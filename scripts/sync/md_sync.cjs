...
// 功能描述：SAP 机种-目的地同步（Sap_Data.PP_SapModelDest → takt_logistics_materials_model_destination；含 delta/oper 日志）
//
...
(async () => {
  ...
  const cjsResult = await execSQL(makeCjsSyncSql(), 'CJS 机种-目的地同步', { filePrefix: 'md_sync' });
  const cjsCounts = parseInsUpdSummaryCounts(cjsResult);

  console.log('\n==========================================');
  console.log('📊 最终汇总日志');
  console.log('------------------------------------------');
  console.log('【CJS 机种-目的地】');
