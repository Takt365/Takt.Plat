...
// 功能描述：SAP 物料主数据同步（Sap_Data.PP_SapMaterial → takt_logistics_materials_material_plant；含 delta/oper 日志）
//
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
    ...
    S.[material_status],S.[tenant_code],S.[company_code],S.[ext_field_json],S.[remark],S.[created_by],
...
(async () => {
  ...
  const syncResult = await execSQL(syncSql, 'ST 物料同步', { filePrefix: 'ma_sync' });
