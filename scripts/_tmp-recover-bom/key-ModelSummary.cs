...
<!-- 命名空间：@/views/logistics/manufacturing/bom/material-cost-analysis/components -->
<!-- 文件名称：material-cost-transposed-table.vue -->
...
<template>
  <div>
    <div v-if="summaryText" class="mb-3 text-sm text-text-secondary">
      {{ summaryText }}
    </div>
    <TaktSingleTable
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="String(column.key).startsWith('period_')">
          {{ formatPeriodCost(record as BomMaterialCostTransposed, String(column.key)) }}
        </template>
      </template>
        ...
        <a-table-summary fixed>
          <a-table-summary-row>
            ...
            <a-table-summary-cell :index="1">
              <span class="font-medium text-text">{{ modelSummary.modelCode }}</span>
            </a-table-summary-cell>
...
import type {
  BomMaterialCostModelSummary,
  BomMaterialCostTransposed,
} from '@/types/logistics/manufacturing/bom/material-cost-analysis';
import {
  MATERIAL_COST_ANALYSIS_LOCALE_PREFIX,
  useMaterialCostAnalysis,
} from '../composables/use-material-cost-analysis';

const props = defineProps<{
  /** 机种编码 */
  modelCode?: string;
  /** 机种平均成本汇总（全量成品，不受分页影响） */
  modelSummary?: BomMaterialCostModelSummary | null;
  /** 表格行 */
  rows: BomMaterialCostTransposed[];
  /** 期间列顺序 */
  periodOrder: string[];
  /** 加载态 */
  loading?: boolean;
 ...
 * @returns 产品编码
 */
function getRowKey(record: BomMaterialCostTransposed) {
 ...
 * @returns 展示文本
 */
function formatPeriodCost(record: BomMaterialCostTransposed, columnKey: string) {
