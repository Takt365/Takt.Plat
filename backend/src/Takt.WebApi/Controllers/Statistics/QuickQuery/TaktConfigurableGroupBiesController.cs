...
/// </summary>
[ApiModule(9, "统计看板")]
[Route("api/[controller]", Name = "定制报表分组")]
public class TaktConfigurableGroupBiesController : TaktControllerBase
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="configurableGroupByService">定制报表分组服务</param>
    ...
    [TaktPermission("statistics:quickquery:configurable:list", "定制报表分组列表")]
    [HttpGet("list")]
    ...
    /// <returns>定制报表分组DTO</returns>
    [TaktPermission("statistics:quickquery:configurable:query", "定制报表分组详情")]
    [HttpGet("{id}")]
    ...
    [TaktPermission("statistics:quickquery:configurable:query", "定制报表分组选项")]
    [HttpGet("options")]
    public async Task<IActionResult> GetConfigurableGroupByOptionsAsync([FromQuery] string? plantCode = null, [FromQuery] string? keyword = null)
    {
        try
        {
            var result = await _configurableGroupByService.GetConfigurableGroupByOptionsAsync(plantCode, keyword);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
    ...
    /// <returns>定制报表分组DTO</returns>
    [TaktPermission("statistics:quickquery:configurable:create", "创建定制报表分组")]
    [HttpPost]
    ...
    /// <returns>定制报表分组DTO</returns>
    [TaktPermission("statistics:quickquery:configurable:update", "更新定制报表分组")]
    [HttpPut("{id}")]
    ...
    [TaktPermission("statistics:quickquery:configurable:delete", "删除定制报表分组")]
    [HttpDelete("{id}")]
    ...
    /// <returns>操作结果</returns>
    [TaktPermission("statistics:quickquery:configurable:delete", "批量删除定制报表分组")]
    [HttpDelete("batch")]
    ...
    /// <returns>定制报表分组DTO</returns>
    [TaktPermission("statistics:quickquery:configurable:update", "更新定制报表分组排序")]
    [HttpPut("sort")]
    ...
    [TaktPermission("statistics:quickquery:configurable:import", "获取定制报表分组导入模板")]
    [HttpGet("template")]
    ...
    [TaktPermission("statistics:quickquery:configurable:import", "导入定制报表分组")]
    [HttpPost("import")]
    ...
    /// </summary>
    /// <returns>Excel文件</returns>
    [TaktPermission("statistics:quickquery:configurable:export", "导出定制报表分组")]
    [HttpGet("export")]
