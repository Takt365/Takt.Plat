...
public class TaktSalesOrderItemsController : TaktControllerBase
{
    ...
    [HttpGet("options")]
    public async Task<IActionResult> GetSalesOrderItemOptionsAsync([FromQuery] string? plantCode = null, [FromQuery] string? keyword = null)
    {
        try
        {
            var result = await _salesOrderItemService.GetSalesOrderItemOptionsAsync(plantCode, keyword);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
