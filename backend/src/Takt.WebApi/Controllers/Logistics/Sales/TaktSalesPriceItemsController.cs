...
public class TaktSalesPriceItemsController : TaktControllerBase
{
    ...
    [TaktPermission("logistics:sales:price:list", "销售价格明细列表")]
    [HttpGet("list")]
    ...
    [TaktPermission("logistics:sales:price:query", "销售价格明细详情")]
    [HttpGet("{id}")]
    ...
    [TaktPermission("logistics:sales:price:query", "销售价格明细选项")]
    [HttpGet("options")]
    public async Task<IActionResult> GetSalesPriceItemOptionsAsync([FromQuery] string? plantCode = null, [FromQuery] string? keyword = null)
    {
        try
        {
            var result = await _salesPriceItemService.GetSalesPriceItemOptionsAsync(plantCode, keyword);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
    ...
    /// <returns>销售价格明细DTO</returns>
    [TaktPermission("logistics:sales:price:create", "创建销售价格明细")]
    [HttpPost]
    ...
    /// <returns>销售价格明细DTO</returns>
    [TaktPermission("logistics:sales:price:update", "更新销售价格明细")]
    [HttpPut("{id}")]
    ...
    [TaktPermission("logistics:sales:price:delete", "删除销售价格明细")]
    [HttpDelete("{id}")]
    ...
    /// <returns>操作结果</returns>
    [TaktPermission("logistics:sales:price:delete", "批量删除销售价格明细")]
    [HttpDelete("batch")]
    ...
    /// <returns>销售价格明细DTO</returns>
    [TaktPermission("logistics:sales:price:update", "更新销售价格明细作废状态")]
    [HttpPut("obsolete")]
    public async Task<IActionResult> UpdateSalesPriceItemObsoleteAsync([FromBody] TaktSalesPriceItemObsoleteDto dto)
    {
        try
        {
            var result = await _salesPriceItemService.UpdateSalesPriceItemObsoleteAsync(dto);
            return Success(result, "更新成功");
        }
        catch (Exception ex)
        {
    ...
    [TaktPermission("logistics:sales:price:import", "获取销售价格明细导入模板")]
    [HttpGet("template")]
    ...
    [TaktPermission("logistics:sales:price:import", "导入销售价格明细")]
    [HttpPost("import")]
    ...
    /// </summary>
    /// <returns>Excel文件</returns>
    [TaktPermission("logistics:sales:price:export", "导出销售价格明细")]
    [HttpGet("export")]
