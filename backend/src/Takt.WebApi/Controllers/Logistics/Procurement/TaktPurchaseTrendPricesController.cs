...
/// <summary>
/// 采购价格月推移 / 机种推移转置分析控制器（与 TaktPurchasePrices CRUD 分离）
/// </summary>
[ApiModule(4, "后勤管理")]
...
public class TaktPurchaseTrendPricesController : TaktControllerBase
{
    ...
    public async Task<IActionResult> GetPurchasePriceTrendPlantOptionsAsync()
    {
        try
        {
            var result = await _purchaseTrendPriceService.GetPurchasePriceTrendPlantOptionsAsync();
    ...
    public async Task<IActionResult> GetPurchasePriceTrendPriceTypeOptionsAsync([FromQuery] string plantCode)
    {
        try
        {
            ...
            var result = await _purchaseTrendPriceService.GetPurchasePriceTrendPriceTypeOptionsAsync(plantCode);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
        ...
        [FromQuery] string? priceType = null)
    {
        try
        {
            ...
            var result = await _purchaseTrendPriceService.GetPurchasePriceTrendSupplierOptionsAsync(plantCode, priceType);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
        ...
        [FromQuery] string? supplierCode = null)
    {
        try
        {
            ...
            var result = await _purchaseTrendPriceService.GetPurchasePriceTrendMaterialOptionsAsync(
    ...
    /// <param name="queryDto">查询 DTO</param>
    /// <returns>转置分析结果</returns>
    [TaktPermission("logistics:procurement:model:purchase:trend:list", "机种采购推移")]
    ...
    /// <param name="exportName">导出文件名</param>
    /// <returns>Excel 文件</returns>
    [TaktPermission("logistics:procurement:model:purchase:trend:export", "清单导出机种采购推移")]
