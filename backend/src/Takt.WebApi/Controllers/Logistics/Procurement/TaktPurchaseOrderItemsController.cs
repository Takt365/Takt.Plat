...
public class TaktPurchaseOrderItemsController : TaktControllerBase
{
    ...
    [TaktPermission("logistics:procurement:purchase:order:list", "采购订单明细列表")]
    [HttpGet("list")]
    ...
    [TaktPermission("logistics:procurement:purchase:order:query", "采购订单明细详情")]
    [HttpGet("{id:long}")]
    ...
    [TaktPermission("logistics:procurement:purchase:order:query", "采购订单明细选项")]
    [HttpGet("options")]
    public async Task<IActionResult> GetPurchaseOrderItemOptionsAsync([FromQuery] string? plantCode = null, [FromQuery] string? keyword = null)
    {
        try
        {
            var result = await _purchaseOrderItemService.GetPurchaseOrderItemOptionsAsync(plantCode, keyword);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
    ...
    /// <returns>采购订单明细DTO</returns>
    [TaktPermission("logistics:procurement:purchase:order:create", "创建采购订单明细")]
    [HttpPost]
    ...
    /// <returns>采购订单明细DTO</returns>
    [TaktPermission("logistics:procurement:purchase:order:update", "更新采购订单明细")]
    [HttpPut("{id}")]
    ...
    [TaktPermission("logistics:procurement:purchase:order:delete", "删除采购订单明细")]
    [HttpDelete("{id}")]
    ...
    /// <returns>操作结果</returns>
    [TaktPermission("logistics:procurement:purchase:order:delete", "批量删除采购订单明细")]
    [HttpDelete("batch")]
    ...
    /// <returns>采购订单明细DTO</returns>
    [TaktPermission("logistics:procurement:purchase:order:update", "更新采购订单明细状态")]
    [HttpPut("status")]
    ...
    [TaktPermission("logistics:procurement:purchase:order:import", "获取采购订单明细导入模板")]
    [HttpGet("template")]
    ...
    [TaktPermission("logistics:procurement:purchase:order:import", "导入采购订单明细")]
    [HttpPost("import")]
    ...
    /// </summary>
    /// <returns>Excel文件</returns>
    [TaktPermission("logistics:procurement:purchase:order:export", "导出采购订单明细")]
    [HttpGet("export")]
