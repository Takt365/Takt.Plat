...
public class TaktPurchasePricesController : TaktControllerBase
{
    ...
    [TaktPermission("logistics:procurement:purchase:price:list", "采购价格列表")]
    [HttpGet("list")]
    ...
    [TaktPermission("logistics:procurement:purchase:price:query", "采购价格详情")]
    [HttpGet("{id}")]
    ...
    [TaktPermission("logistics:procurement:purchase:price:query", "采购价格选项")]
    [HttpGet("options")]
    ...
    /// <returns>下拉选项</returns>
    [TaktPermission("logistics:procurement:purchase:price:query", "采购价格供应商选项")]
    [HttpGet("supplier-options")]
    public async Task<IActionResult> GetPurchasePriceSupplierOptionsAsync([FromQuery] string? plantCode = null)
    {
        try
        {
            var result = await _purchasePriceService.GetPurchasePriceSupplierOptionsAsync(plantCode);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
    ...
    /// <param name="plantCode">工厂代码</param>
    /// <returns>下拉选项</returns>
    [TaktPermission("logistics:procurement:purchase:price:query", "采购价格物料选项")]
    [HttpGet("material-options")]
    public async Task<IActionResult> GetPurchasePriceMaterialOptionsAsync([FromQuery] string? plantCode = null)
    {
        try
        {
            var result = await _purchasePriceService.GetPurchasePriceMaterialOptionsAsync(plantCode);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
    ...
    /// <returns>采购价格DTO</returns>
    [TaktPermission("logistics:procurement:purchase:price:create", "创建采购价格")]
    [HttpPost]
    ...
    /// <returns>采购价格DTO</returns>
    [TaktPermission("logistics:procurement:purchase:price:update", "更新采购价格")]
    [HttpPut("{id}")]
    ...
    [TaktPermission("logistics:procurement:purchase:price:delete", "删除采购价格")]
    [HttpDelete("{id}")]
    ...
    /// <returns>操作结果</returns>
    [TaktPermission("logistics:procurement:purchase:price:delete", "批量删除采购价格")]
    [HttpDelete("batch")]
    ...
    [TaktPermission("logistics:procurement:purchase:price:import", "获取采购价格导入模板")]
    [HttpGet("template")]
    ...
    [TaktPermission("logistics:procurement:purchase:price:import", "导入采购价格")]
    [HttpPost("import")]
    ...
    /// </summary>
    /// <returns>Excel文件</returns>
    [TaktPermission("logistics:procurement:purchase:price:export", "导出采购价格")]
    [HttpGet("export")]
