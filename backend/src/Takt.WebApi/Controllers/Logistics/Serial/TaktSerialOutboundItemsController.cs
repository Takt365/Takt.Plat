...
public class TaktSerialOutboundItemsController : TaktControllerBase
{
    ...
    [HttpGet("options")]
    public async Task<IActionResult> GetSerialOutboundItemOptionsAsync([FromQuery] string? plantCode = null, [FromQuery] string? keyword = null)
    {
        try
        {
            var result = await _serialOutboundItemService.GetSerialOutboundItemOptionsAsync(plantCode, keyword);
            return Success(result, "查询成功");
        }
        catch (Exception ex)
        {
    ...
    /// <returns>序列号出库明细DTO</returns>
    [TaktPermission("logistics:serial:outbound:update", "更新序列号出库明细作废状态")]
    [HttpPut("obsolete")]
    public async Task<IActionResult> UpdateSerialOutboundItemObsoleteAsync([FromBody] TaktSerialOutboundItemObsoleteDto dto)
    {
        try
        {
            var result = await _serialOutboundItemService.UpdateSerialOutboundItemObsoleteAsync(dto);
            return Success(result, "更新成功");
        }
        catch (Exception ex)
        {
