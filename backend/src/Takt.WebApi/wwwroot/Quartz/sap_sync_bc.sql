...
-- 2) 按完整唯一键去重：
--    Plant+Product+SequenceNo+BomLevel+BomItemNo+Component
...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ORDER BY
        N.[plant_code], N.[product_code], N.[sequence_no], N.[bom_level],
        N.[bom_item_no], N.[component_code], N.[component_quantity],
  ...
  FROM (
    SELECT
      ...
      ISNULL(NULLIF(LTRIM(RTRIM(R.[moving_price_currency])), ''), '') AS [moving_price_currency],
      ...
      ISNULL(NULLIF(LTRIM(RTRIM(R.[purchase_currency])), ''), '') AS [purchase_currency],
      ...
      ROW_NUMBER() OVER (
        PARTITION BY
          ...
          CASE
            ...
            ELSE LTRIM(RTRIM(R.[product_code]))
          END,
          LTRIM(RTRIM(R.[sequence_no])),
          ISNULL(NULLIF(LTRIM(RTRIM(R.[bom_level])), ''), ''),
          LTRIM(RTRIM(R.[bom_item_no])),
          CASE
            WHEN LEN(LTRIM(RTRIM(R.[component_code]))) = 18
              AND LTRIM(RTRIM(R.[component_code])) NOT LIKE '%[^0-9]%'
            THEN RIGHT(LTRIM(RTRIM(R.[component_code])), 10)
    ...
    FROM [Sap_Data].[dbo].[PP_Sap_Zp002] R
  ) N
  WHERE N.dup_rn = 1
) S
...
DECLARE @sap_raw_count INT = (SELECT COUNT(*) FROM [Sap_Data].[dbo].[PP_Sap_Zp002]);
...
IF EXISTS (
  ...
  FROM #st_source
  GROUP BY
    [plant_code], [product_code], [sequence_no], [bom_level], [bom_item_no],
...
BEGIN
  ...
  SELECT TOP 1
    @dup_key = CONCAT(
      [plant_code], N' / ', [product_code], N' / ', [sequence_no], N' / ',
      [bom_level], N' / ', [bom_item_no], N' / ', [component_code], N' / ',
...
AND LTRIM(RTRIM(T.[sequence_no])) = S.[sequence_no]
AND ISNULL(LTRIM(RTRIM(T.[bom_level])), N'') = ISNULL(LTRIM(RTRIM(S.[bom_level])), N'')
AND LTRIM(RTRIM(T.[bom_item_no])) = S.[bom_item_no]
...
WHEN MATCHED AND (
  ...
  OR LTRIM(RTRIM(ISNULL(T.[moving_price_currency], N''))) <> LTRIM(RTRIM(ISNULL(S.[moving_price_currency], N'')))
  ...
  OR LTRIM(RTRIM(ISNULL(T.[purchase_currency], N''))) <> LTRIM(RTRIM(ISNULL(S.[purchase_currency], N'')))
) THEN
  UPDATE SET
    ...
    T.[moving_price_currency] = S.[moving_price_currency],
    ...
    T.[purchase_currency] = S.[purchase_currency],
...
WHEN NOT MATCHED THEN
  INSERT (
    [id],[plant_code],[product_code],[sequence_no],[product_description],[bom_level],
    [bom_item_no],[component_code],[component_description],[component_quantity],
    [batch_indicator],[production_related],[purchase_type],[special_procurement_type],
    [profit_center_code],[moving_average_price],[moving_price_unit],[moving_price_currency],
    [purchase_organization],[purchase_group],[supplier_code],[net_purchase_price],
    [purchase_price_unit],[purchase_currency],[costing_date],
    [tenant_code],[company_code],[ext_field_json],[remark],
    [created_by],[created_at],[updated_by],[updated_at],[is_deleted]
  )
  VALUES (
    S.[id],S.[plant_code],S.[product_code],S.[sequence_no],S.[product_description],S.[bom_level],
    S.[bom_item_no],S.[component_code],S.[component_description],S.[component_quantity],
    S.[batch_indicator],S.[production_related],S.[purchase_type],S.[special_procurement_type],
    S.[profit_center_code],S.[moving_average_price],S.[moving_price_unit],S.[moving_price_currency],
    S.[purchase_organization],S.[purchase_group],S.[supplier_code],S.[net_purchase_price],
    S.[purchase_price_unit],S.[purchase_currency],S.[costing_date],
...
WHERE T.[tenant_code] = @tenant_code
  ...
  AND NOT EXISTS (
    ...
    WHERE S.[plant_code] = LTRIM(RTRIM(T.[plant_code]))
      ...
      AND S.[sequence_no] = LTRIM(RTRIM(T.[sequence_no]))
      AND ISNULL(LTRIM(RTRIM(S.[bom_level])), N'') = ISNULL(LTRIM(RTRIM(T.[bom_level])), N'')
      AND S.[bom_item_no] = LTRIM(RTRIM(T.[bom_item_no]))
...
SELECT
  ...
  N'MERGE SAP BomMaterialCostItem Sync',
...
VALUES (
  ...
  N'exec_sql_merge',
  'SQL',
  N'/sync/sap/bom-material-cost-item',
