...
WHEN MATCHED AND (
  T.[is_deleted] <> 0
  OR LTRIM(RTRIM(ISNULL(T.[material_name], N''))) <> LTRIM(RTRIM(ISNULL(S.[material_name], N'')))
  ...
  OR LTRIM(RTRIM(ISNULL(T.[ext_field_json], N''))) <> LTRIM(RTRIM(ISNULL(S.[ext_field_json], N'')))
...
WHEN NOT MATCHED THEN
  INSERT (
    ...
    [material_status],[tenant_code],[company_code],[ext_field_json],[remark],[created_by],
  ...
  VALUES (
    ...
    S.[material_status],S.[tenant_code],S.[company_code],S.[ext_field_json],S.[remark],S.[updated_by],
...
SELECT
  ...
  ISNULL((
    SELECT
      CASE WHEN d.oper_type = 'UPDATE' THEN ISNULL(d.material_name_old, 'null') END AS [material_name.old],
      CASE WHEN d.oper_type = 'UPDATE' THEN ISNULL(d.material_name_new, 'null') END AS [material_name.new],
