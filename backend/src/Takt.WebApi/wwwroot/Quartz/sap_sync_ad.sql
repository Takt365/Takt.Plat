...
-- Sap_Data.dbo.takt_foundation_admin_division → 当前租户库同名表
...
FROM (
  ...
  FROM (
    SELECT
      ...
      ISNULL(NULLIF(LTRIM(RTRIM(R.[culture_code])), N''), N'') AS [culture_code],
    ...
    FROM [Sap_Data].[dbo].[takt_foundation_admin_division] R
    LEFT JOIN [Sap_Data].[dbo].[takt_foundation_admin_division] P
      ON P.[id] = R.[parent_id]
...
WHEN NOT MATCHED THEN
  INSERT (
    ...
    [division_path],[is_leaf],[postal_code],[culture_code],[currency_code],[phone_code],
  ...
  VALUES (
    ...
    S.[division_path],S.[is_leaf],S.[postal_code],S.[culture_code],S.[currency_code],S.[phone_code],
    S.[is_built_in],S.[sort_order],S.[division_status],
    S.[tenant_code],S.[ext_field],S.[remark],
...
OUTPUT
  ...
  DELETED.[culture_code], INSERTED.[culture_code],
...
SELECT
  ...
  N'MERGE SAP AdminDivision Sync',
