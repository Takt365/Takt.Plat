...
-- Sap_Data.dbo.takt_logistics_sales_price* → 当前租户库同名表
-- 明细/等级表无 plant_code（仅主表有）；item 按 sales_price_code 关联主表回填 sales_price_id
-- 业务唯一键：主表 plant+sales_price_code；明细 code+seq；等级 code+seq+scale_seq+门槛
...
CREATE TABLE #hdr (
  ...
  [customer_code] NVARCHAR(40), [material_code] NVARCHAR(40),
...
SELECT S.rn, @base_id + S.rn,
  S.[plant_code], S.[sales_price_code], S.[price_type], S.[customer_code], S.[material_code],
  S.[sales_group], S.[tax_code],
  S.[gr_based_invoice_inspection], S.[pricing_date_control],
  S.[valid_from], S.[valid_to], S.[sales_quotation_id], S.[sales_quotation_code], S.[variable_key]
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (ORDER BY LTRIM(RTRIM(R.[plant_code])), LTRIM(RTRIM(R.[sales_price_code]))) AS rn
  FROM [Sap_Data].[dbo].[takt_logistics_sales_price] R
...
DECLARE @hdr_sap_raw INT = (
  SELECT COUNT(*) FROM [Sap_Data].[dbo].[takt_logistics_sales_price] R
  WHERE LTRIM(RTRIM(ISNULL(R.[sales_price_code], N''))) <> N''
    AND LTRIM(RTRIM(ISNULL(R.[plant_code], N''))) <> N''
);
...
END;
IF EXISTS (SELECT 1 FROM #hdr GROUP BY [plant_code], [sales_price_code] HAVING COUNT(*) > 1)
BEGIN
  DECLARE @hdr_dup NVARCHAR(400);
  SELECT TOP 1 @hdr_dup = CONCAT([plant_code], N' / ', [sales_price_code], N' x', COUNT(*))
  FROM #hdr GROUP BY [plant_code], [sales_price_code] HAVING COUNT(*) > 1;
...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (ORDER BY LTRIM(RTRIM(R.[sales_price_code])), COALESCE(TRY_CAST(R.[sales_price_seq] AS INT), 10)) AS rn
  FROM [Sap_Data].[dbo].[takt_logistics_sales_price_item] R
  WHERE LTRIM(RTRIM(ISNULL(R.[sales_price_code], N''))) <> N''
    AND EXISTS (
      SELECT 1 FROM #hdr H
...
DECLARE @item_sap_raw INT = (
  SELECT COUNT(*) FROM [Sap_Data].[dbo].[takt_logistics_sales_price_item] R
  WHERE LTRIM(RTRIM(ISNULL(R.[sales_price_code], N''))) <> N''
    AND EXISTS (
      SELECT 1 FROM #hdr H
      WHERE H.[sales_price_code] = LTRIM(RTRIM(R.[sales_price_code]))
...
END;
IF EXISTS (SELECT 1 FROM #item GROUP BY [sales_price_code], [sales_price_seq] HAVING COUNT(*) > 1)
BEGIN
  DECLARE @item_dup NVARCHAR(400);
  SELECT TOP 1 @item_dup = CONCAT([sales_price_code], N' / ', [sales_price_seq], N' x', COUNT(*))
  FROM #item GROUP BY [sales_price_code], [sales_price_seq] HAVING COUNT(*) > 1;
...
FROM (
  SELECT
    ...
    COALESCE(TRY_CAST(R.[is_obsolete] AS INT), 0) AS [is_obsolete],
    ROW_NUMBER() OVER (
      ORDER BY LTRIM(RTRIM(R.[sales_price_code])), COALESCE(TRY_CAST(R.[sales_price_seq] AS INT), 10),
        COALESCE(TRY_CAST(R.[sales_scale_seq] AS INT), 10), COALESCE(TRY_CAST(R.[scale_quantity] AS DECIMAL(18,8)), 0)
    ) AS rn
  FROM [Sap_Data].[dbo].[takt_logistics_sales_price_scale_quantity] R
  WHERE LTRIM(RTRIM(ISNULL(R.[sales_price_code], N''))) <> N''
    AND EXISTS (
...
DECLARE @sq_sap_raw INT = (
  SELECT COUNT(*) FROM [Sap_Data].[dbo].[takt_logistics_sales_price_scale_quantity] R
  WHERE LTRIM(RTRIM(ISNULL(R.[sales_price_code], N''))) <> N''
    AND EXISTS (
      SELECT 1 FROM #item I
      WHERE I.[sales_price_code] = LTRIM(RTRIM(R.[sales_price_code]))
...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ORDER BY LTRIM(RTRIM(R.[sales_price_code])), COALESCE(TRY_CAST(R.[sales_price_seq] AS INT), 10),
        COALESCE(TRY_CAST(R.[sales_scale_seq] AS INT), 10), COALESCE(TRY_CAST(R.[scale_value] AS DECIMAL(18,8)), 0)
    ) AS rn
  FROM [Sap_Data].[dbo].[takt_logistics_sales_price_scale_value] R
  WHERE LTRIM(RTRIM(ISNULL(R.[sales_price_code], N''))) <> N''
    AND EXISTS (
      SELECT 1 FROM #item I
      WHERE I.[sales_price_code] = LTRIM(RTRIM(R.[sales_price_code]))
...
DECLARE @sv_sap_raw INT = (
  SELECT COUNT(*) FROM [Sap_Data].[dbo].[takt_logistics_sales_price_scale_value] R
  WHERE LTRIM(RTRIM(ISNULL(R.[sales_price_code], N''))) <> N''
    AND EXISTS (
      SELECT 1 FROM #item I
      WHERE I.[sales_price_code] = LTRIM(RTRIM(R.[sales_price_code]))
...
DECLARE @hdr_before INT = (SELECT COUNT(*) FROM [takt_logistics_sales_price] WHERE [tenant_code]=@tenant_code AND [company_code]=@company_code AND [is_deleted]=0);
DECLARE @item_before INT = (SELECT COUNT(*) FROM [takt_logistics_sales_price_item] WHERE [tenant_code]=@tenant_code AND [company_code]=@company_code AND [is_deleted]=0);
DECLARE @sq_before INT = (SELECT COUNT(*) FROM [takt_logistics_sales_price_scale_quantity] WHERE [tenant_code]=@tenant_code AND [company_code]=@company_code AND [is_deleted]=0);
DECLARE @sv_before INT = (SELECT COUNT(*) FROM [takt_logistics_sales_price_scale_value] WHERE [tenant_code]=@tenant_code AND [company_code]=@company_code AND [is_deleted]=0);
...
WHEN NOT MATCHED THEN INSERT (
  [id],[plant_code],[sales_price_code],[price_type],[customer_code],[material_code],
  [sales_group],[tax_code],[gr_based_invoice_inspection],[pricing_date_control],
  [valid_from],[valid_to],[sales_quotation_id],[sales_quotation_code],[variable_key],
  [tenant_code],[company_code],[ext_field_json],[remark],
  [created_by],[created_at],[updated_by],[updated_at],[is_deleted]
) VALUES (
  S.[id],S.[plant_code],S.[sales_price_code],S.[price_type],S.[customer_code],S.[material_code],
...
WHERE T.[tenant_code]=@tenant_code AND T.[company_code]=@company_code AND T.[is_deleted]=0
  AND NOT EXISTS (SELECT 1 FROM #hdr S WHERE S.[plant_code]=LTRIM(RTRIM(T.[plant_code])) AND S.[sales_price_code]=LTRIM(RTRIM(T.[sales_price_code])));
DECLARE @hdr_del INT = @@ROWCOUNT;

UPDATE I SET I.[sales_price_id]=H.[id]
FROM #item I
INNER JOIN #hdr H ON H.[sales_price_code]=I.[sales_price_code];
...
FROM #sq Q INNER JOIN #item I ON I.[sales_price_code]=Q.[sales_price_code] AND I.[sales_price_seq]=Q.[sales_price_seq];
DELETE FROM #sq WHERE [sales_price_item_id]=0 OR [sales_price_item_id] IS NULL;

UPDATE V SET V.[sales_price_item_id]=I.[id]
FROM #sv V INNER JOIN #item I ON I.[sales_price_code]=V.[sales_price_code] AND I.[sales_price_seq]=V.[sales_price_seq];
...
WHEN MATCHED AND (
  T.[is_deleted]<>0 OR LTRIM(RTRIM(ISNULL(T.[sales_price_code],N'')))<>S.[sales_price_code]
...
DECLARE @hdr_after INT = (SELECT COUNT(*) FROM [takt_logistics_sales_price] WHERE [tenant_code]=@tenant_code AND [company_code]=@company_code AND [is_deleted]=0);
DECLARE @item_after INT = (SELECT COUNT(*) FROM [takt_logistics_sales_price_item] WHERE [tenant_code]=@tenant_code AND [company_code]=@company_code AND [is_deleted]=0);
DECLARE @sq_after INT = (SELECT COUNT(*) FROM [takt_logistics_sales_price_scale_quantity] WHERE [tenant_code]=@tenant_code AND [company_code]=@company_code AND [is_deleted]=0);
DECLARE @sv_after INT = (SELECT COUNT(*) FROM [takt_logistics_sales_price_scale_value] WHERE [tenant_code]=@tenant_code AND [company_code]=@company_code AND [is_deleted]=0);
...
INSERT INTO [takt_statistics_logging_oper_log] (
  ...
  @base_id + 1, N'SYSTEM_SAP_SYNC', N'SAP_SYNC', N'销售价格',
  N'exec_sql_merge', 'SQL', N'/sync/sap/sales-price', CONCAT('batch_size=', @batch_size),
