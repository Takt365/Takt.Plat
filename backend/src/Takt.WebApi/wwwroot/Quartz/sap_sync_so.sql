...
DECLARE @target_before INT = (
  SELECT COUNT(*)
  FROM [takt_logistics_manufacturing_planning_production_order]
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
    ...
    '',NULL,NULL,NULL,1,
    @tenant_code,@company_code,'{}',N'mismatch update',
...
WHERE T.[tenant_code] = @tenant_code
  AND T.[company_code] = @company_code
  AND T.[is_deleted] = 0
  AND ISNULL(T.[serial_no], '') <> ISNULL(S.serial_no, '');
...
SELECT
  ...
  @now,0,
  @tenant_code,@company_code,'{}',N'SAP_SYNC',@sync_user_id,@now
FROM #order_delta d;
...
DECLARE @json_result NVARCHAR(MAX) =
  ...
  + N',"serial_no":' + CAST(@ser_upd AS NVARCHAR) + N'}';
