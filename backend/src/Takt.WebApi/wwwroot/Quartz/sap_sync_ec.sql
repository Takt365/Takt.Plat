...
CREATE TABLE #source_main (
  ...
  [source_pp_no] NVARCHAR(MAX),
  [source_technical_notice_no] NVARCHAR(MAX),
...
FROM (
  SELECT *,
    LTRIM(RTRIM([D_SAP_ZPABD_Z001])) AS source_ec_no,
...
BEGIN
  DECLARE @main_src_msg NVARCHAR(200) = CONCAT(
    N'主表SAP源行数与装入不一致: sap=', @main_sap_raw_count, N', loaded=', @main_source_count);
  THROW 50003, @main_src_msg, 1;
END;

IF EXISTS (
  SELECT 1 FROM #source_main GROUP BY [source_ec_no] HAVING COUNT(*) > 1
)
BEGIN
  DECLARE @main_dup NVARCHAR(400);
  SELECT TOP 1 @main_dup = CONCAT([source_ec_no], N' x', COUNT(*))
  FROM #source_main GROUP BY [source_ec_no] HAVING COUNT(*) > 1;
...
FROM (
  SELECT
    LTRIM(RTRIM(Sub.[D_SAP_ZPABD_S001])) AS source_ec_no,
    ISNULL(Sub.[D_SAP_ZPABD_S004], N'') AS legacy_part_no,
    ...
    ISNULL(CAST(Sub.[D_SAP_ZPABD_S008] AS NVARCHAR(MAX)), N'') AS source_replacement_part_no,
    ...
    ISNULL(CAST(Sub.[D_SAP_ZPABD_S012] AS NVARCHAR(MAX)), N'') AS source_bom_no,
  ...
  INNER JOIN #source_main M
    ON M.[source_ec_no] = LTRIM(RTRIM(Sub.[D_SAP_ZPABD_S001]))
...
BEGIN
  DECLARE @detail_src_msg NVARCHAR(200) = CONCAT(
    N'子表SAP源行数与装入不一致: sap=', @detail_sap_raw_count, N', loaded=', @detail_source_count);
  THROW 50003, @detail_src_msg, 1;
END;

DECLARE @main_target_before INT = (
  SELECT COUNT(*)
  FROM [takt_logistics_manufacturing_source_ec]
...
MERGE INTO [takt_logistics_manufacturing_source_ec] AS T
USING #source_main AS S
ON T.[tenant_code] = @tenant_code
AND T.[company_code] = @company_code
AND LTRIM(RTRIM(T.[source_ec_no])) = S.[source_ec_no]
WHEN MATCHED AND (
  T.[is_deleted] <> 0
  ...
  OR LTRIM(RTRIM(ISNULL(T.[source_pp_no], N''))) <> LTRIM(RTRIM(ISNULL(S.[source_pp_no], N'')))
  OR LTRIM(RTRIM(ISNULL(T.[source_technical_notice_no], N''))) <> LTRIM(RTRIM(ISNULL(S.[source_technical_notice_no], N'')))
  ...
  UPDATE SET
    ...
    T.[source_pp_no] = S.[source_pp_no],
    T.[source_technical_notice_no] = S.[source_technical_notice_no],
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
    S.[id],S.[source_ec_no],S.[source_model],S.[source_title],S.[source_status],
    S.[source_issue_date],S.[source_tcj_owner],S.[source_tcj_dependency],
    S.[source_ec_meeting],S.[source_pp_no],S.[source_technical_notice_no],
    ...
    S.[source_ec_content],@tenant_code,@company_code,
    @sync_user_id,@now,@sync_user_id,@now,0
  )
OUTPUT S.rn, $action, INSERTED.[id], INSERTED.[source_ec_no]
INTO #main_delta(rn, oper_type, id, source_ec_no);
...
SET
  ...
  T.[updated_by] = @sync_user_id,
  T.[updated_at] = @now
OUTPUT INSERTED.[id], INSERTED.[source_ec_no]
INTO #main_soft_deleted_rows ([id], [source_ec_no])
FROM [takt_logistics_manufacturing_source_ec] T
WHERE T.[tenant_code] = @tenant_code
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM #source_main S
    WHERE S.[source_ec_no] = LTRIM(RTRIM(T.[source_ec_no]))
  );
...
SELECT @main_soft_deleted_keys = STRING_AGG(
  CONCAT(CAST([id] AS NVARCHAR(30)), N'|', ISNULL([source_ec_no], N'')),
  N'; '
)
...
DECLARE @detail_target_before INT = (
  SELECT COUNT(*)
  FROM [takt_logistics_manufacturing_source_ec_detail]
  WHERE [tenant_code] = @tenant_code
    AND [company_code] = @company_code
    AND [is_deleted] = 0
);
...
SET
  ...
  T.[updated_by] = @sync_user_id,
  T.[updated_at] = @now
OUTPUT INSERTED.[id], INSERTED.[source_ec_id], INSERTED.[source_legacy_part_no]
INTO #detail_soft_deleted_rows ([id], [source_ec_id], [legacy_part_no])
FROM [takt_logistics_manufacturing_source_ec_detail] T
WHERE T.[tenant_code] = @tenant_code
  AND T.[company_code] = @company_code
  AND T.[is_deleted] = 0;
...
INNER JOIN [takt_logistics_manufacturing_source_ec] M
 ...
 AND M.[source_ec_no] = S.[source_ec_no]
 AND M.[is_deleted] = 0;

INSERT INTO #detail_delta(rn, oper_type, id, source_ec_id, legacy_part_no)
SELECT
  S.[rn],
  N'INSERT',
  S.[id],
...
SELECT @detail_soft_deleted_keys = STRING_AGG(
  CONCAT(
    ...
    ISNULL([legacy_part_no], N'')
  ),
  N'; '
)
FROM (
  SELECT TOP (200) [id], [source_ec_id], [legacy_part_no]
...
DECLARE @detail_target_count INT = (
  SELECT COUNT(*)
  FROM [takt_logistics_manufacturing_source_ec_detail] T
  INNER JOIN [takt_logistics_manufacturing_source_ec] M ON M.[id] = T.[source_ec_id]
  WHERE T.[tenant_code] = @tenant_code
    AND T.[company_code] = @company_code
    AND T.[is_deleted] = 0
...
VALUES (
  ...
  N'exec_sql_merge','SQL',N'/sync/sap/ec','',
  @json_result,
...
SELECT
  ...
  @main_sap_raw_count AS [sap_raw_count],
