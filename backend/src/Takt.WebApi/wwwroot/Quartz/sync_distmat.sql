...
CREATE TABLE #st_source (
  ...
  [seller_material_code] NVARCHAR(40),
  [seller_material_description] NVARCHAR(40),
  [seller_material_specification] NVARCHAR(70),
...
-- 源表装入：
-- 1) 物料码含全角括号的不参与同步
-- 2) 18 位纯数字本厂/内部物料码截末 10 位（销售商物料码 Length=40，不截断）
...
FROM (
  ...
  FROM (
    SELECT
      ...
      LEFT(LTRIM(RTRIM(ISNULL(R.[seller_material_code], N''))), 40) AS [seller_material_code],
      ...
      CASE WHEN ISNULL(R.[is_deleted], 0) = 0 THEN 0 ELSE 1 END AS [is_deleted],
        ...
        PARTITION BY
          ...
          END
        ORDER BY
          LEN(ISNULL(LTRIM(RTRIM(R.[seller_material_code])), N'')) DESC,
          LEN(ISNULL(LTRIM(RTRIM(R.[material_description])), N'')) DESC
      ) AS dup_rn
    FROM [{{SourceDatabase}}].[dbo].[takt_logistics_sales_seller_material] R
    WHERE LTRIM(RTRIM(ISNULL(R.[internal_material_code], N''))) <> N''
      AND LTRIM(RTRIM(ISNULL(R.[material_code], N''))) <> N''
      AND LTRIM(RTRIM(ISNULL(R.[seller_material_code], N''))) <> N''
...
DECLARE @sap_raw_count INT = (SELECT COUNT(*) FROM [{{SourceDatabase}}].[dbo].[takt_logistics_sales_seller_material]);
...
CREATE TABLE #delta (
  ...
  tenant_code NVARCHAR(3),
  change_by BIGINT,
  seller_material_code_old NVARCHAR(40),
  seller_material_code_new NVARCHAR(40),
  material_description_old NVARCHAR(40),
  material_description_new NVARCHAR(40),
  customer_code_old NVARCHAR(10),
  customer_code_new NVARCHAR(10)
);
...
MERGE INTO [takt_logistics_sales_seller_material] AS T
...
WHEN MATCHED AND (
  ...
  OR LTRIM(RTRIM(ISNULL(T.[client_code], N''))) <> LTRIM(RTRIM(ISNULL(S.[client_code], N'')))
  OR LTRIM(RTRIM(ISNULL(T.[client_short_name], N''))) <> LTRIM(RTRIM(ISNULL(S.[client_short_name], N'')))
  ...
  OR LTRIM(RTRIM(ISNULL(T.[seller_material_code], N''))) <> S.[seller_material_code]
  OR LTRIM(RTRIM(ISNULL(T.[seller_material_description], N''))) <> S.[seller_material_description]
...
WHEN NOT MATCHED THEN
  INSERT (
    ...
    [seller_material_code],[seller_material_description],[seller_material_specification],[tenant_code],[ext_field],[remark],
...
OUTPUT
  ...
  INSERTED.[seller_material_code],
  INSERTED.[tenant_code],
  INSERTED.[updated_by],
  DELETED.[seller_material_code], INSERTED.[seller_material_code],
  DELETED.[material_description], INSERTED.[material_description],
  DELETED.[customer_code], INSERTED.[customer_code]
INTO #delta(
  rn, oper_type, id, internal_material_code, material_code, seller_material_code,
  tenant_code, change_by,
  seller_material_code_old, seller_material_code_new,
  material_description_old, material_description_new,
  customer_code_old, customer_code_new
);
...
FROM [takt_logistics_sales_seller_material] T
...
SELECT
  ...
  ISNULL((
    SELECT
      d.seller_material_code_old AS [seller_material_code],
      d.material_description_old AS [material_description],
      d.customer_code_old AS [customer_code]
    WHERE d.oper_type = 'UPDATE'
    FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
  ), '{}'),
  (
    SELECT
      d.seller_material_code_new AS [seller_material_code],
      d.material_description_new AS [material_description],
      d.customer_code_new AS [customer_code]
    FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
  ),
  ISNULL((
    SELECT
      CASE WHEN d.oper_type = 'UPDATE' THEN ISNULL(d.seller_material_code_old, 'null') END AS [seller_material_code.old],
      CASE WHEN d.oper_type = 'UPDATE' THEN ISNULL(d.seller_material_code_new, 'null') END AS [seller_material_code.new],
