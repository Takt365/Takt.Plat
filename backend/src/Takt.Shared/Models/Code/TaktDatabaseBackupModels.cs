...
{
    /// <summary>目标租户</summary>
    public string TargetTenantCode { get; set; } = string.Empty;

    /// <summary>目标数据库名</summary>
    public string TargetDatabaseName { get; set; } = string.Empty;

    /// <summary>备份类型：1=Full 2=Delta</summary>
    public int BackupType { get; set; } = 1;

    /// <summary>路径类型：1=本地(服务器) 2=文件服务器 3=FTP 4=客户端</summary>
    public int BackupPathType { get; set; } = 4;

    /// <summary>备份目录（服务器本地/UNC/FTP 远程目录；客户端为目录标识，实际落盘见临时目录）</summary>
    public string BackupDirectory { get; set; } = string.Empty;

    /// <summary>备份文件名（含 .bak）</summary>
    public string BackupFileName { get; set; } = string.Empty;

    /// <summary>主机（FTP/网络）</summary>
    public string? BackupHost { get; set; }

    /// <summary>端口（FTP）</summary>
    public int? BackupPort { get; set; }

    /// <summary>用户名</summary>
    public string? BackupUserName { get; set; }

    /// <summary>明文密码（仅执行时）</summary>
    public string? BackupPassword { get; set; }

    /// <summary>暂存根（默认 wwwroot/Backup；远程 SQL 须 UNC）</summary>
...
public class TaktDatabaseBackupResult
{
    ...
    /// <summary>实际备份文件路径（本地/UNC/远程）</summary>
    public string BackupFilePath { get; set; } = string.Empty;

    /// <summary>备份类型</summary>
    public int BackupType { get; set; }

    /// <summary>文件大小字节</summary>
