...
// 文件名称：TaktEcSeikan.cs
// 创建时间：2026-06-29
// 创建人：Takt365(Cursor AI)
// 功能描述：设变生管课部门执行表（按设变明细ID+根物料编码唯一）
//
...
/// <summary>
/// 设变生管课（D0420）部门执行表（按设变明细 + 根物料编码唯一）
/// </summary>
[SugarTable("takt_logistics_manufacturing_ec_seikan", "设变生管执行表")]
[SugarIndex("ix_ec_seikan_tenant", nameof(TenantCode), OrderByType.Asc, nameof(CompanyCode), OrderByType.Asc, false)]
[SugarIndex("ix_takt_logistics_manufacturing_ec_seikan_unique", nameof(TenantCode), OrderByType.Asc, nameof(CompanyCode), OrderByType.Asc, nameof(EcDetailId), OrderByType.Asc, nameof(EcRootMaterialCode), OrderByType.Asc, true)]
...
public class TaktEcSeikan : TaktCompanyEntityBase, ITaktEcDeptExecEntity, ITaktEcExecModelRootMaterialRow
{
    /// <summary>
    /// 行号（项号/序号，固定步长=10）
    /// </summary>
