...
// 功能描述：设变制造2课部门执行表（仅采购类型非 F；按设变明细ID+完成品唯一，对齐制一）
//
...
/// <summary>
/// 设变制造2课（D0626）部门执行表（仅采购类型非 F；按设变明细 + 完成品唯一）
/// </summary>
...
[SugarIndex("ix_takt_logistics_manufacturing_ec_seizounika_unique", nameof(TenantCode), OrderByType.Asc, nameof(CompanyCode), OrderByType.Asc, nameof(EcDetailId), OrderByType.Asc, nameof(EcFinishedGoods), OrderByType.Asc, true)]
...
public class TaktEcSeizounika : TaktCompanyEntityBase, ITaktEcDeptExecEntity
{
    /// <summary>
    /// 行号（项号/序号，固定步长=10）
    /// </summary>
