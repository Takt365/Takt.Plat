...
// 功能描述：设变品管课部门执行表（按设变明细ID+根物料编码唯一）
//
...
/// <summary>
/// 设变品管课（D0820）部门执行表（按设变明细 + 根物料编码唯一）
/// </summary>
...
[SugarIndex("ix_takt_logistics_manufacturing_ec_hinkan_unique", nameof(TenantCode), OrderByType.Asc, nameof(CompanyCode), OrderByType.Asc, nameof(EcDetailId), OrderByType.Asc, nameof(EcRootMaterialCode), OrderByType.Asc, true)]
...
public class TaktEcHinkan : TaktCompanyEntityBase, ITaktEcDeptExecEntity, ITaktEcExecModelRootMaterialRow
{
    /// <summary>
    /// 行号（项号/序号，固定步长=10）
    /// </summary>
