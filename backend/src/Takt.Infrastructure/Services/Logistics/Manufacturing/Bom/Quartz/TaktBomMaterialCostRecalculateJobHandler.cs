...
// 功能描述：Quartz 重算成本（仅 CostingDate 当月；完成后由 Executor 落库消息并推送）
//
...
public sealed class TaktBomMaterialCostRecalculateJobHandler : ITaktQuartzJobHandler
{
    ...
    public TaktBomMaterialCostRecalculateJobHandler(ITaktBomMaterialCostItemService bomMaterialCostItemService)
    {
    ...
    public async Task ExecuteAsync(TaktQuartzJobContext context, CancellationToken cancellationToken = default)
    {
        ...
        var result = await _bomMaterialCostItemService.RunScheduledBomMaterialCostRecalculateAsync(
            force: TryParseForce(context.ExecuteParams),
            asOfDate: DateTime.Today);
        if (result == null)
        {
            context.ExecuteMessage = "成本重算无结果（当月无明细或上下文无效）";
            TaktLogger.Warning(
        ...
        context.ExecuteMessage =
            $"成本重算完成（当月 {result.ProcessedMonth}）：扫描 {result.ScannedRowCount}，刷新 {result.RefreshedGroupCount}，跳过 {result.SkippedGroupCount}，重置 {result.ResetGroupCount}";
