...
public class TaktDeptSeedData : ITaktSeedDataCoordinator
{
        ...
        string companyCode)
    {
        ...
        var (headOffice, i0a) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "HEAD_OFFICE", "TEAC", 1, 0, 0);
        insertCount += (i0a ? 1 : 0); updateCount += (i0a ? 0 : 1);

        // DTA（组织编码 D0000, ParentId=headOffice.Id）
        var (dta, iD0) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0000", "DTA", 1, headOffice.Id, 1);
        insertCount += (iD0 ? 1 : 0); updateCount += (iD0 ? 0 : 1);

        // —— DTA 下一级（D1000～D0900, ParentId=dta.Id）——
        var (d1000, i1) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D1000", "总经理室", 1, dta.Id, 1);
        insertCount += (i1 ? 1 : 0); updateCount += (i1 ? 0 : 1);
        
        var (d0100, i2) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0100", "总务部", 1, dta.Id, 2);
        insertCount += (i2 ? 1 : 0); updateCount += (i2 ? 0 : 1);
        
        var (d0200, i3) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0200", "财务部", 1, dta.Id, 3);
        insertCount += (i3 ? 1 : 0); updateCount += (i3 ? 0 : 1);
        
        var (d0300, i4) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0300", "IT部", 2, dta.Id, 4);
        insertCount += (i4 ? 1 : 0); updateCount += (i4 ? 0 : 1);
        
        var (d0400, i5) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0400", "管理部", 1, dta.Id, 5);
        insertCount += (i5 ? 1 : 0); updateCount += (i5 ? 0 : 1);
        
        var (d0500, i6) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0500", "资材部", 1, dta.Id, 6);
        insertCount += (i6 ? 1 : 0); updateCount += (i6 ? 0 : 1);
        
        var (d0600, i7) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0600", "生产部", 2, dta.Id, 7);
        insertCount += (i7 ? 1 : 0); updateCount += (i7 ? 0 : 1);
        
        var (d0700, i8) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0700", "技术部", 2, dta.Id, 8);
        insertCount += (i8 ? 1 : 0); updateCount += (i8 ? 0 : 1);
        
        var (d0800, i9) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0800", "品保部", 2, dta.Id, 9);
        insertCount += (i9 ? 1 : 0); updateCount += (i9 ? 0 : 1);
        
        var (d0900, i10) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0900", "OEM部", 1, dta.Id, 10);
        insertCount += (i10 ? 1 : 0); updateCount += (i10 ? 0 : 1);

        // —— 总务部 D0100 → D0110 (ParentId=d0100.Id) ——
        var (_, i11) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0110", "总务课", 1, d0100.Id, 1);
        insertCount += (i11 ? 1 : 0); updateCount += (i11 ? 0 : 1);

        // —— 财务部 D0200 → D0210 (ParentId=d0200.Id) ——
        var (_, i12) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0210", "财务课", 1, d0200.Id, 1);
        insertCount += (i12 ? 1 : 0); updateCount += (i12 ? 0 : 1);

        // —— IT 部 D0300 → D0310 (ParentId=d0300.Id) ——
        var (_, i13) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0310", "电脑课", 2, d0300.Id, 1);
        insertCount += (i13 ? 1 : 0); updateCount += (i13 ? 0 : 1);

        // —— 管理部 D0400 → 报关 / 生管 / 部管 (ParentId=d0400.Id) ——
        var (_, i14) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0410", "报关课", 1, d0400.Id, 1);
        insertCount += (i14 ? 1 : 0); updateCount += (i14 ? 0 : 1);
        
        var (_, i15) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0420", "生管课", 1, d0400.Id, 2);
        insertCount += (i15 ? 1 : 0); updateCount += (i15 ? 0 : 1);
        
        var (_, i16) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0430", "部管课", 1, d0400.Id, 3);
        insertCount += (i16 ? 1 : 0); updateCount += (i16 ? 0 : 1);

        // —— 资材部 D0500 → D0510 (ParentId=d0500.Id) ——
        var (_, i17) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0510", "采购课", 1, d0500.Id, 1);
        insertCount += (i17 ? 1 : 0); updateCount += (i17 ? 0 : 1);

        // —— 生产部 D0600 → 制造一课 / 制造二课 / 制造技术课 (ParentId=d0600.Id) ——
        var (_, i18) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0610", "制造1课", 2, d0600.Id, 1);
        insertCount += (i18 ? 1 : 0); updateCount += (i18 ? 0 : 1);
        
        var (d0620, i19) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0620", "制造2课", 2, d0600.Id, 2);
        insertCount += (i19 ? 1 : 0); updateCount += (i19 ? 0 : 1);
        
        var (_, i20) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0630", "制造技术课", 2, d0600.Id, 3);
        insertCount += (i20 ? 1 : 0); updateCount += (i20 ? 0 : 1);

        // —— 制造2课 D0620 下级：SMT / 自插 / 修正 / 手插 / 物料 / 制造2课-间接 (ParentId=d0620.Id) ——
        var (_, i21) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0621", "SMT", 2, d0620.Id, 1);
        insertCount += (i21 ? 1 : 0); updateCount += (i21 ? 0 : 1);
        
        var (_, i22) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0622", "自插", 2, d0620.Id, 2);
        insertCount += (i22 ? 1 : 0); updateCount += (i22 ? 0 : 1);
        
        var (_, i23) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0623", "修正", 2, d0620.Id, 3);
        insertCount += (i23 ? 1 : 0); updateCount += (i23 ? 0 : 1);
        
        var (_, i24) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0624", "手插", 2, d0620.Id, 4);
        insertCount += (i24 ? 1 : 0); updateCount += (i24 ? 0 : 1);
        
        var (_, i25) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0625", "物料", 1, d0620.Id, 5);
        insertCount += (i25 ? 1 : 0); updateCount += (i25 ? 0 : 1);
        
        var (_, i26) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0626", "制造2课-间接", 2, d0620.Id, 6);
        insertCount += (i26 ? 1 : 0); updateCount += (i26 ? 0 : 1);

        // —— 技术部 D0700 → D0710 (ParentId=d0700.Id) ——
        var (_, i27) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0710", "技术课", 2, d0700.Id, 1);
        insertCount += (i27 ? 1 : 0); updateCount += (i27 ? 0 : 1);

        // —— 品保部 D0800 → 受检课 / 品管课 (ParentId=d0800.Id) ——
        var (_, i28) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0810", "受检课", 2, d0800.Id, 1);
        insertCount += (i28 ? 1 : 0); updateCount += (i28 ? 0 : 1);
        
        var (_, i29) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0820", "品管课", 2, d0800.Id, 2);
        insertCount += (i29 ? 1 : 0); updateCount += (i29 ? 0 : 1);

        // —— OEM 部 D0900 → OEM QA课 / OEM管理课 (ParentId=d0900.Id) ——
        var (_, i30) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0910", "OEM QA课", 2, d0900.Id, 1);
        insertCount += (i30 ? 1 : 0); updateCount += (i30 ? 0 : 1);
        
        var (_, i31) = await CreateOrUpdateDeptAsync(repository, tenantCode, companyCode, "D0920", "OEM管理课", 1, d0900.Id, 2);
