...
public class TaktDictTypeSeedData : ITaktSeedDataCoordinator
{
    ...
    private static List<(string DictTypeCode, string DictTypeName, string Remark, int SortOrder)> GetStandardDictTypes()
    {
        return new List<(string, string, string, int)>
        {
            ...
            ("logistics_inventory_provision_scope","存货跌价计提范围","存货跌价准备计提范围（TaktInventoryImpairmentProvision.ProvisionScope）。1=按单个存货项目，2=按存货类别。对齐 CAS《存货》与 IAS 2",395),
            ...
            ("sys_reset_period_config","重置周期","编码规则流水号重置周期（与 date_format 粒度匹配，排序：none=不重置，year=按年，month=按月，day=按日，hour=按时）",126),
            ...
            ("sys_numbering_date_format_config","编码日期格式","编码规则日期格式（与 TaktNumbering.date_format 一致）。none=不使用；yyyy/yyyyMM/yyyyMMdd/yyyyMMddHH 须与 reset_period 按年/月/日/时匹配",140),
