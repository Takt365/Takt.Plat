...
public class TaktDictTypeSeedData : ITaktSeedDataCoordinator
{
    ...
    private static List<(string DictTypeCode, string DictTypeName, string Remark, int SortOrder)> GetStandardDictTypes()
    {
        return new List<(string, string, string, int)>
        {
            ...
            ("logistics_defect_category","不良区分","不良区分/检出工程（TaktAssyDefectDetail.DefectCategory、TaktPcbaRepairDetail.DefectEngineering 共用）。自插、部品、设计、修正、加工、手插、组立、SMT、其他。原编码 defect_category / assy_location。",76),
            ...
            ("logistics_pcb_location_category","PCB个所","不良个所（TaktAssyDefectDetail.DefectLocation、TaktPcbaInspectionDetail.DefectLocation 共用）。翘脚、生锡、空焊、漏件等。原编码 pcb_location / defect_location。",84),
            ...
            ("logistics_points_to_minutes_rate","点数转分钟汇率","点数转分钟汇率（TaktRoutingItem/TaktStandardOperationTime.PointsToMinutesRate）。普通=1，AI=0.028，SMT=0.045",376),
            ...
            ("routine_document_change_type","文档变更类型","文档变更类型（TaktDocumentChangeLog.ChangeType）。0=创建，1=修订，2=发布，3=归档，4=删除",303),
            ...
            ("sys_entity_change_type","实体变更类型","通用实体变更日志类型（工单/知识库/IT设备等 ChangeLog.ChangeType）。0=创建，1=更新，2=删除，3=状态变更",307),
            ...
            ("logistics_quality_standard_status","检验标准状态","检验标准/抽样方案状态（TaktInspectionStandard.StandardStatus、TaktSamplingScheme.SamplingSchemeStatus）",386),
            ("logistics_quality_inspection_item_type","检验项目类型","检验项目类型（TaktInspectionStandardItem.ItemType）",387),
            ("logistics_quality_inspection_mode","检验计数计量","检验计数/计量方式（TaktInspectionStandardItem.InspectionMode）",388),
            ("logistics_quality_sampling_scheme_type","抽样方案类型","抽样方案类型（TaktSamplingScheme.SamplingSchemeType）",389),
            ("logistics_quality_sampling_standard","抽样标准","抽样标准（TaktSamplingScheme.SamplingStandard）",390),
            ("logistics_quality_inspection_level","检验水平","检验水平（TaktSamplingScheme.InspectionLevel）",391),
            ("logistics_quality_inspection_strictness","检验严格度","检验严格度（TaktSamplingScheme.InspectionStrictness）",392),
            ("logistics_accounting_document_type","会计凭证类型","SAP 会计凭证类型（TaktSalesInvoiceItem.DocumentType）。DictValue=AA/AB/…",393),
            ("logistics_team_category","班组分类","生产班组分类。A=组立，P=PCBA，S=SMT，Q=质检，O=其他",165),
            ("logistics_prod_category","生产类别","生产类别。RD=研发，EVT=工程验证测试，DVT=设计验证测试，EPP=工程试产，PP=试产，FPP=正式生产，MP=大规模生产，RPR=维修生产，RWR=返工生产",278),
            ("logistics_prod_order_type","生产工单类型","生产工单类型。ZDTA=通常生産，ZDTB=改造改修，ZDTC=開発試作，ZDTD=通常生産PCBA，ZDTE=改造改修PCBA，ZDTF=開発試作PCBA",286),
