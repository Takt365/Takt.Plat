...
public class TaktDictTypeSeedData : ITaktSeedDataCoordinator
{
    ...
    private static List<(string DictTypeCode, string DictTypeName, string Remark, int SortOrder)> GetStandardDictTypes()
    {
        return new List<(string, string, string, int)>
        {
            ...
            ("accounting_chart_of_accounts","帐目表","科目表（TaktCompany.ChartOfAccounts；SAP KTOPL）。DictValue=INT/TEAC",415),
            ...
            ("accounting_tax_code","税码","SAP 税码 MWSKZ。CultureCode：zh-CN 中国税码、ja-JP 日本税码；GetDataDictAll 按 Accept-Language 与全局项加载",404),
            ...
            ("logistics_calculation_type","计算类型","SAP 定价计算类型 KRECH（TaktPurchasePriceItem.CalculationType）。A=百分数，B=固定金额，C=数量，…，V=数量调整；默认 A",405),
            ("logistics_scale_type","等级类型","SAP 定价等级类型 STFKZ（TaktPurchasePriceItem.ScaleType）。A=基础等级，B=到等级，C=未使用，D=累进间隔等级",406),
            ("logistics_scale_basis","等级基础","SAP 定价等级基础 KZBZG（TaktPurchasePriceItem.ScaleBasis）。B=价值等级，C=数量规模，D=毛重，…，X=保留(IS-OIL)",407),
            ...
            ("logistics_shipping_conditions","装运条件","装运条件（TaktCustomer/TaktClient.ShippingConditions；SAP VSBED）。DictValue=Z1～Z3",419),
            ("logistics_incoterms1","国际贸易条件1","国际贸易条件1（Incoterms；SAP INCO1）。DictValue=C&F/CFR/CIF/…/FOB 等；默认 FOB",420),
            ("logistics_account_assignment_group","帐户分配组","帐户分配组（TaktCustomer/TaktClient.AccountAssignmentGroup；SAP KTGRD）。DictValue=01/02/03/Y1～Y4/Z0～ZD",421),
            ("logistics_customer_pricing_procedure","客户定价过程","客户定价过程（TaktCustomer/TaktClient.CustomerPricingProcedure；SAP KALKS）。1=Standard，2=Stand Incl Sales Tax，3=Intercompany；默认 1",422),
            ...
            ("logistics_customer_group","客户组","客户组（TaktCustomer/TaktClient.CustomerGroup；SAP KDGRP）。DictValue=Z1～Z4",418),
            ...
            ("logistics_work_center_category","工作中心类别","工作中心类别（SAP VERWE）。0001=机器，0002=机器组，0003=工时，0004=工时组，0005=工厂维护，0006=项目管理，0007=生产线，0008=加工单位，0009=人员资源，0010=线，0011=地区，0012=站，0014=存储资源，0015=处理单位/存储资源",92),
