...
public class TaktDictTypeSeedData : ITaktSeedDataCoordinator
{
    ...
    private static List<(string DictTypeCode, string DictTypeName, string Remark, int SortOrder)> GetStandardDictTypes()
    {
        return new List<(string, string, string, int)>
        {
            ...
            ("accounting_financial_account_title_type","科目类型","总账科目类型（SKA1-KTOKS）。DictValue 为 X/P/S/N/C",79),
            ...
            ("accounting_financial_auxiliary_type","辅助核算类型","统驭/子账类型（SKB1-MITKZ）。DictValue 为 D/K/A/S/M",83),
            ...
            ("accounting_financial_chart_of_accounts","帐目表","科目表（TaktCompany.ChartOfAccounts；KTOPL）。DictValue=INT/TEAC",87),
            ("accounting_controlling_cost_center_category","成本中心类别","成本中心/利润中心共用分类。用于 CO 核算管理",88),
            ("accounting_controlling_cost_center_type","成本中心类型","成本中心类型（TaktCostCenter.CostCenterType）。0=成本中心，1=利润中心，2=投资中心",89),
            ("accounting_controlling_cost_element_category","成本要素类别","CSKB-KATYP 成本要素类别（CHAR2）",89),
            ("accounting_controlling_cost_element_type","成本要素类型","初级/次级成本要素（Primary/Secondary），由 KATYP 推导",90),
            ...
            ("accounting_financial_posting_period_variant","记帐期间变式","过帐期间变式（TaktCompany.PostingPeriodVariant；PERIV）。原则上一个公司对应一个变式；DictValue=0001/Z001～Z009",98),
            ...
            ("accounting_financial_tax_code","税码","税码 MWSKZ。CultureCode：zh-CN 中国税码、ja-JP 日本税码；GetDataDictAll 按 Accept-Language 与全局项加载",102),
            ...
            ("logistics_sales_account_assignment_group","帐户分配组","帐户分配组（TaktCustomer/TaktClient.AccountAssignmentGroup；KTGRD）。DictValue=01/02/03/Y1～Y4/Z0～ZD",180),
            ...
            ("logistics_procurement_calculation_type","计算类型","定价计算类型 KRECH（TaktPurchasePriceItem.CalculationType）。A=百分数，B=固定金额，C=数量，…，V=数量调整；默认 A",187),
            ...
            ("logistics_sales_customer_group","客户组","客户组（TaktCustomer/TaktClient.CustomerGroup；KDGRP）。DictValue=Z1～Z4",193),
            ("logistics_sales_customer_level","客户等级","客户/客户端等级。0=普通，1=重要，2=VIP，3=战略",194),
            ("logistics_sales_customer_pricing_procedure","客户定价过程","客户定价过程（TaktCustomer/TaktClient.CustomerPricingProcedure；KALKS）。1=Standard，2=Stand Incl Sales Tax，3=Intercompany；默认 1",195),
            ...
            ("logistics_sales_incoterms1","国际贸易条件1","国际贸易条件1（Incoterms；INCO1）。DictValue=C&F/CFR/CIF/…/FOB 等；默认 FOB",218),
            ("logistics_materials_industry_sector","行业领域","MBRSH 行业领域。A=工厂工程/装备制造，C=化工行业，M=机械工程，P=制药/医药",219),
            ...
            ("logistics_materials_material_type","物料类型","MTART 制造业常用物料类型。ROH=原材料，HALB=半成品，FERT=成品，HAWA=贸易货物，DIEN=服务，ERSA=备件，VERP=包装",241),
            ...
            ("logistics_materials_movement_type","移动类型","物料移动类型（101=GR 收货等）",243),
            ...
            ("logistics_procurement_price_type","价格条件类型","采购/价格条件类型（4 位）。PB00=采购总价（Gross Price），PR00=基本价格（Base Price），MWST=销项税/增值税，MWRK=不可抵扣进项税，NLXV=购置税",256),
            ...
            ("logistics_sales_price_type","销售价格条件类型","SD 定价条件类型 KSCHL（如 PR00 标准净价、PB00 毛价、K004 物料折扣等）",309),
            ("logistics_quality_sampling_scheme_type","物料抽样标准","抽样方案类型。用于抽样检验方案管理",311),
            ("logistics_procurement_scale","等级类型","定价等级类型 STFKZ（TaktPurchasePriceItem.ScaleType）。A=基础等级，B=到等级，C=未使用，D=累进间隔等级",312),
            ("logistics_procurement_scale_basis","等级基础","定价等级基础 KZBZG（TaktPurchasePriceItem.ScaleBasis）。B=价值等级，C=数量规模，D=毛重，…，X=保留(IS-OIL)",313),
            ...
            ("logistics_sales_shipping_conditions","装运条件","装运条件（TaktCustomer/TaktClient.ShippingConditions；VSBED）。DictValue=Z1～Z3",325),
