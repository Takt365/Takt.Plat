...
public class TaktDictTypeSeedData : ITaktSeedDataCoordinator
{
    ...
    /// 示例：sys_numbering_date_forma、logistics_nonachievement_reason_category
    ...
    private static List<(string DictTypeCode, string DictTypeName, string Remark, int SortOrder)> GetStandardDictTypes()
    {
        return new List<(string, string, string, int)>
        {
            ...
            ("accounting_account_title_type","科目类型","总账科目类型（SKA1-KTOKS）。DictValue 为 X/P/S/N/C",54),
            ("accounting_auxiliary_type","辅助核算类型","统驭/子账类型（SKB1-MITKZ）。DictValue 为 D/K/A/S/M",55),
            ...
            ("accounting_cost_element_category","成本要素类别","CSKB-KATYP 成本要素类别（CHAR2）",5),
            ("accounting_cost_element_type","成本要素类型","初级/次级成本要素（Primary/Secondary），由 KATYP 推导",51),
            ("accounting_currency_code","币种","货币类型。国际贸易和财务结算通用币种",6),
            ("accounting_chart_of_accounts","帐目表","科目表（TaktCompany.ChartOfAccounts；KTOPL）。DictValue=INT/TEAC",415),
            ("accounting_posting_period_variant","记帐期间变式","过帐期间变式（TaktCompany.PostingPeriodVariant；PERIV）。原则上一个公司对应一个变式；DictValue=0001/Z001～Z009",416),
            ("accounting_fiscal_year_variant","会计年度变式","会计年度变式（TaktCompany.FiscalYearVariant）。DictValue=K4/Z1/Z2",417),
            ("accounting_exchange_rate_type","汇率类型","汇率类型（TaktExchangeRate.ExchangeRateType）。M=平均汇率，B=银行买入价，G=历史汇率，P=计划汇率，Z=自定义汇率，E=期末汇率，K=现金余额汇率，X=平均买入价",379),
            ...
            ("accounting_budget_measure_type","预算计量类型","预算计量类型（TaktBudgetActual.MeasureType）。1=金额，2=数量",403),
            ("accounting_payment_terms_param","付款条件","付款条件。用于客户和供应商财务付款条款管理",7),
            ("accounting_tax_rate_param","税率","增值税/消费税税率。CultureCode：zh-CN 中国档位、ja-JP 日本档位；GetDataDictAll 按 Accept-Language 与全局项加载",10),
            ("accounting_tax_code","税码","税码 MWSKZ。CultureCode：zh-CN 中国税码、ja-JP 日本税码；GetDataDictAll 按 Accept-Language 与全局项加载",404),
            ("accounting_tax_category","税别","采购/销售定价税别。0=不含税，1=含税，2=免税，3=增值税专票，4=增值税普票",395),
            ("accounting_payment_method_type","支付方式","收付款方式。0=现金，1=银行转账，2=支票，3=信用证，4=其他",57),
            ("accounting_sepa_rtrans_type","SEPA拒付交易支持标识","银行信息 SEPA R-Transaction 支持标识（TaktBank.SddRtrans）。0=未知（未维护），1=同时支持 B2B 和 Core，2=仅支持 B2B，3=仅支持 Core，9=不支持",409),
            ("accounting_expense_type","费用类型","费用单类型：月结供应商除原材料外、月结货款及其他、杂项购置费用",11),
            ("logistics_allocation_category","分配类别","分配类别：A=资产，K=成本中心，F=订单（会签/采购/费用明细共用）",12),
            ...
            ("hr_political_affiliation","政治面貌","政治面貌（国家标准十三类）。0=群众，1=共青团员，2=中共党员，3=中共预备党员，4=民革党员，5=民盟盟员，6=民建会员，7=民进会员，8=农工党党员，9=致公党党员，10=九三学社社员，11=台盟盟员，12=无党派民主人士",40),
            ...
            ("logistics_equip_category","设备类别","设备分类类别。用于设备资产管理",50),
            ...
            ("logistics_industry_sector","行业领域","MBRSH 行业领域。A=工厂工程/装备制造，C=化工行业，M=机械工程，P=制药/医药",62),
            ("logistics_material_type","物料类型","MTART 制造业常用物料类型。ROH=原材料，HALB=半成品，FERT=成品，HAWA=贸易货物，DIEN=服务，ERSA=备件，VERP=包装",63),
            ...
            ("logistics_price_type","价格条件类型","采购/价格条件类型（4 位）。PB00=采购总价（Gross Price），PR00=基本价格（Base Price），MWST=销项税/增值税，MWRK=不可抵扣进项税，NLXV=购置税",66),
            ("logistics_calculation_type","计算类型","定价计算类型 KRECH（TaktPurchasePriceItem.CalculationType）。A=百分数，B=固定金额，C=数量，…，V=数量调整；默认 A",405),
            ("logistics_scale_type","等级类型","定价等级类型 STFKZ（TaktPurchasePriceItem.ScaleType）。A=基础等级，B=到等级，C=未使用，D=累进间隔等级",406),
            ("logistics_scale_basis","等级基础","定价等级基础 KZBZG（TaktPurchasePriceItem.ScaleBasis）。B=价值等级，C=数量规模，D=毛重，…，X=保留(IS-OIL)",407),
            ("logistics_pricing_date_control","定价日期控制","定价日期控制。1=采购订单日期，2=交货日期，3=当前日期，4=手动，5=收货日期；默认 1",408),
            ...
            ("logistics_mds_product_category","销售预测产品类别","销售预测四阶第 2 层产品类别（TaktSalesForecastItem.ProductCategoryCode）。CAD/ISD/PAD",421),
            ("logistics_inventory_reserve_scope","存货跌价计提范围","存货跌价准备计提范围（TaktInventoryReserve.ProvisionScope）。1=按单个存货项目，2=按存货类别。对齐 CAS《存货》与 IAS 2",395),
            ...
            ("logistics_movement_type","移动类型","物料移动类型（101=GR 收货等）",153),
            ...
            ("logistics_shipping_conditions","装运条件","装运条件（TaktCustomer/TaktClient.ShippingConditions；VSBED）。DictValue=Z1～Z3",419),
            ("logistics_incoterms1","国际贸易条件1","国际贸易条件1（Incoterms；INCO1）。DictValue=C&F/CFR/CIF/…/FOB 等；默认 FOB",420),
            ("logistics_account_assignment_group","帐户分配组","帐户分配组（TaktCustomer/TaktClient.AccountAssignmentGroup；KTGRD）。DictValue=01/02/03/Y1～Y4/Z0～ZD",421),
            ("logistics_customer_pricing_procedure","客户定价过程","客户定价过程（TaktCustomer/TaktClient.CustomerPricingProcedure；KALKS）。1=Standard，2=Stand Incl Sales Tax，3=Intercompany；默认 1",422),
            ...
            ("logistics_customer_group","客户组","客户组（TaktCustomer/TaktClient.CustomerGroup；KDGRP）。DictValue=Z1～Z4",418),
            ...
            ("logistics_sales_price_type","销售价格条件类型","SD 定价条件类型 KSCHL（如 PR00 标准净价、PB00 毛价、K004 物料折扣等）",171),
            ...
            ("logistics_nonachievement_reason_category","未达成原因","未达成原因。清机、测试慢/测试修理机、修理试机、转机、人员欠缺、部品不良/欠料、ST差异大、仪器设备/设置/调试/检查/故障、请假/旷工、其他、切换机种/仕向、作业困难、组立慢、下机慢、加工多、升级慢、改修、坏机多/不良多、人员借调、返工、工程复杂/下机慢、学习中/新人员学习/开会、正常",83),
            ...
            ("logistics_pcba_function_category","PCBA功能类别","PCBA 功能类别。电源、前板、IO板等。原编码 pcba_function_category。",87),
            ...
            ("logistics_stop_reason_category","停线原因","停线原因。切换停止时间、切换机种、早会、班会、周会、学习、组立、读工程表、程序烧录错误、拆机、仪设、清洁、欠料、停电、消防演习、其他",90),
            ...
            ("logistics_work_center_category","工作中心类别","工作中心类别（VERWE）。0001=机器，0002=机器组，0003=工时，0004=工时组，0005=工厂维护，0006=项目管理，0007=生产线，0008=加工单位，0009=人员资源，0010=线，0011=地区，0012=站，0014=存储资源，0015=处理单位/存储资源",92),
            ("sys_culture_code","区域文化编码","BCP47 区域文化编码（与 sys_country_code 一对一，约 250 项）。DictValue=大写 BCP47（如 JA-JP）；DictLabel=本族语；Remark=区域文化编码.中文；ExtLabel=英文语言名；ExtValue=JSON{countryCode,language}。UI 翻译包仍以 TaktCulture 为准",93),
            ("sys_admin_division_level_type","行政区划层级","世界通用六级行政区划（TaktAdminDivision.Level）。1=国家，2=州省，3=地市，4=区县，5=乡镇街道，6=行政村",413),
            ("sys_country_code","国家地区","国家/地区（ISO 3166-1 alpha-2，含非主权地区/海外属地等约 250 项）。DictValue=代码；DictLabel=本国语言官方名；Remark=国家地区.中文名；ExtLabel=英文名；ExtValue=JSON{phoneCode,currencyCode,language}",414),
            ...
            ("sys_archive_key_kind","归档键类型","数据表归档键类型（TaktTableArchive.ArchiveKeyKind）。yyyyMMddHHmmss/yyyyMM/yyyy 等",96),
            ("sys_dept_type","部门类型","部门类型。0=直接，1=间接",97),
            ("sys_flow_category","流程分类","流程分类。0=通用流程，1=业务流程，2=系统流程",98),
            ("sys_flow_status","流程状态","流程实例运行状态（TaktFlowInstance.InstanceStatus）。0=运行中，1=已完成，2=已驳回，3=已挂起，4=已终止，5=草稿",99),
            ("sys_flow_add_sign_type","加签方式","流程加签审批方式（TaktFlowAddSign.SignType；与引擎 approveType 一致）。sequential=顺序加签，all=会签加签，one=或签加签",100),
            ("sys_flow_suspension_state","流程定义挂起状态","流程定义挂起状态（TaktFlowScheme.SuspensionState）。1=激活，2=挂起",101),
            ("sys_flow_task_status","流程任务状态","流程用户任务状态（TaktFlowTask.TaskStatus）。0=待办，1=已完成，2=已取消",102),
            ("sys_flow_sign_type","会签类型","流程任务会签类型（TaktFlowTask.SignType）。1=或签，2=会签",103),
            ("sys_flow_action_type","流程动作类型","流程流转动作类型（TaktFlowTransition.ActionType）。0=发起，1=通过，2=驳回，3=撤回，4=转办，5=加签，6=减签，7=挂起，8=恢复，9=终止，10=抄送",104),
            ("sys_flow_variable_type","流程变量类型","流程变量类型（TaktFlowVariable.VariableType）。0=字符串，1=长整型，2=双精度，3=布尔，4=JSON",105),
            ...
            ("sys_builtin_type","内置","内置标志。1=是/内置，0=否/自定义",103),
            ("sys_default_type","是否默认","通用默认标志。1=是/默认，0=否/非默认",104),
            ("sys_public_type","公开","公开标志。0=公开，1=私有",105),
            ...
            ("sys_style_type​","字典样式类别","字典标签 ListClass/CssClass（0-69），与 frontend dict-tag-base.css 一致",107),
            ...
            ("sys_message_group","消息分组","消息分组。Collaboration=协同，Message=消息，Reminder=提醒",111),
            ("sys_message_type","消息类型","消息类型。Text=文本，System=系统，Multimedia=多媒体",112),
            ("sys_news_type","新闻分类","新闻分类。0=公司新闻，1=行业动态，2=技术分享，3=产品发布，4=活动资讯，5=其他",113),
            ...
            ("sys_ticket_status","工单状态","通用工单状态（字典 sys_ticket_status；TaktTicket.TicketStatus、TaktCustomerServiceTicket.TicketStatus、TaktMaintenanceWorkOrder.WorkOrderStatus 等共用）。0=新建，1=已分配，2=处理中，3=待确认，4=已完成，5=已关闭，6=已取消，7=重新打开",116),
            ...
            ("sys_oss_provider","OSS提供商类型","OSS对象存储提供商类型。aliyun=阿里云OSS，tencent=腾讯云COS，huawei=华为云OBS，aws=AWS S3",120),
            ("sys_post_category","岗位类别","岗位类别（与 TaktPost.PostCategory 一致）。MGT=管理岗，PRO=专业岗，TEC=技术岗，SUP=支持岗，OPS=操作岗",121),
            ("sys_post_level","岗位职级","岗位职级（与 TaktPost.PostLevel 一致）。P1~P4 专业序列，M1~M5 管理序列",122),
            ("sys_priority_level","优先级","优先级（字典 sys_priority_level）。1=最高，2=高，3=普通，4=低",123),
            ("sys_publish_scope","发布范围","发布范围（字典 sys_publish_scope；TaktAnnouncement/TaktDocument/TaktNews.TargetScope 共用）。0=全部，1=指定部门，2=指定用户",124),
            ("sys_read_status","读取状态","读取状态。0=未读，1=已读",125),
            ("sys_reset_period","重置周期","编码规则流水号重置周期（与 date_format 粒度匹配；DictValue：None|Annually|Monthly|Daily）",126),
            ...
            ("sys_setting_group","设置分组","设置分组。backend=后端，frontend=前端",129),
            ("sys_sort_type","排序类型","排序类型。asc=升序，desc=降序",130),
            ("sys_storage_naming","存储命名规则","存储命名规则。0=原文件+哈希值，1=自动生成，2=自定义",131),
            ("sys_storage_type","存储方式","存储方式。0=本地存储，1=OSS对象存储，2=FTP，3=其他",132),
            ("sys_urgency_level","紧急度","紧急度 Urgency（字典 sys_urgency_level）。1=High/高，2=Medium/中，3=Low/低",133),
            ("sys_impact_level","影响范围","影响范围 Impact（字典 sys_impact_level）。1=High/高，2=Medium/中，3=Low/低",134),
            ("sys_gender_type","用户性别","用户性别。0=未知，1=男，2=女",135),
            ...
            ("sys_word_filter_level","敏感词过滤等级","敏感词过滤等级（与 Takt.Domain.Entities.Foundation.TaktVocabulary.FilterLevel 一致，字典 sys_word_filter_level）。1=低，2=中，3=高",138),
            ("sys_yes_no","是否","通用布尔标志。1=是/启用，0=否/禁用",139),
            ("sys_numbering_date_forma","编码日期格式","编码规则日期格式（与 TaktNumbering.date_format 一致）。none=不使用；yyyy/yyyyMM/yyyyMMdd/yyyyMMddHH 须与 reset_period（None|Annually|Monthly|Daily）匹配",140),
            ("routine_ticket_source","工单来源","服务台工单来源。0=门户，1=邮件，2=电话，3=API",142),
            ...
            ("sys_attachment_file","附件文件类型","通用多媒体附件类型（字典 sys_attachment_file；SOP/新闻/公告/通知/文管等共用）。1=图片，2=视频，3=文档",146),
            ...
            ("hr_employee_address_type","员工地址类型","员工地址类型（TaktEmployeeAddress.AddressType）。1=家庭，2=工作，3=常住",178),
            ...
            ("logistics_service_ticket_type","服务工单类型","服务工单类型（TaktCustomerServiceTicket.TicketType）。0=维修，1=巡检，2=安装，3=升级，4=其他",309),
            ("logistics_service_contract_type","服务合同类型","服务合同类型（TaktCustomerServiceContract.ContractType）。0=维保，1=单次，2=框架，3=SLA，4=其他",310),
            ("logistics_service_contract_status","服务合同状态","服务合同状态（TaktCustomerServiceContract.ContractStatus）。0=草稿，1=生效，2=暂停，3=到期，4=终止",311),
            ("logistics_service_payment_terms","服务付款条件","服务合同/订单付款条件（TaktCustomerServiceContract.PaymentTerms）。0=预付，1=后付，2=月结30天，3=月结60天，4=其他",312),
            ("logistics_service_order_type","服务订单类型","服务订单类型（TaktCustomerServiceOrder.OrderType）。0=现场服务，1=远程支持，2=备件更换，3=安装调试，4=其他",313),
            ("logistics_service_order_status","服务订单状态","服务订单状态（TaktCustomerServiceOrder.OrderStatus）。0=草稿，1=已确认，2=执行中，3=已完成，4=已结算，5=已取消",314),
            ("logistics_service_request_type","服务请求类型","服务请求类型（TaktCustomerServiceRequest.RequestType）。0=咨询，1=报修，2=投诉，3=安装，4=巡检，5=其他",315),
            ("logistics_service_source_channel","服务请求来源","服务请求来源渠道（TaktCustomerServiceRequest.SourceChannel）。0=电话，1=邮件，2=门户，3=现场，4=其他",316),
            ("logistics_service_request_status","服务请求状态","服务请求状态（TaktCustomerServiceRequest.RequestStatus）。0=草稿，1=已提交，2=处理中，3=已完成，4=已关闭，5=已取消",317),
            ("logistics_acceptance_result","验收结果","服务验收结果（TaktCustomerServiceTicket.AcceptanceResult）。0=不合格，1=合格，2=部分合格",318),
            ...
            ("logistics_accounting_document_type","会计凭证类型","会计凭证类型（TaktSalesInvoiceItem.DocumentType）。DictValue=AA/AB/…",394),
            ...
            ("logistics_team_category","班组分类","生产班组分类。A=组立，P=PCBA，Q=质检，O=其他",165),
            ...
            ("sys_job_run_status","作业运行状态","归档/备份等作业运行状态（TaktArchiveLog.RunStatus、TaktBackupLog.RunStatus）。0=进行中，1=成功，2=失败",170),
            ("sys_success_fail_status","成功失败状态","通用成功/失败结果（TaktOperLog.OperStatus、TaktQuartzLog.ExecuteStatus）。0=失败，1=成功",171),
            ("sys_tracking_level","追踪级别","前端交互追踪级别（TaktTrackingLog.TrackingLevel）。1=warn，2=error",172),
            ("sys_backup_sync_mode","备份同步模式","备份同步模式快照（TaktBackupLog.SyncMode）。1=完整，2=增量",173),
            ("sys_backup_execute_mode","备份执行方式","备份执行方式快照（TaktBackupLog.ExecuteMode）。1=立即，2=后台",174),
            ("sys_backup_path_type","备份路径类型","备份路径类型快照（TaktBackupLog.PathType）。0=无，1=本地，2=网络，3=FTP",175),
            ("sys_configurable_join_type","报表关联类型","自定义报表 JOIN 类型（TaktConfigurableJoin.JoinType）。1=内连接，2=左连接，3=右连接，4=全连接",176),
            ("sys_configurable_aggregate_func","报表聚合函数","自定义报表聚合函数（TaktConfigurableField.AggregateFunc）。0=无，1=COUNT，2=SUM，3=AVG，4=MIN，5=MAX",177),
            ("sys_configurable_sort_direction","报表排序方向","自定义报表排序方向（TaktConfigurableOrderBy.SortDirection）。1=升序，2=降序",178),
