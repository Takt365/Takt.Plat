...
public class TaktGenTableCreateValidator : AbstractValidator<TaktGenTableCreateDto>
{
    ...
    public TaktGenTableCreateValidator()
    {
        ...
        RuleFor(x => x.TableName)
            ...
            .MaximumLength(40).WithMessage("数据表名称长度不能超过40个字符");
        RuleFor(x => x.TableComment)
            .MaximumLength(500).WithMessage("表描述长度不能超过500个字符");
        RuleFor(x => x.SubTableName)
            .MaximumLength(40).WithMessage("关联父表名长度不能超过40个字符");
        RuleFor(x => x.SubTableFkName)
            .MaximumLength(40).WithMessage("本表关联父表的外键名长度不能超过40个字符");
        RuleFor(x => x.TreeCode)
            .MaximumLength(40).WithMessage("树编码字段长度不能超过40个字符");
        RuleFor(x => x.TreeParentCode)
            .MaximumLength(40).WithMessage("树父编码字段长度不能超过40个字符");
        RuleFor(x => x.TreeName)
            .MaximumLength(40).WithMessage("树名称字段长度不能超过40个字符");
        ...
        RuleFor(x => x.GenModuleName)
            .MaximumLength(40).WithMessage("模块名长度不能超过40个字符");
        RuleFor(x => x.GenBusinessName)
            .NotEmpty().WithMessage("业务名不能为空")
            .MaximumLength(40).WithMessage("业务名长度不能超过40个字符");
        RuleFor(x => x.GenFunctionName)
            .MaximumLength(40).WithMessage("功能名长度不能超过40个字符");
...
public class TaktGenTableImportValidator : AbstractValidator<TaktGenTableImportDto>
{
    ...
    public TaktGenTableImportValidator()
    {
        ...
        RuleFor(x => x.SubTableName)
            .MaximumLength(40).WithMessage("关联父表名长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.SubTableName));
        RuleFor(x => x.SubTableFkName)
            .MaximumLength(40).WithMessage("本表关联父表的外键名长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.SubTableFkName));
        RuleFor(x => x.TreeCode)
            .MaximumLength(40).WithMessage("树编码字段长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.TreeCode));
        RuleFor(x => x.TreeParentCode)
            .MaximumLength(40).WithMessage("树父编码字段长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.TreeParentCode));
        RuleFor(x => x.TreeName)
            .MaximumLength(40).WithMessage("树名称字段长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.TreeName));
        RuleFor(x => x.GenTemplateCategory)
            .NotEmpty().WithMessage("生成模板类型不能为空")
            .MaximumLength(50).WithMessage("生成模板类型长度不能超过50个字符");
        RuleFor(x => x.GenModuleName)
            .MaximumLength(40).WithMessage("模块名长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.GenModuleName));
        RuleFor(x => x.GenBusinessName)
            .NotEmpty().WithMessage("业务名不能为空")
            .MaximumLength(40).WithMessage("业务名长度不能超过40个字符");
        RuleFor(x => x.ExtFieldJson)
