...
public class TaktConfigurableFieldCreateValidator : AbstractValidator<TaktConfigurableFieldCreateDto>
{
    ...
    public TaktConfigurableFieldCreateValidator()
    {
        ...
        RuleFor(x => x.OutputAlias)
            .MaximumLength(128).WithMessage("输出别名长度不能超过128个字符");
        RuleFor(x => x.AggregateFunc)
            .IsInEnum().WithMessage("聚合函数无效");
        RuleFor(x => x.IsVisible)
            .IsInEnum().WithMessage("是否输出无效");
...
public class TaktConfigurableFieldImportValidator : AbstractValidator<TaktConfigurableFieldImportDto>
{
    ...
    public TaktConfigurableFieldImportValidator()
    {
        ...
        RuleFor(x => x.DisplayName)
            ...
            .MaximumLength(40).WithMessage("显示名称长度不能超过40个字符");
        RuleFor(x => x.OutputAlias)
            .MaximumLength(128).WithMessage("输出别名长度不能超过128个字符").When(x => !string.IsNullOrWhiteSpace(x.OutputAlias));
        RuleFor(x => x.AggregateFunc)
            .IsInEnum().WithMessage("聚合函数无效");
        RuleFor(x => x.IsVisible)
            .IsInEnum().WithMessage("是否输出无效");
