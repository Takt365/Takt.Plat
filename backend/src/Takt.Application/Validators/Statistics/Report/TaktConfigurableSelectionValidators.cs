...
public class TaktConfigurableSelectionCreateValidator : AbstractValidator<TaktConfigurableSelectionCreateDto>
{
    ...
    public TaktConfigurableSelectionCreateValidator()
    {
        ...
        RuleFor(x => x.DisplayName)
            ...
            .MaximumLength(40).WithMessage("显示名称长度不能超过40个字符");
        RuleFor(x => x.FilterOperator)
            .IsInEnum().WithMessage("比较运算符无效");
        RuleFor(x => x.DefaultValue)
            .MaximumLength(500).WithMessage("默认值长度不能超过500个字符");
        RuleFor(x => x.DefaultValueTo)
            .MaximumLength(500).WithMessage("区间结束值长度不能超过500个字符");
        RuleFor(x => x.IsRequired)
            .IsInEnum().WithMessage("是否必填无效");
...
public class TaktConfigurableSelectionImportValidator : AbstractValidator<TaktConfigurableSelectionImportDto>
{
    ...
    public TaktConfigurableSelectionImportValidator()
    {
        ...
        RuleFor(x => x.FilterOperator)
            .IsInEnum().WithMessage("比较运算符无效");
        RuleFor(x => x.DefaultValue)
            .MaximumLength(500).WithMessage("默认值长度不能超过500个字符").When(x => !string.IsNullOrWhiteSpace(x.DefaultValue));
        RuleFor(x => x.DefaultValueTo)
            .MaximumLength(500).WithMessage("区间结束值长度不能超过500个字符").When(x => !string.IsNullOrWhiteSpace(x.DefaultValueTo));
        RuleFor(x => x.IsRequired)
            .IsInEnum().WithMessage("是否必填无效");
        RuleFor(x => x.SortOrder)
            .GreaterThanOrEqualTo(0).WithMessage("排序号不能为负数");
