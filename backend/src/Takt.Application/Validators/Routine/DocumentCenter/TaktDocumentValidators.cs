...
public class TaktDocumentCreateValidator : AbstractValidator<TaktDocumentCreateDto>
{
    ...
    public TaktDocumentCreateValidator()
    {
        ...
        RuleFor(x => x.DocumentCategory)
            .IsInEnum().WithMessage("文档分类无效");
        RuleFor(x => x.DocumentStatus)
            .IsInEnum().WithMessage("文档状态无效");
        RuleFor(x => x.ConfidentialLevel)
            .IsInEnum().WithMessage("密级无效");
        RuleFor(x => x.Summary)
            .MaximumLength(2000).WithMessage("文档摘要长度不能超过2000个字符");
        RuleFor(x => x.Tags)
        ...
        RuleFor(x => x.FileType)
            .MaximumLength(100).WithMessage("当前文件类型长度不能超过100个字符");
        RuleFor(x => x.FileExtension)
            .MaximumLength(20).WithMessage("当前文件扩展名长度不能超过20个字符");
...
public class TaktDocumentImportValidator : AbstractValidator<TaktDocumentImportDto>
{
    ...
    public TaktDocumentImportValidator()
    {
        ...
        RuleFor(x => x.ConfidentialLevel)
            .IsInEnum().WithMessage("密级无效");
        RuleFor(x => x.Summary)
            .MaximumLength(2000).WithMessage("文档摘要长度不能超过2000个字符").When(x => !string.IsNullOrWhiteSpace(x.Summary));
        RuleFor(x => x.Tags)
