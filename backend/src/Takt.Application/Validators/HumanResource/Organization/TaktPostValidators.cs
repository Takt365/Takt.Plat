...
public class TaktPostCreateValidator : AbstractValidator<TaktPostCreateDto>
{
    ...
    public TaktPostCreateValidator()
    {
        ...
        RuleFor(x => x.PostCode)
            ...
            .MaximumLength(40).WithMessage("岗位编码长度不能超过40个字符");
        RuleFor(x => x.PostName)
            .NotEmpty().WithMessage("岗位名称不能为空")
            .MaximumLength(40).WithMessage("岗位名称长度不能超过40个字符");
        RuleFor(x => x.DeptId)
            .GreaterThanOrEqualTo(0).WithMessage("所属部门ID不能为负数");
        RuleFor(x => x.PostType)
            .IsInEnum().WithMessage("岗位类型无效");
        RuleFor(x => x.PostLevel)
            .IsInEnum().WithMessage("岗位职级无效");
...
public class TaktPostImportValidator : AbstractValidator<TaktPostImportDto>
{
    ...
    public TaktPostImportValidator()
    {
        ...
        RuleFor(x => x.PostLevel)
            .IsInEnum().WithMessage("岗位职级无效");
        RuleFor(x => x.Responsibilities)
            .MaximumLength(2000).WithMessage("岗位职责长度不能超过2000个字符").When(x => !string.IsNullOrWhiteSpace(x.Responsibilities));
        RuleFor(x => x.Requirements)
            .MaximumLength(2000).WithMessage("任职要求长度不能超过2000个字符").When(x => !string.IsNullOrWhiteSpace(x.Requirements));
        RuleFor(x => x.PostStatus)
            .IsInEnum().WithMessage("状态无效");
        RuleFor(x => x.ExtFieldJson)
