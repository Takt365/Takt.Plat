...
public class TaktPayrollCreateValidator : AbstractValidator<TaktPayrollCreateDto>
{
    ...
    public TaktPayrollCreateValidator()
    {
        ...
        RuleFor(x => x.PayScaleId)
            .GreaterThanOrEqualTo(0).WithMessage("关联薪级表 ID不能为负数");
        RuleFor(x => x.FormulaSetCode)
            .MaximumLength(40).WithMessage("默认公式方案编码长度不能超过40个字符");
        RuleFor(x => x.Description)
            .MaximumLength(500).WithMessage("说明长度不能超过500个字符");
...
public class TaktPayrollImportValidator : AbstractValidator<TaktPayrollImportDto>
{
    ...
    public TaktPayrollImportValidator()
    {
        ...
        RuleFor(x => x.PayScaleId)
            .GreaterThanOrEqualTo(0).WithMessage("关联薪级表 ID不能为负数");
        RuleFor(x => x.FormulaSetCode)
            .MaximumLength(40).WithMessage("默认公式方案编码长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.FormulaSetCode));
        RuleFor(x => x.Description)
            .MaximumLength(500).WithMessage("说明长度不能超过500个字符").When(x => !string.IsNullOrWhiteSpace(x.Description));
        RuleFor(x => x.RelatedPlant)
