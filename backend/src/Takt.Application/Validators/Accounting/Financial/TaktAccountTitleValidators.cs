...
public class TaktAccountTitleCreateValidator : AbstractValidator<TaktAccountTitleCreateDto>
{
    ...
    public TaktAccountTitleCreateValidator()
    {
        ...
        RuleFor(x => x.TitleCode)
            ...
            .MaximumLength(40).WithMessage("科目编码长度不能超过40个字符");
        RuleFor(x => x.TitleName)
            .NotEmpty().WithMessage("科目名称不能为空")
            .MaximumLength(40).WithMessage("科目名称长度不能超过40个字符");
        RuleFor(x => x.ShortName)
            .MaximumLength(40).WithMessage("科目简称长度不能超过40个字符");
        RuleFor(x => x.TitleDesc)
            .MaximumLength(200).WithMessage("科目描述长度不能超过200个字符");
...
public class TaktAccountTitleImportValidator : AbstractValidator<TaktAccountTitleImportDto>
{
    ...
    public TaktAccountTitleImportValidator()
    {
        ...
        RuleFor(x => x.TitleName)
            ...
            .MaximumLength(40).WithMessage("科目名称长度不能超过40个字符");
        RuleFor(x => x.ShortName)
            .MaximumLength(40).WithMessage("科目简称长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.ShortName));
        RuleFor(x => x.TitleDesc)
            .MaximumLength(200).WithMessage("科目描述长度不能超过200个字符").When(x => !string.IsNullOrWhiteSpace(x.TitleDesc));
        RuleFor(x => x.ParentId)
            .GreaterThanOrEqualTo(0).WithMessage("父级 ID不能为负数");
        RuleFor(x => x.ExtFieldJson)
