...
public class TaktQuartzTaskCreateValidator : AbstractValidator<TaktQuartzTaskCreateDto>
{
    ...
    public TaktQuartzTaskCreateValidator()
    {
        ...
        RuleFor(x => x.TriggerType)
            .InclusiveBetween(0, 1).WithMessage("触发器类型无效");
        ...
        RuleFor(x => x.TaskStatus)
            .InclusiveBetween(0, 1).WithMessage("任务状态无效");
        RuleFor(x => x.Concurrent)
            .InclusiveBetween(0, 1).WithMessage("是否允许并发执行无效");
        RuleFor(x => x.MisfirePolicy)
            .InclusiveBetween(0, 3).WithMessage("Misfire 策略无效");
