...
public class TaktPackagingCreateValidator : AbstractValidator<TaktPackagingCreateDto>
{
    ...
    public TaktPackagingCreateValidator()
    {
        ...
        RuleFor(x => x.HsCode)
            .MaximumLength(40).WithMessage("海关商品编码长度不能超过40个字符");
        RuleFor(x => x.HsName)
            .MaximumLength(40).WithMessage("商品名称长度不能超过40个字符");
        RuleFor(x => x.AdditionalCode)
            .MaximumLength(40).WithMessage("附加编码长度不能超过40个字符");
        RuleFor(x => x.OriginCountryRegionCode)
            .MaximumLength(40).WithMessage("原产国/地区编码长度不能超过40个字符");
        RuleFor(x => x.OriginCountryRegionName)
            .MaximumLength(40).WithMessage("原产国/地区名称长度不能超过40个字符");
        RuleFor(x => x.DestinationCountryRegionCode)
            .MaximumLength(40).WithMessage("目的国/地区编码长度不能超过40个字符");
        RuleFor(x => x.DestinationCountryRegionName)
            .MaximumLength(40).WithMessage("目的国/地区名称长度不能超过40个字符");
        RuleFor(x => x.RegulatoryConditionCode)
            .MaximumLength(40).WithMessage("监管条件代码长度不能超过40个字符");
...
public class TaktPackagingImportValidator : AbstractValidator<TaktPackagingImportDto>
{
    ...
    public TaktPackagingImportValidator()
    {
        ...
        RuleFor(x => x.PlantCode)
            .MaximumLength(40).WithMessage("工厂代码长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.PlantCode));
        RuleFor(x => x.MaterialCode)
            .NotEmpty().WithMessage("物料编码不能为空")
            .MaximumLength(40).WithMessage("物料编码长度不能超过40个字符");
        RuleFor(x => x.HsCode)
            .MaximumLength(40).WithMessage("海关商品编码长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.HsCode));
        RuleFor(x => x.HsName)
            .MaximumLength(40).WithMessage("商品名称长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.HsName));
        RuleFor(x => x.AdditionalCode)
            .MaximumLength(40).WithMessage("附加编码长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.AdditionalCode));
        RuleFor(x => x.OriginCountryRegionCode)
            .MaximumLength(40).WithMessage("原产国/地区编码长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.OriginCountryRegionCode));
        RuleFor(x => x.OriginCountryRegionName)
            .MaximumLength(40).WithMessage("原产国/地区名称长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.OriginCountryRegionName));
        RuleFor(x => x.DestinationCountryRegionCode)
            .MaximumLength(40).WithMessage("目的国/地区编码长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.DestinationCountryRegionCode));
        RuleFor(x => x.DestinationCountryRegionName)
            .MaximumLength(40).WithMessage("目的国/地区名称长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.DestinationCountryRegionName));
        RuleFor(x => x.RegulatoryConditionCode)
            .MaximumLength(40).WithMessage("监管条件代码长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.RegulatoryConditionCode));
        RuleFor(x => x.TariffRateType)
