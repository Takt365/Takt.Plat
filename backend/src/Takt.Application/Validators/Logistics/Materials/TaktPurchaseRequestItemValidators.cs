...
public class TaktPurchaseRequestItemCreateValidator : AbstractValidator<TaktPurchaseRequestItemCreateDto>
{
    ...
    public TaktPurchaseRequestItemCreateValidator()
    {
        ...
        RuleFor(x => x.PurchaseRequestId)
            .GreaterThanOrEqualTo(0).WithMessage("采购申请ID不能为负数");
        RuleFor(x => x.PurchaseRequestCode)
            .NotEmpty().WithMessage("采购申请编码不能为空")
            .MaximumLength(40).WithMessage("采购申请编码长度不能超过40个字符");
        ...
        RuleFor(x => x.ReferenceSupplierCode)
            .MaximumLength(40).WithMessage("参考供应商编码长度不能超过40个字符");
        RuleFor(x => x.ReferenceSupplierName)
            .MaximumLength(40).WithMessage("参考供应商名称长度不能超过40个字符");
...
public class TaktPurchaseRequestItemImportValidator : AbstractValidator<TaktPurchaseRequestItemImportDto>
{
    ...
    public TaktPurchaseRequestItemImportValidator()
    {
        ...
        RuleFor(x => x.RequestUnit)
            ...
            .MaximumLength(20).WithMessage("申请单位长度不能超过20个字符");
        RuleFor(x => x.ReferenceSupplierCode)
            .MaximumLength(40).WithMessage("参考供应商编码长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.ReferenceSupplierCode));
        RuleFor(x => x.ReferenceSupplierName)
            .MaximumLength(40).WithMessage("参考供应商名称长度不能超过40个字符").When(x => !string.IsNullOrWhiteSpace(x.ReferenceSupplierName));
        RuleFor(x => x.ExtFieldJson)
