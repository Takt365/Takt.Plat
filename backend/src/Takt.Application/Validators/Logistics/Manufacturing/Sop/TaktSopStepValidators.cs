...
public class TaktSopStepCreateValidator : AbstractValidator<TaktSopStepCreateDto>
{
    ...
    public TaktSopStepCreateValidator()
    {
        ...
        RuleFor(x => x.StepDescription)
            .MaximumLength(4000).WithMessage("作业说明长度不能超过4000个字符");
        RuleFor(x => x.SafetyAlert)
            .MaximumLength(500).WithMessage("安全警示长度不能超过500个字符");
...
public class TaktSopStepImportValidator : AbstractValidator<TaktSopStepImportDto>
{
    ...
    public TaktSopStepImportValidator()
    {
        ...
        RuleFor(x => x.StepTitle)
            ...
            .MaximumLength(200).WithMessage("工步标题长度不能超过200个字符");
        RuleFor(x => x.StepDescription)
            .MaximumLength(4000).WithMessage("作业说明长度不能超过4000个字符").When(x => !string.IsNullOrWhiteSpace(x.StepDescription));
        RuleFor(x => x.SafetyAlert)
            .MaximumLength(500).WithMessage("安全警示长度不能超过500个字符").When(x => !string.IsNullOrWhiteSpace(x.SafetyAlert));
        RuleFor(x => x.ExtField)
