...
public class TaktPersonnelOperationRateCreateValidator : AbstractValidator<TaktPersonnelOperationRateCreateDto>
{
    ...
    public TaktPersonnelOperationRateCreateValidator()
    {
        ...
        RuleFor(x => x.ProductionLineName)
            .MaximumLength(100).WithMessage("生产线名称长度不能超过100个字符");
        RuleFor(x => x.IdleReason)
            .MaximumLength(500).WithMessage("空闲原因描述长度不能超过500个字符");
        RuleFor(x => x.TeamLeader)
            .MaximumLength(50).WithMessage("班组长长度不能超过50个字符");
        RuleFor(x => x.Supervisor)
            .MaximumLength(50).WithMessage("主管长度不能超过50个字符");
...
public class TaktPersonnelOperationRateImportValidator : AbstractValidator<TaktPersonnelOperationRateImportDto>
{
    ...
    public TaktPersonnelOperationRateImportValidator()
    {
        ...
        RuleFor(x => x.ProductionLine)
            ...
            .MaximumLength(20).WithMessage("生产线长度不能超过20个字符");
        RuleFor(x => x.ProductionLineName)
            .MaximumLength(100).WithMessage("生产线名称长度不能超过100个字符").When(x => !string.IsNullOrWhiteSpace(x.ProductionLineName));
        RuleFor(x => x.ExtFieldJson)
