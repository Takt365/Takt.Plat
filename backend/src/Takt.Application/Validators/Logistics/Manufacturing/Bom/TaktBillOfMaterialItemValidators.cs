...
public class TaktBillOfMaterialItemCreateValidator : AbstractValidator<TaktBillOfMaterialItemCreateDto>
{
    ...
    public TaktBillOfMaterialItemCreateValidator()
    {
        ...
        RuleFor(x => x.BomCode)
            ...
            .MaximumLength(40).WithMessage("BOM编码长度不能超过40个字符");
        RuleFor(x => x.MaterialId)
            .GreaterThanOrEqualTo(0).WithMessage("子项物料ID不能为负数");
        RuleFor(x => x.MaterialCode)
            .NotEmpty().WithMessage("子项物料编码不能为空")
            .MaximumLength(40).WithMessage("子项物料编码长度不能超过40个字符");
        ...
        RuleFor(x => x.Position)
            .MaximumLength(200).WithMessage("位号长度不能超过200个字符");
        RuleFor(x => x.SubstituteGroup)
            .MaximumLength(10).WithMessage("替代组号长度不能超过10个字符");
...
public class TaktBillOfMaterialItemImportValidator : AbstractValidator<TaktBillOfMaterialItemImportDto>
{
    ...
    public TaktBillOfMaterialItemImportValidator()
    {
        ...
        RuleFor(x => x.Position)
            .MaximumLength(200).WithMessage("位号长度不能超过200个字符").When(x => !string.IsNullOrWhiteSpace(x.Position));
        RuleFor(x => x.SubstituteGroup)
            .MaximumLength(10).WithMessage("替代组号长度不能超过10个字符").When(x => !string.IsNullOrWhiteSpace(x.SubstituteGroup));
        RuleFor(x => x.ExtFieldJson)
