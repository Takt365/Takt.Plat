...
public class TaktServiceTicketCreateValidator : AbstractValidator<TaktServiceTicketCreateDto>
{
    ...
    public TaktServiceTicketCreateValidator()
    {
        ...
        RuleFor(x => x.FaultDescription)
            .MaximumLength(2000).WithMessage("故障/问题描述长度不能超过2000个字符");
        RuleFor(x => x.SolutionDescription)
            .MaximumLength(2000).WithMessage("处理方案/解决说明长度不能超过2000个字符");
        RuleFor(x => x.ServiceLocation)
            .MaximumLength(500).WithMessage("服务地点长度不能超过500个字符");
