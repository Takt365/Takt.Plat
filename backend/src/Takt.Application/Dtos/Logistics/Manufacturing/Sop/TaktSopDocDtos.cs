...
public class TaktSopDocCreateDto
{
    ...
    /// 默认语言（zh-CN 简体 / en-US 英文 / ja-JP 日文 / zh-HK 香港繁体；与 TaktCulture.CultureCode 一致）
    /// </summary>
    [Required(ErrorMessage = "默认语言（zh-CN 简体 / en-US 英文 / ja-JP 日文 / zh-HK 香港繁体；与 TaktCulture.CultureCode 一致）不能为空")]
    public string DefaultLang { get; set; } = string.Empty;
...
public class TaktSopDocStatusDto
{
    ...
    /// 状态（字典 sys_normal_disable_status，0=停用，1=正常）
    /// </summary>
    [Required(ErrorMessage = "状态（字典 sys_normal_disable_status，0=停用，1=正常）不能为空")]
    public int SopStatus { get; set; } = 0;
}
