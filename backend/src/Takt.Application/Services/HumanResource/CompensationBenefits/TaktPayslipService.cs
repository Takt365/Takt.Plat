...
public class TaktPayslipService : TaktServiceBase, ITaktPayslipService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="payslipRepository">薪资条仓储</param>
    ...
    public async Task<TaktPayslipDto> UpdatePayslipAsync(long id, TaktPayslipUpdateDto dto)
    {
        ...
        if (entity == null)
        {
            throw new TaktBusinessException("薪资条不存在");
        }
        dto.Adapt(entity);
        await _payslipRepository.UpdateAsync(entity);
        return await GetPayslipByIdAsync(id) ?? throw new TaktBusinessException("薪资条不存在");
    }
    ...
    public async Task DeletePayslipByIdAsync(long id)
    {
        ...
        if (!deleted)
        {
            throw new TaktBusinessException("薪资条不存在或已删除");
        }
    }
    ...
    public async Task<TaktPayslipDto> UpdatePayslipStatusAsync(TaktPayslipStatusDto dto)
    {
        ...
        await _payslipRepository.UpdateAsync(entity);
        return await GetPayslipByIdAsync(dto.PayslipId) ?? throw new TaktBusinessException("薪资条不存在");
    }
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportPayslipAsync(Stream fileStream, string? sheetName = null)
    {
        ...
        var success = 0;
        var fail = 0;
        var rows = await TaktExcelHelper.ImportAsync<TaktPayslipImportDto>(fileStream, sheetName ?? "薪资条导入模板");
    ...
    private static Expression<Func<TaktPayslip, bool>> QueryExpression(TaktPayslipQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.KeyWords))
        {
            ...
            exp = exp.And(x =>
                ...
                || SqlFunc.ToString(x.PositionAllowance).Contains(keywords)
                || SqlFunc.ToString(x.PerformanceBonus).Contains(keywords)
