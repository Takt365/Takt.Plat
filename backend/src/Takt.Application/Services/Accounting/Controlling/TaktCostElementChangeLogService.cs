...
public class TaktCostElementChangeLogService : TaktServiceBase, ITaktCostElementChangeLogService
{
    ...
    public async Task<List<TaktSelectOption>> GetCostElementChangeLogOptionsAsync()
    {
        ...
        var list = await _costElementChangeLogRepository.GetListAsync(
            ...
            x => x.CostElementCode ?? string.Empty,
            false);
    ...
    public async Task<TaktCostElementChangeLogDto> CreateCostElementChangeLogAsync(TaktCostElementChangeLogCreateDto dto)
    {
        ...
        await StampCostElementChangeLogCostElementAsync(entity, dto);
    ...
    /// <summary>
    /// 同步成本要素变更记录主表外键（ManyToOne → 成本要素）
    /// </summary>
    /// <param name="entity">当前实体</param>
    /// <param name="dto">创建 DTO</param>
    /// <returns>任务</returns>
    private async Task StampCostElementChangeLogCostElementAsync(TaktCostElementChangeLog entity, TaktCostElementChangeLogCreateDto dto)
    {
        if (dto.CostElementId <= 0)
        {
            return;
        }
        var master = await _costElementRepository.GetByIdAsync(dto.CostElementId);
        if (master == null)
        {
            throw new TaktBusinessException("成本要素不存在");
        }
        entity.CostElementId = master.Id;
    }
    // ========================================
    // 查询表达式
    // ========================================
