...
public class TaktPurchaseInvoiceService : TaktServiceBase, ITaktPurchaseInvoiceService
{
    ...
    /// <summary>
    /// 保存采购发票子表级联（采购发票明细；Create/Update 后按主表 Id 先删后插）
    /// </summary>
    /// <param name="entity">主表实体</param>
    ...
    private async Task SavePurchaseInvoiceChildrenAsync(TaktPurchaseInvoice entity, TaktPurchaseInvoiceCreateDto dto)
    {
        ...
        {
            var items = dto.Items.Adapt<List<TaktPurchaseInvoiceItem>>();
            ...
            if (itemsNeedLine.Count > 0)
            {
                ...
                var maxLine = await _purchaseInvoiceItemRepository.GetMaxIntAsync(
                    x => x.TenantCode == CurrentTenantCode && x.CompanyCode == CurrentCompanyCode && x.PurchaseInvoiceId == entity.Id,
                    x => x.LineNumber);
                ...
                foreach (var child in items)
                {
                    if (child.LineNumber <= 0)
                    {
                        ...
                        {
                            var key = $"{items[i].CompanyCode}|{items[i].PurchaseInvoiceId}|{items[i].LineNumber}";
                            if (!seenKeys.Add(key))
                            {
            ...
            var isUnique_ix_takt_logistics_procurement_purchase_invoice_item_invoice_line_unique = await _uniqueValidator.IsUniqueAsync(
                ...
                x => x.CompanyCode == child.CompanyCode
                    && x.PurchaseInvoiceId == child.PurchaseInvoiceId
            ...
            await _purchaseInvoiceItemRepository.CreateRangeAsync(items);
        }
    }
    // ========================================
    // 查询表达式
