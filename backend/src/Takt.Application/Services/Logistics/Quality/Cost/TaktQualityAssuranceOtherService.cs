...
public class TaktQualityAssuranceOtherService : TaktServiceBase, ITaktQualityAssuranceOtherService
{
    ...
    /// <summary>
    /// 获取品质业务其他通常业务费用明细列表（分页；无业务查询条件时返回空结果）
    /// </summary>
    ...
    public async Task<TaktPagedResult<TaktQualityAssuranceOtherDto>> GetQualityAssuranceOtherListAsync(TaktQualityAssuranceOtherQueryDto queryDto)
    {
        if (!HasAnyListQueryFilter(queryDto))
        {
            return TaktPagedResult<TaktQualityAssuranceOtherDto>.Create(
                new List<TaktQualityAssuranceOtherDto>(),
                0,
                queryDto.PageIndex,
                queryDto.PageSize);
        }
    ...
    public async Task<List<TaktSelectOption>> GetQualityAssuranceOtherOptionsAsync(string? plantCode = null, string? keyword = null)
    {
    ...
    {
        var queryDto = query ?? new TaktQualityAssuranceOtherQueryDto();
        if (!HasAnyListQueryFilter(queryDto))
        {
    ...
    private static Expression<Func<TaktQualityAssuranceOther, bool>> QueryExpression(TaktQualityAssuranceOtherQueryDto? queryDto)
    {
        ...
        {
            var operationsCost = queryDto.OperationsCost.Value;
            exp = exp.And(x => x.OperationsCost == operationsCost);
        }
        ...
        if (!string.IsNullOrWhiteSpace(queryDto?.OtherNote))
        {
            var otherNote = queryDto.OtherNote;
            exp = exp.And(x => x.OtherNote != null && x.OtherNote.Contains(otherNote));
        }
    ...
    private static bool HasAnyListQueryFilter(TaktQualityAssuranceOtherQueryDto? queryDto)
    {
        if (queryDto == null)
        {
            return false;
        ...
        if (queryDto.OperationsCost.HasValue)
        {
            return true;
        }
