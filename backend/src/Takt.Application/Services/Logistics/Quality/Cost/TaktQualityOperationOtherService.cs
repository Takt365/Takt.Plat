...
public class TaktQualityOperationOtherService : TaktServiceBase, ITaktQualityOperationOtherService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="qualityOperationOtherRepository">品质业务其他通常业务费用明细仓储</param>
    ...
    public async Task<TaktPagedResult<TaktQualityOperationOtherDto>> GetQualityOperationOtherListAsync(TaktQualityOperationOtherQueryDto queryDto)
    {
        ...
        var (data, total) = await _qualityOperationOtherRepository.GetPagedAsync(
            queryDto.PageIndex,
            queryDto.PageSize,
            predicate);
    ...
    {
        var entity = await _qualityOperationOtherRepository.GetByIdAsync(id);
        if (entity == null || entity.TenantCode != CurrentTenantCode || entity.CompanyCode != CurrentCompanyCode)
        {
            return null;
        }
        return entity.Adapt<TaktQualityOperationOtherDto>();
    }
    ...
    public async Task<List<TaktSelectOption>> GetQualityOperationOtherOptionsAsync()
    {
        ...
        var list = await _qualityOperationOtherRepository.GetListAsync(
    ...
    public async Task<TaktQualityOperationOtherDto> CreateQualityOperationOtherAsync(TaktQualityOperationOtherCreateDto dto)
    {
        var entity = dto.Adapt<TaktQualityOperationOther>();
                await StampQualityOperationOtherQualityOperationAsync(entity, dto);
        ...
        {
            throw new TaktBusinessException("品质业务其他通常业务费用明细的QualityOperationId、LineNumber已存在");
        }
        ...
        entity = await _qualityOperationOtherRepository.CreateAsync(entity);
    ...
    public async Task<TaktQualityOperationOtherDto> UpdateQualityOperationOtherAsync(long id, TaktQualityOperationOtherUpdateDto dto)
    {
        ...
        await _qualityOperationOtherRepository.UpdateAsync(entity);
        return await GetQualityOperationOtherByIdAsync(id) ?? throw new TaktBusinessException("品质业务其他通常业务费用明细不存在");
    }
    ...
    {
        var deleted = await _qualityOperationOtherRepository.DeleteAsync(id);
        if (!deleted)
        {
    ...
    public async Task DeleteQualityOperationOtherBatchAsync(IEnumerable<long> ids)
    {
        ...
        {
            await DeleteQualityOperationOtherByIdAsync(id);
        }
    }
    ...
    public async Task<(string fileName, byte[] content)> GetQualityOperationOtherTemplateAsync(string? sheetName = null, string? fileName = null)
    {
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportQualityOperationOtherAsync(Stream fileStream, string? sheetName = null)
    {
        var errors = new List<string>();
        var success = 0;
        var fail = 0;
        var rows = await TaktExcelHelper.ImportAsync<TaktQualityOperationOtherImportDto>(fileStream, sheetName ?? "品质业务其他通常业务费用明细导入模板");
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            try
            {
                var entity = rows[i].Adapt<TaktQualityOperationOther>();
                var importDto = rows[i].Adapt<TaktQualityOperationOtherCreateDto>();
                await StampQualityOperationOtherQualityOperationAsync(entity, importDto);
                ...
                await _qualityOperationOtherRepository.CreateAsync(entity);
                success += 1;
            }
            catch (Exception ex)
            {
    ...
    public async Task<(string fileName, byte[] fileContent)> ExportQualityOperationOtherAsync(TaktQualityOperationOtherQueryDto? query = null, string? sheetName = null, string? fileName = null)
    {
        ...
        var exportData = list.Adapt<List<TaktQualityOperationOtherExportDto>>();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            sheetName ?? "品质业务其他通常业务费用明细数据",
            fileName ?? "品质业务其他通常业务费用明细导出.xlsx");
    ...
    /// <returns>任务</returns>
    private async Task StampQualityOperationOtherQualityOperationAsync(TaktQualityOperationOther entity, TaktQualityOperationOtherCreateDto dto)
    {
