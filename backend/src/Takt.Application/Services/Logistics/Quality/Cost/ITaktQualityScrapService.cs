...
public interface ITaktQualityScrapService
{
    ...
    Task<(string fileName, byte[] content)> GetQualityScrapTemplateAsync(string? sheetName = null, string? fileName = null);
    ...
    Task<(int success, int fail, List<string> errors)> ImportQualityScrapAsync(Stream fileStream, string? sheetName = null);
    ...
    /// <param name="fileName">文件名</param>
    /// <returns>Excel 文件</returns>
    Task<(string fileName, byte[] fileContent)> ExportQualityScrapAsync(TaktQualityScrapQueryDto? query = null, string? sheetName = null, string? fileName = null);
