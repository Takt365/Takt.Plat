...
public class TaktQualityOperationCustomerResponseService : TaktServiceBase, ITaktQualityOperationCustomerResponseService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="qualityOperationCustomerResponseRepository">品质业务顾客品质要求对应费用明细仓储</param>
    ...
    public async Task<TaktPagedResult<TaktQualityOperationCustomerResponseDto>> GetQualityOperationCustomerResponseListAsync(TaktQualityOperationCustomerResponseQueryDto queryDto)
    {
        ...
        var (data, total) = await _qualityOperationCustomerResponseRepository.GetPagedAsync(
            queryDto.PageIndex,
            queryDto.PageSize,
            predicate);
    ...
    {
        var entity = await _qualityOperationCustomerResponseRepository.GetByIdAsync(id);
        if (entity == null || entity.TenantCode != CurrentTenantCode || entity.CompanyCode != CurrentCompanyCode)
        {
            return null;
        }
        return entity.Adapt<TaktQualityOperationCustomerResponseDto>();
    }
    ...
    public async Task<List<TaktSelectOption>> GetQualityOperationCustomerResponseOptionsAsync()
    {
        ...
        var list = await _qualityOperationCustomerResponseRepository.GetListAsync(
    ...
    public async Task<TaktQualityOperationCustomerResponseDto> CreateQualityOperationCustomerResponseAsync(TaktQualityOperationCustomerResponseCreateDto dto)
    {
        ...
        {
            throw new TaktBusinessException("品质业务顾客品质要求对应费用明细的QualityOperationId、LineNumber已存在");
        }
        ...
        entity = await _qualityOperationCustomerResponseRepository.CreateAsync(entity);
    ...
    public async Task<TaktQualityOperationCustomerResponseDto> UpdateQualityOperationCustomerResponseAsync(long id, TaktQualityOperationCustomerResponseUpdateDto dto)
    {
        ...
        await _qualityOperationCustomerResponseRepository.UpdateAsync(entity);
        return await GetQualityOperationCustomerResponseByIdAsync(id) ?? throw new TaktBusinessException("品质业务顾客品质要求对应费用明细不存在");
    }
    ...
    {
        var deleted = await _qualityOperationCustomerResponseRepository.DeleteAsync(id);
        if (!deleted)
        {
    ...
    public async Task DeleteQualityOperationCustomerResponseBatchAsync(IEnumerable<long> ids)
    {
        ...
        {
            await DeleteQualityOperationCustomerResponseByIdAsync(id);
        }
    }
    ...
    public async Task<(string fileName, byte[] content)> GetQualityOperationCustomerResponseTemplateAsync(string? sheetName = null, string? fileName = null)
    {
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportQualityOperationCustomerResponseAsync(Stream fileStream, string? sheetName = null)
    {
        var errors = new List<string>();
        var success = 0;
        var fail = 0;
        var rows = await TaktExcelHelper.ImportAsync<TaktQualityOperationCustomerResponseImportDto>(fileStream, sheetName ?? "品质业务顾客品质要求对应费用明细导入模板");
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            try
            {
                var entity = rows[i].Adapt<TaktQualityOperationCustomerResponse>();
                var importDto = rows[i].Adapt<TaktQualityOperationCustomerResponseCreateDto>();
                await StampQualityOperationCustomerResponseQualityOperationAsync(entity, importDto);
                ...
                await _qualityOperationCustomerResponseRepository.CreateAsync(entity);
                success += 1;
            }
            catch (Exception ex)
            {
    ...
    public async Task<(string fileName, byte[] fileContent)> ExportQualityOperationCustomerResponseAsync(TaktQualityOperationCustomerResponseQueryDto? query = null, string? sheetName = null, string? fileName = null)
    {
        ...
        var exportData = list.Adapt<List<TaktQualityOperationCustomerResponseExportDto>>();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            sheetName ?? "品质业务顾客品质要求对应费用明细数据",
            fileName ?? "品质业务顾客品质要求对应费用明细导出.xlsx");
    ...
    /// <returns>任务</returns>
    private async Task StampQualityOperationCustomerResponseQualityOperationAsync(TaktQualityOperationCustomerResponse entity, TaktQualityOperationCustomerResponseCreateDto dto)
    {
