...
// 文件名称：TaktQualityOperationFirstArticleService.cs
// 创建时间：2026-06-06
...
/// 品质业务初期定期检定费用明细应用服务
/// </summary>
public class TaktQualityOperationFirstArticleService : TaktServiceBase, ITaktQualityOperationFirstArticleService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="qualityOperationFirstArticleRepository">品质业务初期定期检定费用明细仓储</param>
    /// <param name="qualityOperationRepository">品质业务主仓储</param>
    /// <param name="lineNumberGenerator">明细行号生成器</param>
    /// <param name="uniqueValidator">唯一性验证器</param>
    /// <param name="userContext">用户上下文</param>
    /// <param name="localizationService">本地化服务</param>
    public TaktQualityOperationFirstArticleService(
    ...
    public async Task<TaktPagedResult<TaktQualityOperationFirstArticleDto>> GetQualityOperationFirstArticleListAsync(TaktQualityOperationFirstArticleQueryDto queryDto)
    {
        var predicate = QueryExpression(queryDto);
        var (data, total) = await _qualityOperationFirstArticleRepository.GetPagedAsync(
            queryDto.PageIndex,
            queryDto.PageSize,
            predicate);
        return TaktPagedResult<TaktQualityOperationFirstArticleDto>.Create(
            data.Adapt<List<TaktQualityOperationFirstArticleDto>>(),
            total,
            queryDto.PageIndex,
            queryDto.PageSize);
    }
    ...
    /// <returns>DTO</returns>
    public async Task<TaktQualityOperationFirstArticleDto?> GetQualityOperationFirstArticleByIdAsync(long id)
    {
        var entity = await _qualityOperationFirstArticleRepository.GetByIdAsync(id);
        if (entity == null || entity.TenantCode != CurrentTenantCode || entity.CompanyCode != CurrentCompanyCode)
        {
            return null;
        }
        return entity.Adapt<TaktQualityOperationFirstArticleDto>();
    }
    ...
    public async Task<List<TaktSelectOption>> GetQualityOperationFirstArticleOptionsAsync()
    {
        EnsureThreeLayerContext();
        var list = await _qualityOperationFirstArticleRepository.GetListAsync(
    ...
    /// <returns>DTO</returns>
    public async Task<TaktQualityOperationFirstArticleDto> CreateQualityOperationFirstArticleAsync(TaktQualityOperationFirstArticleCreateDto dto)
    {
        var entity = dto.Adapt<TaktQualityOperationFirstArticle>();
                await StampQualityOperationFirstArticleQualityOperationAsync(entity, dto);
        ...
        {
            throw new TaktBusinessException("品质业务初期定期检定费用明细的QualityOperationId、LineNumber已存在");
        }
        ...
        entity = await _qualityOperationFirstArticleRepository.CreateAsync(entity);
        return await GetQualityOperationFirstArticleByIdAsync(entity.Id) ?? entity.Adapt<TaktQualityOperationFirstArticleDto>();
    }
    ...
    /// <returns>DTO</returns>
    public async Task<TaktQualityOperationFirstArticleDto> UpdateQualityOperationFirstArticleAsync(long id, TaktQualityOperationFirstArticleUpdateDto dto)
    {
        var entity = await _qualityOperationFirstArticleRepository.GetByIdAsync(id);
        if (entity == null)
        {
        ...
        await _qualityOperationFirstArticleRepository.UpdateAsync(entity);
        return await GetQualityOperationFirstArticleByIdAsync(id) ?? throw new TaktBusinessException("品质业务初期定期检定费用明细不存在");
    }
    ...
    /// <param name="id">品质业务初期定期检定费用明细ID</param>
    /// <returns>任务</returns>
    public async Task DeleteQualityOperationFirstArticleByIdAsync(long id)
    {
        var deleted = await _qualityOperationFirstArticleRepository.DeleteAsync(id);
        if (!deleted)
        {
    ...
    /// <returns>任务</returns>
    public async Task DeleteQualityOperationFirstArticleBatchAsync(IEnumerable<long> ids)
    {
        ...
        {
            await DeleteQualityOperationFirstArticleByIdAsync(id);
        }
    }
    ...
    public async Task<(string fileName, byte[] content)> GetQualityOperationFirstArticleTemplateAsync(string? sheetName = null, string? fileName = null)
    {
        return await TaktExcelHelper.GenerateTemplateAsync<TaktQualityOperationFirstArticleTemplateDto>(
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportQualityOperationFirstArticleAsync(Stream fileStream, string? sheetName = null)
    {
        var errors = new List<string>();
        var success = 0;
        var fail = 0;
        var rows = await TaktExcelHelper.ImportAsync<TaktQualityOperationFirstArticleImportDto>(fileStream, sheetName ?? "品质业务初期定期检定费用明细导入模板");
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            try
            {
                var entity = rows[i].Adapt<TaktQualityOperationFirstArticle>();
                var importDto = rows[i].Adapt<TaktQualityOperationFirstArticleCreateDto>();
                await StampQualityOperationFirstArticleQualityOperationAsync(entity, importDto);
                ...
                await _qualityOperationFirstArticleRepository.CreateAsync(entity);
                success += 1;
            }
            catch (Exception ex)
            {
    ...
    public async Task<(string fileName, byte[] fileContent)> ExportQualityOperationFirstArticleAsync(TaktQualityOperationFirstArticleQueryDto? query = null, string? sheetName = null, string? fileName = null)
    {
        ...
        if (list == null || list.Count == 0)
        {
            return await TaktExcelHelper.ExportAsync(
                new List<TaktQualityOperationFirstArticleExportDto>(),
                sheetName ?? "品质业务初期定期检定费用明细数据",
                fileName ?? "品质业务初期定期检定费用明细导出.xlsx");
        }
        var exportData = list.Adapt<List<TaktQualityOperationFirstArticleExportDto>>();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            sheetName ?? "品质业务初期定期检定费用明细数据",
            fileName ?? "品质业务初期定期检定费用明细导出.xlsx");
    ...
    /// <returns>任务</returns>
    private async Task StampQualityOperationFirstArticleQualityOperationAsync(TaktQualityOperationFirstArticle entity, TaktQualityOperationFirstArticleCreateDto dto)
    {
    ...
    private static Expression<Func<TaktQualityOperationFirstArticle, bool>> QueryExpression(TaktQualityOperationFirstArticleQueryDto? queryDto)
    {
        var exp = Expressionable.Create<TaktQualityOperationFirstArticle>();
