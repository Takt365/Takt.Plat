...
public class TaktQualityScrapItemService : TaktServiceBase, ITaktQualityScrapItemService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="qualityScrapItemRepository">品质废弃明细仓储</param>
    ...
    public async Task<TaktPagedResult<TaktQualityScrapItemDto>> GetQualityScrapItemListAsync(TaktQualityScrapItemQueryDto queryDto)
    {
        ...
        var (data, total) = await _qualityScrapItemRepository.GetPagedAsync(
            queryDto.PageIndex,
            queryDto.PageSize,
            predicate);
    ...
    {
        var entity = await _qualityScrapItemRepository.GetByIdAsync(id);
        if (entity == null || entity.TenantCode != CurrentTenantCode || entity.CompanyCode != CurrentCompanyCode)
        {
            return null;
        }
        return entity.Adapt<TaktQualityScrapItemDto>();
    }
    ...
    public async Task<List<TaktSelectOption>> GetQualityScrapItemOptionsAsync()
    {
        ...
        var list = await _qualityScrapItemRepository.GetListAsync(
    ...
    public async Task<TaktQualityScrapItemDto> CreateQualityScrapItemAsync(TaktQualityScrapItemCreateDto dto)
    {
        var entity = dto.Adapt<TaktQualityScrapItem>();
                await StampQualityScrapItemQualityScrapAsync(entity, dto);
        var isUnique_ix_takt_logistics_quality_scrap_item_line_number_unique = await _uniqueValidator.IsUniqueAsync(
            ...
            x => x.QualityScrapId == entity.QualityScrapId
                && x.LineNumber == entity.LineNumber
                && x.MaterialCode == entity.MaterialCode);
        if (!isUnique_ix_takt_logistics_quality_scrap_item_line_number_unique)
        {
            throw new TaktBusinessException("品质废弃明细的QualityScrapId、LineNumber、MaterialCode已存在");
        }
        if (entity.LineNumber <= 0)
        {
            var maxLine = await _qualityScrapItemRepository.GetMaxIntAsync(
                x => x.TenantCode == CurrentTenantCode && x.CompanyCode == CurrentCompanyCode && x.QualityScrapId == entity.QualityScrapId,
                x => x.LineNumber);
            var businessCode = !string.IsNullOrWhiteSpace(entity.QualityScrapCode) ? entity.QualityScrapCode : entity.QualityScrapId.ToString();
            entity.LineNumber = _lineNumberGenerator.GenerateNext(businessCode, maxLine);
        }
        entity = await _qualityScrapItemRepository.CreateAsync(entity);
    ...
    public async Task<TaktQualityScrapItemDto> UpdateQualityScrapItemAsync(long id, TaktQualityScrapItemUpdateDto dto)
    {
        ...
        if (entity == null)
        {
            throw new TaktBusinessException("品质废弃明细不存在");
        }
        dto.Adapt(entity);
                await StampQualityScrapItemQualityScrapAsync(entity, dto);
        var isUnique_ix_takt_logistics_quality_scrap_item_line_number_unique = await _uniqueValidator.IsUniqueAsync(
        ...
        await _qualityScrapItemRepository.UpdateAsync(entity);
        return await GetQualityScrapItemByIdAsync(id) ?? throw new TaktBusinessException("品质废弃明细不存在");
    }
    ...
    {
        var deleted = await _qualityScrapItemRepository.DeleteAsync(id);
        if (!deleted)
        {
            throw new TaktBusinessException("品质废弃明细不存在或已删除");
        }
    }
    ...
    public async Task DeleteQualityScrapItemBatchAsync(IEnumerable<long> ids)
    {
        ...
        {
            await DeleteQualityScrapItemByIdAsync(id);
        }
    }
    ...
    public async Task<(string fileName, byte[] content)> GetQualityScrapItemTemplateAsync(string? sheetName = null, string? fileName = null)
    {
        return await TaktExcelHelper.GenerateTemplateAsync<TaktQualityScrapItemTemplateDto>(
            sheetName ?? "品质废弃明细导入模板",
            fileName ?? "品质废弃明细导入模板.xlsx");
    }
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportQualityScrapItemAsync(Stream fileStream, string? sheetName = null)
    {
        var errors = new List<string>();
        var success = 0;
        var fail = 0;
        var rows = await TaktExcelHelper.ImportAsync<TaktQualityScrapItemImportDto>(fileStream, sheetName ?? "品质废弃明细导入模板");
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            try
            {
                var entity = rows[i].Adapt<TaktQualityScrapItem>();
                var importDto = rows[i].Adapt<TaktQualityScrapItemCreateDto>();
                await StampQualityScrapItemQualityScrapAsync(entity, importDto);
                var importKey = $"{entity.QualityScrapId}|{entity.LineNumber}|{entity.MaterialCode}";
                if (!importSeenKeys.Add(importKey))
                {
                    throw new TaktBusinessException("与Excel中其他行重复（QualityScrapId、LineNumber、MaterialCode）");
                }
                var isUnique_ix_takt_logistics_quality_scrap_item_line_number_unique = await _uniqueValidator.IsUniqueAsync(
                    _qualityScrapItemRepository,
                    x => x.QualityScrapId == entity.QualityScrapId
                ...
                await _qualityScrapItemRepository.CreateAsync(entity);
                success += 1;
            }
            catch (Exception ex)
            {
    ...
    public async Task<(string fileName, byte[] fileContent)> ExportQualityScrapItemAsync(TaktQualityScrapItemQueryDto? query = null, string? sheetName = null, string? fileName = null)
    {
        ...
        if (list == null || list.Count == 0)
        {
            return await TaktExcelHelper.ExportAsync(
                ...
                sheetName ?? "品质废弃明细数据",
                fileName ?? "品质废弃明细导出.xlsx");
        }
        var exportData = list.Adapt<List<TaktQualityScrapItemExportDto>>();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            sheetName ?? "品质废弃明细数据",
            fileName ?? "品质废弃明细导出.xlsx");
    ...
    /// <summary>
    /// 同步品质废弃明细主表外键（ManyToOne → 品质废弃主）
    /// </summary>
    /// <param name="entity">当前实体</param>
    /// <param name="dto">创建 DTO</param>
    /// <returns>任务</returns>
    private async Task StampQualityScrapItemQualityScrapAsync(TaktQualityScrapItem entity, TaktQualityScrapItemCreateDto dto)
    {
        if (dto.QualityScrapId <= 0)
        {
            return;
        }
        var master = await _qualityScrapRepository.GetByIdAsync(dto.QualityScrapId);
        if (master == null)
        {
            throw new TaktBusinessException("品质废弃主不存在");
        }
        entity.QualityScrapId = master.Id;
    }
    // ========================================
    // 查询表达式
    // ========================================
    ...
    private static Expression<Func<TaktQualityScrapItem, bool>> QueryExpression(TaktQualityScrapItemQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.KeyWords))
        {
            ...
            exp = exp.And(x =>
                SqlFunc.ToString(x.QualityScrapId).Contains(keywords)
