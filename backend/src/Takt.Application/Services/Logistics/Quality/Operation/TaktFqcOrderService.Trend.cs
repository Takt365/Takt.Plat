...
// 文件名称：TaktFqcOrderService.Trend.cs
// 创建时间：2026-07-18
// 创建人：Takt365(Cursor AI)
// 功能描述：FQC 成品检验月推移转置分析
//
...
public partial class TaktFqcOrderService
{
    /// <inheritdoc />
        ...
        IReadOnlyList<TaktFqcOrderMonthlyTrendDto> rows)
    {
        ...
        var map = customers
            ...
            .ToDictionary(
                g => g.Key,
                g => g.Select(x => x.CustomerName?.Trim()).FirstOrDefault(n => !string.IsNullOrWhiteSpace(n)) ?? string.Empty,
