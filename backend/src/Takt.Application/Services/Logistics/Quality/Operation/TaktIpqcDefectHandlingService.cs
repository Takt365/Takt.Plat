...
public class TaktIpqcDefectHandlingService : TaktServiceBase, ITaktIpqcDefectHandlingService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="ipqcDefectHandlingRepository">制程检验不良处理记录仓储</param>
    ...
    /// <summary>
    /// 获取制程检验不良处理记录列表（分页；无业务查询条件时返回空结果）
    /// </summary>
    ...
    public async Task<TaktPagedResult<TaktIpqcDefectHandlingDto>> GetIpqcDefectHandlingListAsync(TaktIpqcDefectHandlingQueryDto queryDto)
    {
        if (!HasAnyListQueryFilter(queryDto))
        {
            return TaktPagedResult<TaktIpqcDefectHandlingDto>.Create(
                new List<TaktIpqcDefectHandlingDto>(),
                0,
                queryDto.PageIndex,
                queryDto.PageSize);
        }
        var predicate = QueryExpression(queryDto);
        var (data, total) = await _ipqcDefectHandlingRepository.GetPagedAsync(
            queryDto.PageIndex,
            queryDto.PageSize,
            predicate);
        return TaktPagedResult<TaktIpqcDefectHandlingDto>.Create(
    ...
    public async Task<TaktIpqcDefectHandlingDto?> GetIpqcDefectHandlingByIdAsync(long id)
    {
        ...
        return entity.Adapt<TaktIpqcDefectHandlingDto>();
    }
    ...
    public async Task<List<TaktSelectOption>> GetIpqcDefectHandlingOptionsAsync(string? plantCode = null, string? keyword = null)
    {
        EnsureThreeLayerContext();
        var list = await _ipqcDefectHandlingRepository.GetListAsync(
            x => x.TenantCode == CurrentTenantCode && x.CompanyCode == CurrentCompanyCode && x.HandlingStatus == 1 && x.IsObsolete == 0,
            x => x.IpqcDefectHandlingCode ?? string.Empty,
            false);
        return list.Select(e => new TaktSelectOption
        {
            DictValue = e.IpqcDefectHandlingCode,
            DictLabel = e.IpqcDefectHandlingCode,
        }).ToList();
    }
    ...
    public async Task<TaktIpqcDefectHandlingDto> CreateIpqcDefectHandlingAsync(TaktIpqcDefectHandlingCreateDto dto)
    {
        ...
        var isUnique_ix_takt_logistics_quality_ipqc_defect_handling_unique = await _uniqueValidator.IsUniqueAsync(
            ...
            x => x.IpqcOrderItemId == entity.IpqcOrderItemId
        ...
        {
            throw new TaktBusinessException("制程检验不良处理记录的IpqcOrderItemId、DefectCode、HandlingMethod已存在");
        }
        if (entity.LineNumber <= 0)
        {
            var maxLine = await _ipqcDefectHandlingRepository.GetMaxIntAsync(
                x => x.TenantCode == CurrentTenantCode && x.CompanyCode == CurrentCompanyCode && x.IpqcOrderItemId == entity.IpqcOrderItemId,
                x => x.LineNumber);
            var businessCode = entity.IpqcOrderItemId.ToString();
            entity.LineNumber = _lineNumberGenerator.GenerateNext(businessCode, maxLine);
        }
        entity = await _ipqcDefectHandlingRepository.CreateAsync(entity);
    ...
    public async Task<TaktIpqcDefectHandlingDto> UpdateIpqcDefectHandlingAsync(long id, TaktIpqcDefectHandlingUpdateDto dto)
    {
        ...
        return await GetIpqcDefectHandlingByIdAsync(id) ?? throw new TaktBusinessException("制程检验不良处理记录不存在");
    }
    ...
    public async Task DeleteIpqcDefectHandlingBatchAsync(IEnumerable<long> ids)
    {
        ...
        {
            await DeleteIpqcDefectHandlingByIdAsync(id);
        }
    }
    ...
    /// <returns>DTO</returns>
    public async Task<TaktIpqcDefectHandlingDto> UpdateIpqcDefectHandlingStatusAsync(TaktIpqcDefectHandlingStatusDto dto)
    {
    ...
    public async Task<(string fileName, byte[] content)> GetIpqcDefectHandlingTemplateAsync(string? sheetName = null, string? fileName = null)
    {
        return await TaktExcelHelper.GenerateTemplateAsync<TaktIpqcDefectHandlingTemplateDto>(
            sheetName ?? "制程检验不良处理记录导入模板",
            fileName ?? "制程检验不良处理记录导入模板.xlsx");
    }
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportIpqcDefectHandlingAsync(Stream fileStream, string? sheetName = null)
    {
        var errors = new List<string>();
        var success = 0;
        var fail = 0;
        var rows = await TaktExcelHelper.ImportAsync<TaktIpqcDefectHandlingImportDto>(fileStream, sheetName ?? "制程检验不良处理记录导入模板");
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            try
            {
                var entity = rows[i].Adapt<TaktIpqcDefectHandling>();
                var importDto = rows[i].Adapt<TaktIpqcDefectHandlingCreateDto>();
                await StampIpqcDefectHandlingIpqcOrderItemAsync(entity, importDto);
                var importKey = $"{entity.IpqcOrderItemId}|{entity.DefectCode}|{entity.HandlingMethod}";
                if (!importSeenKeys.Add(importKey))
                {
                    throw new TaktBusinessException("与Excel中其他行重复（IpqcOrderItemId、DefectCode、HandlingMethod）");
                }
                var isUnique_ix_takt_logistics_quality_ipqc_defect_handling_unique = await _uniqueValidator.IsUniqueAsync(
                    _ipqcDefectHandlingRepository,
                    x => x.IpqcOrderItemId == entity.IpqcOrderItemId
                ...
                await _ipqcDefectHandlingRepository.CreateAsync(entity);
                success += 1;
            }
            catch (Exception ex)
            {
    ...
    {
        var queryDto = query ?? new TaktIpqcDefectHandlingQueryDto();
        if (!HasAnyListQueryFilter(queryDto))
        {
            return await TaktExcelHelper.ExportAsync(
                new List<TaktIpqcDefectHandlingExportDto>(),
                sheetName ?? "制程检验不良处理记录数据",
                fileName ?? "制程检验不良处理记录导出.xlsx");
        }
        var predicate = QueryExpression(queryDto);
        ...
        var exportData = list.Adapt<List<TaktIpqcDefectHandlingExportDto>>();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            sheetName ?? "制程检验不良处理记录数据",
            fileName ?? "制程检验不良处理记录导出.xlsx");
    ...
    /// <summary>
    /// 同步制程检验不良处理记录主表外键（ManyToOne → 制程检验单明细）
    /// </summary>
    /// <param name="entity">当前实体</param>
    /// <param name="dto">创建 DTO</param>
    /// <returns>任务</returns>
    private async Task StampIpqcDefectHandlingIpqcOrderItemAsync(TaktIpqcDefectHandling entity, TaktIpqcDefectHandlingCreateDto dto)
    {
        if (dto.IpqcOrderItemId <= 0)
        {
            return;
        }
        var master = await _ipqcOrderItemRepository.GetByIdAsync(dto.IpqcOrderItemId);
        if (master == null)
        {
            throw new TaktBusinessException("制程检验单明细不存在");
        }
        entity.IpqcOrderItemId = master.Id;
        ...
        if (string.IsNullOrEmpty(entity.IpqcOrderCode))
        {
            entity.IpqcOrderCode = master.IpqcOrderCode;
        }
    }
    // ========================================
    // 查询表达式
    ...
    private static Expression<Func<TaktIpqcDefectHandling, bool>> QueryExpression(TaktIpqcDefectHandlingQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrWhiteSpace(queryDto?.KeyWords))
        {
            ...
            exp = exp.And(x =>
                ...
                || (x.IpqcDefectHandlingCode != null && x.IpqcDefectHandlingCode.Contains(keywords))
        ...
        if (!string.IsNullOrWhiteSpace(queryDto?.IpqcDefectHandlingCode))
        {
            var ipqcDefectHandlingCode = queryDto.IpqcDefectHandlingCode;
            exp = exp.And(x => x.IpqcDefectHandlingCode != null && x.IpqcDefectHandlingCode.Contains(ipqcDefectHandlingCode));
        }

        if (queryDto?.IpqcOrderItemId.HasValue == true)
        {
            var ipqcOrderItemId = queryDto.IpqcOrderItemId.Value;
            exp = exp.And(x => x.IpqcOrderItemId == ipqcOrderItemId);
        }
    ...
    private static bool HasAnyListQueryFilter(TaktIpqcDefectHandlingQueryDto? queryDto)
    {
        if (queryDto == null)
        {
            return false;
        ...
        if (queryDto.IpqcOrderItemId.HasValue)
        {
            return true;
        }
        if (!string.IsNullOrWhiteSpace(queryDto.IpqcOrderCode))
