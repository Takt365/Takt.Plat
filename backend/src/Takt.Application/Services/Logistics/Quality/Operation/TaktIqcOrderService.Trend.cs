...
// 文件名称：TaktIqcOrderService.Trend.cs
// 创建时间：2026-07-18
// 创建人：Takt365(Cursor AI)
// 功能描述：IQC 进货检验月推移转置分析
//
...
public partial class TaktIqcOrderService
{
    /// <inheritdoc />
        ...
        IReadOnlyList<TaktIqcOrderMonthlyTrendDto> rows)
    {
        ...
        var map = suppliers
            ...
            .ToDictionary(
                g => g.Key,
                g => g.Select(x => x.SupplierName?.Trim()).FirstOrDefault(n => !string.IsNullOrWhiteSpace(n)) ?? string.Empty,
