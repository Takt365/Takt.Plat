...
public class TaktIqcOrderService : TaktServiceBase, ITaktIqcOrderService
{
    ...
    public async Task DeleteIqcOrderByIdAsync(long id)
    {
        ...
        await _iqcOrderChangeLogRepository.DeleteAsync(x => x.IqcOrderId == entity.Id);
    ...
    /// <summary>
    /// 填充进货检验单详情（加载 OneToMany 子表：进货检验单明细、进货检验单变更日志）
    /// </summary>
    ...
    private async Task FillIqcOrderDetailsAsync(TaktIqcOrderDto dto, TaktIqcOrder entity)
    {
        ...
        // 进货检验单变更日志 → dto.ChangeLogs
        var changelogs = await _iqcOrderChangeLogRepository.GetListAsync(x => x.IqcOrderId == entity.Id);
        dto.ChangeLogs = changelogs.Adapt<List<TaktIqcOrderChangeLogDto>>();
    }

    /// <summary>
    /// 保存进货检验单子表级联（进货检验单明细、进货检验单变更日志；Create/Update 后按主表 Id 先删后插）
    /// </summary>
    /// <param name="entity">主表实体</param>
    ...
    private async Task SaveIqcOrderChildrenAsync(TaktIqcOrder entity, TaktIqcOrderCreateDto dto)
    {
        ...
        {
            var items = dto.Items.Adapt<List<TaktIqcOrderItem>>();
            ...
            if (itemsNeedLine.Count > 0)
            {
                ...
                var maxLine = await _iqcOrderItemRepository.GetMaxIntAsync(
                    x => x.TenantCode == CurrentTenantCode && x.CompanyCode == CurrentCompanyCode && x.IqcOrderId == entity.Id,
                    x => x.LineNumber);
                ...
                foreach (var child in items)
                {
                    if (child.LineNumber <= 0)
                    {
                        ...
                        {
                            var key = $"{items[i].CompanyCode}|{items[i].IqcOrderId}|{items[i].LineNumber}";
                            if (!seenKeys.Add(key))
                            {
            ...
            await _iqcOrderItemRepository.CreateRangeAsync(items);
        }
        // 进货检验单变更日志（ChangeLogs）
        ...
        {
            var changelogs = dto.ChangeLogs.Adapt<List<TaktIqcOrderChangeLog>>();
            ...
            await _iqcOrderChangeLogRepository.CreateRangeAsync(changelogs);
        }
    }
