...
public class TaktSerialOutboundItemService : TaktServiceBase, ITaktSerialOutboundItemService
{
    ...
    public async Task<List<TaktSelectOption>> GetSerialOutboundItemOptionsAsync()
    {
        ...
        {
            DictValue = e.Id,
            DictLabel = e.OutboundCode ?? e.Id.ToString(),
        }).ToList();
    }
    ...
    {
        var predicate = QueryExpression(query ?? new TaktSerialOutboundItemQueryDto());
    ...
    private static Expression<Func<TaktSerialOutboundItem, bool>> QueryExpression(TaktSerialOutboundItemQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.ReferenceInboundCode))
        {
            exp = exp.And(x => x.ReferenceInboundCode != null && x.ReferenceInboundCode.Contains(queryDto.ReferenceInboundCode));
        }
