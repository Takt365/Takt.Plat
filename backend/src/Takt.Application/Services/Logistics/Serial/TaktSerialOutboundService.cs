...
public class TaktSerialOutboundService : TaktServiceBase, ITaktSerialOutboundService
{
    ...
    public async Task<TaktSerialOutboundDto> CreateSerialOutboundAsync(TaktSerialOutboundCreateDto dto)
    {
        ...
        {
            throw new TaktBusinessException("序列号出库的PlantCode、OutboundNo已存在");
        }
    ...
    {
        var predicate = QueryExpression(query ?? new TaktSerialOutboundQueryDto());
    ...
    /// <summary>
    /// 保存序列号出库子表级联（序列号出库明细；Create/Update 后按主表 Id 先删后插）
    /// </summary>
    /// <param name="entity">主表实体</param>
    ...
    private async Task SaveSerialOutboundChildrenAsync(TaktSerialOutbound entity, TaktSerialOutboundCreateDto dto)
    {
        ...
        {
            var items = dto.Items.Adapt<List<TaktSerialOutboundItem>>();
                ...
                foreach (var child in items)
                {
                    if (child.LineNumber <= 0)
                    {
                        ...
                        for (var i = 0; i < items.Count; i++)
                        {
                            ...
                            if (!seenKeys.Add(key))
                            {
                                throw new TaktBusinessException($"序列号出库明细第{i + 1}项与本次提交的其他项重复（CompanyCode、OutboundSerialNo）");
                            }
                        }
            await _serialOutboundItemRepository.DeleteAsync(x => x.OutboundId == entity.Id);
            foreach (var child in items)
            ...
            {
                throw new TaktBusinessException("序列号出库明细的CompanyCode、OutboundSerialNo已存在");
            }
            }
    ...
    private static Expression<Func<TaktSerialOutbound, bool>> QueryExpression(TaktSerialOutboundQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.KeyWords))
        {
            ...
            exp = exp.And(x =>
                ...
                || SqlFunc.ToString(x.ShippingMethod).Contains(keywords)
