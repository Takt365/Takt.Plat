...
public interface ITaktProductSerialOutboundItemService
{
    ...
    Task<(string fileName, byte[] content)> GetProductSerialOutboundItemTemplateAsync(string? sheetName = null, string? fileName = null);
    ...
    Task<(int success, int fail, List<string> errors)> ImportProductSerialOutboundItemAsync(Stream fileStream, string? sheetName = null);
    ...
    /// <param name="fileName">文件名</param>
    /// <returns>Excel 文件</returns>
    Task<(string fileName, byte[] fileContent)> ExportProductSerialOutboundItemAsync(TaktProductSerialOutboundItemQueryDto? query = null, string? sheetName = null, string? fileName = null);
