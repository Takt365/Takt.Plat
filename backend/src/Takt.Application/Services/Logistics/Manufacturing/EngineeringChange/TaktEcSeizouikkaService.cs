...
public class TaktEcSeizouikkaService : TaktServiceBase, ITaktEcSeizouikkaService
{
    ...
    public async Task<TaktEcSeizouikkaDto?> GetEcSeizouikkaByIdAsync(long id)
    {
        ...
        var dto = entity.Adapt<TaktEcSeizouikkaDto>();
    ...
    public async Task<List<TaktSelectOption>> GetEcSeizouikkaOptionsAsync(string? plantCode = null, string? keyword = null)
    {
    ...
    public async Task<TaktEcSeizouikkaDto> CreateEcSeizouikkaAsync(TaktEcSeizouikkaCreateDto dto)
    {
        ...
        {
            throw new TaktBusinessException("设变制一执行的EcDetailId、EcRootMaterialCode已存在");
        }
    ...
    /// <summary>
    /// 更新设变制一执行（同设变单号+机种+根物料编码的执行行一并写入可填字段）
    /// </summary>
    /// <param name="id">设变制一执行ID</param>
    ...
    public async Task<TaktEcSeizouikkaDto> UpdateEcSeizouikkaAsync(long id, TaktEcSeizouikkaUpdateDto dto)
    {
        ...
        await _ecExecPersistence.FanOutSeizouikkaFillableByEcModelAndRootMaterialAsync(entity);
        await _ecGijutsuStatusSynchronizer.RefreshByEcCodeAsync(entity.EcCode);
    ...
    /// <returns>DTO</returns>
    public async Task<TaktEcSeizouikkaDto> UpdateEcSeizouikkaDiscontinuedStatusAsync(TaktEcSeizouikkaDiscontinuedStatusDto dto)
    {
        var entity = await _ecSeizouikkaRepository.GetByIdAsync(dto.EcSeizouikkaId);
        if (entity == null)
        {
            throw new TaktBusinessException("设变制造一课执行不存在");
        }
        if (entity.TenantCode != CurrentTenantCode || entity.CompanyCode != CurrentCompanyCode)
        {
            throw new TaktBusinessException("设变制造一课执行不存在");
        ...
        return await GetEcSeizouikkaByIdAsync(dto.EcSeizouikkaId) ?? throw new TaktBusinessException("设变制造一课执行不存在");
    }
    ...
    private static Expression<Func<TaktEcSeizouikka, bool>> QueryExpression(TaktEcSeizouikkaQueryDto? queryDto)
    {
        ...
        exp = exp.And(TaktEcSeizouikkaQueryHelper.VisibleExecExpression());
