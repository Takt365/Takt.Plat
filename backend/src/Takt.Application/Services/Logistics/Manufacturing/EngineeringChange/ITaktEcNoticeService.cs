...
public interface ITaktEcNoticeService
{
    ...
    /// <returns>DTO</returns>
    Task<TaktEcNoticeDto> UpdateEcNoticeStatusAsync(TaktEcNoticeStatusDto dto);
    ...
    Task<(string fileName, byte[] content)> GetEcNoticeTemplateAsync(string? sheetName = null, string? fileName = null);
    ...
    Task<(int success, int fail, List<string> errors)> ImportEcNoticeAsync(Stream fileStream, string? sheetName = null);
    ...
    /// <param name="fileName">文件名</param>
    /// <returns>Excel 文件</returns>
    Task<(string fileName, byte[] fileContent)> ExportEcNoticeAsync(TaktEcNoticeQueryDto? query = null, string? sheetName = null, string? fileName = null);
