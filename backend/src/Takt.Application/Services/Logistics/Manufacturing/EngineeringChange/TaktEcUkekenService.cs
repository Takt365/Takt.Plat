...
/// 设变受检执行应用服务
/// </summary>
public class TaktEcUkekenService : TaktServiceBase, ITaktEcUkekenService
{
    private readonly ITaktCompanyRepository<TaktEcUkeken> _ecUkekenRepository;
    ...
    /// 构造函数
    /// </summary>
    /// <param name="ecUkekenRepository">设变受检执行仓储</param>
    ...
    public TaktEcUkekenService(
        ITaktCompanyRepository<TaktEcUkeken> ecUkekenRepository,
    ...
    {
        _ecUkekenRepository = ecUkekenRepository;
    ...
    public async Task<TaktPagedResult<TaktEcUkekenDto>> GetEcUkekenListAsync(TaktEcUkekenQueryDto queryDto)
    {
        var predicate = QueryExpression(queryDto);
        var (data, total) = await _ecUkekenRepository.GetPagedAsync(
            queryDto.PageIndex,
            queryDto.PageSize,
            predicate);
        return TaktPagedResult<TaktEcUkekenDto>.Create(
            data.Adapt<List<TaktEcUkekenDto>>(),
            total,
            queryDto.PageIndex,
            queryDto.PageSize);
    }
    ...
    /// <returns>DTO</returns>
    public async Task<TaktEcUkekenDto?> GetEcUkekenByIdAsync(long id)
    {
        var entity = await _ecUkekenRepository.GetByIdAsync(id);
    ...
    /// <returns>DTO</returns>
    public async Task<TaktEcUkekenDto> CreateEcUkekenAsync(TaktEcUkekenCreateDto dto)
    {
        var entity = dto.Adapt<TaktEcUkeken>();
        entity.IsObsolete = 0;
        ...
        return await GetEcUkekenByIdAsync(entity.Id) ?? entity.Adapt<TaktEcUkekenDto>();
    }

    /// <summary>
    /// 更新设变受检执行（同设变单号+新物料且新品需检验=1 的执行行一并写入可填字段）
    /// </summary>
    /// <param name="id">设变受检执行ID</param>
    /// <param name="dto">更新DTO</param>
    /// <returns>DTO</returns>
    public async Task<TaktEcUkekenDto> UpdateEcUkekenAsync(long id, TaktEcUkekenUpdateDto dto)
    {
        var entity = await _ecUkekenRepository.GetByIdAsync(id);
        if (entity == null)
        {
        ...
        await _ecGijutsuStatusSynchronizer.RefreshByEcCodeAsync(entity.EcCode);
        return await GetEcUkekenByIdAsync(id) ?? throw new TaktBusinessException("设变受检执行不存在");
    }
    ...
    /// <param name="id">设变受检执行ID</param>
    /// <returns>任务</returns>
    public async Task DeleteEcUkekenByIdAsync(long id)
    {
        var entity = await _ecUkekenRepository.GetByIdAsync(id);
        if (entity == null)
        {
            throw new TaktBusinessException("设变受检执行不存在或已删除");
        }
        ...
        {
            throw new TaktBusinessException("设变受检执行已作废");
        }
        entity.IsObsolete = 1;
        await _ecUkekenRepository.UpdateAsync(entity);
        await _ecGijutsuStatusSynchronizer.RefreshByEcCodeAsync(entity.EcCode);
    ...
    /// <returns>任务</returns>
    public async Task DeleteEcUkekenBatchAsync(IEnumerable<long> ids)
    {
    ...
    /// <returns>DTO</returns>
    public async Task<TaktEcUkekenDto> UpdateEcUkekenObsoleteAsync(TaktEcUkekenObsoleteDto dto)
    {
    ...
    public async Task<(string fileName, byte[] content)> GetEcUkekenTemplateAsync(string? sheetName = null, string? fileName = null)
    {
        return await TaktExcelHelper.GenerateTemplateAsync<TaktEcUkekenTemplateDto>(
            sheetName ?? "设变受检执行导入模板",
            fileName ?? "设变受检执行导入模板.xlsx");
    }
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportEcUkekenAsync(Stream fileStream, string? sheetName = null)
    {
        var errors = new List<string>();
        var success = 0;
        var fail = 0;
        var rows = await TaktExcelHelper.ImportAsync<TaktEcUkekenImportDto>(fileStream, sheetName ?? "设变受检执行导入模板");
    ...
    /// <param name="fileName">文件名</param>
    /// <returns>Excel 文件</returns>
    public async Task<(string fileName, byte[] fileContent)> ExportEcUkekenAsync(TaktEcUkekenQueryDto? query = null, string? sheetName = null, string? fileName = null)
    {
        var predicate = QueryExpression(query ?? new TaktEcUkekenQueryDto());
        var list = await _ecUkekenRepository.GetListAsync(predicate);
        if (list == null || list.Count == 0)
        {
            return await TaktExcelHelper.ExportAsync(
                new List<TaktEcUkekenExportDto>(),
                sheetName ?? "设变受检执行数据",
                fileName ?? "设变受检执行导出.xlsx");
        }
        var exportData = list.Adapt<List<TaktEcUkekenExportDto>>();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            sheetName ?? "设变受检执行数据",
            fileName ?? "设变受检执行导出.xlsx");
    ...
    private static Expression<Func<TaktEcUkeken, bool>> QueryExpression(TaktEcUkekenQueryDto? queryDto)
    {
        var exp = Expressionable.Create<TaktEcUkeken>();
        ...
        exp = exp.And(TaktEcUkekenQueryHelper.VisibleExecExpression());
