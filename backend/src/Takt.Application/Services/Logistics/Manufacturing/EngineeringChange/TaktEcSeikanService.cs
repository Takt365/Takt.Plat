...
public class TaktEcSeikanService : TaktServiceBase, ITaktEcSeikanService
{
    ...
    public async Task<TaktEcSeikanDto?> GetEcSeikanByIdAsync(long id)
    {
        ...
        var dto = entity.Adapt<TaktEcSeikanDto>();
    ...
    public async Task<List<TaktSelectOption>> GetEcSeikanOptionsAsync(string? plantCode = null, string? keyword = null)
    {
    ...
    /// <summary>
    /// 更新设变生管执行停产状态（同步明细并自动填充/清除执行内容）
    /// </summary>
    /// <param name="dto">停产状态 DTO</param>
    /// <returns>DTO</returns>
    public async Task<TaktEcSeikanDto> UpdateEcSeikanDiscontinuedStatusAsync(TaktEcSeikanDiscontinuedStatusDto dto)
    {
    ...
    private static Expression<Func<TaktEcSeikan, bool>> QueryExpression(TaktEcSeikanQueryDto? queryDto)
    {
        ...
        exp = exp.And(TaktEcSeikanQueryHelper.VisibleExecExpression());
