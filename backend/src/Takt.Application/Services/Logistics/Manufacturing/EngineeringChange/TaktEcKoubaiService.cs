...
public class TaktEcKoubaiService : TaktServiceBase, ITaktEcKoubaiService
{
    ...
    public async Task<TaktEcKoubaiDto?> GetEcKoubaiByIdAsync(long id)
    {
        ...
        var dto = entity.Adapt<TaktEcKoubaiDto>();
    ...
    public async Task<List<TaktSelectOption>> GetEcKoubaiOptionsAsync(string? plantCode = null, string? keyword = null)
    {
    ...
    public async Task<TaktEcKoubaiDto> CreateEcKoubaiAsync(TaktEcKoubaiCreateDto dto)
    {
        ...
        var isUnique_ix_takt_logistics_manufacturing_ec_koubai_unique = await _uniqueValidator.IsUniqueAsync(
            ...
            x => x.EcDetailId == entity.EcDetailId
                ...
                && x.EcNewPurchaseType == entity.EcNewPurchaseType);
        if (!isUnique_ix_takt_logistics_manufacturing_ec_koubai_unique)
        {
            throw new TaktBusinessException("设变采购执行的EcDetailId、EcNewMaterialCode、EcNewPurchaseType已存在");
        }
    ...
    /// <summary>
    /// 更新设变采购执行停产状态（同步明细并自动填充/清除执行内容）
    /// </summary>
    /// <param name="dto">停产状态 DTO</param>
    /// <returns>DTO</returns>
    public async Task<TaktEcKoubaiDto> UpdateEcKoubaiDiscontinuedStatusAsync(TaktEcKoubaiDiscontinuedStatusDto dto)
    {
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportEcKoubaiAsync(Stream fileStream, string? sheetName = null)
    {
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            try
            {
                ...
                var importKey = $"{entity.EcDetailId}|{entity.EcNewMaterialCode}|{entity.EcNewPurchaseType}";
                if (!importSeenKeys.Add(importKey))
                {
                    throw new TaktBusinessException("与Excel中其他行重复（EcDetailId、EcNewMaterialCode、EcNewPurchaseType）");
                }
                var isUnique_ix_takt_logistics_manufacturing_ec_koubai_unique = await _uniqueValidator.IsUniqueAsync(
                    _ecKoubaiRepository,
                    x => x.EcDetailId == entity.EcDetailId
    ...
    private static Expression<Func<TaktEcKoubai, bool>> QueryExpression(TaktEcKoubaiQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.EcOldPartDisposition))
        {
            exp = exp.And(x => x.EcOldPartDisposition != null && x.EcOldPartDisposition.Contains(queryDto.EcOldPartDisposition));
        }
