...
public class TaktEcHinkanService : TaktServiceBase, ITaktEcHinkanService
{
    ...
    public async Task<TaktPagedResult<TaktEcHinkanDto>> GetEcHinkanListAsync(TaktEcHinkanQueryDto queryDto)
    {
        var predicate = QueryExpression(queryDto);
        var (data, total) = await _ecHinkanRepository.GetPagedAsync(
            queryDto.PageIndex,
            queryDto.PageSize,
            predicate);
        return TaktPagedResult<TaktEcHinkanDto>.Create(
            data.Adapt<List<TaktEcHinkanDto>>(),
            total,
            queryDto.PageIndex,
            queryDto.PageSize);
    }
    ...
    /// <returns>DTO</returns>
    public async Task<TaktEcHinkanDto?> GetEcHinkanByIdAsync(long id)
    {
        var entity = await _ecHinkanRepository.GetByIdAsync(id);
        if (entity == null || entity.TenantCode != CurrentTenantCode || entity.CompanyCode != CurrentCompanyCode)
        {
            return null;
        }
        var dto = entity.Adapt<TaktEcHinkanDto>();
    ...
    public async Task<List<TaktSelectOption>> GetEcHinkanOptionsAsync(string? plantCode = null, string? keyword = null)
    {
        EnsureThreeLayerContext();
        var list = await _ecHinkanRepository.GetListAsync(
    ...
    /// <returns>DTO</returns>
    public async Task<TaktEcHinkanDto> CreateEcHinkanAsync(TaktEcHinkanCreateDto dto)
    {
        var entity = dto.Adapt<TaktEcHinkan>();
        entity.IsObsolete = 0;
        ...
        {
            var maxLine = await _ecHinkanRepository.GetMaxIntAsync(
    ...
    /// <returns>DTO</returns>
    public async Task<TaktEcHinkanDto> UpdateEcHinkanAsync(long id, TaktEcHinkanUpdateDto dto)
    {
        var entity = await _ecHinkanRepository.GetByIdAsync(id);
        if (entity == null)
        {
    ...
    /// <param name="id">设变品管执行ID</param>
    /// <returns>任务</returns>
    public async Task DeleteEcHinkanByIdAsync(long id)
    {
        var entity = await _ecHinkanRepository.GetByIdAsync(id);
        if (entity == null)
        {
            throw new TaktBusinessException("设变品管执行不存在或已删除");
        }
        ...
        {
            throw new TaktBusinessException("设变品管执行已作废");
        }
        entity.IsObsolete = 1;
        await _ecHinkanRepository.UpdateAsync(entity);
        await _ecGijutsuStatusSynchronizer.RefreshByEcCodeAsync(entity.EcCode);
    ...
    /// <returns>任务</returns>
    public async Task DeleteEcHinkanBatchAsync(IEnumerable<long> ids)
    {
        ...
        {
            await DeleteEcHinkanByIdAsync(id);
        }
    }

    /// <summary>
    /// 更新设变品管执行停产状态（同步明细并自动填充/清除执行内容）
    /// </summary>
    /// <param name="dto">停产状态 DTO</param>
    /// <returns>DTO</returns>
    public async Task<TaktEcHinkanDto> UpdateEcHinkanDiscontinuedStatusAsync(TaktEcHinkanDiscontinuedStatusDto dto)
    {
        var entity = await _ecHinkanRepository.GetByIdAsync(dto.EcHinkanId);
        if (entity == null)
        {
            throw new TaktBusinessException("设变品管执行不存在");
        }
    ...
    /// <returns>DTO</returns>
    public async Task<TaktEcHinkanDto> UpdateEcHinkanObsoleteAsync(TaktEcHinkanObsoleteDto dto)
    {
        var entity = await _ecHinkanRepository.GetByIdAsync(dto.EcHinkanId);
        if (entity == null)
        {
    ...
    public async Task<(string fileName, byte[] content)> GetEcHinkanTemplateAsync(string? sheetName = null, string? fileName = null)
    {
        return await TaktExcelHelper.GenerateTemplateAsync<TaktEcHinkanTemplateDto>(
            sheetName ?? "设变品管执行导入模板",
            fileName ?? "设变品管执行导入模板.xlsx");
    }
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportEcHinkanAsync(Stream fileStream, string? sheetName = null)
    {
        var errors = new List<string>();
        var success = 0;
        var fail = 0;
        var rows = await TaktExcelHelper.ImportAsync<TaktEcHinkanImportDto>(fileStream, sheetName ?? "设变品管执行导入模板");
    ...
    /// <param name="fileName">文件名</param>
    /// <returns>Excel 文件</returns>
    public async Task<(string fileName, byte[] fileContent)> ExportEcHinkanAsync(TaktEcHinkanQueryDto? query = null, string? sheetName = null, string? fileName = null)
    {
        var predicate = QueryExpression(query ?? new TaktEcHinkanQueryDto());
        var list = await _ecHinkanRepository.GetListAsync(predicate);
        if (list == null || list.Count == 0)
        {
            return await TaktExcelHelper.ExportAsync(
                new List<TaktEcHinkanExportDto>(),
                sheetName ?? "设变品管执行数据",
                fileName ?? "设变品管执行导出.xlsx");
        }
        var exportData = list.Adapt<List<TaktEcHinkanExportDto>>();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            sheetName ?? "设变品管执行数据",
            fileName ?? "设变品管执行导出.xlsx");
    ...
    private static Expression<Func<TaktEcHinkan, bool>> QueryExpression(TaktEcHinkanQueryDto? queryDto)
    {
        var exp = Expressionable.Create<TaktEcHinkan>();
