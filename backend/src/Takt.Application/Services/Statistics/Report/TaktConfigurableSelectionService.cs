...
public class TaktConfigurableSelectionService : TaktServiceBase, ITaktConfigurableSelectionService
{
    ...
    {
        var predicate = QueryExpression(query ?? new TaktConfigurableSelectionQueryDto());
    ...
    private static Expression<Func<TaktConfigurableSelection, bool>> QueryExpression(TaktConfigurableSelectionQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.KeyWords))
        {
            ...
            exp = exp.And(x =>
                ...
                || SqlFunc.ToString(x.FilterOperator).Contains(keywords)
        ...
        {
            exp = exp.And(x => x.FilterOperator == queryDto.FilterOperator);
        }

        if (!string.IsNullOrEmpty(queryDto?.DefaultValue))
        {
            exp = exp.And(x => x.DefaultValue != null && x.DefaultValue.Contains(queryDto.DefaultValue));
        }

        if (!string.IsNullOrEmpty(queryDto?.DefaultValueTo))
        {
            exp = exp.And(x => x.DefaultValueTo != null && x.DefaultValueTo.Contains(queryDto.DefaultValueTo));
        }
