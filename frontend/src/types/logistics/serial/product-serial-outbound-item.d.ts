// ========================================
// 项目名称：节拍工厂·Takt Plat
// 命名空间：frontend/src/types/logistics/serial
// 文件名称：product-serial-outbound-item.d.ts
// 创建时间：2026-06-06
// 创建人：Takt365(Auto Generated)
// 功能描述：logistics/serial 模块类型定义（自动生成；类型名去 Takt 前缀与末尾 Dto，如 TaktCompanyDto → Company）
// 
// 版权信息：Copyright (c) 2025 Takt  All rights reserved.
// 免责声明：此软件使用 MIT License，作者不承担任何使用风险。
// ========================================

import type {
  CompanyDtoBase,
  TaktPagedQuery
} from '@/types/common';

/**
 * 产品序列号出库明细实体
 * 对应前端 TaktProductSerialOutboundItemDto
 * 继承 TaktCompanyDtoBase
 * 对应前端 ProductSerialOutboundItem
 * @description 对应后端 TaktProductSerialOutboundItemDto
 */
export interface ProductSerialOutboundItem extends CompanyDtoBase {
  /**
   * ProductSerialOutboundItemID（适配实体 Id，序列化为 string 以避免 Javascript 精度问题）
   */
  productSerialOutboundItemId: string;

  /**
   * 出库ID（主子表关系，序列化为string以避免Javascript精度问题）
   */
  outboundId: string;

  /**
   * 出库名称（填充字段）
   */
  outboundName?: string;

  /**
   * 出库单号（冗余字段，便于查询）
   */
  outboundNo: string;

  /**
   * 行号（项号/序号，固定步长=10）
   */
  lineNumber: number;

  /**
   * 出库序列号（唯一索引）
   */
  outboundSerialNo: string;

  /**
   * 关联入库ID(序列化为string以避免Javascript精度问题)
   */
  referenceInboundId: string;

  /**
   * 关联入库名称（填充字段）
   */
  referenceInboundName?: string;

  /**
   * 关联入库单号
   */
  referenceInboundNo: string;

  /**
   * 关联入库行号
   */
  referenceInboundLineNumber: number;

  /**
   * 出库时间
   */
  outboundTime: string;

  /**
   * 出库主表 （主表：TaktProductSerialOutbound）
   */
  outbound?: ProductSerialOutbound;

}


/**
 * ProductSerialOutboundItem 分页查询 DTO
 * 继承 TaktPagedQuery
 * 对应前端 ProductSerialOutboundItemQuery
 * @description 对应后端 TaktProductSerialOutboundItemQueryDto
 */
export interface ProductSerialOutboundItemQuery extends TaktPagedQuery {
  /**
   * 租户编码
   */
  tenantCode?: string;

  /**
   * 公司代码
   */
  companyCode?: string;

  /**
   * 出库ID（主子表关系，序列化为string以避免Javascript精度问题）
   */
  outboundId?: string;

  /**
   * 出库单号（冗余字段，便于查询）
   */
  outboundNo?: string;

  /**
   * 行号（项号/序号，固定步长=10）
   */
  lineNumber?: number;

  /**
   * 出库序列号（唯一索引）
   */
  outboundSerialNo?: string;

  /**
   * 关联入库ID(序列化为string以避免Javascript精度问题)
   */
  referenceInboundId?: string;

  /**
   * 关联入库单号
   */
  referenceInboundNo?: string;

  /**
   * 关联入库行号
   */
  referenceInboundLineNumber?: number;

  /**
   * 出库时间（范围查询-开始）
   */
  outboundTimeStart?: string;

  /**
   * 出库时间（范围查询-结束）
   */
  outboundTimeEnd?: string;

  /**
   * 创建时间（范围查询-开始）
   */
  createdAtStart?: string;

  /**
   * 创建时间（范围查询-结束）
   */
  createdAtEnd?: string;

  /**
   * 扩展字段JSON
   */
  extFieldJson?: string;

  /**
   * 备注（模糊查询）
   */
  remark?: string;

}


/**
 * 创建ProductSerialOutboundItem DTO
 * 对应前端 ProductSerialOutboundItemCreate
 * @description 对应后端 TaktProductSerialOutboundItemCreateDto
 */
export interface ProductSerialOutboundItemCreate {
  /**
   * 租户编码（登录上下文注入，对应请求头 X-Tenant-Code）
   */
  tenantCode: string;

  /**
   * 公司代码（登录或公司切换注入，对应请求头 X-Company-Code）
   */
  companyCode: string;

  /**
   * 当前公司默认区域文化 BCP47（登录或公司切换注入，须与 takt_company.default_culture 一致，用于写入校验）
   */
  companyDefaultCulture: string;

  /**
   * 出库ID（主子表关系，序列化为string以避免Javascript精度问题）
   */
  outboundId: string;

  /**
   * 出库单号（冗余字段，便于查询）
   */
  outboundNo: string;

  /**
   * 行号（项号/序号，固定步长=10）
   */
  lineNumber: number;

  /**
   * 出库序列号（唯一索引）
   */
  outboundSerialNo: string;

  /**
   * 关联入库ID(序列化为string以避免Javascript精度问题)
   */
  referenceInboundId: string;

  /**
   * 关联入库单号
   */
  referenceInboundNo: string;

  /**
   * 关联入库行号
   */
  referenceInboundLineNumber: number;

  /**
   * 出库时间
   */
  outboundTime: string;

  /**
   * 扩展字段JSON
   */
  extFieldJson?: string;

  /**
   * 备注
   */
  remark?: string;

}


/**
 * 更新ProductSerialOutboundItem DTO
 * 继承 TaktProductSerialOutboundItemCreateDto，添加 ProductSerialOutboundItemId 字段
 * 对应前端 ProductSerialOutboundItemUpdate
 * @description 对应后端 TaktProductSerialOutboundItemUpdateDto
 */
export interface ProductSerialOutboundItemUpdate extends ProductSerialOutboundItemCreate {
  /**
   * ProductSerialOutboundItemID（标识要更新的实体）
   */
  productSerialOutboundItemId: string;

}


/**
 * ProductSerialOutboundItem 导入模板行 DTO
 * 对应前端 ProductSerialOutboundItemTemplate
 * @description 对应后端 TaktProductSerialOutboundItemTemplateDto
 */
export interface ProductSerialOutboundItemTemplate {
  /**
   * 租户编码（登录上下文注入，对应请求头 X-Tenant-Code）
   */
  tenantCode?: string;

  /**
   * 公司代码（登录或公司切换注入，对应请求头 X-Company-Code）
   */
  companyCode?: string;

  /**
   * 出库ID（主子表关系，序列化为string以避免Javascript精度问题）
   */
  outboundId?: string;

  /**
   * 出库单号（冗余字段，便于查询）
   */
  outboundNo?: string;

  /**
   * 行号（项号/序号，固定步长=10）
   */
  lineNumber?: number;

  /**
   * 出库序列号（唯一索引）
   */
  outboundSerialNo?: string;

  /**
   * 关联入库ID(序列化为string以避免Javascript精度问题)
   */
  referenceInboundId?: string;

  /**
   * 关联入库单号
   */
  referenceInboundNo?: string;

  /**
   * 关联入库行号
   */
  referenceInboundLineNumber?: number;

  /**
   * 扩展字段JSON
   */
  extFieldJson?: string;

  /**
   * 备注
   */
  remark?: string;

}


/**
 * ProductSerialOutboundItem 导入 DTO（独立实现，不继承 TemplateDto）
 * 对应前端 ProductSerialOutboundItemImport
 * @description 对应后端 TaktProductSerialOutboundItemImportDto
 */
export interface ProductSerialOutboundItemImport {
  /**
   * 租户编码（登录上下文注入，对应请求头 X-Tenant-Code）
   */
  tenantCode?: string;

  /**
   * 公司代码（登录或公司切换注入，对应请求头 X-Company-Code）
   */
  companyCode?: string;

  /**
   * 当前公司默认区域文化 BCP47（登录或公司切换注入，须与 takt_company.default_culture 一致，用于写入校验）
   */
  companyDefaultCulture?: string;

  /**
   * 出库ID（主子表关系，序列化为string以避免Javascript精度问题）
   */
  outboundId?: string;

  /**
   * 出库单号（冗余字段，便于查询）
   */
  outboundNo?: string;

  /**
   * 行号（项号/序号，固定步长=10）
   */
  lineNumber?: number;

  /**
   * 出库序列号（唯一索引）
   */
  outboundSerialNo?: string;

  /**
   * 关联入库ID(序列化为string以避免Javascript精度问题)
   */
  referenceInboundId?: string;

  /**
   * 关联入库单号
   */
  referenceInboundNo?: string;

  /**
   * 关联入库行号
   */
  referenceInboundLineNumber?: number;

  /**
   * 扩展字段JSON
   */
  extFieldJson?: string;

  /**
   * 备注
   */
  remark?: string;

}


/**
 * ProductSerialOutboundItem 导出 DTO（独立实现，不继承响应 Dto）
 * 对应前端 ProductSerialOutboundItemExport
 * @description 对应后端 TaktProductSerialOutboundItemExportDto
 */
export interface ProductSerialOutboundItemExport {
  /**
   * ProductSerialOutboundItemID
   */
  productSerialOutboundItemId: string;

  /**
   * 公司代码
   */
  companyCode: string;

  /**
   * 出库ID（主子表关系，序列化为string以避免Javascript精度问题）
   */
  outboundId: string;

  /**
   * 出库单号（冗余字段，便于查询）
   */
  outboundNo: string;

  /**
   * 行号（项号/序号，固定步长=10）
   */
  lineNumber: number;

  /**
   * 出库序列号（唯一索引）
   */
  outboundSerialNo: string;

  /**
   * 关联入库ID(序列化为string以避免Javascript精度问题)
   */
  referenceInboundId: string;

  /**
   * 关联入库单号
   */
  referenceInboundNo: string;

  /**
   * 关联入库行号
   */
  referenceInboundLineNumber: number;

  /**
   * 出库时间
   */
  outboundTime: string;

  /**
   * 扩展字段JSON
   */
  extFieldJson?: string;

  /**
   * 备注
   */
  remark?: string;

  /**
   * 创建时间
   */
  createdAt: string;

}

