// ========================================
// 项目名称：节拍工厂·Takt Plat
// 命名空间：frontend/src/types/statistics/report
// 文件名称：configurable-selection.d.ts
// 创建时间：2026-06-09
// 创建人：Takt365(Auto Generated)
// 功能描述：statistics/report 模块类型定义（自动生成；类型名去 Takt 前缀与末尾 Dto，如 TaktCompanyDto → Company）
// 
// 版权信息：Copyright (c) 2025 Takt  All rights reserved.
// 免责声明：此软件使用 MIT License，作者不承担任何使用风险。
// ========================================

import type {
  CompanyDtoBase,
  TaktPagedQuery
} from '@/types/common';

/**
 * 自定义报表筛选条件（SQVI 运行时筛选项）
 * 对应前端 TaktConfigurableSelectionDto
 * 继承 TaktCompanyDtoBase
 * 对应前端 ConfigurableSelection
 * @description 对应后端 TaktConfigurableSelectionDto
 */
export interface ConfigurableSelection extends CompanyDtoBase {
  /**
   * ConfigurableSelectionID（适配实体 Id，序列化为 string 以避免 Javascript 精度问题）
   */
  configurableSelectionId: string;

  /**
   * 关联报表主表 ID（主子表关系）
   */
  configurableId: string;

  /**
   * 关联报表主表 名称（填充字段）
   */
  configurableName?: string;

  /**
   * 数据源别名
   */
  sourceAlias: string;

  /**
   * 列名
   */
  columnName: string;

  /**
   * 显示名称（SQVI 筛选项 标签）
   */
  displayName: string;

  /**
   * 比较运算符
   */
  filterOperator: number;

  /**
   * 默认值（单值或 IN 列表逗号分隔）
   */
  defaultValue?: string;

  /**
   * 区间结束值（BETWEEN 时使用）
   */
  defaultValueTo?: string;

  /**
   * 是否必填（0=否 1=是）
   */
  isRequired: number;

  /**
   * 排序号（SQVI 筛选项 展示顺序）
   */
  sortOrder: number;

  /**
   * 关联的报表主表 （主表：TaktConfigurable）
   */
  configurable?: Configurable;

}


/**
 * ConfigurableSelection 分页查询 DTO
 * 继承 TaktPagedQuery
 * 对应前端 ConfigurableSelectionQuery
 * @description 对应后端 TaktConfigurableSelectionQueryDto
 */
export interface ConfigurableSelectionQuery extends TaktPagedQuery {
  /**
   * 租户编码
   */
  tenantCode?: string;

  /**
   * 公司代码
   */
  companyCode?: string;

  /**
   * 关联报表主表 ID（主子表关系）
   */
  configurableId?: string;

  /**
   * 数据源别名
   */
  sourceAlias?: string;

  /**
   * 列名
   */
  columnName?: string;

  /**
   * 显示名称（SQVI 筛选项 标签）
   */
  displayName?: string;

  /**
   * 比较运算符
   */
  filterOperator?: number;

  /**
   * 默认值（单值或 IN 列表逗号分隔）
   */
  defaultValue?: string;

  /**
   * 区间结束值（BETWEEN 时使用）
   */
  defaultValueTo?: string;

  /**
   * 是否必填（0=否 1=是）
   */
  isRequired?: number;

  /**
   * 排序号（SQVI 筛选项 展示顺序）
   */
  sortOrder?: number;

  /**
   * 创建时间（范围查询-开始）
   */
  createdAtStart?: string;

  /**
   * 创建时间（范围查询-结束）
   */
  createdAtEnd?: string;

  /**
   * 扩展字段JSON
   */
  ExtField?: string;

  /**
   * 备注（模糊查询）
   */
  remark?: string;

}


/**
 * 创建ConfigurableSelection DTO
 * 对应前端 ConfigurableSelectionCreate
 * @description 对应后端 TaktConfigurableSelectionCreateDto
 */
export interface ConfigurableSelectionCreate {
  /**
   * 租户编码（登录上下文注入，对应请求头 X-Tenant-Code）
   */
  tenantCode: string;

  /**
   * 公司代码（登录或公司切换注入，对应请求头 X-Company-Code）
   */
  companyCode: string;

  /**
   * 当前公司默认区域文化 BCP47（登录或公司切换注入，须与 takt_company.default_culture 一致，用于写入校验）
   */
  companyDefaultCulture: string;

  /**
   * 关联报表主表 ID（主子表关系）
   */
  configurableId: string;

  /**
   * 数据源别名
   */
  sourceAlias: string;

  /**
   * 列名
   */
  columnName: string;

  /**
   * 显示名称（SQVI 筛选项 标签）
   */
  displayName: string;

  /**
   * 比较运算符
   */
  filterOperator: number;

  /**
   * 默认值（单值或 IN 列表逗号分隔）
   */
  defaultValue?: string;

  /**
   * 区间结束值（BETWEEN 时使用）
   */
  defaultValueTo?: string;

  /**
   * 是否必填（0=否 1=是）
   */
  isRequired: number;

  /**
   * 排序号（SQVI 筛选项 展示顺序）
   */
  sortOrder: number;

  /**
   * 扩展字段JSON
   */
  ExtField?: string;

  /**
   * 备注
   */
  remark?: string;

}


/**
 * 更新ConfigurableSelection DTO
 * 继承 TaktConfigurableSelectionCreateDto，添加 ConfigurableSelectionId 字段
 * 对应前端 ConfigurableSelectionUpdate
 * @description 对应后端 TaktConfigurableSelectionUpdateDto
 */
export interface ConfigurableSelectionUpdate extends ConfigurableSelectionCreate {
  /**
   * ConfigurableSelectionID（标识要更新的实体）
   */
  configurableSelectionId: string;

}


/**
 * ConfigurableSelection 排序更新 DTO
 * 对应前端 ConfigurableSelectionSort
 * @description 对应后端 TaktConfigurableSelectionSortDto
 */
export interface ConfigurableSelectionSort {
  /**
   * ConfigurableSelectionID
   */
  configurableSelectionId: string;

  /**
   * 排序号（SQVI 筛选项 展示顺序）
   */
  sortOrder: number;

}


/**
 * ConfigurableSelection 导入模板行 DTO
 * 对应前端 ConfigurableSelectionTemplate
 * @description 对应后端 TaktConfigurableSelectionTemplateDto
 */
export interface ConfigurableSelectionTemplate {
  /**
   * 租户编码（登录上下文注入，对应请求头 X-Tenant-Code）
   */
  tenantCode?: string;

  /**
   * 公司代码（登录或公司切换注入，对应请求头 X-Company-Code）
   */
  companyCode?: string;

  /**
   * 关联报表主表 ID（主子表关系）
   */
  configurableId?: string;

  /**
   * 数据源别名
   */
  sourceAlias?: string;

  /**
   * 列名
   */
  columnName?: string;

  /**
   * 显示名称（SQVI 筛选项 标签）
   */
  displayName?: string;

  /**
   * 比较运算符
   */
  filterOperator?: number;

  /**
   * 默认值（单值或 IN 列表逗号分隔）
   */
  defaultValue?: string;

  /**
   * 区间结束值（BETWEEN 时使用）
   */
  defaultValueTo?: string;

  /**
   * 是否必填（0=否 1=是）
   */
  isRequired?: number;

  /**
   * 排序号（SQVI 筛选项 展示顺序）
   */
  sortOrder?: number;

  /**
   * 扩展字段JSON
   */
  ExtField?: string;

  /**
   * 备注
   */
  remark?: string;

}


/**
 * ConfigurableSelection 导入 DTO（独立实现，不继承 TemplateDto）
 * 对应前端 ConfigurableSelectionImport
 * @description 对应后端 TaktConfigurableSelectionImportDto
 */
export interface ConfigurableSelectionImport {
  /**
   * 租户编码（登录上下文注入，对应请求头 X-Tenant-Code）
   */
  tenantCode?: string;

  /**
   * 公司代码（登录或公司切换注入，对应请求头 X-Company-Code）
   */
  companyCode?: string;

  /**
   * 当前公司默认区域文化 BCP47（登录或公司切换注入，须与 takt_company.default_culture 一致，用于写入校验）
   */
  companyDefaultCulture?: string;

  /**
   * 关联报表主表 ID（主子表关系）
   */
  configurableId?: string;

  /**
   * 数据源别名
   */
  sourceAlias?: string;

  /**
   * 列名
   */
  columnName?: string;

  /**
   * 显示名称（SQVI 筛选项 标签）
   */
  displayName?: string;

  /**
   * 比较运算符
   */
  filterOperator?: number;

  /**
   * 默认值（单值或 IN 列表逗号分隔）
   */
  defaultValue?: string;

  /**
   * 区间结束值（BETWEEN 时使用）
   */
  defaultValueTo?: string;

  /**
   * 是否必填（0=否 1=是）
   */
  isRequired?: number;

  /**
   * 排序号（SQVI 筛选项 展示顺序）
   */
  sortOrder?: number;

  /**
   * 扩展字段JSON
   */
  ExtField?: string;

  /**
   * 备注
   */
  remark?: string;

}


/**
 * ConfigurableSelection 导出 DTO（独立实现，不继承响应 Dto）
 * 对应前端 ConfigurableSelectionExport
 * @description 对应后端 TaktConfigurableSelectionExportDto
 */
export interface ConfigurableSelectionExport {
  /**
   * ConfigurableSelectionID
   */
  configurableSelectionId: string;

  /**
   * 公司代码
   */
  companyCode: string;

  /**
   * 关联报表主表 ID（主子表关系）
   */
  configurableId: string;

  /**
   * 数据源别名
   */
  sourceAlias: string;

  /**
   * 列名
   */
  columnName: string;

  /**
   * 显示名称（SQVI 筛选项 标签）
   */
  displayName: string;

  /**
   * 比较运算符
   */
  filterOperator: number;

  /**
   * 默认值（单值或 IN 列表逗号分隔）
   */
  defaultValue?: string;

  /**
   * 区间结束值（BETWEEN 时使用）
   */
  defaultValueTo?: string;

  /**
   * 是否必填（0=否 1=是）
   */
  isRequired: number;

  /**
   * 排序号（SQVI 筛选项 展示顺序）
   */
  sortOrder: number;

  /**
   * 扩展字段JSON
   */
  ExtField?: string;

  /**
   * 备注
   */
  remark?: string;

  /**
   * 创建时间
   */
  createdAt: string;

}

