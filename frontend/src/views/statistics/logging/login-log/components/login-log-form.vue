...
<!-- 命名空间：@/views/statistics/logging/login-log/components -->
<!-- 文件名称：login-log-form.vue -->
<!-- 功能描述：登录日志实体维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
/**
 * 登录日志实体维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/statistics/logging/login-log/components
 */
import { reactive, watch, computed, ref } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import type { LoginLogCreate } from '@/types/statistics/logging/login-log'
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","username","loginType","browser","os","userAgent","loginResult","loginMessage","loginIp","loginLocation","logoutAt","extFieldJson","remark"]


/** 父级传入的编辑 DTO；新增时为 undefined 或空对象 */
interface Props {
  formData?: Partial<LoginLogCreate & { loginLogId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.loginLogId
    if (isCreate) {
