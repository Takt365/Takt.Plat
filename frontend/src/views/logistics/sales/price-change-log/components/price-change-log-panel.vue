...
import {
  useSalesPriceChangeLogI18n,
  SALESPRICECHANGELOG_LIST_FIELDS,
  SALESPRICECHANGELOG_QUERY_STRING_FIELDS,
  SALESPRICECHANGELOG_QUERY_FIELDS,
  SALESPRICECHANGELOG_SELF_I18N_KEY,
} from '../composables/use-price-change-log-i18n'
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(SALESPRICECHANGELOG_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
    (typeof SALESPRICECHANGELOG_QUERY_STRING_FIELDS)[number],
    string
  >
  return {
    ...form,
...
const queryFieldsMeta = computed(() =>
  SALESPRICECHANGELOG_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceChangeLog }) =>
      String(getSalesPriceChangeLogField(record, 'salesPriceName') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceChangeLog }) =>
      String(getSalesPriceChangeLogField(record, 'salesPrice') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record: SalesPriceChangeLog, selected: boolean) => {
    ...
    } else if (selectedRow.value && getSalesPriceChangeLogId(selectedRow.value) === getSalesPriceChangeLogId(record)) {
...
function buildListQuery(overrides?: Partial<SalesPriceChangeLogQuery>): SalesPriceChangeLogQuery {
  ...
  for (const key of SALESPRICECHANGELOG_QUERY_STRING_FIELDS) {
