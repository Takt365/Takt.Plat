...
<!-- 文件名称：price-change-log-form.vue -->
<!-- 功能描述：Takt销售价格实体子表 salesPriceChangeLog 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form price-change-log-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    <a-tabs
      ...
      class="price-change-log-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * Takt销售价格实体子表 salesPriceChangeLog 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 salesPriceChangeLogId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.salesPriceChangeLogId) {
...
/** 映射为 Create/Update DTO（含主表外键 salesPriceId） */
function getValues(): Record<string, any> {
  const payload = { ...formState }
  if ('sortOrder' in payload) delete payload.sortOrder
  payload.salesPriceId = props.masterId
  return payload
}
