...
<!-- 文件名称：price-scale-quantity-panel.vue -->
<!-- 功能描述：Takt销售价格明细实体主表实体右侧明细 salesPriceScaleQuantity 独立 CRUD（按主表选中 salesPriceItemId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="price-scale-quantity-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="price-scale-quantity-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
        ...
        id-column-key="salesPriceScaleQuantityId"
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-logistics-sales-price-item-price-scale-quantity"
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="SALESPRICESCALEQUANTITY_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * Takt销售价格明细实体子表 salesPriceScaleQuantity 右栏面板
...
import SalesPriceScaleQuantityForm from './price-scale-quantity-form.vue'
import { useSalesPriceItemMasterContext } from '../composables/use-price-item-master-context'
import {
  getSalesPriceScaleQuantityList,
  getSalesPriceScaleQuantityById,
  createSalesPriceScaleQuantity,
  updateSalesPriceScaleQuantity,
  deleteSalesPriceScaleQuantityById,
  deleteSalesPriceScaleQuantityBatch,
  getSalesPriceScaleQuantityTemplate,
  importSalesPriceScaleQuantity,
  exportSalesPriceScaleQuantity,
} from '@/api/logistics/sales/price-scale-quantity'
import type { SalesPriceScaleQuantity, SalesPriceScaleQuantityQuery } from '@/types/logistics/sales/price-scale-quantity'

import {
  useSalesPriceScaleQuantityI18n,
  SALESPRICESCALEQUANTITY_DEFAULT_VISIBLE_COLUMN_KEYS,
  SALESPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS,
  SALESPRICESCALEQUANTITY_QUERY_STRING_FIELDS,
  SALESPRICESCALEQUANTITY_QUERY_FIELDS,
  SALESPRICESCALEQUANTITY_SELF_I18N_KEY,
} from '../composables/use-price-scale-quantity-i18n'
...
const excelNames = taktExcelEntityNames('TaktSalesPriceScaleQuantity')
/** 快捷查询占位文案 */
...
const dataSource = ref<SalesPriceScaleQuantity[]>([])
const currentPage = ref(getTaktDefaultPageIndex())
const pageSize = ref(getTaktDefaultPageSize())
const total = ref(0)
const queryKeyword = ref('')
const selectedRow = ref<SalesPriceScaleQuantity | null>(null)
const selectedRows = ref<SalesPriceScaleQuantity[]>([])
const selectedRowKeys = ref<(string | number)[]>([])
const formVisible = ref(false)
const formTitle = ref('')
const formData = ref<Partial<SalesPriceScaleQuantity>>({})
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of SALESPRICESCALEQUANTITY_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(SALESPRICESCALEQUANTITY_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  SALESPRICESCALEQUANTITY_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...SALESPRICESCALEQUANTITY_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...SALESPRICESCALEQUANTITY_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'salesPriceScaleQuantityId'
...
function getSalesPriceScaleQuantityId(record: SalesPriceScaleQuantity | Record<string, unknown>): string {
  return String((record as SalesPriceScaleQuantity)?.[entityIdName] ?? '')
}

function getSalesPriceScaleQuantityField(record: SalesPriceScaleQuantity | Record<string, unknown>, field: string): unknown {
  return (record as SalesPriceScaleQuantity)?.[field as keyof SalesPriceScaleQuantity]
}

const columns = computed<TableColumnsType>(() => [
  {
    title: t('common.page.entity.id'),
    dataIndex: 'salesPriceScaleQuantityId',
    key: 'salesPriceScaleQuantityId',
    width: 80,
    resizable: true,
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: SalesPriceScaleQuantity }) =>
      String(getSalesPriceScaleQuantityField(record, 'salesPriceScaleQuantityId') ?? ''),
  },
  {
    title: pi.label('salesPriceItemId'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceScaleQuantity }) =>
      String(getSalesPriceScaleQuantityField(record, 'salesPriceItemId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceScaleQuantity }) =>
      String(getSalesPriceScaleQuantityField(record, 'salesPriceCode') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceScaleQuantity }) =>
      String(getSalesPriceScaleQuantityField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: SalesPriceScaleQuantity) => void handleEdit(record),
      },
      {
        key: 'delete',
        ...
        onClick: (record: SalesPriceScaleQuantity) => void handleDeleteOne(record),
      }],
  })])
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'salesPriceScaleQuantityId',
...
const summarySumFieldSet = new Set<string>(SALESPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    SALESPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof SALESPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of SALESPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS) {
      const num = Number(getSalesPriceScaleQuantityField(row, field))
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: SalesPriceScaleQuantity[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: SalesPriceScaleQuantity, selected: boolean) => {
    if (selected) {
      selectedRow.value = record
    } else if (selectedRow.value && getSalesPriceScaleQuantityId(selectedRow.value) === getSalesPriceScaleQuantityId(record)) {
      selectedRow.value = null
    }
  },
  onSelectAll: (selected: boolean, selectedRowsData: SalesPriceScaleQuantity[]) => {
 ...
 * @param record 行数据
 */
function onClickRow(record: SalesPriceScaleQuantity) {
  const key = getSalesPriceScaleQuantityId(record)
  return {
    onClick: () => {
 ...
 * @returns {SalesPriceScaleQuantityQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<SalesPriceScaleQuantityQuery>): SalesPriceScaleQuantityQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: SalesPriceScaleQuantityQuery = {
  ...
  const assignTrimmed = (key: keyof SalesPriceScaleQuantityQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
async function loadData() {
  ...
  loading.value = true
  try {
    const res = await getSalesPriceScaleQuantityList(buildListQuery())
...
async function handleEdit(record: SalesPriceScaleQuantity) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getSalesPriceScaleQuantityById(getSalesPriceScaleQuantityId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.salesPriceScaleQuantityId
    if (id) {
      await updateSalesPriceScaleQuantity(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createSalesPriceScaleQuantity(payload)
...
async function handleDeleteOne(record: SalesPriceScaleQuantity) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteSalesPriceScaleQuantityById(getSalesPriceScaleQuantityId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getSalesPriceScaleQuantityId(r)).filter(Boolean)
      await deleteSalesPriceScaleQuantityBatch(ids)
      message.success(t('common.feedback.deleted', { target: pi.self() }))
      await loadData()
    },
  })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getSalesPriceScaleQuantityTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importSalesPriceScaleQuantity(file, sheetName)
...
async function handleExport() {
  ...
  try {
    ...
    const exportMeta = await exportSalesPriceScaleQuantity(
