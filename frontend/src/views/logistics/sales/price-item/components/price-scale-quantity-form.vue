...
function getValues(): Record<string, any> {
  ...
  if (props.formData?.salesPriceScaleQuantityId) {
    payload.salesPriceScaleQuantityId = props.formData.salesPriceScaleQuantityId
  }
  ...
  if (masterRow) {
    const masterCode = masterRow.salesPriceItemCode ?? masterRow.SalesPriceItemCode
    const masterName = masterRow.salesPriceItemName ?? masterRow.SalesPriceItemName
    if (masterCode != null && masterCode !== '' && !payload.salesPriceItemCode) {
      payload.salesPriceItemCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.salesPriceItemName) {
      payload.salesPriceItemName = masterName
    }
