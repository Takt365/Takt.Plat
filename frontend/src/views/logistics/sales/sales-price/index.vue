...
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt销售价格实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="logistics-sales-sales-price">
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.salesPrice._self')"
      :width="600"
      :footer="null"
...
/**
 * Takt销售价格实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getSalesPriceField(record, 'priceStatus') ?? ''
  },
  {
    title: t('entity.salesPrice.items'),
    dataIndex: 'items',
    key: 'items',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getSalesPriceField(record, 'items') ?? ''
  },
  {
    title: t('entity.salesPrice.changelogs'),
    dataIndex: 'changeLogs',
    key: 'changeLogs',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getSalesPriceField(record, 'changeLogs') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.salesPrice._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: SalesPrice) {
  formTitle.value = t('common.page.button.edit') + t('entity.salesPrice._self')
