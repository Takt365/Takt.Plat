...
<!-- 文件名称：sales-monthly-trend-panel.vue -->
<!-- 功能描述：月销售推移转置表（工厂×客户×月份） -->
      ...
      >
        <template #bodyCell="{ column, record, text }">
          <template v-if="String(column.key).startsWith('period_')">
            {{ formatPeriodAmount(record as SalesMonthlyTrend, String(column.key)) }}
          </template>
          <template v-else-if="column.key === 'trend'">
            <span :class="trendClass((record as SalesMonthlyTrend).trend || 'none')">
              {{ trendLabel((record as SalesMonthlyTrend).trend || 'none') }}
            </span>
          </template>
          <template v-else-if="column.key === 'varianceAmount'">
            <span :class="varianceClass((record as SalesMonthlyTrend).varianceAmount)">
              {{ formatCost((record as SalesMonthlyTrend).varianceAmount) }}
            </span>
          </template>
          <template v-else-if="column.key === 'variancePercent'">
            <span :class="varianceClass((record as SalesMonthlyTrend).varianceAmount)">
              {{ formatPercent((record as SalesMonthlyTrend).variancePercent) }}
            </span>
          </template>
          <template v-else>
            {{ text }}
...
import {
  exportSalesMonthlyTrendAnalysis,
  getSalesMonthlyTrendAnalysis,
} from '@/api/logistics/sales/monthly-trend'
import type { SalesMonthlyTrend } from '@/types/logistics/sales/monthly-trend'
...
/** 行数据 */
const rows = ref<SalesMonthlyTrend[]>([])
/** 期间列 */
...
const summaryText = computed(() => {
  ...
  return t(`${localePrefix}.summary`, { count: customerCount.value })
})
...
/**
 * 行主键
 * @param {SalesMonthlyTrend} record 行
 * @returns {string} key
 */
function getRowKey(record: SalesMonthlyTrend): string {
 ...
 * @returns {string} 文本
 */
function formatPeriodAmount(record: SalesMonthlyTrend, columnKey: string): string {
...
async function handleExport() {
  ...
  try {
    const exportMeta = await exportSalesMonthlyTrendAnalysis({
      ...query,
      pageIndex: 1,
      pageSize: 1,
    }, '月销售推移表', '月销售推移表.xlsx')
    ...
    const fileName = resolveExportDownloadFileName({
      ...
      fallbackBase: `sales-monthly-trend_${query.plantCode}`,
    })
