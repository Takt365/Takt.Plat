...
<!-- 功能描述：Takt销售价格实体主表实体右侧明细 salesPriceItem 独立 CRUD（按主表选中 salesPriceId 分页） -->
...
<template>
  <div class="price-item-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div
      ...
      class="price-item-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
...
import {
  ...
  SALESPRICEITEM_DEFAULT_VISIBLE_COLUMN_KEYS,
  SALESPRICEITEM_SUMMARY_SUM_FIELDS,
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...SALESPRICEITEM_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...SALESPRICEITEM_DEFAULT_VISIBLE_COLUMN_KEYS]
}
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'salesPriceId') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'salesPriceSeq') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'priceType') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'scaleType') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'scaleBasis') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'scaleQuantity') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'scaleUnit') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'calculationType') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'untaxedPrice') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'taxIncludedPrice') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'priceUnit') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'unitOfMeasure') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'roundingValue') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'plannedDeliveryTimeDays') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesPriceItem }) =>
      String(getSalesPriceItemField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const summarySumFieldSet = new Set<string>(SALESPRICEITEM_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    SALESPRICEITEM_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof SALESPRICEITEM_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of SALESPRICEITEM_SUMMARY_SUM_FIELDS) {
      const num = Number(getSalesPriceItemField(row, field))
