...
<!-- 功能描述：产品成本分析报表：单个产品下明细组件×月材料成本转置涨跌 -->
...
/**
 * 合计文案所在列：跳过序号，取第一个业务数据列（如 BOM 层级）
...
async function handleExport() {
  ...
  try {
    const exportBase = buildBomExportBaseName('DTA 产品成本分析表', [
      query.plantCode,
      query.modelCode,
      query.productCode,
    ])
    const exportMeta = await exportBomMaterialCostItemComponentMovingPriceAnalysis(
      ...
      'DTA 产品成本分析表',
      buildBomExportFileName('DTA 产品成本分析表', [
        query.plantCode,
        query.modelCode,
        query.productCode,
      ]),
