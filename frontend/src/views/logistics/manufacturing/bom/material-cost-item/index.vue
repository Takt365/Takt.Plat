...
<template>
  <div class="p-4">
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-bom-material-cost-item'"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('componentCode')">
        <TaktSelect
          ...
          api-url="TaktMaterialPlants/options"
          :placeholder="pi.queryPh('componentCode', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
...
type BomMaterialCostItemRowRecord = BomMaterialCostItem | Record<string, unknown>
/** i18n 翻译函数 */
const { t } = useI18n()
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<BomMaterialCostItemRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<BomMaterialCostItemRowRecord[]>([])
/** 表格多选 row-key 集合 */
 ...
 */
function buildBomMaterialCostItemListColumn(
  key: string,
  title: string,
...
/** 表格 row-key（优先实体主键字段） */
const getBomMaterialCostItemId = (record: BomMaterialCostItemRowRecord): string => {
 ...
 * @param field 字段名
 */
const getBomMaterialCostItemDictValue = (
  record: BomMaterialCostItemRowRecord,
  field: string,
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: BomMaterialCostItemRowRecord[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: BomMaterialCostItemRowRecord, selected: boolean) => {
    if (selected) {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: BomMaterialCostItemRowRecord[]) => {
    selectedRow.value = selected && selectedRowsData.length === 1 ? (selectedRowsData[0] ?? null) : null
  }
}))

/** 行点击切换选中（与 rowSelection 联动） */
const onClickRow = (record: BomMaterialCostItemRowRecord) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getBomMaterialCostItemId(item)))
...
async function handleExport() {
  try {
    ...
    logger.error('[BomMaterialCostItem] 导出失败', { error })
