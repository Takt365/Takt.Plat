...
<!-- 功能描述：BOM变更记录实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.billOfMaterialChangeLog.billofmaterialid')"
...
/**
 * BOM变更记录实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","billOfMaterialId","bomCode","changeFields","changeTime","changeBy","changeReason","extFieldJson","remark"]
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.billOfMaterialChangeLogId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  billOfMaterialId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.billOfMaterialChangeLog.billofmaterialid') }),
      trigger: 'blur'
    }
  ],
  bomCode: [
  ...
  changeTime: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.billOfMaterialChangeLog.changetime') }),
      trigger: 'blur'
    }
  ],
}))
