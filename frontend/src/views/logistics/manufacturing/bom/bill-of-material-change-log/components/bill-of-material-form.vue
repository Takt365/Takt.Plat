...
<template>
  ...
  >
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="billOfMaterialChangeLogTableRef"
      v-model="childBillOfMaterialChangeLogRows"
      :columns="billOfMaterialChangeLogFormColumns"
      :title="t('entity.billofmaterialchangelog._self')"
      :add-button-entity="t('entity.billofmaterialchangelog._self')"
      id-field="billOfMaterialChangeLogId"
      :default-row="createDefaultBillOfMaterialChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const billOfMaterialChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 billOfMaterialChangeLog 可编辑列 */
const billOfMaterialChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'bomCode',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.billofmaterialchangelog.changefields') }),
  },
  {
    key: 'changeTime',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.billofmaterialchangelog.changeby') }),
  },
  {
    key: 'changeReason',
...
function syncChildRowsFromFormData(val: Partial<BillOfMaterialCreate & { billOfMaterialId?: string }> | null | undefined) {
  childBillOfMaterialChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultBillOfMaterialChangeLogRow(): Record<string, unknown> {
  return {
    bomCode: '',
    changeFields: '',
    changeTime: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    changeLogs: billOfMaterialChangeLogTableRef.value?.getRows?.() ?? childBillOfMaterialChangeLogRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.billOfMaterialId
    if (isCreate) {
...
async function validate() {
  ...
  await billOfMaterialChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  billOfMaterialChangeLogTableRef.value?.resetRows?.()
