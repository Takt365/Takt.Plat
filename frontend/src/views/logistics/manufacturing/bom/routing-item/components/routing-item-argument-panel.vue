...
<template>
  <div class="routing-item-argument-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div
      ...
      class="routing-item-argument-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('paramCode')">
        <a-input
          v-model:value="advancedQueryForm.paramCode"
          :placeholder="pi.queryPh('paramCode', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('paramName')">
        <a-input
          v-model:value="advancedQueryForm.paramName"
          :placeholder="pi.queryPh('paramName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="ROUTINGITEMARGUMENT_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  useRoutingItemArgumentI18n,
  ROUTINGITEMARGUMENT_DEFAULT_VISIBLE_COLUMN_KEYS,
  ROUTINGITEMARGUMENT_SUMMARY_SUM_FIELDS,
  ROUTINGITEMARGUMENT_QUERY_STRING_FIELDS,
  ROUTINGITEMARGUMENT_QUERY_FIELDS,
  ROUTINGITEMARGUMENT_SELF_I18N_KEY,
} from '../composables/use-routing-item-argument-i18n'
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of ROUTINGITEMARGUMENT_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(ROUTINGITEMARGUMENT_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  ROUTINGITEMARGUMENT_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...ROUTINGITEMARGUMENT_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...ROUTINGITEMARGUMENT_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'routingItemArgumentId'
const masterRoutingItemId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['routingItemId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterRoutingItemId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: RoutingItemArgument }) =>
      String(getRoutingItemArgumentField(record, 'routingItemId') ?? ''),
  },
  {
    title: pi.label('paramCode'),
    dataIndex: 'paramCode',
    key: 'paramCode',
    width: 120,
    resizable: true,
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'routingItemArgumentId',
...
const summarySumFieldSet = new Set<string>(ROUTINGITEMARGUMENT_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    ROUTINGITEMARGUMENT_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof ROUTINGITEMARGUMENT_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of ROUTINGITEMARGUMENT_SUMMARY_SUM_FIELDS) {
      const num = Number(getRoutingItemArgumentField(row, field))
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importRoutingItemArgument(file, sheetName)
