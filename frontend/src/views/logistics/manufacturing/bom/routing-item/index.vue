...
<!-- 文件名称：index.vue -->
<!-- 功能描述：工艺路线明细表实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- 免责声明：此软件使用 MIT License，作者不承担任何使用风险。 -->
<!-- ======================================== -->

<template>
  <div class="logistics-manufacturing-bom-routing-item">
    <!-- 查询栏 -->
    <TaktQueryBar
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:bom:routingitem:create"
      update-permission="logistics:manufacturing:bom:routingitem:update"
      delete-permission="logistics:manufacturing:bom:routingitem:delete"
      import-permission="logistics:manufacturing:bom:routingitem:import"
      export-permission="logistics:manufacturing:bom:routingitem:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.routingItem._self')"
      :width="600"
      :footer="null"
...
/**
 * 工艺路线明细表实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
import type { RoutingItem, RoutingItemQuery, RoutingItemCreate, RoutingItemUpdate } from '@/types/logistics/manufacturing/bom/routing-item'
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getRoutingItemField(record, 'routingName') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getRoutingItemField(record, 'pointsToMinutesRate') ?? ''
  },
  {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:bom:routingitem:update',
      ...
      },
      {
        ...
        permission: 'logistics:manufacturing:bom:routingitem:delete',
...
const onClickRow = (record: RoutingItem) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getRoutingItemId(item)))
...
async function loadData() {
  ...
  try {
    ...
    const res = await getRoutingItemList(params)
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.routingItem._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: RoutingItem) {
  formTitle.value = t('common.page.button.edit') + t('entity.routingItem._self')
...
async function handleExport() {
  try {
    loading.value = true
    const kw = (queryKeyword.value ?? '').trim()
    const exportQuery: RoutingItemQuery = {
      pageIndex: 1,
      pageSize: 100000,
      ...advancedQueryForm.value
    }
    ...
    const exportMeta = await exportRoutingItem(exportQuery, excelNames.sheet, excelNames.fileBase)
    const ts = new Date()
...
.logistics-manufacturing-bom-routing-item {
  padding: 16px;
  display: flex;
