...
<!-- 命名空间：@/views/logistics/manufacturing/bom/standard-operation-time-change-log -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：标准工序时间实体管理页面，含查询、增删改，由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:bom:standard:operation:time:change:log:create"
      update-permission="logistics:manufacturing:bom:standard:operation:time:change:log:update"
      delete-permission="logistics:manufacturing:bom:standard:operation:time:change:log:delete"
      import-permission="logistics:manufacturing:bom:standard:operation:time:change:log:import"
      export-permission="logistics:manufacturing:bom:standard:operation:time:change:log:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getStandardOperationTimeId"
      :master-row-selection="rowSelection"
      master-id-column-key="standardOperationTimeId"
    ...
    >
      <template #detail>
        <StandardOperationTimeChangeLogPanel
          ref="standardOperationTimeChangeLogPanelRef"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-bom-standard-operation-time-change-log'"
...
/**
 * 标准工序时间实体管理页 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/bom/standard-operation-time-change-log
 */
...
import StandardOperationTimeChangeLogPanel from './components/standard-operation-time-change-log-panel.vue'
import { provideStandardOperationTimeMasterContext } from './composables/use-standard-operation-time-master-context'
...
/** 主表选中行上下文（右侧明细面板读取） */
const { selectedMasterRow } = provideStandardOperationTimeMasterContext()
const standardOperationTimeChangeLogPanelRef = ref<InstanceType<typeof StandardOperationTimeChangeLogPanel> | null>(null)
...
function syncMasterSelection(record: StandardOperationTime | null) {
  selectedMasterRow.value = record
  selectedMasterKey.value = record ? getStandardOperationTimeId(record) : ''
}
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as StandardOperationTime
  const key = getStandardOperationTimeId(row)
...
async function loadStandardOperationTimeDetail(record: StandardOperationTime): Promise<StandardOperationTime | null> {
  const id = getStandardOperationTimeId(record)
  if (!id) {
    return null
  }
  try {
    const detail = await getStandardOperationTimeById(id)
    const index = dataSource.value.findIndex((row) => getStandardOperationTimeId(row) === id)
    if (index !== -1) {
      dataSource.value[index] = { ...dataSource.value[index], ...detail } as StandardOperationTime
    }
    return detail
  } catch (error: any) {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:bom:standard:operation:time:change:log:update',
      ...
      },
      {
        ...
        permission: 'logistics:manufacturing:bom:standard:operation:time:change:log:delete',
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: StandardOperationTime) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.standardoperationtime._self') })
  formLoading.value = true
  try {
    const detail = await loadStandardOperationTimeDetail(record)
...
async function handleFormSubmit() {
  ...
  standardOperationTimeChangeLogPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
