...
<template>
  ...
  >
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="routingChangeLogTableRef"
      v-model="childRoutingChangeLogRows"
      :columns="routingChangeLogFormColumns"
      :title="t('entity.routingchangelog._self')"
      :add-button-entity="t('entity.routingchangelog._self')"
      id-field="routingChangeLogId"
      :default-row="createDefaultRoutingChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const routingChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 routingChangeLog 可编辑列 */
const routingChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'changeFields',
    title: t('entity.routingchangelog.changefields'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.routingchangelog.changefields') }),
  },
  {
    key: 'changeType',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.routingchangelog.changereason') }),
  },
  {
    key: 'changeBy',
    title: t('entity.routingchangelog.changeby'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.routingchangelog.changeby') }),
  },
  {
    key: 'changeTime',
...
function syncChildRowsFromFormData(val: Partial<RoutingCreate & { routingId?: string }> | null | undefined) {
  childRoutingChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultRoutingChangeLogRow(): Record<string, unknown> {
  return {
    changeFields: '',
    changeType: 0,
    changeReason: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    changeLogs: routingChangeLogTableRef.value?.getRows?.() ?? childRoutingChangeLogRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await routingChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  routingChangeLogTableRef.value?.resetRows?.()
