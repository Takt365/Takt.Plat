...
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt物料清单明细实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- 免责声明：此软件使用 MIT License，作者不承担任何使用风险。 -->
<!-- ======================================== -->

<template>
  <div class="logistics-manufacturing-bom-bill-of-material-item">
    <!-- 查询栏 -->
    <TaktQueryBar
    ...
    <TaktToolsBar
      ...
      import-permission="logistics:manufacturing:bom:billofmaterialitem:import"
      export-permission="logistics:manufacturing:bom:billofmaterialitem:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.billOfMaterialItem._self')"
      :width="600"
      :footer="null"
...
/**
 * Takt物料清单明细实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
import type { BillOfMaterialItem, BillOfMaterialItemQuery, BillOfMaterialItemCreate, BillOfMaterialItemUpdate } from '@/types/logistics/manufacturing/bom/bill-of-material-item'
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getBillOfMaterialItemField(record, 'billOfMaterialName') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getBillOfMaterialItemField(record, 'materialName') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getBillOfMaterialItemField(record, 'materialUnit') ?? ''
  },
  {
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record: BillOfMaterialItem, selected: boolean) => {
    ...
    } else if (getBillOfMaterialItemId(selectedRow.value) === getBillOfMaterialItemId(record)) {
...
const onClickRow = (record: BillOfMaterialItem) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getBillOfMaterialItemId(item)))
...
async function loadData() {
  ...
  try {
    ...
    const params: BillOfMaterialItemQuery = {
    ...
    const res = await getBillOfMaterialItemList(params)
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.billOfMaterialItem._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: BillOfMaterialItem) {
  formTitle.value = t('common.page.button.edit') + t('entity.billOfMaterialItem._self')
...
async function handleExport() {
  try {
    loading.value = true
    const kw = (queryKeyword.value ?? '').trim()
    const exportQuery: BillOfMaterialItemQuery = {
      pageIndex: 1,
      pageSize: 100000,
      ...advancedQueryForm.value
    }
    ...
    const exportMeta = await exportBillOfMaterialItem(exportQuery, excelNames.sheet, excelNames.fileBase)
    const ts = new Date()
...
.logistics-manufacturing-bom-bill-of-material-item {
  padding: 16px;
  display: flex;
