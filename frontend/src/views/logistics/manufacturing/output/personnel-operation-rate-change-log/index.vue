...
<!-- 命名空间：@/views/logistics/manufacturing/output/personnel-operation-rate-change-log -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：人员稼动率实体管理页面，含查询、增删改，由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getPersonnelOperationRateId"
      :master-row-selection="rowSelection"
      master-id-column-key="personnelOperationRateId"
    ...
    >
      <template #detail>
        <PersonnelOperationRateChangeLogPanel
          ref="personnelOperationRateChangeLogPanelRef"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-output-personnel-operation-rate-change-log'"
...
/**
 * 人员稼动率实体管理页 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/output/personnel-operation-rate-change-log
 */
...
import PersonnelOperationRateChangeLogPanel from './components/personnel-operation-rate-change-log-panel.vue'
import { providePersonnelOperationRateMasterContext } from './composables/use-personnel-operation-rate-master-context'
...
/** 主表选中行上下文（右侧明细面板读取） */
const { selectedMasterRow } = providePersonnelOperationRateMasterContext()
const personnelOperationRateChangeLogPanelRef = ref<InstanceType<typeof PersonnelOperationRateChangeLogPanel> | null>(null)
...
function syncMasterSelection(record: PersonnelOperationRate | null) {
  selectedMasterRow.value = record
  selectedMasterKey.value = record ? getPersonnelOperationRateId(record) : ''
}
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as PersonnelOperationRate
  const key = getPersonnelOperationRateId(row)
...
async function loadPersonnelOperationRateDetail(record: PersonnelOperationRate): Promise<PersonnelOperationRate | null> {
  ...
  try {
    ...
    const index = dataSource.value.findIndex((row) => getPersonnelOperationRateId(row) === id)
    if (index !== -1) {
      dataSource.value[index] = { ...dataSource.value[index], ...detail } as PersonnelOperationRate
    }
    return detail
  } catch (error: any) {
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: PersonnelOperationRate) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.personneloperationrate._self') })
  formLoading.value = true
  try {
    const detail = await loadPersonnelOperationRateDetail(record)
...
async function handleFormSubmit() {
  ...
  personnelOperationRateChangeLogPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
