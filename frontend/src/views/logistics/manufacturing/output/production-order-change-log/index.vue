...
<!-- 命名空间：@/views/logistics/manufacturing/output/production-order-change-log -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：生产工单实体管理页面，含查询、增删改，由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getProductionOrderId"
      :master-row-selection="rowSelection"
      master-id-column-key="productionOrderId"
    ...
    >
      <template #detail>
        <ProductionOrderChangeLogPanel
          ref="productionOrderChangeLogPanelRef"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-output-production-order-change-log'"
...
/**
 * 生产工单实体管理页 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/output/production-order-change-log
 */
...
import ProductionOrderChangeLogPanel from './components/production-order-change-log-panel.vue'
import { provideProductionOrderMasterContext } from './composables/use-production-order-master-context'
...
/** 主表选中行上下文（右侧明细面板读取） */
const { selectedMasterRow } = provideProductionOrderMasterContext()
const productionOrderChangeLogPanelRef = ref<InstanceType<typeof ProductionOrderChangeLogPanel> | null>(null)
...
function syncMasterSelection(record: ProductionOrder | null) {
  selectedMasterRow.value = record
  selectedMasterKey.value = record ? getProductionOrderId(record) : ''
}
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as ProductionOrder
  const key = getProductionOrderId(row)
...
async function loadProductionOrderDetail(record: ProductionOrder): Promise<ProductionOrder | null> {
  ...
  try {
    ...
    const index = dataSource.value.findIndex((row) => getProductionOrderId(row) === id)
    if (index !== -1) {
      dataSource.value[index] = { ...dataSource.value[index], ...detail } as ProductionOrder
    }
    return detail
  } catch (error: any) {
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: ProductionOrder) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.productionorder._self') })
  formLoading.value = true
  try {
    const detail = await loadProductionOrderDetail(record)
...
async function handleFormSubmit() {
  ...
  productionOrderChangeLogPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
