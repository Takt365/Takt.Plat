...
<!-- 功能描述：生产工单实体维护弹窗内嵌表单（上主下从级联保存）。由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form production-order-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="productionOrderChangeLogTableRef"
      v-model="childProductionOrderChangeLogRows"
      :columns="productionOrderChangeLogFormColumns"
      :title="t('entity.productionorderchangelog._self')"
      :add-button-entity="t('entity.productionorderchangelog._self')"
      id-field="productionOrderChangeLogId"
      :default-row="createDefaultProductionOrderChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
/**
 * 生产工单实体维护表单 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
...
const childProductionOrderChangeLogRows = ref<Record<string, unknown>[]>([])
const productionOrderChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 productionOrderChangeLog 可编辑列 */
const productionOrderChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'prodOrderCode',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.productionorderchangelog.changefields') }),
  },
  {
    key: 'changeTime',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.productionorderchangelog.changeby') }),
  },
  {
    key: 'changeReason',
    title: t('entity.productionorderchangelog.changereason'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.productionorderchangelog.changereason') }),
  },
  {
    key: 'extField',
...
function syncChildRowsFromFormData(val: Partial<ProductionOrderCreate & { productionOrderId?: string }> | null | undefined) {
  childProductionOrderChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultProductionOrderChangeLogRow(): Record<string, unknown> {
  return {
    prodOrderCode: '',
    changeFields: '',
    changeTime: '',
...
function buildSubmitPayload() {
  const masterId = props.formData?.productionOrderId ?? ''
  return {
    ...formState,
    changeLogs: productionOrderChangeLogTableRef.value?.getRows?.() ?? childProductionOrderChangeLogRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await productionOrderChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childProductionOrderChangeLogRows.value = []
  productionOrderChangeLogTableRef.value?.resetRows?.()
