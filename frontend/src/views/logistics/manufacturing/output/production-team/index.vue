...
<!-- 命名空间：@/views/logistics/manufacturing/output/production-team -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：生产班组实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="logistics-manufacturing-output-production-team">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:output:productionteam:create"
      update-permission="logistics:manufacturing:output:productionteam:update"
      delete-permission="logistics:manufacturing:output:productionteam:delete"
      import-permission="logistics:manufacturing:output:productionteam:import"
      export-permission="logistics:manufacturing:output:productionteam:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.productionTeam._self')"
      :width="600"
      :footer="null"
...
/**
 * 生产班组实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/logistics/manufacturing/output/production-team
 */
...
const total = ref(0)
const selectedRow = ref<ProductionTeam | null>(null)
const selectedRows = ref<ProductionTeam[]>([])
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getProductionTeamField(record, 'status') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:output:productionteam:update',
        onClick: (record: ProductionTeam) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        permission: 'logistics:manufacturing:output:productionteam:delete',
        onClick: (record: ProductionTeam) => handleDeleteOne(record)
      }
    ]
  })
])

const getProductionTeamId = (record: any): string => record?.[entityIdName] ?? ''
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: ProductionTeam[]) => {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: ProductionTeam[]) => {
    selectedRow.value = selected && selectedRowsData.length === 1 ? (selectedRowsData[0] ?? null) : null
  }
}))

const onClickRow = (record: ProductionTeam) => ({
  onClick: () => {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.productionTeam._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: ProductionTeam) {
  formTitle.value = t('common.page.button.edit') + t('entity.productionTeam._self')
...
async function handleImportFile(file: File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> {
  return await importProductionTeam(file, sheetName)
}
