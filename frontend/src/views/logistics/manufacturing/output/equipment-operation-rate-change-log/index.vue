...
<!-- 命名空间：@/views/logistics/manufacturing/output/equipment-operation-rate-change-log -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：机器稼动率实体管理页面，含查询、增删改，由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:output:equipment:operation:rate:create"
      update-permission="logistics:manufacturing:output:equipment:operation:rate:update"
      delete-permission="logistics:manufacturing:output:equipment:operation:rate:delete"
      import-permission="logistics:manufacturing:output:equipment:operation:rate:import"
      export-permission="logistics:manufacturing:output:equipment:operation:rate:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getEquipmentOperationRateId"
      :master-row-selection="rowSelection"
      master-id-column-key="equipmentOperationRateId"
    ...
    >
      <template #detail>
        <EquipmentOperationRateChangeLogPanel
          ref="equipmentOperationRateChangeLogPanelRef"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-output-equipment-operation-rate-change-log'"
...
/**
 * 机器稼动率实体管理页 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/output/equipment-operation-rate-change-log
 */
...
import EquipmentOperationRateChangeLogPanel from './components/equipment-operation-rate-change-log-panel.vue'
import { provideEquipmentOperationRateMasterContext } from './composables/use-equipment-operation-rate-master-context'
import { getEquipmentOperationRateList, getEquipmentOperationRateById, createEquipmentOperationRate, updateEquipmentOperationRate, deleteEquipmentOperationRateById, deleteEquipmentOperationRateBatch, getEquipmentOperationRateTemplate, importEquipmentOperationRate, exportEquipmentOperationRate, updateEquipmentOperationRateStatus } from '@/api/logistics/manufacturing/output/equipment-operation-rate'
import type { EquipmentOperationRate, EquipmentOperationRateQuery } from '@/types/logistics/manufacturing/output/equipment-operation-rate'
...
const queryFieldsMeta = computed(() => [
  ...
  { key: 'startDateStart', label: t('common.page.entity.createdatstart').replace(t('common.page.entity.createdat'), t('entity.equipmentoperationrate.startdate')) },
  { key: 'startDateEnd', label: t('common.page.entity.createdatend').replace(t('common.page.entity.createdat'), t('entity.equipmentoperationrate.startdate')) },
  { key: 'endDateStart', label: t('common.page.entity.createdatstart').replace(t('common.page.entity.createdat'), t('entity.equipmentoperationrate.enddate')) },
  { key: 'endDateEnd', label: t('common.page.entity.createdatend').replace(t('common.page.entity.createdat'), t('entity.equipmentoperationrate.enddate')) },
...
/** 主表选中行上下文（右侧明细面板读取） */
const { selectedMasterRow } = provideEquipmentOperationRateMasterContext()
const equipmentOperationRateChangeLogPanelRef = ref<InstanceType<typeof EquipmentOperationRateChangeLogPanel> | null>(null)
...
function syncMasterSelection(record: EquipmentOperationRate | null) {
  selectedMasterRow.value = record
  selectedMasterKey.value = record ? getEquipmentOperationRateId(record) : ''
}
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as EquipmentOperationRate
  const key = getEquipmentOperationRateId(row)
...
async function loadEquipmentOperationRateDetail(record: EquipmentOperationRate): Promise<EquipmentOperationRate | null> {
  ...
  try {
    ...
    const index = dataSource.value.findIndex((row) => getEquipmentOperationRateId(row) === id)
    if (index !== -1) {
      dataSource.value[index] = { ...dataSource.value[index], ...detail } as EquipmentOperationRate
    }
    return detail
  } catch (error: any) {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:output:equipment:operation:rate:update',
      ...
      },
      {
        ...
        permission: 'logistics:manufacturing:output:equipment:operation:rate:delete',
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: EquipmentOperationRate) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.equipmentoperationrate._self') })
  formLoading.value = true
  try {
    const detail = await loadEquipmentOperationRateDetail(record)
...
async function handleFormSubmit() {
  ...
  equipmentOperationRateChangeLogPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
