    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'status'">
          <a-switch
            :checked="getStandardOperationRateField(record, 'status') === 1"
...
async function handleStatusChange(record: StandardOperationRate, checked: boolean) {
  ...
  const oldVal = getStandardOperationRateField(record, 'status')
  const id = getStandardOperationRateId(record)
  ...
  try {
    await updateStandardOperationRateStatus({ standardOperationRateId: id, status: newVal })
