...
<!-- 功能描述：PCBA日报实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.pcbaOutput.details')"
                name="pcbaOutputDetails"
              >
                <a-input
                  v-model:value="formState.pcbaOutputDetails"
                  :placeholder="t('common.page.form.placeholder.required', { field: t('entity.pcbaOutput.details') })"
                  size="small"
                  allow-clear
                />
              </a-form-item>
...
/**
 * PCBA日报实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","plantCode","prodCategory","prodDate","prodLine","shiftNo","prodOrderCode","modelCode","batchNo","materialCode","prodOrderQty","stdMinutes","stdShorts","stdCapacity","pcbaOutputDetails","extFieldJson","remark"]
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  shiftNo: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.pcbaOutput.shiftno') }),
      trigger: 'change'
    }
  ],
  prodOrderCode: [
  ...
  prodOrderQty: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.pcbaOutput.prodorderqty') }),
      trigger: 'change'
    }
  ],
  stdMinutes: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.pcbaOutput.stdminutes') }),
      trigger: 'change'
    }
  ],
  stdShorts: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.pcbaOutput.stdshorts') }),
      trigger: 'change'
    }
  ],
  stdCapacity: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.pcbaOutput.stdcapacity') }),
      trigger: 'change'
    }
  ],
}))
