...
<!-- 命名空间：@/views/logistics/manufacturing/output/production-changeover -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：生产切换记录实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:output:productionchangeover:create"
      update-permission="logistics:manufacturing:output:productionchangeover:update"
      delete-permission="logistics:manufacturing:output:productionchangeover:delete"
      import-permission="logistics:manufacturing:output:productionchangeover:import"
      export-permission="logistics:manufacturing:output:productionchangeover:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktSingleTable
      ...
      :id-column-key="'productionChangeoverId'"
      table-mode="single"
      :data-source="dataSource"
      :loading="loading"
      :stripe="true"
      :row-key="getProductionChangeoverId"
    ...
    >
      <ProductionChangeoverForm
        :key="formData?.productionChangeoverId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-output-production-changeover'"
...
/**
 * 生产切换记录实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/output/production-changeover
 */
...
/** 分页列表数据 */
const dataSource = ref<ProductionChangeover[]>([])
/** 当前页码 */
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<ProductionChangeover> | null>(null)
/** 表单提交 loading */
 ...
 * @returns {ProductionChangeoverQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<ProductionChangeoverQuery>): ProductionChangeoverQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: ProductionChangeoverQuery = {
  ...
  const assignTrimmed = (key: keyof ProductionChangeoverQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:output:productionchangeover:update',
      ...
      },
      {
        ...
        permission: 'logistics:manufacturing:output:productionchangeover:delete',
...
const onClickRow = (record: ProductionChangeover) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getProductionChangeoverId(item)))
...
async function loadData() {
  loading.value = true
  try {
    const res = await getProductionChangeoverList(buildListQuery())
    dataSource.value = res.data ?? []
    total.value = res.total ?? 0
  } catch (error: any) {
    logger.error('[ProductionChangeover] 加载数据失败', { error })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getProductionChangeoverTemplate(sheetName, fileName)
...
async function handleExport() {
  try {
    loading.value = true
    const exportMeta = await exportProductionChangeover(
