...
<template>
  <div class="production-team-equipment-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="production-team-equipment-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
        ...
        id-column-key="productionTeamEquipmentId"
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="PRODUCTIONTEAMEQUIPMENT_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 生产班组实体子表 productionTeamEquipment 右栏面板
...
import {
  getProductionTeamEquipmentList,
  getProductionTeamEquipmentById,
  createProductionTeamEquipment,
  updateProductionTeamEquipment,
  deleteProductionTeamEquipmentById,
  deleteProductionTeamEquipmentBatch,
  getProductionTeamEquipmentTemplate,
  importProductionTeamEquipment,
  exportProductionTeamEquipment,
} from '@/api/logistics/manufacturing/mps/production-team-equipment'
import type { ProductionTeamEquipment, ProductionTeamEquipmentQuery } from '@/types/logistics/manufacturing/mps/production-team-equipment'

import {
  useProductionTeamEquipmentI18n,
  PRODUCTIONTEAMEQUIPMENT_DEFAULT_VISIBLE_COLUMN_KEYS,
  PRODUCTIONTEAMEQUIPMENT_SUMMARY_SUM_FIELDS,
  PRODUCTIONTEAMEQUIPMENT_QUERY_STRING_FIELDS,
  PRODUCTIONTEAMEQUIPMENT_QUERY_FIELDS,
  PRODUCTIONTEAMEQUIPMENT_SELF_I18N_KEY,
} from '../composables/use-production-team-equipment-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useProductionTeamEquipmentI18n()

const { t } = useI18n()
const { selectedMasterRow } = useProductionTeamMasterContext()

/** Excel 导入/导出默认 sheet 名与文件名前缀 */
const excelNames = taktExcelEntityNames('TaktProductionTeamEquipment')
/** 快捷查询占位文案 */
...
const dataSource = ref<ProductionTeamEquipment[]>([])
const currentPage = ref(getTaktDefaultPageIndex())
const pageSize = ref(getTaktDefaultPageSize())
const total = ref(0)
const queryKeyword = ref('')
const selectedRow = ref<ProductionTeamEquipment | null>(null)
const selectedRows = ref<ProductionTeamEquipment[]>([])
const selectedRowKeys = ref<(string | number)[]>([])
const formVisible = ref(false)
const formTitle = ref('')
const formData = ref<Partial<ProductionTeamEquipment>>({})
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...PRODUCTIONTEAMEQUIPMENT_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'productionTeamEquipmentId'
const masterProductionTeamId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['productionTeamId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterProductionTeamId.value !== '')
const updateDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length !== 1)
const deleteDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length === 0)

function getProductionTeamEquipmentId(record: ProductionTeamEquipment | Record<string, unknown>): string {
  return String((record as ProductionTeamEquipment)?.[entityIdName] ?? '')
}

function getProductionTeamEquipmentField(record: ProductionTeamEquipment | Record<string, unknown>, field: string): unknown {
  return (record as ProductionTeamEquipment)?.[field as keyof ProductionTeamEquipment]
}

const columns = computed<TableColumnsType>(() => [
  {
    title: t('common.page.entity.id'),
    dataIndex: 'productionTeamEquipmentId',
    key: 'productionTeamEquipmentId',
    width: 80,
    resizable: true,
    ellipsis: true,
    fixed: 'left',
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: ProductionTeamEquipment }) =>
      String(getProductionTeamEquipmentField(record, 'teamCode') ?? ''),
  },
  {
    ...
    actions: [
      {
        ...
        onClick: (record: ProductionTeamEquipment) => void handleEdit(record),
      },
      {
        key: 'delete',
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'productionTeamEquipmentId',
...
const summarySumFieldSet = new Set<string>(PRODUCTIONTEAMEQUIPMENT_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    PRODUCTIONTEAMEQUIPMENT_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof PRODUCTIONTEAMEQUIPMENT_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of PRODUCTIONTEAMEQUIPMENT_SUMMARY_SUM_FIELDS) {
      const num = Number(getProductionTeamEquipmentField(row, field))
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: ProductionTeamEquipment[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: ProductionTeamEquipment, selected: boolean) => {
    if (selected) {
      selectedRow.value = record
    } else if (selectedRow.value && getProductionTeamEquipmentId(selectedRow.value) === getProductionTeamEquipmentId(record)) {
      selectedRow.value = null
    }
  },
  onSelectAll: (selected: boolean, selectedRowsData: ProductionTeamEquipment[]) => {
 ...
 * @param record 行数据
 */
function onClickRow(record: ProductionTeamEquipment) {
  const key = getProductionTeamEquipmentId(record)
  return {
    onClick: () => {
 ...
 */
function buildListQuery(overrides?: Partial<ProductionTeamEquipmentQuery>): ProductionTeamEquipmentQuery {
  ...
  const assignTrimmed = (key: keyof ProductionTeamEquipmentQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
async function loadData() {
  ...
  loading.value = true
  try {
    const res = await getProductionTeamEquipmentList(buildListQuery())
...
async function handleEdit(record: ProductionTeamEquipment) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getProductionTeamEquipmentById(getProductionTeamEquipmentId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.productionTeamEquipmentId
    if (id) {
      await updateProductionTeamEquipment(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createProductionTeamEquipment(payload)
...
async function handleDeleteOne(record: ProductionTeamEquipment) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteProductionTeamEquipmentById(getProductionTeamEquipmentId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getProductionTeamEquipmentId(r)).filter(Boolean)
      await deleteProductionTeamEquipmentBatch(ids)
      message.success(t('common.feedback.deleted', { target: pi.self() }))
      await loadData()
    },
  })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getProductionTeamEquipmentTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importProductionTeamEquipment(file, sheetName)
