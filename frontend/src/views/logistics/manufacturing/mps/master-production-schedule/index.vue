...
<!-- 命名空间：@/views/logistics/manufacturing/mps/master-production-schedule -->
<!-- 文件名称：index.vue -->
  ...
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    >
      <template #master-toolbar>
        ...
        />
        <TaktToolsBar
      create-permission="logistics:manufacturing:mps:master:production:schedule:create"
      update-permission="logistics:manufacturing:mps:master:production:schedule:update"
      delete-permission="logistics:manufacturing:mps:master:production:schedule:delete"
      import-permission="logistics:manufacturing:mps:master:production:schedule:import"
      export-permission="logistics:manufacturing:mps:master:production:schedule:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'scheduleStatus'">
          <a-switch
            :checked="getMasterProductionScheduleDictValue(record, 'scheduleStatus') === 1"
        ...
        <template v-else-if="column.key === 'bucketType'">
          <TaktDictTag
            :value="getMasterProductionScheduleDictValue(record, 'bucketType')"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-mps-master-production-schedule'"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('planPeriodStartStart')">
        <a-input
          ...
          :placeholder="pi.queryPh('planPeriodStartStart', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('planPeriodStartEnd')">
        <a-input
          ...
          :placeholder="pi.queryPh('planPeriodStartEnd', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('planPeriodEndStart')">
        <a-input
          ...
          :placeholder="pi.queryPh('planPeriodEndStart', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('planPeriodEndEnd')">
        <a-input
          ...
          :placeholder="pi.queryPh('planPeriodEndEnd', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('bucketType')">
        <TaktSelect
          v-model:value="advancedQueryForm.bucketType"
          dict-type="mps_time_bucket_type"
          :placeholder="pi.queryPh('bucketType', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('scheduleStatus')">
        <TaktSelect
          ...
          :placeholder="pi.queryPh('scheduleStatus', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="MASTERPRODUCTIONSCHEDULE_SELF_I18N_KEY"
        file-type="xlsx"
 ...
 * @module views/logistics/manufacturing/mps/master-production-schedule
 */
...
import { provideMasterProductionScheduleMasterContext, type MasterProductionScheduleRowRecord } from './composables/use-master-production-schedule-master-context'
import { getMasterProductionScheduleList, getMasterProductionScheduleById, createMasterProductionSchedule, updateMasterProductionSchedule, deleteMasterProductionScheduleById, deleteMasterProductionScheduleBatch, getMasterProductionScheduleTemplate, importMasterProductionSchedule, exportMasterProductionSchedule, updateMasterProductionScheduleStatus } from '@/api/logistics/manufacturing/mps/master-production-schedule'
import type { MasterProductionSchedule, MasterProductionScheduleQuery } from '@/types/logistics/manufacturing/mps/master-production-schedule'
...
import {
  useMasterProductionScheduleI18n,
  MASTERPRODUCTIONSCHEDULE_LIST_FIELDS,
  MASTERPRODUCTIONSCHEDULE_QUERY_STRING_FIELDS,
  MASTERPRODUCTIONSCHEDULE_QUERY_FIELDS,
  MASTERPRODUCTIONSCHEDULE_SELF_I18N_KEY,
} from './composables/use-master-production-schedule-i18n'
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<MasterProductionScheduleRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<MasterProductionScheduleRowRecord[]>([])
/** 表格多选 row-key 集合 */
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(MASTERPRODUCTIONSCHEDULE_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  MASTERPRODUCTIONSCHEDULE_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
/** 高级查询当前可见字段 key */
...
function buildListQuery(overrides?: Partial<MasterProductionScheduleQuery>): MasterProductionScheduleQuery {
  ...
  for (const key of MASTERPRODUCTIONSCHEDULE_QUERY_STRING_FIELDS) {
...
function syncMasterSelection(record: MasterProductionScheduleRowRecord | null) {
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as MasterProductionScheduleRowRecord
...
async function loadMasterProductionScheduleDetail(record: MasterProductionScheduleRowRecord): Promise<MasterProductionSchedule | null> {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:mps:master:production:schedule:update',
        onClick: (record: MasterProductionScheduleRowRecord) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        permission: 'logistics:manufacturing:mps:master:production:schedule:delete',
        onClick: (record: MasterProductionScheduleRowRecord) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getMasterProductionScheduleId = (record: MasterProductionScheduleRowRecord): string => {
 ...
 * @param field 字段名
 */
const getMasterProductionScheduleDictValue = (
  record: MasterProductionScheduleRowRecord,
  field: string,
...
/** 将行字段/字典值转为有限 number */
const toMasterProductionScheduleNumber = (value: string | number | undefined | null): number => {
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: MasterProductionScheduleRowRecord[]) => {
  ...
  onSelect: (record: MasterProductionScheduleRowRecord, selected: boolean) => {
    if (selected) {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: MasterProductionScheduleRowRecord[]) => {
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: MasterProductionScheduleRowRecord) {
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importMasterProductionSchedule(file, sheetName)
...
/** 删除单行 */
async function handleDeleteOne(record: MasterProductionScheduleRowRecord) {
  Modal.confirm({
 ...
 * @param checked 是否启用
 */
async function handleScheduleStatusChange(record: MasterProductionScheduleRowRecord, checked: boolean) {
  const newVal = checked ? 1 : 0
  const oldVal = toMasterProductionScheduleNumber(getMasterProductionScheduleDictValue(record, 'scheduleStatus'))
  const id = getMasterProductionScheduleId(record)
