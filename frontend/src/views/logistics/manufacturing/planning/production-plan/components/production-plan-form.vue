...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form production-plan-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
...
function syncChildRowsFromFormData(val: Partial<ProductionPlanCreate & { productionPlanId?: string }> | null | undefined) {
  childProductionPlanItemRows.value = ((val as any)?.items ?? []) as Record<string, unknown>[]
}

function createDefaultProductionPlanItemRow(): Record<string, unknown> {
  return {
    lineNumber: (childProductionPlanItemRows.value.length + 1) * 10,
    salesPlanId: '',
    salesPlanCode: '',
