...
<!-- 命名空间：@/views/logistics/manufacturing/planning/production-team -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：生产班组实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:planning:production:team:create"
      update-permission="logistics:manufacturing:planning:production:team:update"
      delete-permission="logistics:manufacturing:planning:production:team:delete"
      import-permission="logistics:manufacturing:planning:production:team:import"
      export-permission="logistics:manufacturing:planning:production:team:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-planning-production-team'"
...
/**
 * 生产班组实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/planning/production-team
 */
...
import { getProductionTeamList, getProductionTeamById, createProductionTeam, updateProductionTeam, deleteProductionTeamById, deleteProductionTeamBatch, getProductionTeamTemplate, importProductionTeam, exportProductionTeam, updateProductionTeamStatus } from '@/api/logistics/manufacturing/planning/production-team'
import type { ProductionTeam, ProductionTeamQuery } from '@/types/logistics/manufacturing/planning/production-team'
...
type ProductionTeamRowRecord = ProductionTeam | Record<string, unknown>
/** i18n 翻译函数 */
const { t } = useI18n()
 ...
 */
function buildProductionTeamListColumn(
  key: string,
  title: string,
...
const columns = computed<TableColumnsType>(() => [
  buildProductionTeamListColumn('productionTeamId', t('common.page.entity.id'), { width: 80, fixed: 'left' }),
  ...PRODUCTIONTEAM_LIST_FIELDS.map((key) => buildProductionTeamListColumn(key, pi.label(key))),
  CreateActionColumn({
    actions: [
      {
        key: 'update',
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:planning:production:team:update',
      ...
      },
      {
        ...
        permission: 'logistics:manufacturing:planning:production:team:delete',
...
const onClickRow = (record: ProductionTeamRowRecord) => ({
  onClick: () => {
    const key = getProductionTeamId(record)
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getProductionTeamId(item)))
...
async function handleEdit(record: ProductionTeamRowRecord) {
  ...
  try {
    ...
    formData.value = detail ?? ({ ...record } as Partial<ProductionTeam>)
    formVisible.value = true
  } catch (error: unknown) {
