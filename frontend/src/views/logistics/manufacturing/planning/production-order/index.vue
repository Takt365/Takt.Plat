...
<!-- 命名空间：@/views/logistics/manufacturing/planning/production-order -->
<!-- 文件名称：index.vue -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:planning:production:order:create"
      update-permission="logistics:manufacturing:planning:production:order:update"
      delete-permission="logistics:manufacturing:planning:production:order:delete"
      import-permission="logistics:manufacturing:planning:production:order:import"
      export-permission="logistics:manufacturing:planning:production:order:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-planning-production-order'"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('producedQty')">
        <a-input-number
          v-model:value="advancedQueryForm.producedQty"
          :placeholder="pi.queryPh('producedQty', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('prodBatch')">
        <a-input
          v-model:value="advancedQueryForm.prodBatch"
          :placeholder="pi.queryPh('prodBatch', 'required')"
          show-count
          :maxlength="80"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('plannedOrderId')">
        <TaktSelect
          ...
          :placeholder="pi.queryPh('plannedOrderId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="PRODUCTIONORDER_SELF_I18N_KEY"
        file-type="xlsx"
 ...
 * @module views/logistics/manufacturing/planning/production-order
 */
...
import { getProductionOrderList, getProductionOrderById, createProductionOrder, updateProductionOrder, deleteProductionOrderById, deleteProductionOrderBatch, getProductionOrderTemplate, importProductionOrder, exportProductionOrder, updateProductionOrderStatus } from '@/api/logistics/manufacturing/planning/production-order'
import type { ProductionOrder, ProductionOrderQuery } from '@/types/logistics/manufacturing/planning/production-order'
...
import {
  useProductionOrderI18n,
  PRODUCTIONORDER_LIST_FIELDS,
  PRODUCTIONORDER_QUERY_STRING_FIELDS,
  PRODUCTIONORDER_QUERY_FIELDS,
  PRODUCTIONORDER_SELF_I18N_KEY,
} from './composables/use-production-order-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useProductionOrderI18n()
/** 表格行类型（TaktSingleTable slot record 与 dataSource 行兼容） */
type ProductionOrderRowRecord = ProductionOrder | Record<string, unknown>
/** i18n 翻译函数 */
const { t } = useI18n()
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<ProductionOrderRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<ProductionOrderRowRecord[]>([])
/** 表格多选 row-key 集合 */
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(PRODUCTIONORDER_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  PRODUCTIONORDER_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
/** 高级查询当前可见字段 key */
...
function buildListQuery(overrides?: Partial<ProductionOrderQuery>): ProductionOrderQuery {
  ...
  for (const key of PRODUCTIONORDER_QUERY_STRING_FIELDS) {
 ...
 */
function buildProductionOrderListColumn(
  key: string,
  title: string,
...
const columns = computed<TableColumnsType>(() => [
  buildProductionOrderListColumn('productionOrderId', t('common.page.entity.id'), { width: 80, fixed: 'left' }),
  ...PRODUCTIONORDER_LIST_FIELDS.map((key) => buildProductionOrderListColumn(key, pi.label(key))),
  CreateActionColumn({
    actions: [
      {
        key: 'update',
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:planning:production:order:update',
        onClick: (record: ProductionOrderRowRecord) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        permission: 'logistics:manufacturing:planning:production:order:delete',
        onClick: (record: ProductionOrderRowRecord) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getProductionOrderId = (record: ProductionOrderRowRecord): string => {
 ...
 * @param field 字段名
 */
const getProductionOrderDictValue = (
  record: ProductionOrderRowRecord,
  field: string,
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: ProductionOrderRowRecord[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: ProductionOrderRowRecord, selected: boolean) => {
    if (selected) {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: ProductionOrderRowRecord[]) => {
    selectedRow.value = selected && selectedRowsData.length === 1 ? (selectedRowsData[0] ?? null) : null
  }
}))

/** 行点击切换选中（与 rowSelection 联动） */
const onClickRow = (record: ProductionOrderRowRecord) => ({
  onClick: () => {
...
async function handleEdit(record: ProductionOrderRowRecord) {
  const id = getProductionOrderId(record)
  if (!id) {
    return
  }
  ...
  try {
    const detail = await getProductionOrderById(id)
    formData.value = detail ?? ({ ...record } as Partial<ProductionOrder>)
    formVisible.value = true
  } catch (error: unknown) {
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importProductionOrder(file, sheetName)
...
/** 删除单行 */
async function handleDeleteOne(record: ProductionOrderRowRecord) {
  Modal.confirm({
