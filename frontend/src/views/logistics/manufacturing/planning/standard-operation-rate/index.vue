...
<!-- 命名空间：@/views/logistics/manufacturing/planning/standard-operation-rate -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：标准生产稼动率实体 OperationRate 为标准对标目标值管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:planning:standard:operation:rate:create"
      update-permission="logistics:manufacturing:planning:standard:operation:rate:update"
      delete-permission="logistics:manufacturing:planning:standard:operation:rate:delete"
      import-permission="logistics:manufacturing:planning:standard:operation:rate:import"
      export-permission="logistics:manufacturing:planning:standard:operation:rate:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'rateStatus'">
          <a-switch
            :checked="getStandardOperationRateDictValue(record, 'rateStatus') === 1"
            :checked-children="t('common.page.button.enable')" :un-checked-children="t('common.page.button.disable')"
            @change="(checked: unknown) => handleRateStatusChange(record, Boolean(checked))"
          />
        </template>
      </template>
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-planning-standard-operation-rate'"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('financialYear')">
        <a-input
          v-model:value="advancedQueryForm.financialYear"
          :placeholder="pi.queryPh('financialYear', 'required')"
          show-count
          :maxlength="4"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('operationType')">
        <a-input-number
          v-model:value="advancedQueryForm.operationType"
          :placeholder="pi.queryPh('operationType', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('operationRate')">
      <a-form-item :label="pi.queryLabel('operationRate')">
        <a-input-number
          v-model:value="advancedQueryForm.operationRate"
          :placeholder="pi.queryPh('operationRate', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="STANDARDOPERATIONRATE_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 标准生产稼动率实体 OperationRate 为标准对标目标值管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/planning/standard-operation-rate
 */
...
import {
  useStandardOperationRateI18n,
  STANDARDOPERATIONRATE_LIST_FIELDS,
  STANDARDOPERATIONRATE_QUERY_STRING_FIELDS,
  STANDARDOPERATIONRATE_QUERY_FIELDS,
  STANDARDOPERATIONRATE_SELF_I18N_KEY,
} from './composables/use-standard-operation-rate-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useStandardOperationRateI18n()
/** 表格行类型（TaktSingleTable slot record 与 dataSource 行兼容） */
type StandardOperationRateRowRecord = StandardOperationRate | Record<string, unknown>
/** i18n 翻译函数 */
const { t } = useI18n()
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<StandardOperationRateRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<StandardOperationRateRowRecord[]>([])
/** 表格多选 row-key 集合 */
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(STANDARDOPERATIONRATE_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  STANDARDOPERATIONRATE_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
/** 高级查询当前可见字段 key */
...
function buildListQuery(overrides?: Partial<StandardOperationRateQuery>): StandardOperationRateQuery {
  ...
  for (const key of STANDARDOPERATIONRATE_QUERY_STRING_FIELDS) {
 ...
 */
function buildStandardOperationRateListColumn(
  key: string,
  title: string,
...
const columns = computed<TableColumnsType>(() => [
  buildStandardOperationRateListColumn('standardOperationRateId', t('common.page.entity.id'), { width: 80, fixed: 'left' }),
  ...STANDARDOPERATIONRATE_LIST_FIELDS.map((key) => buildStandardOperationRateListColumn(key, pi.label(key))),
  CreateActionColumn({
    actions: [
      {
        key: 'update',
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:planning:standard:operation:rate:update',
        onClick: (record: StandardOperationRateRowRecord) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        permission: 'logistics:manufacturing:planning:standard:operation:rate:delete',
        onClick: (record: StandardOperationRateRowRecord) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getStandardOperationRateId = (record: StandardOperationRateRowRecord): string => {
 ...
 * @param field 字段名
 */
const getStandardOperationRateDictValue = (
  record: StandardOperationRateRowRecord,
  field: string,
...
/** 将行字段/字典值转为有限 number */
const toStandardOperationRateNumber = (value: string | number | undefined | null): number => {
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: StandardOperationRateRowRecord[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: StandardOperationRateRowRecord, selected: boolean) => {
    if (selected) {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: StandardOperationRateRowRecord[]) => {
    selectedRow.value = selected && selectedRowsData.length === 1 ? (selectedRowsData[0] ?? null) : null
  }
}))

/** 行点击切换选中（与 rowSelection 联动） */
const onClickRow = (record: StandardOperationRateRowRecord) => ({
  onClick: () => {
...
async function handleEdit(record: StandardOperationRateRowRecord) {
  ...
  try {
    ...
    formData.value = detail ?? ({ ...record } as Partial<StandardOperationRate>)
    formVisible.value = true
  } catch (error: unknown) {
...
/** 删除单行 */
async function handleDeleteOne(record: StandardOperationRateRowRecord) {
  Modal.confirm({
 ...
 * @param checked 是否启用
 */
async function handleRateStatusChange(record: StandardOperationRateRowRecord, checked: boolean) {
  const newVal = checked ? 1 : 0
  const oldVal = toStandardOperationRateNumber(getStandardOperationRateDictValue(record, 'rateStatus'))
  const id = getStandardOperationRateId(record)
  const row = dataSource.value.find((item) => getStandardOperationRateId(item) === id)
  if (row) {
    row.rateStatus = newVal
  }
  try {
    await updateStandardOperationRateStatus({ standardOperationRateId: id, rateStatus: newVal })
    message.success(t('common.feedback.updated'))
    
  } catch (error: unknown) {
    if (row) {
      row.rateStatus = oldVal
    }
