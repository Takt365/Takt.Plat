...
<template>
  ...
  >
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="apsScheduleChangeLogTableRef"
      v-model="childApsScheduleChangeLogRows"
      :columns="apsScheduleChangeLogFormColumns"
      :title="t('entity.apsschedulechangelog._self')"
      :add-button-entity="t('entity.apsschedulechangelog._self')"
      id-field="apsScheduleChangeLogId"
      :default-row="createDefaultApsScheduleChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const apsScheduleChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 apsScheduleChangeLog 可编辑列 */
const apsScheduleChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'changeFields',
    title: t('entity.apsschedulechangelog.changefields'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.apsschedulechangelog.changefields') }),
  },
  {
    key: 'changeType',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.apsschedulechangelog.changereason') }),
  },
  {
    key: 'changeBy',
    title: t('entity.apsschedulechangelog.changeby'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.apsschedulechangelog.changeby') }),
  },
  {
    key: 'changeTime',
...
function syncChildRowsFromFormData(val: Partial<ApsScheduleCreate & { apsScheduleId?: string }> | null | undefined) {
  childApsScheduleChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultApsScheduleChangeLogRow(): Record<string, unknown> {
  return {
    changeFields: '',
    changeType: 0,
    changeReason: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    changeLogs: apsScheduleChangeLogTableRef.value?.getRows?.() ?? childApsScheduleChangeLogRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await apsScheduleChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  apsScheduleChangeLogTableRef.value?.resetRows?.()
