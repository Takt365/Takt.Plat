...
<!-- 命名空间：@/views/logistics/manufacturing/labor-hour/pcba-mi-labor-hour/components -->
<!-- 文件名称：pcba-mi-labor-hour-form.vue -->
<!-- 功能描述：PCBA手插工数统计实体维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="pcba-mi-labor-hour-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * PCBA手插工数统计实体维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/labor-hour/pcba-mi-labor-hour/components
 */
import { reactive, watch, computed, ref, onMounted } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import { usePcbaMiLaborHourI18n } from '../composables/use-pcba-mi-labor-hour-i18n'

/** 实体字段 i18n */
const pi = usePcbaMiLaborHourI18n()
import type { PcbaMiLaborHourCreate } from '@/types/logistics/manufacturing/labor-hour/pcba-mi-labor-hour'
...
interface Props {
  formData?: Partial<PcbaMiLaborHourCreate & { pcbaMiLaborHourId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 pcbaMiLaborHourId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.pcbaMiLaborHourId) {
...
watch(
  ...
  () => {
    if (!props.formData?.pcbaMiLaborHourId) {
...
function resetFields() {
  ...
  applyFormDefaults(formState)
  applyScopeDefaults(formState as Record<string, unknown>, !props.formData?.pcbaMiLaborHourId)
