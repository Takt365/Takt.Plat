...
<!-- 命名空间：@/views/logistics/manufacturing/labor-hour/assy-labor-hour/components -->
<!-- 文件名称：assy-labor-hour-form.vue -->
<!-- 功能描述：组立工数统计实体维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="assy-labor-hour-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * 组立工数统计实体维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/labor-hour/assy-labor-hour/components
 */
import { reactive, watch, computed, ref, onMounted } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import { useAssyLaborHourI18n } from '../composables/use-assy-labor-hour-i18n'

/** 实体字段 i18n */
const pi = useAssyLaborHourI18n()
import type { AssyLaborHourCreate } from '@/types/logistics/manufacturing/labor-hour/assy-labor-hour'
...
interface Props {
  formData?: Partial<AssyLaborHourCreate & { assyLaborHourId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 assyLaborHourId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.assyLaborHourId) {
...
watch(
  ...
  () => {
    if (!props.formData?.assyLaborHourId) {
...
function resetFields() {
  ...
  applyFormDefaults(formState)
  applyScopeDefaults(formState as Record<string, unknown>, !props.formData?.assyLaborHourId)
