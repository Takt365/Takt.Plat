...
<!-- 命名空间：@/views/logistics/manufacturing/mrp/production-plan -->
<!-- 文件名称：index.vue -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-mrp-production-plan'"
 ...
 * @module views/logistics/manufacturing/mrp/production-plan
 */
...
import { getProductionPlanList, getProductionPlanById, createProductionPlan, updateProductionPlan, deleteProductionPlanById, deleteProductionPlanBatch, getProductionPlanTemplate, importProductionPlan, exportProductionPlan, updateProductionPlanStatus } from '@/api/logistics/manufacturing/mrp/production-plan'
import type { ProductionPlan, ProductionPlanQuery } from '@/types/logistics/manufacturing/mrp/production-plan'
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getProductionPlanField(record, 'materialRequirementsPlanningId') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getProductionPlanField(record, 'materialRequirementsPlanningCode') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getProductionPlanField(record, 'salesForecastId') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getProductionPlanField(record, 'salesForecastCode') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getProductionPlanField(record, 'remark') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
