...
<!-- 文件名称：schedule-item-panel.vue -->
<!-- 功能描述：APS排程主表主表实体右侧明细 apsScheduleItem 独立 CRUD（按主表选中 apsScheduleId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="schedule-item-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="schedule-item-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="APSSCHEDULEITEM_SELF_I18N_KEY"
        file-type="xlsx"
...
} from '@/api/logistics/manufacturing/aps/schedule-item'
import type { ApsScheduleItem, ApsScheduleItemQuery } from '@/types/logistics/manufacturing/aps/schedule-item'

import {
  useApsScheduleItemI18n,
  APSSCHEDULEITEM_DEFAULT_VISIBLE_COLUMN_KEYS,
  APSSCHEDULEITEM_SUMMARY_SUM_FIELDS,
  APSSCHEDULEITEM_QUERY_STRING_FIELDS,
  APSSCHEDULEITEM_QUERY_FIELDS,
  APSSCHEDULEITEM_SELF_I18N_KEY,
} from '../composables/use-schedule-item-i18n'
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...APSSCHEDULEITEM_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'apsScheduleItemId'
const masterApsScheduleId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['apsScheduleId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterApsScheduleId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: pi.label('processSequence'),
  ...
  {
    title: pi.label('processStandardST'),
  ...
  {
    title: pi.label('processStandardSTUnit'),
...
const summarySumFieldSet = new Set<string>(APSSCHEDULEITEM_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    APSSCHEDULEITEM_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof APSSCHEDULEITEM_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of APSSCHEDULEITEM_SUMMARY_SUM_FIELDS) {
      const num = Number(getApsScheduleItemField(row, field))
