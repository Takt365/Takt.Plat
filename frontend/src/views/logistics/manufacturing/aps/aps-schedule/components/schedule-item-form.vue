...
function getValues(): Record<string, any> {
  ...
  if (props.formData?.apsScheduleItemId) {
    payload.apsScheduleItemId = props.formData.apsScheduleItemId
  }
  ...
  if (masterRow) {
    const masterCode = masterRow.apsScheduleCode ?? masterRow.ApsScheduleCode
    const masterName = masterRow.apsScheduleName ?? masterRow.ApsScheduleName
    if (masterCode != null && masterCode !== '' && !payload.apsScheduleCode) {
      payload.apsScheduleCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.apsScheduleName) {
      payload.apsScheduleName = masterName
    }
