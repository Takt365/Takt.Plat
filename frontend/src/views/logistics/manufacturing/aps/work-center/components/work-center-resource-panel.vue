...
<template>
  <div class="work-center-resource-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    />
    <TaktToolsBar
      create-permission="logistics:manufacturing:aps:work:center:create"
      update-permission="logistics:manufacturing:aps:work:center:update"
      delete-permission="logistics:manufacturing:aps:work:center:delete"
      import-permission="logistics:manufacturing:aps:work:center:import"
      export-permission="logistics:manufacturing:aps:work:center:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-expand="false"
    ...
    <div
      ...
      class="work-center-resource-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-logistics-manufacturing-aps-work-center-work-center-resource"
    ...
    >
      ...
      <div v-show="isFieldVisible('resourceCode')">
      <a-form-item :label="pi.queryLabel('resourceCode')">
        <a-input
          v-model:value="advancedQueryForm.resourceCode"
          :placeholder="pi.queryPh('resourceCode', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <div v-show="isFieldVisible('resourceName')">
      <a-form-item :label="pi.queryLabel('resourceName')">
        <a-input
          v-model:value="advancedQueryForm.resourceName"
          :placeholder="pi.queryPh('resourceName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <div v-show="isFieldVisible('resourceType')">
      <a-form-item :label="pi.queryLabel('resourceType')">
        <TaktSelect
          v-model:value="advancedQueryForm.resourceType"
          dict-type="work_center_resource_type"
          :placeholder="pi.queryPh('resourceType', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      ...
      <div v-show="isFieldVisible('efficiencyRate')">
      <a-form-item :label="pi.queryLabel('efficiencyRate')">
        <a-input-number
          v-model:value="advancedQueryForm.efficiencyRate"
          :placeholder="pi.queryPh('efficiencyRate', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('resourceStatus')">
      <a-form-item :label="pi.queryLabel('resourceStatus')">
        <TaktSelect
          v-model:value="advancedQueryForm.resourceStatus"
          dict-type="sys_normal_disable"
          :placeholder="pi.queryPh('resourceStatus', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="WORKCENTERRESOURCE_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  getWorkCenterResourceList,
  getWorkCenterResourceById,
  createWorkCenterResource,
  updateWorkCenterResource,
  deleteWorkCenterResourceById,
  deleteWorkCenterResourceBatch,
  getWorkCenterResourceTemplate,
  importWorkCenterResource,
  exportWorkCenterResource,
} from '@/api/logistics/manufacturing/aps/work-center-resource'
import type { WorkCenterResource, WorkCenterResourceQuery } from '@/types/logistics/manufacturing/aps/work-center-resource'

import {
  useWorkCenterResourceI18n,
  WORKCENTERRESOURCE_DEFAULT_VISIBLE_COLUMN_KEYS,
  WORKCENTERRESOURCE_SUMMARY_SUM_FIELDS,
  WORKCENTERRESOURCE_QUERY_STRING_FIELDS,
  WORKCENTERRESOURCE_QUERY_FIELDS,
  WORKCENTERRESOURCE_SELF_I18N_KEY,
} from '../composables/use-work-center-resource-i18n'
...
const excelNames = taktExcelEntityNames('TaktWorkCenterResource')
/** 快捷查询占位文案 */
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(WORKCENTERRESOURCE_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
  ...
  return {
    ...
    efficiencyRate: undefined as number | undefined,
    resourceStatus: undefined as number | undefined,
  }
}
...
const queryFieldsMeta = computed(() =>
  WORKCENTERRESOURCE_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...WORKCENTERRESOURCE_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...WORKCENTERRESOURCE_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'workCenterResourceId'
const masterWorkCenterId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['workCenterId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterWorkCenterId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: WorkCenterResource }) =>
      String(getWorkCenterResourceField(record, 'workCenterId') ?? ''),
  },
  {
    title: pi.label('workCenterCode'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: WorkCenterResource }) =>
      String(getWorkCenterResourceField(record, 'workCenterCode') ?? ''),
  },
  {
    title: pi.label('resourceCode'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: WorkCenterResource }) =>
      String(getWorkCenterResourceField(record, 'resourceCode') ?? ''),
  },
  {
    title: pi.label('resourceName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: WorkCenterResource }) =>
      String(getWorkCenterResourceField(record, 'resourceName') ?? ''),
  },
  {
    title: pi.label('resourceType'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: WorkCenterResource }) =>
      String(getWorkCenterResourceField(record, 'resourceType') ?? ''),
  },
  {
    title: pi.label('parallelCapacity'),
    dataIndex: 'parallelCapacity',
    key: 'parallelCapacity',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: WorkCenterResource }) =>
      String(getWorkCenterResourceField(record, 'parallelCapacity') ?? ''),
  },
  {
    title: pi.label('efficiencyRate'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: WorkCenterResource }) =>
      String(getWorkCenterResourceField(record, 'efficiencyRate') ?? ''),
  },
  {
    title: pi.label('resourceStatus'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: WorkCenterResource }) =>
      String(getWorkCenterResourceField(record, 'resourceStatus') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:aps:work:center:update',
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'workCenterResourceId',
...
const summarySumFieldSet = new Set<string>(WORKCENTERRESOURCE_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    WORKCENTERRESOURCE_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof WORKCENTERRESOURCE_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of WORKCENTERRESOURCE_SUMMARY_SUM_FIELDS) {
      const num = Number(getWorkCenterResourceField(row, field))
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record: WorkCenterResource, selected: boolean) => {
    ...
    } else if (selectedRow.value && getWorkCenterResourceId(selectedRow.value) === getWorkCenterResourceId(record)) {
...
function buildListQuery(overrides?: Partial<WorkCenterResourceQuery>): WorkCenterResourceQuery {
  ...
  for (const key of WORKCENTERRESOURCE_QUERY_STRING_FIELDS) {
    assignTrimmed(key, form[key])
  }
  if (form.resourceType !== undefined && form.resourceType !== null) {
  ...
  if (form.efficiencyRate !== undefined && form.efficiencyRate !== null) {
    query.efficiencyRate = form.efficiencyRate
  }
  if (form.resourceStatus !== undefined && form.resourceStatus !== null) {
    query.resourceStatus = form.resourceStatus
  }
  return query
}
...
async function loadData() {
  ...
  loading.value = true
  try {
    const res = await getWorkCenterResourceList(buildListQuery())
...
async function handleEdit(record: WorkCenterResource) {
  ...
  try {
    const detail = await getWorkCenterResourceById(getWorkCenterResourceId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    if (id) {
      await updateWorkCenterResource(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createWorkCenterResource(payload)
...
async function handleDeleteOne(record: WorkCenterResource) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteWorkCenterResourceById(getWorkCenterResourceId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      ...
      await deleteWorkCenterResourceBatch(ids)
      message.success(t('common.feedback.deleted', { target: pi.self() }))
      await loadData()
    },
  })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getWorkCenterResourceTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importWorkCenterResource(file, sheetName)
...
async function handleExport() {
  ...
  try {
    loading.value = true
    const exportMeta = await exportWorkCenterResource(
