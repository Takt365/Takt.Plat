...
<!-- 命名空间：@/views/logistics/manufacturing/engineering-change/bukan -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：设变部管部门页面 -->
...
<template>
  <div class="p-4">
    ...
    <TaktToolsBar
      ...
      :show-import="false"
      :show-export="true"
      update-permission="logistics:manufacturing:engineeringchange:bukan:update"
      export-permission="logistics:manufacturing:engineeringchange:bukan:export"
...
import { getEcBukanList, updateEcBukan, exportEcBukanData } from '@/api/logistics/manufacturing/engineering-change/bukan';
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getEcBukanList({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
...
async function handleFormSubmit() {
  ...
  try {
    await updateEcBukan(String(formData.value.ecDetailId), dto);
...
async function handleExport() {
  try {
    loading.value = true;
    const blob = await exportEcBukanData({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
    const url = window.URL.createObjectURL(blob as Blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = '部管部门.xlsx';
