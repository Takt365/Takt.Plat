...
<!-- 命名空间：@/views/logistics/manufacturing/engineering-change/batch -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：设变投入批次页面 -->
...
<template>
  <div class="p-4">
    ...
    <TaktToolsBar
      ...
      :show-import="false"
      :show-export="true"
      update-permission="logistics:manufacturing:engineeringchange:batch:update"
      export-permission="logistics:manufacturing:engineeringchange:batch:export"
    ...
    <TaktModal v-model:open="formVisible" :title="t('common.page.button.update')" width="900px" :confirm-loading="formLoading" @ok="handleFormSubmit">
      <BatchForm ref="formRef" :form-data="formData" :loading="formLoading" />
    </TaktModal>
  </div>
</template>
...
import { getEcBatchList, updateEcBatch, exportEcBatchData } from '@/api/logistics/manufacturing/engineering-change/batch';
...
/** 选中行 */
const selectedRows = ref<EcBatch[]>([]);
/** 表单可见 */
const formVisible = ref(false);
/** 表单 loading */
const formLoading = ref(false);
/** 编辑数据 */
const formData = ref<EcBatch | null>(null);
/** 表单 ref */
const formRef = ref<InstanceType<typeof BatchForm> | null>(null);
/** 列定义 */
const columns = ref([
...
/** 行选择 */
const rowSelection = computed(() => ({ selectedRowKeys: selectedRowKeys.value, onChange: (keys: (string | number)[], rows: EcBatch[]) => { selectedRowKeys.value = keys; selectedRows.value = rows; } }));
/** 更新按钮禁用 */
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getEcBatchList({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
...
function onClickRow(record: Record<string, unknown>) {
  return { onClick: () => { const id = getEcDetailId(record); selectedRowKeys.value = [id]; selectedRows.value = [record as unknown as EcBatch]; } };
}
/** 编辑 */
...
async function handleFormSubmit() {
  ...
  const dto: EcBatchUpdate = formRef.value.getValues();
  formLoading.value = true;
  try {
    await updateEcBatch(String(formData.value.ecDetailId), dto);
...
async function handleExport() {
  try {
    loading.value = true;
    const blob = await exportEcBatchData({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
    const url = window.URL.createObjectURL(blob as Blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = '投入批次.xlsx';
