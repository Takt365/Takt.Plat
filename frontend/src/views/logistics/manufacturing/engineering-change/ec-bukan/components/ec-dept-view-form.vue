...
<!-- 功能描述：设变部管部门表单；冗余字段 disabled；IsImplemented/ExecContent/课别填报可编辑；defineExpose validate/getValues/resetFields -->
  ...
  >
    <a-tabs v-model:active-key="activeTab">
      <a-tab-pane key="tab-0" :tab="pi.t('common.page.form.tabs.basicinfo') + ' (1/2)'" force-render>
        <a-row :gutter="24">
      <a-col :span="12"><a-form-item :label="pi.label('ecBukanId')"><a-input v-model:value="formState.ecBukanId" disabled /></a-form-item></a-col>
...
const { rules } = useEcDeptViewFormRules('ecbukan', pi.label)
const formRef = ref();
...
function getValues(): EcBukanUpdate {
  ...
  return { ...formState } as EcBukanUpdate;
}
function resetFields() {
  Object.assign(formState, {
    ecBukanId: '',
    tenantCode: '', companyCode: '', cultureCode: '', plantCode: '',
    ecCode: '', lineNumber: undefined, ecModelCode: '', ecRootMaterialCode: '',
    ecRootMaterialDescription: '', discontinuedStatus: 'Z0',
