...
<!-- 功能描述：设变部管部门表单；defineExpose validate/getValues/resetFields -->
...
<template>
  <a-form ref="formRef" :model="formState" layout="horizontal" label-align="right" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
    <a-row :gutter="24">
      ...
      <a-col :span="12"><a-form-item :label="t('entity.ecdept.outbounddate')"><a-date-picker v-model:value="formState.outboundDate" value-format="YYYY-MM-DD" class="w-full" /></a-form-item></a-col>
      <a-col :span="12"><a-form-item :label="t('entity.ecdept.outboundbatch')"><a-input v-model:value="formState.outboundBatch" /></a-form-item></a-col>
