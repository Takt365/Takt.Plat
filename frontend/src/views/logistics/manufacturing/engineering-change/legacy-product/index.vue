...
import { getEcLegacyProductList, updateEcLegacyProduct, exportEcLegacyProductData } from '@/api/logistics/manufacturing/engineering-change/legacy-product';
...
/** 选中行 */
const selectedRows = ref<EcLegacyProduct[]>([]);
/** 表单可见 */
const formVisible = ref(false);
/** 表单 loading */
const formLoading = ref(false);
/** 编辑数据 */
const formData = ref<EcLegacyProduct | null>(null);
/** 表单 ref */
...
/** 行选择 */
const rowSelection = computed(() => ({ selectedRowKeys: selectedRowKeys.value, onChange: (keys: (string | number)[], rows: EcLegacyProduct[]) => { selectedRowKeys.value = keys; selectedRows.value = rows; } }));
/** 更新按钮禁用 */
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getEcLegacyProductList({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
...
function onClickRow(record: Record<string, unknown>) {
  return { onClick: () => { const id = getEcDetailId(record); selectedRowKeys.value = [id]; selectedRows.value = [record as unknown as EcLegacyProduct]; } };
}
/** 编辑 */
...
async function handleFormSubmit() {
  ...
  const dto: EcLegacyProductUpdate = formRef.value.getValues();
...
async function handleExport() {
  try {
    loading.value = true;
    const blob = await exportEcLegacyProductData({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
