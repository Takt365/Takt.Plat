...
<!-- 功能描述：设变技术课子表 ecDetail 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
/**
 * 设变技术课子表 ecDetail 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
function getValues(): Record<string, any> {
  ...
  if (props.formData?.ecDetailId) {
    payload.ecDetailId = props.formData.ecDetailId
  }
  ...
  if (masterRow) {
    const masterCode = masterRow.ecGijutsuCode ?? masterRow.EcGijutsuCode
    const masterName = masterRow.ecGijutsuName ?? masterRow.EcGijutsuName
    if (masterCode != null && masterCode !== '' && !payload.ecGijutsuCode) {
      payload.ecGijutsuCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.ecGijutsuName) {
      payload.ecGijutsuName = masterName
    }
