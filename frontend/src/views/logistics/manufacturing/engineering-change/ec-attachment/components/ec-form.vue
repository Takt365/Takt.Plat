...
<template>
  ...
  >
    ...
    <!-- 下：子表 attachments -->
    <TaktEditableTable
      ref="ecAttachmentTableRef"
      v-model="childEcAttachmentRows"
      :columns="ecAttachmentFormColumns"
      :title="t('entity.ecattachment._self')"
      :add-button-entity="t('entity.ecattachment._self')"
      id-field="ecAttachmentId"
      :default-row="createDefaultEcAttachmentRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const ecAttachmentTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 ecAttachment 可编辑列 */
const ecAttachmentFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'ecNo',
  ...
  {
    key: 'ExtField',
    title: t('entity.ecattachment.extfield'),
    editor: 'textarea',
    rows: 1,
...
function syncChildRowsFromFormData(val: Partial<EcCreate & { ecId?: string }> | null | undefined) {
  childEcAttachmentRows.value = ((val as any)?.attachments ?? []) as Record<string, unknown>[]
}

function createDefaultEcAttachmentRow(): Record<string, unknown> {
  return {
    ecNo: '',
    lineNumber: (childEcAttachmentRows.value.length + 1) * 10,
    attachmentType: '',
    docNo: '',
    fileName: '',
    accessUrl: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    attachments: ecAttachmentTableRef.value?.getRows?.() ?? childEcAttachmentRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await ecAttachmentTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  ecAttachmentTableRef.value?.resetRows?.()
