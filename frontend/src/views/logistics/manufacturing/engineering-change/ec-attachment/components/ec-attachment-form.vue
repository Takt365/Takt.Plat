...
<!-- 功能描述：设变附件实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ecAttachment.ecid')"
                name="ecId"
              >
                <a-input
...
/**
 * 设变附件实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","ecId","ecNo","lineNumber","attachmentType","docNo","fileName","accessUrl","extFieldJson","remark"]
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ecId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.ecAttachment.ecid') }),
      trigger: 'blur'
    }
  ],
  ecNo: [
  ...
  lineNumber: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.ecAttachment.linenumber') }),
      trigger: 'change'
    }
  ],
  attachmentType: [
