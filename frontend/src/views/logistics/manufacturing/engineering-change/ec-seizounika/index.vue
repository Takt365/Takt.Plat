...
<template>
  <div class="p-4">
    ...
    <TaktModal v-model:open="formVisible" :title="t('common.dialog.title.edit', { entity: t('menu.logistics.manufacturing.engineering.change.seizounika') })" width="900px" :confirm-loading="formLoading" @ok="handleFormSubmit">
...
import { getEcSeizounikaList, updateEcSeizounika, exportEcSeizounikaData } from '@/api/logistics/manufacturing/engineering-change/ec-seizounika';
...
/** 数据源 */
const dataSource = ref<EcSeizounika[]>([]);
/** 当前页 */
...
/** 选中行 */
const selectedRows = ref<EcSeizounika[]>([]);
/** 表单可见 */
const formVisible = ref(false);
/** 表单 loading */
const formLoading = ref(false);
/** 编辑数据 */
const formData = ref<EcSeizounika | null>(null);
/** 表单 ref */
...
/** 行选择 */
const rowSelection = computed(() => ({ selectedRowKeys: selectedRowKeys.value, onChange: (keys: (string | number)[], rows: EcSeizounika[]) => { selectedRowKeys.value = keys; selectedRows.value = rows; } }));
/** 更新按钮禁用 */
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getEcSeizounikaList({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
...
function onClickRow(record: Record<string, unknown>) {
  return { onClick: () => { const id = getEcDetailId(record); selectedRowKeys.value = [id]; selectedRows.value = [record as unknown as EcSeizounika]; } };
}
/** 编辑 */
...
async function handleFormSubmit() {
  ...
  const dto: EcSeizounikaUpdate = formRef.value.getValues();
  formLoading.value = true;
  try {
    await updateEcSeizounika(String(formData.value.ecDetailId), dto);
    message.success(t('common.feedback.updated', { target: t('menu.logistics.manufacturing.engineering.change.seizounika') }));
...
async function handleExport() {
  try {
    loading.value = true;
    const blob = await exportEcSeizounikaData({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
    const url = window.URL.createObjectURL(blob as Blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `${t('menu.logistics.manufacturing.engineering.change.seizounika')}.xlsx`;
...
useEcExecSignalRGroup(TaktEcExecCodes.Pcba);
onMounted(loadData);
</script>
