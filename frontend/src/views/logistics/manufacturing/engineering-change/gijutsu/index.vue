...
<!-- 命名空间：@/views/logistics/manufacturing/engineering-change/gijutsu -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：设变技术部门页面 -->
...
<template>
  <div class="p-4">
    ...
    <TaktToolsBar
      ...
      :show-import="false"
      :show-export="true"
      update-permission="logistics:manufacturing:engineeringchange:gijutsu:update"
      export-permission="logistics:manufacturing:engineeringchange:gijutsu:export"
...
import { getEcGijutsuList, updateEcGijutsu, exportEcGijutsuData } from '@/api/logistics/manufacturing/engineering-change/gijutsu';
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getEcGijutsuList({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
...
async function handleFormSubmit() {
  ...
  try {
    await updateEcGijutsu(String(formData.value.ecDetailId), dto);
...
async function handleExport() {
  try {
    loading.value = true;
    const blob = await exportEcGijutsuData({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
    const url = window.URL.createObjectURL(blob as Blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = '技术部门.xlsx';
