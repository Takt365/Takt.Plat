...
<!-- 命名空间：@/views/logistics/manufacturing/engineering-change/seizouikka -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：设变制造一课页面 -->
...
<template>
  <div class="p-4">
    ...
    <TaktToolsBar
      ...
      :show-import="false"
      :show-export="true"
      update-permission="logistics:manufacturing:engineeringchange:seizouikka:update"
      export-permission="logistics:manufacturing:engineeringchange:seizouikka:export"
...
import { getEcSeizouikkaList, updateEcSeizouikka, exportEcSeizouikkaData } from '@/api/logistics/manufacturing/engineering-change/seizouikka';
...
/** 数据源 */
const dataSource = ref<EcDeptView[]>([]);
/** 当前页 */
...
/** 选中行 */
const selectedRows = ref<EcDeptView[]>([]);
/** 表单可见 */
const formVisible = ref(false);
/** 表单 loading */
const formLoading = ref(false);
/** 编辑数据 */
const formData = ref<EcDeptView | null>(null);
/** 表单 ref */
...
/** 行选择 */
const rowSelection = computed(() => ({ selectedRowKeys: selectedRowKeys.value, onChange: (keys: (string | number)[], rows: EcDeptView[]) => { selectedRowKeys.value = keys; selectedRows.value = rows; } }));
/** 更新按钮禁用 */
...
function onClickRow(record: Record<string, unknown>) {
  return { onClick: () => { const id = getEcDetailId(record); selectedRowKeys.value = [id]; selectedRows.value = [record as unknown as EcDeptView]; } };
}
/** 编辑 */
...
async function handleFormSubmit() {
  ...
  const dto: EcDeptViewUpdate = formRef.value.getValues();
