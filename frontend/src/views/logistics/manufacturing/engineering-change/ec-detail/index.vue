...
<template>
  <div class="logistics-manufacturing-engineering-change-ec-detail">
    <!-- 查询栏 -->
    <TaktQueryBar
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:engineeringchange:ecdetail:create"
      update-permission="logistics:manufacturing:engineeringchange:ecdetail:update"
      delete-permission="logistics:manufacturing:engineeringchange:ecdetail:delete"
      import-permission="logistics:manufacturing:engineeringchange:ecdetail:import"
      export-permission="logistics:manufacturing:engineeringchange:ecdetail:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.ecDetail._self')"
      :width="600"
      :footer="null"
...
import type { EcDetail, EcDetailQuery, EcDetailCreate, EcDetailUpdate } from '@/types/logistics/manufacturing/engineering-change/ec-detail'
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getEcDetailField(record, 'ecName') ?? ''
  },
  {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:engineeringchange:ecdetail:update',
      ...
      },
      {
        ...
        permission: 'logistics:manufacturing:engineeringchange:ecdetail:delete',
...
const onClickRow = (record: EcDetail) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getEcDetailId(item)))
...
async function loadData() {
  ...
  try {
    ...
    const res = await getEcDetailList(params)
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.ecDetail._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: EcDetail) {
  formTitle.value = t('common.page.button.edit') + t('entity.ecDetail._self')
...
async function handleExport() {
  try {
    loading.value = true
    const kw = (queryKeyword.value ?? '').trim()
    const exportQuery: EcDetailQuery = {
      pageIndex: 1,
      pageSize: 100000,
      ...advancedQueryForm.value
    }
    ...
    const exportMeta = await exportEcDetail(exportQuery, excelNames.sheet, excelNames.fileBase)
    const ts = new Date()
...
.logistics-manufacturing-engineering-change-ec-detail {
  padding: 16px;
  display: flex;
