...
<template>
  <div class="p-4">
    ...
    <TaktModal v-model:open="formVisible" :title="t('common.dialog.title.edit', { entity: t('menu.logistics.manufacturing.engineering.change.seikan') })" width="900px" :confirm-loading="formLoading" @ok="handleFormSubmit">
...
import { getEcSeikanList, updateEcSeikan, exportEcSeikanData } from '@/api/logistics/manufacturing/engineering-change/ec-seikan';
...
/** 选中行 */
const selectedRows = ref<EcSeikan[]>([]);
/** 表单可见 */
const formVisible = ref(false);
/** 表单 loading */
const formLoading = ref(false);
/** 编辑数据 */
const formData = ref<EcSeikan | null>(null);
/** 表单 ref */
...
/** 行选择 */
const rowSelection = computed(() => ({ selectedRowKeys: selectedRowKeys.value, onChange: (keys: (string | number)[], rows: EcSeikan[]) => { selectedRowKeys.value = keys; selectedRows.value = rows; } }));
/** 更新按钮禁用 */
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getEcSeikanList({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
...
function onClickRow(record: Record<string, unknown>) {
  return { onClick: () => { const id = getEcDetailId(record); selectedRowKeys.value = [id]; selectedRows.value = [record as unknown as EcSeikan]; } };
}
/** 编辑 */
...
async function handleFormSubmit() {
  ...
  const dto: EcSeikanUpdate = formRef.value.getValues();
  formLoading.value = true;
  try {
    await updateEcSeikan(String(formData.value.ecDetailId), dto);
    message.success(t('common.feedback.updated', { target: t('menu.logistics.manufacturing.engineering.change.seikan') }));
...
async function handleExport() {
  try {
    loading.value = true;
    const blob = await exportEcSeikanData({ pageIndex: currentPage.value, pageSize: pageSize.value, keyWords: queryKeyword.value || undefined });
    const url = window.URL.createObjectURL(blob as Blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `${t('menu.logistics.manufacturing.engineering.change.seikan')}.xlsx`;
...
useEcExecSignalRGroup(TaktEcExecCodes.Pmc);
onMounted(loadData);
</script>
