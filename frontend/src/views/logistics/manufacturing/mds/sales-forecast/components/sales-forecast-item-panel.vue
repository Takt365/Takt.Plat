...
<!-- 文件名称：sales-forecast-item-panel.vue -->
<!-- 功能描述：Takt销售预测实体主表实体右侧明细 salesForecastItem 独立 CRUD（按主表选中 salesForecastId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="sales-forecast-item-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="sales-forecast-item-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
        ...
        :stripe="true"
        :virtual="true"
        :row-key="getSalesForecastItemId"
        :row-selection="rowSelection"
        :custom-row="onClickRow"
        :visible-column-keys="visibleColumnKeys"
        id-column-key="salesForecastItemId"
    ...
    >
      <SalesForecastItemForm
        ref="formRef"
        :form-data="formData"
        :master-id="masterSalesForecastId"
        :loading="formLoading"
      />
    </TaktModal>
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-logistics-manufacturing-mds-sales-forecast-sales-forecast-item"
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="SALESFORECASTITEM_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * Takt销售预测实体子表 salesForecastItem 右栏面板
...
import SalesForecastItemForm from './sales-forecast-item-form.vue'
import { useSalesForecastMasterContext } from '../composables/use-sales-forecast-master-context'
import {
  getSalesForecastItemList,
  getSalesForecastItemById,
  createSalesForecastItem,
  updateSalesForecastItem,
  deleteSalesForecastItemById,
  deleteSalesForecastItemBatch,
  getSalesForecastItemTemplate,
  importSalesForecastItem,
  exportSalesForecastItem,
} from '@/api/logistics/manufacturing/mds/sales-forecast-item'
import type { SalesForecastItem, SalesForecastItemQuery } from '@/types/logistics/manufacturing/mds/sales-forecast-item'

import {
  useSalesForecastItemI18n,
  SALESFORECASTITEM_DEFAULT_VISIBLE_COLUMN_KEYS,
  SALESFORECASTITEM_SUMMARY_SUM_FIELDS,
  SALESFORECASTITEM_QUERY_STRING_FIELDS,
  SALESFORECASTITEM_QUERY_FIELDS,
  SALESFORECASTITEM_SELF_I18N_KEY,
} from '../composables/use-sales-forecast-item-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useSalesForecastItemI18n()

const { t } = useI18n()
const { selectedMasterRow } = useSalesForecastMasterContext()

/** Excel 导入/导出默认 sheet 名与文件名前缀 */
const excelNames = taktExcelEntityNames('TaktSalesForecastItem')
/** 快捷查询占位文案 */
...
const dataSource = ref<SalesForecastItem[]>([])
const currentPage = ref(getTaktDefaultPageIndex())
const pageSize = ref(getTaktDefaultPageSize())
const total = ref(0)
const queryKeyword = ref('')
const selectedRow = ref<SalesForecastItem | null>(null)
const selectedRows = ref<SalesForecastItem[]>([])
const selectedRowKeys = ref<(string | number)[]>([])
const formVisible = ref(false)
const formTitle = ref('')
const formData = ref<Partial<SalesForecastItem>>({})
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(SALESFORECASTITEM_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  SALESFORECASTITEM_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...SALESFORECASTITEM_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...SALESFORECASTITEM_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'salesForecastItemId'
const masterSalesForecastId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['salesForecastId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterSalesForecastId.value !== '')
const updateDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length !== 1)
const deleteDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length === 0)

function getSalesForecastItemId(record: SalesForecastItem | Record<string, unknown>): string {
  return String((record as SalesForecastItem)?.[entityIdName] ?? '')
}

function getSalesForecastItemField(record: SalesForecastItem | Record<string, unknown>, field: string): unknown {
  return (record as SalesForecastItem)?.[field as keyof SalesForecastItem]
}

const columns = computed<TableColumnsType>(() => [
  {
    title: t('common.page.entity.id'),
    dataIndex: 'salesForecastItemId',
    key: 'salesForecastItemId',
    width: 80,
    resizable: true,
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: SalesForecastItem }) =>
      String(getSalesForecastItemField(record, 'salesForecastItemId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesForecastItem }) =>
      String(getSalesForecastItemField(record, 'salesForecastId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesForecastItem }) =>
      String(getSalesForecastItemField(record, 'salesForecastCode') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesForecastItem }) =>
      String(getSalesForecastItemField(record, 'convertedQuantity') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesForecastItem }) =>
      String(getSalesForecastItemField(record, 'estimatedUnitPrice') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesForecastItem }) =>
      String(getSalesForecastItemField(record, 'estimatedAmount') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: SalesForecastItem }) =>
      String(getSalesForecastItemField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: SalesForecastItem) => void handleEdit(record),
      },
      {
        key: 'delete',
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'salesForecastItemId',
...
const summarySumFieldSet = new Set<string>(SALESFORECASTITEM_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    SALESFORECASTITEM_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof SALESFORECASTITEM_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of SALESFORECASTITEM_SUMMARY_SUM_FIELDS) {
      const num = Number(getSalesForecastItemField(row, field))
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: SalesForecastItem[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: SalesForecastItem, selected: boolean) => {
    if (selected) {
      selectedRow.value = record
    } else if (selectedRow.value && getSalesForecastItemId(selectedRow.value) === getSalesForecastItemId(record)) {
      selectedRow.value = null
    }
  },
  onSelectAll: (selected: boolean, selectedRowsData: SalesForecastItem[]) => {
 ...
 * @param record 行数据
 */
function onClickRow(record: SalesForecastItem) {
  const key = getSalesForecastItemId(record)
  return {
    onClick: () => {
 ...
 * @returns {SalesForecastItemQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<SalesForecastItemQuery>): SalesForecastItemQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: SalesForecastItemQuery = {
    pageIndex: currentPage.value,
    pageSize: pageSize.value,
    salesForecastId: masterSalesForecastId.value,
    ...overrides,
  }
  if (kw.length > 0) {
    query.keyWords = kw
  }
  const assignTrimmed = (key: keyof SalesForecastItemQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
      query[key] = v as never
    }
  }
  for (const key of SALESFORECASTITEM_QUERY_STRING_FIELDS) {
...
async function loadData() {
  ...
  loading.value = true
  try {
    const res = await getSalesForecastItemList(buildListQuery())
...
async function handleEdit(record: SalesForecastItem) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getSalesForecastItemById(getSalesForecastItemId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.salesForecastItemId
    if (id) {
      await updateSalesForecastItem(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createSalesForecastItem(payload)
...
async function handleDeleteOne(record: SalesForecastItem) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteSalesForecastItemById(getSalesForecastItemId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getSalesForecastItemId(r)).filter(Boolean)
      await deleteSalesForecastItemBatch(ids)
      message.success(t('common.feedback.deleted', { target: pi.self() }))
      await loadData()
    },
  })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getSalesForecastItemTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importSalesForecastItem(file, sheetName)
...
async function handleExport() {
  ...
  try {
    loading.value = true
    const exportMeta = await exportSalesForecastItem(
