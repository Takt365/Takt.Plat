...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form assy-defect-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    ...
    <TaktEditableTable
      ...
      :title="t('entity.assydefectdetail._self')"
      :add-button-entity="t('entity.assydefectdetail._self')"
...
const assyDefectDetailFormColumns = computed<TaktEditableTableColumn[]>(() => [
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.assydefectdetail.randomcardno') }),
  },
  {
    key: 'occurrenceEngineering',
    title: t('entity.assydefectdetail.occurrenceengineering'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.assydefectdetail.occurrenceengineering') }),
  },
  {
    key: 'testStep',
...
function syncChildRowsFromFormData(val: Partial<AssyDefectCreate & { assyDefectId?: string }> | null | undefined) {
  childAssyDefectDetailRows.value = ((val as any)?.assyDefectDetails ?? []) as Record<string, unknown>[]
}

function createDefaultAssyDefectDetailRow(): Record<string, unknown> {
  return {
    prodOrderCode: '',
    lineNumber: (childAssyDefectDetailRows.value.length + 1) * 10,
    defectCategory: '',
    defectQty: 0,
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    assyDefectDetails: assyDefectDetailTableRef.value?.getRows?.() ?? childAssyDefectDetailRows.value.map((rest) => ({
      ...rest,
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  shiftNo: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.assydefect.shiftno') }))
      }
  ...
  prodOrderQty: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.assydefect.prodorderqty') }))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.assydefect.prodorderqty') }))
  ...
  prodActualQty: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.assydefect.prodactualqty') }))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.assydefect.prodactualqty') }))
  ...
  goodQuantity: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.assydefect.goodquantity') }))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.assydefect.goodquantity') }))
