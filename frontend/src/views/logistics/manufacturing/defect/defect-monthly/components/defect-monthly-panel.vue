...
<!-- 文件名称：defect-monthly-panel.vue -->
<!-- 功能描述：月生产不良推移转置表（工厂×机种×不良类别×月份不良率%） -->
      ...
      >
        <template #bodyCell="{ column, record, text }">
          <template v-if="String(column.key).startsWith('period_')">
            <span :title="resolvePeriodTooltip(record as DefectMonthlyTrend, String(column.key))">
              {{ formatPeriodRate(record as DefectMonthlyTrend, String(column.key)) }}
            </span>
          </template>
          <template v-else-if="column.key === 'defectCategory'">
            {{ resolveCategoryLabel(record as DefectMonthlyTrend) }}
          </template>
          <template v-else-if="column.key === 'trend'">
            <span :class="trendClass((record as DefectMonthlyTrend).trend || 'none')">
              {{ trendLabel((record as DefectMonthlyTrend).trend || 'none') }}
            </span>
          </template>
          <template v-else-if="column.key === 'varianceAmount'">
            <span :class="varianceClass((record as DefectMonthlyTrend).varianceAmount)">
              {{ formatRateDiff((record as DefectMonthlyTrend).varianceAmount) }}
            </span>
          </template>
          <template v-else-if="column.key === 'variancePercent'">
            <span :class="varianceClass((record as DefectMonthlyTrend).varianceAmount)">
              {{ formatPercent((record as DefectMonthlyTrend).variancePercent) }}
            </span>
          </template>
          <template v-else>
            {{ text }}
...
import {
  exportDefectMonthlyTrendAnalysis,
  getDefectMonthlyTrendAnalysis,
} from '@/api/logistics/manufacturing/defect/defect-monthly'
import type { DefectMonthlyTrend } from '@/types/logistics/manufacturing/defect/defect-monthly-trend'
...
/** 行数据 */
const rows = ref<DefectMonthlyTrend[]>([])
/** 期间列 */
...
const columns = computed<TableColumnsType>(() => {
  const cols: TableColumnsType = [
    ...
    {
      title: t(`${localePrefix}.defectCategory`),
...
/**
 * 行主键
 * @param {DefectMonthlyTrend} record 行
 * @returns {string} key
 */
function getRowKey(record: DefectMonthlyTrend): string {
  return `${record.plantCode}|${record.modelCode}|${record.defectCategory}`
}
 ...
 * @returns {string} 文案
 */
function resolveCategoryLabel(record: DefectMonthlyTrend): string {
  if (record.defectCategoryName?.trim()) {
    return record.defectCategoryName.trim()
  }
  const key = record.defectCategory as 'assy' | 'pcba'
  if (key === 'assy' || key === 'pcba') {
    return t(`${localePrefix}.defectCategoryOptions.${key}`)
  }
  return record.defectCategory
}

/**
 * 格式化不良率（百分比）
 * @param {number | null | undefined} value 比率 0~1
 * @returns {string} 文本
 */
...
/**
 * 格式化环比率差（百分点）
 * @param {number | null | undefined} value 比率差 0~1
 * @returns {string} 文本
 */
function formatRateDiff(value?: number | null): string {
  if (value == null || Number.isNaN(value)) return '—'
  return `${(value * 100).toFixed(2)}%`
}
...
/**
 * 差额样式
 * @param {number | null | undefined} amount 率差
 * @returns {string} class
 */
 ...
 * @returns {string} 文本
 */
function formatPeriodRate(record: DefectMonthlyTrend, columnKey: string): string {
 ...
 */
function resolvePeriodTooltip(record: DefectMonthlyTrend, columnKey: string): string {
  const period = columnKey.replace(/^period_/, '')
  const actual = record.periodActualQuantities?.[period]
  const defect = record.periodDefectQuantities?.[period]
  if (actual == null && defect == null) return ''
  return `${actual ?? '—'} / ${defect ?? '—'}`
}
...
function buildQuery() {
  ...
  return {
    plantCode: plant,
    modelCode: props.modelCode?.trim() || undefined,
    defectCategory: props.defectCategory?.trim() || undefined,
...
async function loadData() {
  ...
  loading.value = true
  try {
    const result = await getDefectMonthlyTrendAnalysis(query)
...
async function handleExport() {
  ...
  try {
    const exportMeta = await exportDefectMonthlyTrendAnalysis({
      ...query,
      pageIndex: 1,
      pageSize: 1,
    })
    ...
    const fileName = resolveExportDownloadFileName({
      ...
      fallbackBase: `defect-monthly-trend_${query.plantCode}`,
    })
