...
<!-- 命名空间：@/views/logistics/manufacturing/defect/assy-order-defect -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：组立工单不良统计实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:manufacturing:defect:assy:order:create"
      update-permission="logistics:manufacturing:defect:assy:order:update"
      delete-permission="logistics:manufacturing:defect:assy:order:delete"
      import-permission="logistics:manufacturing:defect:assy:order:import"
      export-permission="logistics:manufacturing:defect:assy:order:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktSingleTable
      ...
      :id-column-key="'assyOrderDefectId'"
      table-mode="single"
      :data-source="dataSource"
      :loading="loading"
      :stripe="true"
      :row-key="getAssyOrderDefectId"
    ...
    >
      <AssyOrderDefectForm
        :key="formData?.assyOrderDefectId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-defect-assy-order-defect'"
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="ASSYORDERDEFECT_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 组立工单不良统计实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/defect/assy-order-defect
 */
...
import AssyOrderDefectForm from './components/assy-order-defect-form.vue'
import { getAssyOrderDefectList, getAssyOrderDefectById, createAssyOrderDefect, updateAssyOrderDefect, deleteAssyOrderDefectById, deleteAssyOrderDefectBatch, getAssyOrderDefectTemplate, importAssyOrderDefect, exportAssyOrderDefect, updateAssyOrderDefectStatus } from '@/api/logistics/manufacturing/defect/assy-order-defect'
import type { AssyOrderDefect, AssyOrderDefectQuery } from '@/types/logistics/manufacturing/defect/assy-order-defect'
...
import {
  useAssyOrderDefectI18n,
  ASSYORDERDEFECT_LIST_FIELDS,
  ASSYORDERDEFECT_QUERY_STRING_FIELDS,
  ASSYORDERDEFECT_QUERY_FIELDS,
  ASSYORDERDEFECT_SELF_I18N_KEY,
} from './composables/use-assy-order-defect-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useAssyOrderDefectI18n()
/** 表格行类型（TaktSingleTable slot record 与 dataSource 行兼容） */
type AssyOrderDefectRowRecord = AssyOrderDefect | Record<string, unknown>
/** i18n 翻译函数 */
const { t } = useI18n()
/** Excel 导入/导出默认 sheet 名与文件名前缀 */
const excelNames = taktExcelEntityNames('TaktAssyOrderDefect')
/** 列表快捷查询占位文案 */
...
/** 分页列表数据 */
const dataSource = ref<AssyOrderDefect[]>([])
/** 当前页码 */
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<AssyOrderDefectRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<AssyOrderDefectRowRecord[]>([])
/** 表格多选 row-key 集合 */
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<AssyOrderDefect> | null>(null)
/** 表单提交 loading */
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(ASSYORDERDEFECT_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  ASSYORDERDEFECT_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
/** 高级查询当前可见字段 key */
...
const entityIdName = 'assyOrderDefectId'
/** 工具栏「编辑」是否禁用（须恰好选中一行） */
 ...
 * @returns {AssyOrderDefectQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<AssyOrderDefectQuery>): AssyOrderDefectQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: AssyOrderDefectQuery = {
  ...
  const assignTrimmed = (key: keyof AssyOrderDefectQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
      query[key] = v as never
    }
  }
  for (const key of ASSYORDERDEFECT_QUERY_STRING_FIELDS) {
 ...
 */
function buildAssyOrderDefectListColumn(
  key: string,
  title: string,
...
const columns = computed<TableColumnsType>(() => [
  buildAssyOrderDefectListColumn('assyOrderDefectId', t('common.page.entity.id'), { width: 80, fixed: 'left' }),
  ...ASSYORDERDEFECT_LIST_FIELDS.map((key) => buildAssyOrderDefectListColumn(key, pi.label(key))),
  CreateActionColumn({
    actions: [
      {
        key: 'update',
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:manufacturing:defect:assy:order:update',
        onClick: (record: AssyOrderDefectRowRecord) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        permission: 'logistics:manufacturing:defect:assy:order:delete',
        onClick: (record: AssyOrderDefectRowRecord) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getAssyOrderDefectId = (record: AssyOrderDefectRowRecord): string => {
 ...
 * @param field 字段名
 */
const getAssyOrderDefectDictValue = (
  record: AssyOrderDefectRowRecord,
  field: string,
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: AssyOrderDefectRowRecord[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: AssyOrderDefectRowRecord, selected: boolean) => {
    if (selected) {
      selectedRow.value = record
    } else if (selectedRow.value && getAssyOrderDefectId(selectedRow.value) === getAssyOrderDefectId(record)) {
      selectedRow.value = null
    }
  },
  onSelectAll: (selected: boolean, selectedRowsData: AssyOrderDefectRowRecord[]) => {
    selectedRow.value = selected && selectedRowsData.length === 1 ? (selectedRowsData[0] ?? null) : null
  }
}))

/** 行点击切换选中（与 rowSelection 联动） */
const onClickRow = (record: AssyOrderDefectRowRecord) => ({
  onClick: () => {
    const key = getAssyOrderDefectId(record)
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getAssyOrderDefectId(item)))
...
async function loadData() {
  loading.value = true
  try {
    const res = await getAssyOrderDefectList(buildListQuery())
    dataSource.value = res.data ?? []
    total.value = res.total ?? 0
  } catch (error: any) {
    logger.error('[AssyOrderDefect] 加载数据失败', { error })
...
async function handleEdit(record: AssyOrderDefectRowRecord) {
  const id = getAssyOrderDefectId(record)
  if (!id) {
    return
  }
  ...
  try {
    const detail = await getAssyOrderDefectById(id)
    formData.value = detail ?? ({ ...record } as Partial<AssyOrderDefect>)
    formVisible.value = true
  } catch (error: unknown) {
...
async function handleFormSubmit() {
  ...
  try {
    ...
    if (id) {
      await updateAssyOrderDefect(id, payload as any)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createAssyOrderDefect(payload as any)
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getAssyOrderDefectTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importAssyOrderDefect(file, sheetName)
...
async function handleExport() {
  try {
    loading.value = true
    const exportMeta = await exportAssyOrderDefect(
    ...
    logger.error('[AssyOrderDefect] 导出失败', { error })
...
/** 删除单行 */
async function handleDeleteOne(record: AssyOrderDefectRowRecord) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteAssyOrderDefectById((record as any)[entityIdName])
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      ...
      await deleteAssyOrderDefectBatch(ids)
      message.success(t('common.feedback.deleted', { target: pi.self() }))
      loadData()
    }
  })
