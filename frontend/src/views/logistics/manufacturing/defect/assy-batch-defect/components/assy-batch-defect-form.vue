...
<!-- 命名空间：@/views/logistics/manufacturing/defect/assy-batch-defect/components -->
<!-- 文件名称：assy-batch-defect-form.vue -->
<!-- 功能描述：组立批量不良统计实体维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="assy-batch-defect-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * 组立批量不良统计实体维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/defect/assy-batch-defect/components
 */
import { reactive, watch, computed, ref, onMounted } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import { useAssyBatchDefectI18n } from '../composables/use-assy-batch-defect-i18n'

/** 实体字段 i18n */
const pi = useAssyBatchDefectI18n()
import type { AssyBatchDefectCreate } from '@/types/logistics/manufacturing/defect/assy-batch-defect'
...
interface Props {
  formData?: Partial<AssyBatchDefectCreate & { assyBatchDefectId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 assyBatchDefectId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.assyBatchDefectId) {
...
watch(
  ...
  () => {
    if (!props.formData?.assyBatchDefectId) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  batchOrderQty: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(pi.ph('batchOrderQty'))
      }
  ...
  batchStatus: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(pi.ph('batchStatus'))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(pi.ph('batchStatus'))
...
function getValues(): Record<string, any> {
  ...
  if ('batchOrderQty' in payload) {
    const rawbatchOrderQty = payload.batchOrderQty
    payload.batchOrderQty = typeof rawbatchOrderQty === 'number' ? rawbatchOrderQty : Number(rawbatchOrderQty)
  }
  ...
  if ('batchStatus' in payload) {
    const rawbatchStatus = payload.batchStatus
    payload.batchStatus = typeof rawbatchStatus === 'number' ? rawbatchStatus : Number(rawbatchStatus)
  }
...
function resetFields() {
  ...
  applyFormDefaults(formState)
  applyScopeDefaults(formState as Record<string, unknown>, !props.formData?.assyBatchDefectId)
