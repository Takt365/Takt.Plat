...
<!-- 功能描述：PCBA改修明细实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.pcbaRepairDetail.pcbarepairid')"
                name="pcbaRepairId"
              >
                <a-input
                  v-model:value="formState.pcbaRepairId"
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.pcbaRepairDetail.defectreason')"
                name="defectReason"
              >
                <a-input
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.pcbaRepairDetail.defectqty')"
                name="defectQty"
              >
                <a-input-number
...
/**
 * PCBA改修明细实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","pcbaRepairId","prodOrderCode","lineNumber","pcbaBoardType","prodActualQty","prodLine","cardNo","defectSymptom","defectEngineering","defectReason","defectQty","defectResponsibility","defectNature","repairOperator","extFieldJson","remark"]
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.pcbaRepairDetailId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  pcbaRepairId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.pcbaRepairDetail.pcbarepairid') }),
      trigger: 'blur'
    }
  ],
  prodOrderCode: [
  ...
  lineNumber: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.pcbaRepairDetail.linenumber') }),
      trigger: 'change'
    }
  ],
  prodActualQty: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.pcbaRepairDetail.prodactualqty') }),
      trigger: 'change'
    }
  ],
  defectQty: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.pcbaRepairDetail.defectqty') }),
      trigger: 'change'
    }
  ],
}))
