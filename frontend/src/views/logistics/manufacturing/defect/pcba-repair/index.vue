...
<!-- 文件名称：index.vue -->
<!-- 功能描述：PCBA改修日报实体 不良率管理页面，含查询、增删改，由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getPcbaRepairId"
      :master-row-selection="rowSelection"
      master-id-column-key="pcbaRepairId"
    ...
    >
      ...
      <template #detail>
        <PcbaRepairDetailPanel
          ref="pcbaRepairDetailPanelRef"
    ...
    >
      <PcbaRepairForm
        :key="formData?.pcbaRepairId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="PCBAREPAIR_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * PCBA改修日报实体 不良率管理页 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
...
import { providePcbaRepairMasterContext, type PcbaRepairRowRecord } from './composables/use-pcba-repair-master-context'
...
import {
  usePcbaRepairI18n,
  PCBAREPAIR_LIST_FIELDS,
  PCBAREPAIR_QUERY_STRING_FIELDS,
  PCBAREPAIR_QUERY_FIELDS,
  PCBAREPAIR_SELF_I18N_KEY,
} from './composables/use-pcba-repair-i18n'
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<PcbaRepairRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<PcbaRepairRowRecord[]>([])
/** 表格多选 row-key 集合 */
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<PcbaRepair> | null>(null)
/** 表单提交 loading */
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of PCBAREPAIR_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(PCBAREPAIR_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  PCBAREPAIR_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
/** 高级查询当前可见字段 key */
...
/** 主表选中行上下文（右侧明细面板读取） */
const { selectedMasterRow } = providePcbaRepairMasterContext()
const pcbaRepairDetailPanelRef = ref<InstanceType<typeof PcbaRepairDetailPanel> | null>(null)

/**
 * 构建列表/导出查询参数（空字符串与未填数值/日期不下发，避免后端 DateTime? 模型绑定 400；无参不补默认）
 * @param overrides 覆盖分页或导出上限等字段
 * @returns {PcbaRepairQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<PcbaRepairQuery>): PcbaRepairQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: PcbaRepairQuery = {
  ...
  const assignTrimmed = (key: keyof PcbaRepairQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
function syncMasterSelection(record: PcbaRepairRowRecord | null) {
  selectedMasterRow.value = record
  selectedMasterKey.value = record ? getPcbaRepairId(record) : ''
}
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as PcbaRepairRowRecord
  const key = getPcbaRepairId(row)
...
async function loadPcbaRepairDetail(record: PcbaRepairRowRecord): Promise<PcbaRepair | null> {
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPcbaRepairField(record, 'prodOrderType') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPcbaRepairField(record, 'remark') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: PcbaRepairRowRecord) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        onClick: (record: PcbaRepairRowRecord) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getPcbaRepairId = (record: PcbaRepairRowRecord): string => {
 ...
 * @param field 字段名
 */
const getPcbaRepairDictValue = (
  record: PcbaRepairRowRecord,
  field: string,
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: PcbaRepairRowRecord[]) => {
  ...
  onSelect: (record: PcbaRepairRowRecord, selected: boolean) => {
    if (selected) {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: PcbaRepairRowRecord[]) => {
...
async function loadData() {
  ...
  try {
    ...
    const res = await getPcbaRepairList(buildListQuery())
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: PcbaRepairRowRecord) {
...
async function handleFormSubmit() {
  ...
  pcbaRepairDetailPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importPcbaRepair(file, sheetName)
...
async function handleExport() {
  try {
    ...
    const exportMeta = await exportPcbaRepair(
...
/** 删除单行 */
async function handleDeleteOne(record: PcbaRepairRowRecord) {
  Modal.confirm({
