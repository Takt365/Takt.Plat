...
<!-- 命名空间：@/views/logistics/manufacturing/defect/group/components -->
<!-- 文件名称：group-form.vue -->
<!-- 功能描述：不良组主数据实体维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
        ...
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              ...
              >
                <a-input
                  v-model:value="formState.defectGroupCode"
                  :placeholder="pi.ph('defectGroupCode')"
                  show-count
                  :maxlength="3"
                  allow-clear
                  :disabled="!!formData?.defectGroupId"
                />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item
                :label="pi.label('defectGroupName')"
            ...
            <a-col :span="24">
              <a-form-item
                :label="pi.label('defectGroupDescription')"
                name="defectGroupDescription"
              >
                <a-textarea
...
/**
 * 不良组主数据实体维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/defect/group/components
 */
import { reactive, watch, computed, ref, onMounted } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import { useDefectGroupI18n } from '../composables/use-group-i18n'

/** 实体字段 i18n */
const pi = useDefectGroupI18n()
import type { DefectGroupCreate } from '@/types/logistics/manufacturing/defect/group'
...
interface Props {
  formData?: Partial<DefectGroupCreate & { defectGroupId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 defectGroupId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.defectGroupId) {
...
watch(
  ...
  () => {
    if (!props.formData?.defectGroupId) {
...
function resetFields() {
  ...
  applyFormDefaults(formState)
  applyScopeDefaults(formState as Record<string, unknown>, !props.formData?.defectGroupId)
