...
<!-- 命名空间：@/views/logistics/manufacturing/sop/sop-exec-scan -->
<!-- 文件名称：index.vue -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-sop-sop-exec-scan'"
 ...
 * @module views/logistics/manufacturing/sop/sop-exec-scan
 */
...
import SopExecScanPanel from './components/sop-exec-scan-panel.vue'
import { provideSopExecMasterContext } from './composables/use-exec-master-context'
...
const sopExecScanPanelRef = ref<InstanceType<typeof SopExecScanPanel> | null>(null)
...
function buildListQuery(overrides?: Partial<SopExecQuery>): SopExecQuery {
  ...
  assignTrimmed('currentStepId', form.currentStepId)
...
function syncMasterSelection(record: SopExec | null) {
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as SopExec
...
async function handleFormSubmit() {
  ...
  sopExecScanPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
