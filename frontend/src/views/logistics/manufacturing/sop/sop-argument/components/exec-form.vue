...
<template>
  ...
  >
    ...
    <!-- 下：子表 arguments -->
    <TaktEditableTable
      ref="sopArgumentTableRef"
      v-model="childSopArgumentRows"
      :columns="sopArgumentFormColumns"
      :title="t('entity.sopargument._self')"
      :add-button-entity="t('entity.sopargument._self')"
      id-field="sopArgumentId"
      :default-row="createDefaultSopArgumentRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
/** 子表 sopArgument 可编辑列 */
const sopArgumentFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'execId',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.sopargument.execstepid') }),
  },
  {
    key: 'routingItemParameterId',
    title: t('entity.sopargument.routingitemparameterid'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.sopargument.routingitemparameterid') }),
  },
  {
    key: 'paramCode',
...
function syncChildRowsFromFormData(val: Partial<SopExecCreate & { sopExecId?: string }> | null | undefined) {
  childSopArgumentRows.value = ((val as any)?.arguments ?? []) as Record<string, unknown>[]
}

function createDefaultSopArgumentRow(): Record<string, unknown> {
  return {
    execId: '',
    execStepId: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    arguments: sopArgumentTableRef.value?.getRows?.() ?? childSopArgumentRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await sopArgumentTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  sopArgumentTableRef.value?.resetRows?.()
