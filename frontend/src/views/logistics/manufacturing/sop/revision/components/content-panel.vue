...
<template>
  <div class="content-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-logistics-manufacturing-sop-revision-content"
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record: SopContent, selected: boolean) => {
    ...
    } else if (selectedRow.value && getSopContentId(selectedRow.value) === getSopContentId(record)) {
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importSopContent(file, sheetName)
