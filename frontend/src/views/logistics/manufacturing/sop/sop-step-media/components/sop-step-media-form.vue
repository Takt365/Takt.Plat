...
<!-- 文件名称：sop-step-media-form.vue -->
<!-- 功能描述：SOP 工步实体子表 sopStepMedia 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form sop-step-media-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    <a-tabs
      ...
      class="sop-step-media-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * SOP 工步实体子表 sopStepMedia 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
 * @module views/logistics/manufacturing/sop/sop-step-media/components
 */
import { reactive, watch, computed, ref, onMounted } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import type { SopStepMediaCreate } from '@/types/logistics/manufacturing/sop/sop-step-media'
...
interface Props {
  formData?: Partial<SopStepMediaCreate & { sopStepMediaId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 sopStepMediaId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.sopStepMediaId) {
