...
<!-- 命名空间：@/views/logistics/manufacturing/sop/doc -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：SOP 文档头实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'sopStatus'">
          <a-switch
            :checked="getSopDocDictValue(record, 'sopStatus') === 1"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-sop-doc'"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('sopCode')">
        <a-input
          v-model:value="advancedQueryForm.sopCode"
          :placeholder="pi.queryPh('sopCode', 'required')"
          show-count
          :maxlength="50"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('sopName')">
        <a-input
          v-model:value="advancedQueryForm.sopName"
          :placeholder="pi.queryPh('sopName', 'required')"
          show-count
          :maxlength="200"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('currentRevisionId')">
        <TaktSelect
          v-model:value="advancedQueryForm.currentRevisionId"
          api-url="TaktSopRevisions/options"
          :placeholder="pi.queryPh('currentRevisionId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('sopStatus')">
      <a-form-item :label="pi.queryLabel('sopStatus')">
        <TaktSelect
          v-model:value="advancedQueryForm.sopStatus"
          dict-type="sys_normal_disable"
          :placeholder="pi.queryPh('sopStatus', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="SOPDOC_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * SOP 文档头实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/manufacturing/sop/doc
 */
...
import {
  ...
  SOPDOC_QUERY_FIELDS,
  SOPDOC_SELF_I18N_KEY,
} from './composables/use-doc-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useSopDocI18n()
/** 表格行类型（TaktSingleTable slot record 与 dataSource 行兼容） */
type SopDocRowRecord = SopDoc | Record<string, unknown>
/** i18n 翻译函数 */
const { t } = useI18n()
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<SopDocRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<SopDocRowRecord[]>([])
/** 表格多选 row-key 集合 */
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of SOPDOC_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(SOPDOC_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  SOPDOC_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
/** 高级查询当前可见字段 key */
 ...
 */
function buildSopDocListColumn(
  key: string,
  title: string,
...
const columns = computed<TableColumnsType>(() => [
  buildSopDocListColumn('sopDocId', t('common.page.entity.id'), { width: 80, fixed: 'left' }),
  ...SOPDOC_LIST_FIELDS.map((key) => buildSopDocListColumn(key, pi.label(key))),
  CreateActionColumn({
    actions: [
      {
        key: 'update',
        ...
        onClick: (record: SopDocRowRecord) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        onClick: (record: SopDocRowRecord) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getSopDocId = (record: SopDocRowRecord): string => {
 ...
 * @param field 字段名
 */
const getSopDocDictValue = (
  record: SopDocRowRecord,
  field: string,
...
/** 将行字段/字典值转为有限 number */
const toSopDocNumber = (value: string | number | undefined | null): number => {
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: SopDocRowRecord[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: SopDocRowRecord, selected: boolean) => {
    if (selected) {
      selectedRow.value = record
    } else if (selectedRow.value && getSopDocId(selectedRow.value) === getSopDocId(record)) {
      selectedRow.value = null
    }
  },
  onSelectAll: (selected: boolean, selectedRowsData: SopDocRowRecord[]) => {
    selectedRow.value = selected && selectedRowsData.length === 1 ? (selectedRowsData[0] ?? null) : null
  }
}))

/** 行点击切换选中（与 rowSelection 联动） */
const onClickRow = (record: SopDocRowRecord) => ({
  onClick: () => {
    const key = getSopDocId(record)
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getSopDocId(item)))
...
async function handleEdit(record: SopDocRowRecord) {
  ...
  try {
    ...
    formData.value = detail ?? ({ ...record } as Partial<SopDoc>)
    formVisible.value = true
  } catch (error: unknown) {
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importSopDoc(file, sheetName)
...
/** 删除单行 */
async function handleDeleteOne(record: SopDocRowRecord) {
  Modal.confirm({
 ...
 * @param checked 是否启用
 */
async function handleSopStatusChange(record: SopDocRowRecord, checked: boolean) {
  const newVal = checked ? 1 : 0
  const oldVal = toSopDocNumber(getSopDocDictValue(record, 'sopStatus'))
  const id = getSopDocId(record)
