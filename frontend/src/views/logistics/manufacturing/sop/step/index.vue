...
<!-- 命名空间：@/views/logistics/manufacturing/sop/step -->
<!-- 文件名称：index.vue -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-manufacturing-sop-step'"
 ...
 * @module views/logistics/manufacturing/sop/step
 */
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record: SopStepRowRecord, selected: boolean) => {
    ...
    } else if (selectedRow.value && getSopStepId(selectedRow.value) === getSopStepId(record)) {
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importSopStep(file, sheetName)
