...
<template>
  <div class="step-media-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div
      ...
      class="step-media-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('mediaType')">
        <TaktSelect
          v-model:value="advancedQueryForm.mediaType"
          dict-type="logistics_sop_media_type"
          :placeholder="pi.queryPh('mediaType', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('fileExt')">
        <a-input
          v-model:value="advancedQueryForm.fileExt"
          :placeholder="pi.queryPh('fileExt', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="SOPSTEPMEDIA_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  useSopStepMediaI18n,
  SOPSTEPMEDIA_DEFAULT_VISIBLE_COLUMN_KEYS,
  SOPSTEPMEDIA_SUMMARY_SUM_FIELDS,
  SOPSTEPMEDIA_QUERY_STRING_FIELDS,
  SOPSTEPMEDIA_QUERY_FIELDS,
  SOPSTEPMEDIA_SELF_I18N_KEY,
} from '../composables/use-step-media-i18n'
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of SOPSTEPMEDIA_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(SOPSTEPMEDIA_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  SOPSTEPMEDIA_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...SOPSTEPMEDIA_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...SOPSTEPMEDIA_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'sopStepMediaId'
const masterSopStepId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['sopStepId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterSopStepId.value !== '')
...
const summarySumFieldSet = new Set<string>(SOPSTEPMEDIA_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    SOPSTEPMEDIA_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof SOPSTEPMEDIA_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of SOPSTEPMEDIA_SUMMARY_SUM_FIELDS) {
      const num = Number(getSopStepMediaField(row, field))
