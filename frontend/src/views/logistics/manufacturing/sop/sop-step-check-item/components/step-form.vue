...
<template>
  ...
  >
    ...
    <!-- 下：子表 checkItems -->
    <TaktEditableTable
      ref="sopStepCheckItemTableRef"
      v-model="childSopStepCheckItemRows"
      :columns="sopStepCheckItemFormColumns"
      :title="t('entity.sopstepcheckitem._self')"
      :add-button-entity="t('entity.sopstepcheckitem._self')"
      id-field="sopStepCheckItemId"
      :default-row="createDefaultSopStepCheckItemRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const childSopStepCheckItemRows = ref<Record<string, unknown>[]>([])
const sopStepCheckItemTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 sopStepCheckItem 可编辑列 */
const sopStepCheckItemFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'stepId',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.sopstepcheckitem.checkmethod') }),
  },
  {
    key: 'checkStandard',
    title: t('entity.sopstepcheckitem.checkstandard'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.sopstepcheckitem.checkstandard') }),
  },
  {
    key: 'isRequired',
...
function syncChildRowsFromFormData(val: Partial<SopStepCreate & { sopStepId?: string }> | null | undefined) {
  childSopStepCheckItemRows.value = ((val as any)?.checkItems ?? []) as Record<string, unknown>[]
}

function createDefaultSopStepCheckItemRow(): Record<string, unknown> {
  return {
    stepId: '',
    checkItemName: '',
    checkMethod: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    checkItems: sopStepCheckItemTableRef.value?.getRows?.() ?? childSopStepCheckItemRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await sopStepCheckItemTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childSopStepCheckItemRows.value = []
  sopStepCheckItemTableRef.value?.resetRows?.()
