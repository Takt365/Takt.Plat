...
<template>
  ...
  >
    ...
    <!-- 下：子表 otherItems -->
    <TaktEditableTable
      ref="qualityAssuranceOtherTableRef"
      v-model="childQualityAssuranceOtherRows"
      :columns="qualityAssuranceOtherFormColumns"
      :title="t('entity.qualityassuranceother._self')"
      :add-button-entity="t('entity.qualityassuranceother._self')"
      id-field="qualityAssuranceOtherId"
      :default-row="createDefaultQualityAssuranceOtherRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const childQualityAssuranceOtherRows = ref<Record<string, unknown>[]>([])
const qualityAssuranceOtherTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 qualityAssuranceOther 可编辑列 */
const qualityAssuranceOtherFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'lineNumber',
...
function syncChildRowsFromFormData(val: Partial<QualityAssuranceCreate & { qualityAssuranceId?: string }> | null | undefined) {
  childQualityAssuranceOtherRows.value = ((val as any)?.otherItems ?? []) as Record<string, unknown>[]
}

function createDefaultQualityAssuranceOtherRow(): Record<string, unknown> {
  return {
    lineNumber: (childQualityAssuranceOtherRows.value.length + 1) * 10,
    operationsCost: 0,
    workTimeMinutes: 0,
    otherExpenses: 0,
    otherNote: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    otherItems: qualityAssuranceOtherTableRef.value?.getRows?.() ?? childQualityAssuranceOtherRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await qualityAssuranceOtherTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childQualityAssuranceOtherRows.value = []
  qualityAssuranceOtherTableRef.value?.resetRows?.()
