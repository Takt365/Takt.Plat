...
<!-- 文件名称：index.vue -->
<!-- 功能描述：品质事故主表管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="logistics-quality-cost-quality-incident">
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.qualityIncident._self')"
      :width="600"
      :footer="null"
...
/**
 * 品质事故主表管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: t('entity.qualityIncident.incidentitems'),
    dataIndex: 'incidentItems',
    key: 'incidentItems',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getQualityIncidentField(record, 'incidentItems') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.qualityIncident._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: QualityIncident) {
  formTitle.value = t('common.page.button.edit') + t('entity.qualityIncident._self')
