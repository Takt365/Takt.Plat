...
<!-- 文件名称：index.vue -->
<!-- 功能描述：品质业务主表管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="logistics-quality-cost-quality-operation">
    ...
    >
      ...
      <a-form-item :label="t('entity.qualityOperation.debitnoteno')">
        <a-input
          ...
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.qualityOperation.debitnoteno') })"
          allow-clear
        />
      </a-form-item>
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.qualityOperation._self')"
      :width="600"
      :footer="null"
...
/**
 * 品质业务主表管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: t('entity.qualityOperation.incomingitems'),
    dataIndex: 'incomingItems',
    key: 'incomingItems',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getQualityOperationField(record, 'incomingItems') ?? ''
  },
  {
    title: t('entity.qualityOperation.firstarticleitems'),
    dataIndex: 'firstArticleItems',
    key: 'firstArticleItems',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getQualityOperationField(record, 'firstArticleItems') ?? ''
  },
  {
    title: t('entity.qualityOperation.calibrationitems'),
    dataIndex: 'calibrationItems',
    key: 'calibrationItems',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getQualityOperationField(record, 'calibrationItems') ?? ''
  },
  {
    title: t('entity.qualityOperation.otheritems'),
    dataIndex: 'otherItems',
    key: 'otherItems',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getQualityOperationField(record, 'otherItems') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.qualityOperation._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: QualityOperation) {
  formTitle.value = t('common.page.button.edit') + t('entity.qualityOperation._self')
