...
function getValues(): Record<string, any> {
  ...
  if (props.formData?.qualityIssueMeetingId) {
    payload.qualityIssueMeetingId = props.formData.qualityIssueMeetingId
  }
  ...
  if (masterRow) {
    const masterCode = masterRow.qualityIssueCode ?? masterRow.QualityIssueCode
    const masterName = masterRow.qualityIssueName ?? masterRow.QualityIssueName
    if (masterCode != null && masterCode !== '' && !payload.qualityIssueCode) {
      payload.qualityIssueCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.qualityIssueName) {
      payload.qualityIssueName = masterName
    }
