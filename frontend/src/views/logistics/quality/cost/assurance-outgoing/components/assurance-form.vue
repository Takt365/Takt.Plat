...
<template>
  ...
  >
    ...
    <!-- 下：子表 outgoingItems -->
    <TaktEditableTable
      ref="qualityAssuranceOutgoingTableRef"
      v-model="childQualityAssuranceOutgoingRows"
      :columns="qualityAssuranceOutgoingFormColumns"
      :title="t('entity.qualityassuranceoutgoing._self')"
      :add-button-entity="t('entity.qualityassuranceoutgoing._self')"
      id-field="qualityAssuranceOutgoingId"
      :default-row="createDefaultQualityAssuranceOutgoingRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const childQualityAssuranceOutgoingRows = ref<Record<string, unknown>[]>([])
const qualityAssuranceOutgoingTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 qualityAssuranceOutgoing 可编辑列 */
const qualityAssuranceOutgoingFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'lineNumber',
...
function syncChildRowsFromFormData(val: Partial<QualityAssuranceCreate & { qualityAssuranceId?: string }> | null | undefined) {
  childQualityAssuranceOutgoingRows.value = ((val as any)?.outgoingItems ?? []) as Record<string, unknown>[]
}

function createDefaultQualityAssuranceOutgoingRow(): Record<string, unknown> {
  return {
    lineNumber: (childQualityAssuranceOutgoingRows.value.length + 1) * 10,
    inspectionCost: 0,
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    outgoingItems: qualityAssuranceOutgoingTableRef.value?.getRows?.() ?? childQualityAssuranceOutgoingRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).outgoingItems
...
async function validate() {
  ...
  await qualityAssuranceOutgoingTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childQualityAssuranceOutgoingRows.value = []
  qualityAssuranceOutgoingTableRef.value?.resetRows?.()
