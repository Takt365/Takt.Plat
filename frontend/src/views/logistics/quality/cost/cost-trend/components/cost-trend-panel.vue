...
<!-- 文件名称：cost-trend-panel.vue -->
<!-- 功能描述：质量成本月推移转置表（工厂×成本类别×币种×月份） -->
      ...
      >
        <template #bodyCell="{ column, record, text }">
          <template v-if="String(column.key).startsWith('period_')">
            {{ formatPeriodAmount(record as QualityCostTrend, String(column.key)) }}
          </template>
          ...
          <template v-else-if="column.key === 'trend'">
            <span :class="trendClass((record as QualityCostTrend).trend || 'none')">
              {{ trendLabel((record as QualityCostTrend).trend || 'none') }}
            </span>
          </template>
          <template v-else-if="column.key === 'varianceAmount'">
            <span :class="varianceClass((record as QualityCostTrend).varianceAmount)">
              {{ formatCost((record as QualityCostTrend).varianceAmount) }}
            </span>
          </template>
          <template v-else-if="column.key === 'variancePercent'">
            <span :class="varianceClass((record as QualityCostTrend).varianceAmount)">
              {{ formatPercent((record as QualityCostTrend).variancePercent) }}
            </span>
          </template>
          <template v-else>
            {{ text }}
...
import {
  exportQualityCostMonthlyTrendAnalysis,
  getQualityCostMonthlyTrendAnalysis,
} from '@/api/logistics/quality/cost/cost-trend'
import type { QualityCostTrend } from '@/types/logistics/quality/cost/cost-trend'
...
/** 行数据 */
const rows = ref<QualityCostTrend[]>([])
/** 期间列 */
...
const columns = computed<TableColumnsType>(() => {
  const cols: TableColumnsType = [
    ...
    {
      title: t(`${localePrefix}.costCategory`),
  ...
  for (const period of periodOrder.value) {
    cols.push({
      title: period,
      dataIndex: ['periodAmounts', period],
      key: `period_${period}`,
      width: 120,
      align: 'right',
    })
...
/**
 * 行主键
 * @param {QualityCostTrend} record 行
 * @returns {string} key
 */
 ...
 * @returns {string} 文案
 */
function resolveCategoryLabel(record: QualityCostTrend): string {
  if (record.costCategoryName?.trim()) {
    return record.costCategoryName.trim()
  }
  const key = record.costCategory as 'assurance' | 'issue' | 'incident'
  if (key === 'assurance' || key === 'issue' || key === 'incident') {
    return t(`${localePrefix}.costCategoryOptions.${key}`)
  }
  return record.costCategory
}
 ...
 * @returns {string} 文本
 */
function formatPeriodAmount(record: QualityCostTrend, columnKey: string): string {
  const period = columnKey.replace(/^period_/, '')
  const value = record.periodAmounts?.[period]
  if (value == null || Number.isNaN(value)) return '—'
  return value.toFixed(2)
}
...
async function loadData() {
  ...
  loading.value = true
  try {
    const result = await getQualityCostMonthlyTrendAnalysis(query)
...
async function handleExport() {
  ...
  try {
    const exportMeta = await exportQualityCostMonthlyTrendAnalysis({
      ...query,
      pageIndex: 1,
      pageSize: 1,
    })
    ...
    const fileName = resolveExportDownloadFileName({
      ...
      fallbackBase: `quality-cost-trend_${query.plantCode}`,
    })
