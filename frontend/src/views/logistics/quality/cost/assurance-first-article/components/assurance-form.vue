...
<template>
  ...
  >
    ...
    <!-- 下：子表 firstArticleItems -->
    <TaktEditableTable
      ref="qualityAssuranceFirstArticleTableRef"
      v-model="childQualityAssuranceFirstArticleRows"
      :columns="qualityAssuranceFirstArticleFormColumns"
      :title="t('entity.qualityassurancefirstarticle._self')"
      :add-button-entity="t('entity.qualityassurancefirstarticle._self')"
      id-field="qualityAssuranceFirstArticleId"
      :default-row="createDefaultQualityAssuranceFirstArticleRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const childQualityAssuranceFirstArticleRows = ref<Record<string, unknown>[]>([])
const qualityAssuranceFirstArticleTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 qualityAssuranceFirstArticle 可编辑列 */
const qualityAssuranceFirstArticleFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'lineNumber',
...
function syncChildRowsFromFormData(val: Partial<QualityAssuranceCreate & { qualityAssuranceId?: string }> | null | undefined) {
  childQualityAssuranceFirstArticleRows.value = ((val as any)?.firstArticleItems ?? []) as Record<string, unknown>[]
}

function createDefaultQualityAssuranceFirstArticleRow(): Record<string, unknown> {
  return {
    lineNumber: (childQualityAssuranceFirstArticleRows.value.length + 1) * 10,
    qualificationCost: 0,
    workTimeMinutes: 0,
    otherExpenses: 0,
    qualificationNote: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    firstArticleItems: qualityAssuranceFirstArticleTableRef.value?.getRows?.() ?? childQualityAssuranceFirstArticleRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).firstArticleItems
...
async function validate() {
  ...
  await qualityAssuranceFirstArticleTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childQualityAssuranceFirstArticleRows.value = []
  qualityAssuranceFirstArticleTableRef.value?.resetRows?.()
