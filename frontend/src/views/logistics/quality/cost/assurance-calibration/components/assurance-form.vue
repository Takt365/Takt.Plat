...
<template>
  ...
  >
    ...
    <!-- 下：子表 calibrationItems -->
    <TaktEditableTable
      ref="qualityAssuranceCalibrationTableRef"
      v-model="childQualityAssuranceCalibrationRows"
      :columns="qualityAssuranceCalibrationFormColumns"
      :title="t('entity.qualityassurancecalibration._self')"
      :add-button-entity="t('entity.qualityassurancecalibration._self')"
      id-field="qualityAssuranceCalibrationId"
      :default-row="createDefaultQualityAssuranceCalibrationRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const childQualityAssuranceCalibrationRows = ref<Record<string, unknown>[]>([])
const qualityAssuranceCalibrationTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 qualityAssuranceCalibration 可编辑列 */
const qualityAssuranceCalibrationFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'lineNumber',
...
function syncChildRowsFromFormData(val: Partial<QualityAssuranceCreate & { qualityAssuranceId?: string }> | null | undefined) {
  childQualityAssuranceCalibrationRows.value = ((val as any)?.calibrationItems ?? []) as Record<string, unknown>[]
}

function createDefaultQualityAssuranceCalibrationRow(): Record<string, unknown> {
  return {
    lineNumber: (childQualityAssuranceCalibrationRows.value.length + 1) * 10,
    calibrationCost: 0,
    workTimeMinutes: 0,
    externalAgentServiceFee: 0,
    otherExpenses: 0,
    calibrationNote: '',
    extField: '',
    remark: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    calibrationItems: qualityAssuranceCalibrationTableRef.value?.getRows?.() ?? childQualityAssuranceCalibrationRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).calibrationItems
...
async function validate() {
  ...
  await qualityAssuranceCalibrationTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childQualityAssuranceCalibrationRows.value = []
  qualityAssuranceCalibrationTableRef.value?.resetRows?.()
