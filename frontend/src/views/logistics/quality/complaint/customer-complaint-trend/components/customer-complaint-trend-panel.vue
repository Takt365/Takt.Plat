...
<!-- 文件名称：customer-complaint-trend-panel.vue -->
<!-- 功能描述：顾客投诉月推移转置表（工厂×客户×月份） -->
      ...
      >
        <template #bodyCell="{ column, record, text }">
          <template v-if="String(column.key).startsWith('period_')">
            {{ formatPeriodCount(record as CustomerComplaintMonthlyTrend, String(column.key)) }}
          </template>
          <template v-else-if="column.key === 'trend'">
            <span :class="trendClass((record as CustomerComplaintMonthlyTrend).trend || 'none')">
              {{ trendLabel((record as CustomerComplaintMonthlyTrend).trend || 'none') }}
            </span>
          </template>
          <template v-else-if="column.key === 'varianceAmount'">
            <span :class="varianceClass((record as CustomerComplaintMonthlyTrend).varianceAmount)">
              {{ formatCount((record as CustomerComplaintMonthlyTrend).varianceAmount) }}
            </span>
          </template>
          <template v-else-if="column.key === 'variancePercent'">
            <span :class="varianceClass((record as CustomerComplaintMonthlyTrend).varianceAmount)">
              {{ formatPercent((record as CustomerComplaintMonthlyTrend).variancePercent) }}
            </span>
          </template>
          <template v-else>
            {{ text }}
...
/** 行数据 */
const rows = ref<CustomerComplaintMonthlyTrend[]>([])
/** 期间列 */
...
/**
 * 行主键
 * @param {CustomerComplaintMonthlyTrend} record 行
 * @returns {string} key
 */
function getRowKey(record: CustomerComplaintMonthlyTrend): string {
 ...
 * @returns {string} 文本
 */
function formatPeriodCount(record: CustomerComplaintMonthlyTrend, columnKey: string): string {
...
function buildQuery() {
  ...
  return {
    ...
    complaintType: props.complaintType,
...
async function loadData() {
  ...
  loading.value = true
  try {
    const result = await getCustomerComplaintMonthlyTrendAnalysis(query)
...
async function handleExport() {
  ...
  try {
    const exportMeta = await exportCustomerComplaintMonthlyTrendAnalysis({
      ...query,
      pageIndex: 1,
      pageSize: 1,
    })
    ...
    const fileName = resolveExportDownloadFileName({
      ...
      fallbackBase: `customer-complaint-trend_${query.plantCode}`,
    })
