...
<!-- 功能描述：客诉主表实体主表实体右侧明细 customerComplaintItem 独立 CRUD（按主表选中 customerComplaintCode 分页） -->
...
<template>
  <div class="customer-complaint-item-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div
      ...
      class="customer-complaint-item-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('complaintId')">
        <TaktSelect
          v-model:value="advancedQueryForm.complaintId"
          api-url="TaktCustomerComplaints/options"
          :placeholder="pi.queryPh('complaintId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('improvementResponsible')">
        <TaktSelect
          v-model:value="advancedQueryForm.improvementResponsible"
          api-url="TaktEmployees/options"
          :placeholder="pi.queryPh('improvementResponsible', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('attachmentPaths')">
        <a-input
          v-model:value="advancedQueryForm.attachmentPaths"
          :placeholder="pi.queryPh('attachmentPaths', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="CUSTOMERCOMPLAINTITEM_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  useCustomerComplaintItemI18n,
  CUSTOMERCOMPLAINTITEM_DEFAULT_VISIBLE_COLUMN_KEYS,
  CUSTOMERCOMPLAINTITEM_SUMMARY_SUM_FIELDS,
  CUSTOMERCOMPLAINTITEM_QUERY_STRING_FIELDS,
  CUSTOMERCOMPLAINTITEM_QUERY_FIELDS,
  CUSTOMERCOMPLAINTITEM_SELF_I18N_KEY,
} from '../composables/use-customer-complaint-item-i18n'
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...CUSTOMERCOMPLAINTITEM_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'customerComplaintItemId'
const masterCustomerComplaintId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['customerComplaintId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterCustomerComplaintId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: CustomerComplaintItem }) =>
      String(getCustomerComplaintItemField(record, 'defectLevel') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: CustomerComplaintItem }) =>
      String(getCustomerComplaintItemField(record, 'defectQuantity') ?? ''),
  },
  {
    title: pi.label('defectRate'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: CustomerComplaintItem }) =>
      String(getCustomerComplaintItemField(record, 'defectRate') ?? ''),
  },
  {
    title: pi.label('causeAnalysis'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: CustomerComplaintItem }) =>
      String(getCustomerComplaintItemField(record, 'causeAnalysis') ?? ''),
  },
  {
    title: pi.label('improvementAction'),
    dataIndex: 'improvementAction',
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: CustomerComplaintItem }) =>
      String(getCustomerComplaintItemField(record, 'plannedCompletionDate') ?? ''),
  },
  {
    title: pi.label('actualCompletionDate'),
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: CustomerComplaintItem }) =>
      String(getCustomerComplaintItemField(record, 'improvementStatus') ?? ''),
  },
  {
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'customerComplaintItemId',
...
const summarySumFieldSet = new Set<string>(CUSTOMERCOMPLAINTITEM_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    CUSTOMERCOMPLAINTITEM_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof CUSTOMERCOMPLAINTITEM_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of CUSTOMERCOMPLAINTITEM_SUMMARY_SUM_FIELDS) {
      const num = Number(getCustomerComplaintItemField(row, field))
...
function buildListQuery(overrides?: Partial<CustomerComplaintItemQuery>): CustomerComplaintItemQuery {
  ...
  const query: CustomerComplaintItemQuery = {
    ...
    customerComplaintCode: masterCustomerComplaintId.value,
    ...overrides,
  }
  if (kw.length > 0) {
    query.keyWords = kw
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importCustomerComplaintItem(file, sheetName)
