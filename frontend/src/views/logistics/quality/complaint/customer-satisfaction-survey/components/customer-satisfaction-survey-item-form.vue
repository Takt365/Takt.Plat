...
/** 映射为 Create/Update DTO（含主表外键 customerSatisfactionSurveyId） */
function getValues(): Record<string, any> {
  const payload = { ...formState }
  if ('sortOrder' in payload) delete payload.sortOrder

  if (props.formData?.customerSatisfactionSurveyItemId) {
    payload.customerSatisfactionSurveyItemId = props.formData.customerSatisfactionSurveyItemId
  }
  payload.customerSatisfactionSurveyId = props.masterId
  // 主表冗余码/名：左侧选中行回填（后端 Stamp 仍按主表 FK 兜底；不限人事）
  const masterRow = props.masterRow as Record<string, unknown> | null | undefined
  if (masterRow) {
    const masterCode = masterRow.customerSatisfactionSurveyCode ?? masterRow.CustomerSatisfactionSurveyCode
    const masterName = masterRow.customerSatisfactionSurveyName ?? masterRow.CustomerSatisfactionSurveyName
    if (masterCode != null && masterCode !== '' && !payload.customerSatisfactionSurveyCode) {
      payload.customerSatisfactionSurveyCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.customerSatisfactionSurveyName) {
      payload.customerSatisfactionSurveyName = masterName
    }
