...
function buildSubmitPayload() {
  ...
  return {
    ...
    items: customerSatisfactionSurveyItemTableRef.value?.getRows?.() ?? childCustomerSatisfactionSurveyItemRows.value.map((rest) => ({
      ...
      customerSatisfactionSurveyId: masterId,
    })),
  }
}
...
function getValues(): Record<string, any> {
  ...
  if (props.formData?.customerSatisfactionSurveyId) {
    payload.customerSatisfactionSurveyId = props.formData.customerSatisfactionSurveyId
  }
  return payload
}
