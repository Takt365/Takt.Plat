...
<!-- 文件名称：index.vue -->
<!-- 功能描述：IQC进货检验单明细实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- 免责声明：此软件使用 MIT License，作者不承担任何使用风险。 -->
<!-- ======================================== -->

<template>
  <div class="logistics-quality-operation-iqc-order-item">
    <!-- 查询栏 -->
    <TaktQueryBar
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.iqcOrderItem._self')"
      :width="600"
      :footer="null"
...
/**
 * IQC进货检验单明细实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
import { getIqcOrderItemList, getIqcOrderItemById, createIqcOrderItem, updateIqcOrderItem, deleteIqcOrderItemById, deleteIqcOrderItemBatch, getIqcOrderItemTemplate, importIqcOrderItem, exportIqcOrderItem } from '@/api/logistics/quality/operation/iqc-order-item'
import type { IqcOrderItem, IqcOrderItemQuery, IqcOrderItemCreate, IqcOrderItemUpdate } from '@/types/logistics/quality/operation/iqc-order-item'
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getIqcOrderItemField(record, 'iqcOrderName') ?? ''
  },
  {
...
const onClickRow = (record: IqcOrderItem) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getIqcOrderItemId(item)))
...
async function loadData() {
  ...
  try {
    ...
    const params: IqcOrderItemQuery = {
    ...
    const res = await getIqcOrderItemList(params)
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.iqcOrderItem._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: IqcOrderItem) {
  formTitle.value = t('common.page.button.edit') + t('entity.iqcOrderItem._self')
...
async function handleExport() {
  try {
    loading.value = true
    const kw = (queryKeyword.value ?? '').trim()
    const exportQuery: IqcOrderItemQuery = {
      pageIndex: 1,
      pageSize: 100000,
      ...advancedQueryForm.value
    }
    ...
    const exportMeta = await exportIqcOrderItem(exportQuery, excelNames.sheet, excelNames.fileBase)
    const ts = new Date()
...
.logistics-quality-operation-iqc-order-item {
  padding: 16px;
  display: flex;
