...
<!-- 功能描述：检验标准实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.inspectionStandard.items')"
                name="items"
              >
                <a-input
                  v-model:value="formState.items"
                  :placeholder="t('common.page.form.placeholder.required', { field: t('entity.inspectionStandard.items') })"
                  size="small"
                  allow-clear
                />
              </a-form-item>
...
/**
 * 检验标准实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","plantCode","standardCode","standardName","inspectionType","materialCategoryCode","materialCategoryName","samplingSchemeCode","samplingSchemeName","isEnabled","standardStatus","standardDescription","items","extFieldJson","remark"]
