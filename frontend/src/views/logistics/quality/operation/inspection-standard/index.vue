...
<!-- 文件名称：index.vue -->
<!-- 功能描述：检验标准实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="logistics-quality-operation-inspection-standard">
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.inspectionStandard._self')"
      :width="600"
      :footer="null"
...
/**
 * 检验标准实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: t('entity.inspectionStandard.items'),
    dataIndex: 'items',
    key: 'items',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getInspectionStandardField(record, 'items') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.inspectionStandard._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: InspectionStandard) {
  formTitle.value = t('common.page.button.edit') + t('entity.inspectionStandard._self')
