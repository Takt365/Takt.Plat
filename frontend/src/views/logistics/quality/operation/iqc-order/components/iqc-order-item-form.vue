...
<!-- 功能描述：IQC进货检验单实体子表 iqcOrderItem 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="pi.label('purchaseQuantity')"
                name="purchaseQuantity"
              >
                <a-input-number
                  v-model:value="formState.purchaseQuantity"
                  :placeholder="pi.ph('purchaseQuantity')"
                  style="width: 100%"
                />
              </a-form-item>
            </a-col>
...
/**
 * IQC进货检验单实体子表 iqcOrderItem 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
const formFields = ["tenantCode","companyCode","cultureCode","plantCode","lineNumber","materialCode","materialDescription","batchCode","purchaseQuantity","standardCode","samplingSchemeCode","inspectionMethod","sampleQuantity","qualifiedQuantity","unqualifiedQuantity","inspectionReturnQuantity","sampleSerialCode","inspectionDescription","inspectorBy","inspectionDate","judgeStatus","isObsolete","defectHandlings"]
...
watch(
  ...
  () => {
    if (!props.formData?.iqcOrderItemId) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  purchaseQuantity: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(pi.ph('purchaseQuantity'))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(pi.ph('purchaseQuantity'))
...
function getValues(): Record<string, any> {
  ...
  if ('purchaseQuantity' in payload) {
    ...
    if (rawpurchaseQuantity === undefined || rawpurchaseQuantity === null || rawpurchaseQuantity === '') {
      delete payload.purchaseQuantity
    } else {
      const numpurchaseQuantity = typeof rawpurchaseQuantity === 'number' ? rawpurchaseQuantity : Number(rawpurchaseQuantity)
      if (Number.isFinite(numpurchaseQuantity)) payload.purchaseQuantity = numpurchaseQuantity
      else delete payload.purchaseQuantity
    }
  }
  ...
  if (props.formData?.iqcOrderItemId) {
    payload.iqcOrderItemId = props.formData.iqcOrderItemId
  }
  ...
  if (masterRow) {
    const masterCode = masterRow.iqcOrderCode ?? masterRow.IqcOrderCode
    const masterName = masterRow.iqcOrderName ?? masterRow.IqcOrderName
    if (masterCode != null && masterCode !== '' && !payload.iqcOrderCode) {
      payload.iqcOrderCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.iqcOrderName) {
      payload.iqcOrderName = masterName
    }
