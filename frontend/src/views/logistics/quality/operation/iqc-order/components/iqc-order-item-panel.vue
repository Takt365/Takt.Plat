...
<!-- 功能描述：IQC进货检验单实体主表实体右侧明细 iqcOrderItem 独立 CRUD（按主表选中 iqcOrderId 分页） -->
...
<template>
  <div class="iqc-order-item-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div
      ...
      class="iqc-order-item-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('iqcOrderCode')">
        <a-input
          v-model:value="advancedQueryForm.iqcOrderCode"
          :placeholder="pi.queryPh('iqcOrderCode', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('purchaseQuantity')">
        <a-input-number
          v-model:value="advancedQueryForm.purchaseQuantity"
          :placeholder="pi.queryPh('purchaseQuantity', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="IQCORDERITEM_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  useIqcOrderItemI18n,
  IQCORDERITEM_DEFAULT_VISIBLE_COLUMN_KEYS,
  IQCORDERITEM_SUMMARY_SUM_FIELDS,
  IQCORDERITEM_QUERY_STRING_FIELDS,
  IQCORDERITEM_QUERY_FIELDS,
  IQCORDERITEM_SELF_I18N_KEY,
} from '../composables/use-iqc-order-item-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useIqcOrderItemI18n()
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of IQCORDERITEM_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(IQCORDERITEM_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  IQCORDERITEM_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...IQCORDERITEM_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...IQCORDERITEM_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'iqcOrderItemId'
const masterIqcOrderId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['iqcOrderId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterIqcOrderId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: IqcOrderItem }) =>
      String(getIqcOrderItemField(record, 'iqcOrderId') ?? ''),
  },
  {
  ...
  {
    title: pi.label('purchaseQuantity'),
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: IqcOrderItem }) =>
      String(getIqcOrderItemField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const summarySumFieldSet = new Set<string>(IQCORDERITEM_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    IQCORDERITEM_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof IQCORDERITEM_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of IQCORDERITEM_SUMMARY_SUM_FIELDS) {
      const num = Number(getIqcOrderItemField(row, field))
