...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="IQCORDER_SELF_I18N_KEY"
        file-type="xlsx"
...
import IqcOrderItemPanel from './components/iqc-order-item-panel.vue'
import { provideIqcOrderMasterContext, type IqcOrderRowRecord } from './composables/use-iqc-order-master-context'
...
import {
  useIqcOrderI18n,
  IQCORDER_LIST_FIELDS,
  IQCORDER_QUERY_STRING_FIELDS,
  IQCORDER_QUERY_FIELDS,
  IQCORDER_SELF_I18N_KEY,
} from './composables/use-iqc-order-i18n'
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<IqcOrderRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<IqcOrderRowRecord[]>([])
/** 表格多选 row-key 集合 */
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of IQCORDER_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(IQCORDER_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
    (typeof IQCORDER_QUERY_STRING_FIELDS)[number],
    string
  >
  return {
    ...form,
...
const queryFieldsMeta = computed(() =>
  IQCORDER_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
/** 高级查询当前可见字段 key */
...
const iqcOrderItemPanelRef = ref<InstanceType<typeof IqcOrderItemPanel> | null>(null)
...
function syncMasterSelection(record: IqcOrderRowRecord | null) {
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as IqcOrderRowRecord
...
async function loadIqcOrderDetail(record: IqcOrderRowRecord): Promise<IqcOrder | null> {
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: pi.label('totalPurchaseQuantity'),
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getIqcOrderField(record, 'remark') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: IqcOrderRowRecord) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        onClick: (record: IqcOrderRowRecord) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getIqcOrderId = (record: IqcOrderRowRecord): string => {
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: IqcOrderRowRecord[]) => {
  ...
  onSelect: (record: IqcOrderRowRecord, selected: boolean) => {
    if (selected) {
      selectedRow.value = record
      syncMasterSelection(record)
    } else if (selectedRow.value && getIqcOrderId(selectedRow.value) === getIqcOrderId(record)) {
      selectedRow.value = null
      syncMasterSelection(null)
    }
  },
  onSelectAll: (selected: boolean, selectedRowsData: IqcOrderRowRecord[]) => {
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: IqcOrderRowRecord) {
...
async function handleFormSubmit() {
  ...
  iqcOrderItemPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importIqcOrder(file, sheetName)
...
/** 删除单行 */
async function handleDeleteOne(record: IqcOrderRowRecord) {
  Modal.confirm({
