...
<template>
  ...
  >
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="fqcOrderChangeLogTableRef"
      v-model="childFqcOrderChangeLogRows"
      :columns="fqcOrderChangeLogFormColumns"
      :title="t('entity.fqcorderchangelog._self')"
      :add-button-entity="t('entity.fqcorderchangelog._self')"
      id-field="fqcOrderChangeLogId"
      :default-row="createDefaultFqcOrderChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const fqcOrderChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 fqcOrderChangeLog 可编辑列 */
const fqcOrderChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'changeFields',
    title: t('entity.fqcorderchangelog.changefields'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.fqcorderchangelog.changefields') }),
  },
  {
    key: 'changeType',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.fqcorderchangelog.changereason') }),
  },
  {
    key: 'changeBy',
    title: t('entity.fqcorderchangelog.changeby'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.fqcorderchangelog.changeby') }),
  },
  {
    key: 'changeTime',
...
function syncChildRowsFromFormData(val: Partial<FqcOrderCreate & { fqcOrderId?: string }> | null | undefined) {
  childFqcOrderChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultFqcOrderChangeLogRow(): Record<string, unknown> {
  return {
    changeFields: '',
    changeType: 0,
    changeReason: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    changeLogs: fqcOrderChangeLogTableRef.value?.getRows?.() ?? childFqcOrderChangeLogRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await fqcOrderChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  fqcOrderChangeLogTableRef.value?.resetRows?.()
