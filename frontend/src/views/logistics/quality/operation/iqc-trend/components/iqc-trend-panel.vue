...
<!-- 命名空间：@/views/logistics/quality/operation/iqc-trend/components -->
<!-- 文件名称：iqc-trend-panel.vue -->
<!-- 功能描述：进货检验推移转置表（工厂×供应商×月份不良率） -->
      ...
      >
        <template #bodyCell="{ column, record, text }">
          <template v-if="String(column.key).startsWith('period_')">
            {{ formatPeriodDefectRate(record as IqcOrderMonthlyTrend, String(column.key)) }}
          </template>
          <template v-else-if="column.key === 'trend'">
            <span :class="trendClass((record as IqcOrderMonthlyTrend).trend || 'none')">
              {{ trendLabel((record as IqcOrderMonthlyTrend).trend || 'none') }}
            </span>
          </template>
          <template v-else-if="column.key === 'varianceAmount'">
            <span :class="varianceClass((record as IqcOrderMonthlyTrend).varianceAmount)">
              {{ formatRateDelta((record as IqcOrderMonthlyTrend).varianceAmount) }}
            </span>
          </template>
          <template v-else-if="column.key === 'variancePercent'">
            <span :class="varianceClass((record as IqcOrderMonthlyTrend).varianceAmount)">
              {{ formatPercent((record as IqcOrderMonthlyTrend).variancePercent) }}
            </span>
          </template>
          <template v-else>
            {{ text }}
...
import {
  exportIqcOrderMonthlyTrendAnalysis,
  getIqcOrderMonthlyTrendAnalysis,
} from '@/api/logistics/quality/operation/iqc-order-trend'
import type { IqcOrderMonthlyTrend } from '@/types/logistics/quality/operation/inspection-trend'
...
const localePrefix = 'logistics.quality.operation.iqc-trend.page'
const { t } = useI18n()

/** 行数据 */
const rows = ref<IqcOrderMonthlyTrend[]>([])
/** 期间列 */
...
const columns = computed<TableColumnsType>(() => {
  const cols: TableColumnsType = [
    ...
    {
      title: t('entity.supplier.suppliername'),
...
/**
 * 行主键
 * @param {IqcOrderMonthlyTrend} record 行
 * @returns {string} key
 */
function getRowKey(record: IqcOrderMonthlyTrend): string {
  return `${record.plantCode}|${record.supplierCode}`
}
 ...
 * @returns {string} 文本
 */
function formatPeriodDefectRate(record: IqcOrderMonthlyTrend, columnKey: string): string {
...
async function loadData() {
  ...
  loading.value = true
  try {
    const result = await getIqcOrderMonthlyTrendAnalysis(query)
...
async function handleExport() {
  ...
  try {
    const exportMeta = await exportIqcOrderMonthlyTrendAnalysis(
    ...
    const fileName = resolveExportDownloadFileName({
      ...
      fallbackBase: `iqc-trend_${query.plantCode}`,
    })
