...
<template>
  ...
  >
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorder.plantcode')"
                name="plantCode"
              >
                <a-input
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="24">
              <a-form-item
                :label="t('entity.ipqcorder.extfield')"
                name="ExtField"
              >
                <a-textarea
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="ipqcOrderChangeLogTableRef"
      v-model="childIpqcOrderChangeLogRows"
      :columns="ipqcOrderChangeLogFormColumns"
      :title="t('entity.ipqcorderchangelog._self')"
      :add-button-entity="t('entity.ipqcorderchangelog._self')"
      id-field="ipqcOrderChangeLogId"
      :default-row="createDefaultIpqcOrderChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const childIpqcOrderChangeLogRows = ref<Record<string, unknown>[]>([])
const ipqcOrderChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 ipqcOrderChangeLog 可编辑列 */
const ipqcOrderChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'changeFields',
    title: t('entity.ipqcorderchangelog.changefields'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.ipqcorderchangelog.changefields') }),
  },
  {
    key: 'changeType',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.ipqcorderchangelog.changereason') }),
  },
  {
    key: 'changeBy',
    title: t('entity.ipqcorderchangelog.changeby'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.ipqcorderchangelog.changeby') }),
  },
  {
    key: 'changeTime',
  ...
  {
    key: 'ExtField',
    title: t('entity.ipqcorderchangelog.extfield'),
    editor: 'textarea',
    rows: 1,
...
function syncChildRowsFromFormData(val: Partial<IpqcOrderCreate & { ipqcOrderId?: string }> | null | undefined) {
  childIpqcOrderChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultIpqcOrderChangeLogRow(): Record<string, unknown> {
  return {
    changeFields: '',
    changeType: 0,
    changeReason: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    changeLogs: ipqcOrderChangeLogTableRef.value?.getRows?.() ?? childIpqcOrderChangeLogRows.value.map((rest) => ({
      ...rest,
...
const rules = computed<Record<string, Rule[]>>(() => ({
  plantCode: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.ipqcorder.plantcode') }),
      trigger: 'blur'
    }
  ],
  sourceCode: [
...
async function validate() {
  ...
  await ipqcOrderChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childIpqcOrderChangeLogRows.value = []
  ipqcOrderChangeLogTableRef.value?.resetRows?.()
