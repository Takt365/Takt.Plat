...
<!-- 功能描述：IPQC制程检验单明细实体维护弹窗内嵌表单（上主下从级联保存）。由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.ipqcorderid')"
                name="ipqcOrderId"
              >
                <a-input
                  v-model:value="formState.ipqcOrderId"
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.ipqcordercode')"
                name="ipqcOrderCode"
              >
                <a-input
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.materialdescription')"
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.batchCode')"
                name="batchCode"
              >
                <a-input
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.samplequantity')"
                name="sampleQuantity"
              >
                <a-input-number
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.qualifiedquantity')"
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.unqualifiedquantity')"
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.inspectionreturnquantity')"
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.sampleserialCode')"
            ...
            <a-col :span="24">
              <a-form-item
                :label="t('entity.ipqcorderitem.inspectiondescription')"
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.ipqcorderitem.inspectorby')"
                name="inspectorBy"
              >
                <a-input
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            <a-col :span="24">
              <a-form-item
                :label="t('entity.ipqcorderitem.inspectiondate')"
                name="inspectionDate"
              >
                <a-date-picker
                  v-model:value="formState.inspectionDate"
                  :placeholder="t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.inspectiondate') })"
            ...
            <a-col :span="24">
              <a-form-item
                :label="t('entity.ipqcorderitem.judgestatus')"
                name="judgeStatus"
              >
                <a-input-number
    ...
    <TaktEditableTable
      ref="ipqcDefectHandlingTableRef"
      v-model="childIpqcDefectHandlingRows"
      :columns="ipqcDefectHandlingFormColumns"
      :title="t('entity.ipqcdefecthandling._self')"
      :add-button-entity="t('entity.ipqcdefecthandling._self')"
      id-field="ipqcDefectHandlingId"
      :default-row="createDefaultIpqcDefectHandlingRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
/**
 * IPQC制程检验单明细实体维护表单 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","cultureCode","ipqcOrderId","ipqcOrderCode","lineNumber","materialCode","materialDescription","batchCode","productionQuantity","standardCode","samplingSchemeCode","inspectionMethod","sampleQuantity","qualifiedQuantity","unqualifiedQuantity","inspectionReturnQuantity","sampleSerialCode","inspectionDescription","inspectorBy","inspectionDate","judgeStatus","extField","remark"]

import type { TaktEditableTableColumn } from '@/components/business/takt-editable-table/types'

const childIpqcDefectHandlingRows = ref<Record<string, unknown>[]>([])
const ipqcDefectHandlingTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 ipqcDefectHandling 可编辑列 */
const ipqcDefectHandlingFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
...
function syncChildRowsFromFormData(val: Partial<IpqcOrderItemCreate & { ipqcOrderItemId?: string }> | null | undefined) {
  childIpqcDefectHandlingRows.value = ((val as any)?.defectHandlings ?? []) as Record<string, unknown>[]
}

function createDefaultIpqcDefectHandlingRow(): Record<string, unknown> {
  return {
    ipqcDefectHandlingCode: '',
    ipqcOrderCode: '',
    lineNumber: (childIpqcDefectHandlingRows.value.length + 1) * 10,
    defectType: 0,
    defectCode: '',
    defectDescription: '',
    defectQuantity: 0,
...
function buildSubmitPayload() {
  const masterId = props.formData?.ipqcOrderItemId ?? ''
  return {
    ...formState,
    defectHandlings: ipqcDefectHandlingTableRef.value?.getRows?.() ?? childIpqcDefectHandlingRows.value.map((rest) => ({
      ...rest,
      tenantCode: tenantStore.tenantCode,
      companyCode: tenantStore.companyCode,
      cultureCode: userStore.userInfo?.companyDefaultCulture ?? userStore.userInfo?.cultureCode ?? '',
      ipqcOrderItemId: masterId,
    })),
  }
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.ipqcOrderItemId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ipqcOrderId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.ipqcorderitem.ipqcorderid') }),
      trigger: 'blur'
    }
  ],
  ipqcOrderCode: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.ipqcorderitem.ipqcordercode') }),
      trigger: 'blur'
    }
  ],
  lineNumber: [{
  ...
  materialDescription: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.ipqcorderitem.materialdescription') }),
      trigger: 'blur'
    }
  ],
  productionQuantity: [{
  ...
  sampleQuantity: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.samplequantity') }))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.samplequantity') }))
  ...
  qualifiedQuantity: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.qualifiedquantity') }))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.qualifiedquantity') }))
  ...
  unqualifiedQuantity: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.unqualifiedquantity') }))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.unqualifiedquantity') }))
  ...
  inspectionReturnQuantity: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.inspectionreturnquantity') }))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.inspectionreturnquantity') }))
  ...
  inspectorBy: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.ipqcorderitem.inspectorby') }),
      trigger: 'blur'
    }
  ],
  inspectionDate: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.inspectiondate') }),
      trigger: 'change'
    }
  ],
  judgeStatus: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.judgestatus') }))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(t('common.page.form.placeholder.select', { field: t('entity.ipqcorderitem.judgestatus') }))
...
async function validate() {
  ...
  await ipqcDefectHandlingTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childIpqcDefectHandlingRows.value = []
  ipqcDefectHandlingTableRef.value?.resetRows?.()
