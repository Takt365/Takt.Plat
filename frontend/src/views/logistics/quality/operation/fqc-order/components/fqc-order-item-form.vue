...
function getValues(): Record<string, any> {
  ...
  if (props.formData?.fqcOrderItemId) {
    payload.fqcOrderItemId = props.formData.fqcOrderItemId
  }
  ...
  if (masterRow) {
    const masterCode = masterRow.fqcOrderCode ?? masterRow.FqcOrderCode
    const masterName = masterRow.fqcOrderName ?? masterRow.FqcOrderName
    if (masterCode != null && masterCode !== '' && !payload.fqcOrderCode) {
      payload.fqcOrderCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.fqcOrderName) {
      payload.fqcOrderName = masterName
    }
