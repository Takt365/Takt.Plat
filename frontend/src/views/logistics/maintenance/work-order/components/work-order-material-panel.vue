...
<template>
  <div class="work-order-material-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div
      ...
      class="work-order-material-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('materialId')">
        <a-input
          v-model:value="advancedQueryForm.materialId"
          :placeholder="pi.queryPh('materialId', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('requiredQuantity')">
        <a-input-number
          v-model:value="advancedQueryForm.requiredQuantity"
          :placeholder="pi.queryPh('requiredQuantity', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('issuedQuantity')">
      <a-form-item :label="pi.queryLabel('issuedQuantity')">
        <a-input-number
          v-model:value="advancedQueryForm.issuedQuantity"
          :placeholder="pi.queryPh('issuedQuantity', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('materialUnit')">
        <a-input
          ...
          :placeholder="pi.queryPh('materialUnit', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('warehouseCode')">
        <a-input
          ...
          :placeholder="pi.queryPh('warehouseCode', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('issueStatus')">
        <a-input-number
          v-model:value="advancedQueryForm.issueStatus"
          :placeholder="pi.queryPh('issueStatus', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('issueTimeStart')">
      <a-form-item :label="pi.queryLabel('issueTimeStart')">
        <a-date-picker
          v-model:value="advancedQueryForm.issueTimeStart"
          :placeholder="pi.queryPh('issueTimeStart', 'select')"
      ...
      <a-form-item :label="pi.queryLabel('issueTimeEnd')">
        <a-date-picker
          v-model:value="advancedQueryForm.issueTimeEnd"
          :placeholder="pi.queryPh('issueTimeEnd', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="MAINTENANCEWORKORDERMATERIAL_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  useMaintenanceWorkOrderMaterialI18n,
  MAINTENANCEWORKORDERMATERIAL_DEFAULT_VISIBLE_COLUMN_KEYS,
  MAINTENANCEWORKORDERMATERIAL_SUMMARY_SUM_FIELDS,
  MAINTENANCEWORKORDERMATERIAL_QUERY_STRING_FIELDS,
  MAINTENANCEWORKORDERMATERIAL_QUERY_FIELDS,
  MAINTENANCEWORKORDERMATERIAL_SELF_I18N_KEY,
} from '../composables/use-work-order-material-i18n'
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(MAINTENANCEWORKORDERMATERIAL_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  MAINTENANCEWORKORDERMATERIAL_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...MAINTENANCEWORKORDERMATERIAL_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...MAINTENANCEWORKORDERMATERIAL_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'maintenanceWorkOrderMaterialId'
const masterMaintenanceWorkOrderId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['maintenanceWorkOrderId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterMaintenanceWorkOrderId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: pi.label('maintenanceWorkOrderId'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: MaintenanceWorkOrderMaterial }) =>
      String(getMaintenanceWorkOrderMaterialField(record, 'maintenanceWorkOrderId') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: MaintenanceWorkOrderMaterial }) =>
      String(getMaintenanceWorkOrderMaterialField(record, 'unitPrice') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: MaintenanceWorkOrderMaterial }) =>
      String(getMaintenanceWorkOrderMaterialField(record, 'amount') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: MaintenanceWorkOrderMaterial }) =>
      String(getMaintenanceWorkOrderMaterialField(record, 'warehouseCode') ?? ''),
  },
  {
    title: pi.label('storageLocation'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: MaintenanceWorkOrderMaterial }) =>
      String(getMaintenanceWorkOrderMaterialField(record, 'storageLocation') ?? ''),
  },
  {
    title: pi.label('issueStatus'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: MaintenanceWorkOrderMaterial }) =>
      String(getMaintenanceWorkOrderMaterialField(record, 'issueStatus') ?? ''),
  },
  {
    title: pi.label('issueTime'),
    dataIndex: 'issueTime',
    key: 'issueTime',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: MaintenanceWorkOrderMaterial }) =>
      String(getMaintenanceWorkOrderMaterialField(record, 'issueTime') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: MaintenanceWorkOrderMaterial }) =>
      String(getMaintenanceWorkOrderMaterialField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'maintenanceWorkOrderMaterialId',
...
const summarySumFieldSet = new Set<string>(MAINTENANCEWORKORDERMATERIAL_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    MAINTENANCEWORKORDERMATERIAL_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof MAINTENANCEWORKORDERMATERIAL_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of MAINTENANCEWORKORDERMATERIAL_SUMMARY_SUM_FIELDS) {
      const num = Number(getMaintenanceWorkOrderMaterialField(row, field))
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importMaintenanceWorkOrderMaterial(file, sheetName)
