...
<template>
  ...
  >
    ...
    <!-- 下：子表 maintenanceNotifications -->
    <TaktEditableTable
      ref="maintenanceNotificationTableRef"
      v-model="childMaintenanceNotificationRows"
      :columns="maintenanceNotificationFormColumns"
      :title="t('entity.maintenancenotification._self')"
      :add-button-entity="t('entity.maintenancenotification._self')"
      id-field="maintenanceNotificationId"
      :default-row="createDefaultMaintenanceNotificationRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const childMaintenanceNotificationRows = ref<Record<string, unknown>[]>([])
const maintenanceNotificationTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 maintenanceNotification 可编辑列 */
const maintenanceNotificationFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'plantCode',
  ...
  },
  {
    ...
    editor: 'textarea',
    rows: 1,
    placeholder: t('common.page.form.placeholder.required', { field: t('entity.maintenancenotification.faultdescription') }),
    width: 140,
  },
  {
    key: 'discoveredAt',
...
function syncChildRowsFromFormData(val: Partial<EquipmentCreate & { equipmentId?: string }> | null | undefined) {
  childMaintenanceNotificationRows.value = ((val as any)?.maintenanceNotifications ?? []) as Record<string, unknown>[]
}

function createDefaultMaintenanceNotificationRow(): Record<string, unknown> {
  return {
    plantCode: '',
    notificationCode: '',
    equipmentName: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    maintenanceNotifications: maintenanceNotificationTableRef.value?.getRows?.() ?? childMaintenanceNotificationRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).maintenanceNotifications
...
async function validate() {
  ...
  await maintenanceNotificationTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childMaintenanceNotificationRows.value = []
  maintenanceNotificationTableRef.value?.resetRows?.()
