...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'taxCode'">
          <TaktDictTag
            :value="getPurchaseRequestDictValue(record, 'taxCode')"
    ...
    >
      ...
      <div v-show="isFieldVisible('requestEmployeeId')">
      <a-form-item :label="pi.queryLabel('requestEmployeeId')">
        <TaktSelect
          v-model:value="advancedQueryForm.requestEmployeeId"
          api-url="TaktEmployees/options"
          :placeholder="pi.queryPh('requestEmployeeId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('requestEmployeeName')">
      <a-form-item :label="pi.queryLabel('requestEmployeeName')">
        <a-input
          v-model:value="advancedQueryForm.requestEmployeeName"
          :placeholder="pi.queryPh('requestEmployeeName', 'required')"
          show-count
          :maxlength="80"
          allow-clear
        />
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: pi.label('requestEmployeeId'),
    dataIndex: 'requestEmployeeId',
    key: 'requestEmployeeId',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchaseRequestField(record, 'requestEmployeeId') ?? ''
  },
  {
    title: pi.label('requestEmployeeName'),
    dataIndex: 'requestEmployeeName',
    key: 'requestEmployeeName',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchaseRequestField(record, 'requestEmployeeName') ?? ''
  },
  {
