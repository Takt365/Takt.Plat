...
<template>
  <div class="purchase-request-item-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div
      ...
      class="purchase-request-item-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      ...
      <div v-show="isFieldVisible('purchasePlanItemId')">
      <a-form-item :label="pi.queryLabel('purchasePlanItemId')">
        <a-input
          v-model:value="advancedQueryForm.purchasePlanItemId"
          :placeholder="pi.queryPh('purchasePlanItemId', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <div v-show="isFieldVisible('purchaseRequestUnitPrice')">
      <a-form-item :label="pi.queryLabel('purchaseRequestUnitPrice')">
        <a-input-number
          v-model:value="advancedQueryForm.purchaseRequestUnitPrice"
          :placeholder="pi.queryPh('purchaseRequestUnitPrice', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      ...
      <div v-show="isFieldVisible('requestAmount')">
      <a-form-item :label="pi.queryLabel('requestAmount')">
        <a-input-number
          v-model:value="advancedQueryForm.requestAmount"
          :placeholder="pi.queryPh('requestAmount', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="PURCHASEREQUESTITEM_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  usePurchaseRequestItemI18n,
  PURCHASEREQUESTITEM_DEFAULT_VISIBLE_COLUMN_KEYS,
  PURCHASEREQUESTITEM_SUMMARY_SUM_FIELDS,
  PURCHASEREQUESTITEM_QUERY_STRING_FIELDS,
  PURCHASEREQUESTITEM_QUERY_FIELDS,
  PURCHASEREQUESTITEM_SELF_I18N_KEY,
} from '../composables/use-purchase-request-item-i18n'
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of PURCHASEREQUESTITEM_QUERY_STRING_FIELDS) {
  ...
  if (form.purchaseRequestUnitPrice !== undefined && form.purchaseRequestUnitPrice !== null) {
    return true
  }
  ...
  if (form.requestAmount !== undefined && form.requestAmount !== null) {
    return true
  }
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(PURCHASEREQUESTITEM_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
  ...
  return {
    ...
    purchaseRequestUnitPrice: undefined as number | undefined,
...
const queryFieldsMeta = computed(() =>
  PURCHASEREQUESTITEM_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...PURCHASEREQUESTITEM_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...PURCHASEREQUESTITEM_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'purchaseRequestItemId'
const masterPurchaseRequestId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['purchaseRequestId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterPurchaseRequestId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'purchaseRequestId') ?? ''),
  },
  {
  ...
  {
    title: pi.label('purchasePlanItemId'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'purchasePlanItemId') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'materialDescription') ?? ''),
  },
  {
  ...
  {
    title: pi.label('requestQuantity'),
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'convertedQuantity') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'purchasePerUnit') ?? ''),
  },
  {
    title: pi.label('purchaseRequestUnitPrice'),
    dataIndex: 'purchaseRequestUnitPrice',
    key: 'purchaseRequestUnitPrice',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'purchaseRequestUnitPrice') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'taxCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'taxIncludedAmount') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'untaxedAmount') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'taxAmount') ?? ''),
  },
  {
    title: pi.label('requestAmount'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'requestAmount') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'pricingDate') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'grossWeight') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'netWeight') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'weightUnit') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'volume') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'volumeUnit') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'profitCenterCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchaseRequestItem }) =>
      String(getPurchaseRequestItemField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'purchaseRequestItemId',
...
const summarySumFieldSet = new Set<string>(PURCHASEREQUESTITEM_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    PURCHASEREQUESTITEM_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof PURCHASEREQUESTITEM_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of PURCHASEREQUESTITEM_SUMMARY_SUM_FIELDS) {
      const num = Number(getPurchaseRequestItemField(row, field))
...
function buildListQuery(overrides?: Partial<PurchaseRequestItemQuery>): PurchaseRequestItemQuery {
  ...
  if (form.purchaseRequestUnitPrice !== undefined && form.purchaseRequestUnitPrice !== null) {
    query.purchaseRequestUnitPrice = form.purchaseRequestUnitPrice
  }
  if (form.taxIncludedAmount !== undefined && form.taxIncludedAmount !== null) {
  ...
  if (form.requestAmount !== undefined && form.requestAmount !== null) {
    query.requestAmount = form.requestAmount
  }
  if (form.grossWeight !== undefined && form.grossWeight !== null) {
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importPurchaseRequestItem(file, sheetName)
