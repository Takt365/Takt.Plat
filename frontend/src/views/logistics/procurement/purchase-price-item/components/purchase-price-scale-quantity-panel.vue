...
<!-- 文件名称：purchase-price-scale-quantity-panel.vue -->
<!-- 功能描述：Takt采购价格明细实体主表实体右侧明细 purchasePriceScaleQuantity 独立 CRUD（按主表选中 purchasePriceItemId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="purchase-price-scale-quantity-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="purchase-price-scale-quantity-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
        ...
        :stripe="true"
        :virtual="true"
        :row-key="getPurchasePriceScaleQuantityId"
        :row-selection="rowSelection"
        :custom-row="onClickRow"
        :visible-column-keys="visibleColumnKeys"
        id-column-key="purchasePriceScaleQuantityId"
    ...
    >
      <PurchasePriceScaleQuantityForm
        ref="formRef"
        :form-data="formData"
        :master-id="masterPurchasePriceItemId"
        :loading="formLoading"
      />
    </TaktModal>
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-logistics-procurement-purchase-price-item-purchase-price-scale-quantity"
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="PURCHASEPRICESCALEQUANTITY_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * Takt采购价格明细实体子表 purchasePriceScaleQuantity 右栏面板
...
import PurchasePriceScaleQuantityForm from './purchase-price-scale-quantity-form.vue'
import { usePurchasePriceItemMasterContext } from '../composables/use-purchase-price-item-master-context'
import {
  getPurchasePriceScaleQuantityList,
  getPurchasePriceScaleQuantityById,
  createPurchasePriceScaleQuantity,
  updatePurchasePriceScaleQuantity,
  deletePurchasePriceScaleQuantityById,
  deletePurchasePriceScaleQuantityBatch,
  getPurchasePriceScaleQuantityTemplate,
  importPurchasePriceScaleQuantity,
  exportPurchasePriceScaleQuantity,
} from '@/api/logistics/procurement/purchase-price-scale-quantity'
import type { PurchasePriceScaleQuantity, PurchasePriceScaleQuantityQuery } from '@/types/logistics/procurement/purchase-price-scale-quantity'

import {
  usePurchasePriceScaleQuantityI18n,
  PURCHASEPRICESCALEQUANTITY_DEFAULT_VISIBLE_COLUMN_KEYS,
  PURCHASEPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS,
  PURCHASEPRICESCALEQUANTITY_QUERY_STRING_FIELDS,
  PURCHASEPRICESCALEQUANTITY_QUERY_FIELDS,
  PURCHASEPRICESCALEQUANTITY_SELF_I18N_KEY,
} from '../composables/use-purchase-price-scale-quantity-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = usePurchasePriceScaleQuantityI18n()

const { t } = useI18n()
const { selectedMasterRow } = usePurchasePriceItemMasterContext()

/** Excel 导入/导出默认 sheet 名与文件名前缀 */
const excelNames = taktExcelEntityNames('TaktPurchasePriceScaleQuantity')
/** 快捷查询占位文案 */
...
const dataSource = ref<PurchasePriceScaleQuantity[]>([])
const currentPage = ref(getTaktDefaultPageIndex())
const pageSize = ref(getTaktDefaultPageSize())
const total = ref(0)
const queryKeyword = ref('')
const selectedRow = ref<PurchasePriceScaleQuantity | null>(null)
const selectedRows = ref<PurchasePriceScaleQuantity[]>([])
const selectedRowKeys = ref<(string | number)[]>([])
const formVisible = ref(false)
const formTitle = ref('')
const formData = ref<Partial<PurchasePriceScaleQuantity>>({})
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of PURCHASEPRICESCALEQUANTITY_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(PURCHASEPRICESCALEQUANTITY_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  PURCHASEPRICESCALEQUANTITY_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...PURCHASEPRICESCALEQUANTITY_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...PURCHASEPRICESCALEQUANTITY_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'purchasePriceScaleQuantityId'
const masterPurchasePriceItemId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['purchasePriceItemId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterPurchasePriceItemId.value !== '')
const updateDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length !== 1)
const deleteDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length === 0)

function getPurchasePriceScaleQuantityId(record: PurchasePriceScaleQuantity | Record<string, unknown>): string {
  return String((record as PurchasePriceScaleQuantity)?.[entityIdName] ?? '')
}

function getPurchasePriceScaleQuantityField(record: PurchasePriceScaleQuantity | Record<string, unknown>, field: string): unknown {
  return (record as PurchasePriceScaleQuantity)?.[field as keyof PurchasePriceScaleQuantity]
}

const columns = computed<TableColumnsType>(() => [
  {
    title: t('common.page.entity.id'),
    dataIndex: 'purchasePriceScaleQuantityId',
    key: 'purchasePriceScaleQuantityId',
    width: 80,
    resizable: true,
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: PurchasePriceScaleQuantity }) =>
      String(getPurchasePriceScaleQuantityField(record, 'purchasePriceScaleQuantityId') ?? ''),
  },
  {
    title: pi.label('purchasePriceItemId'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchasePriceScaleQuantity }) =>
      String(getPurchasePriceScaleQuantityField(record, 'purchasePriceItemId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchasePriceScaleQuantity }) =>
      String(getPurchasePriceScaleQuantityField(record, 'purchasePriceCode') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchasePriceScaleQuantity }) =>
      String(getPurchasePriceScaleQuantityField(record, 'taxAmount') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: PurchasePriceScaleQuantity }) =>
      String(getPurchasePriceScaleQuantityField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: PurchasePriceScaleQuantity) => void handleEdit(record),
      },
      {
        key: 'delete',
        ...
        onClick: (record: PurchasePriceScaleQuantity) => void handleDeleteOne(record),
      }],
  })])
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'purchasePriceScaleQuantityId',
...
const summarySumFieldSet = new Set<string>(PURCHASEPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    PURCHASEPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof PURCHASEPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of PURCHASEPRICESCALEQUANTITY_SUMMARY_SUM_FIELDS) {
      const num = Number(getPurchasePriceScaleQuantityField(row, field))
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: PurchasePriceScaleQuantity[]) => {
    selectedRowKeys.value = keys
    selectedRows.value = rows
    selectedRow.value = rows.length === 1 ? (rows[0] ?? null) : null
  },
  onSelect: (record: PurchasePriceScaleQuantity, selected: boolean) => {
    if (selected) {
      selectedRow.value = record
    } else if (selectedRow.value && getPurchasePriceScaleQuantityId(selectedRow.value) === getPurchasePriceScaleQuantityId(record)) {
      selectedRow.value = null
    }
  },
  onSelectAll: (selected: boolean, selectedRowsData: PurchasePriceScaleQuantity[]) => {
 ...
 * @param record 行数据
 */
function onClickRow(record: PurchasePriceScaleQuantity) {
  const key = getPurchasePriceScaleQuantityId(record)
  return {
    onClick: () => {
 ...
 * @returns {PurchasePriceScaleQuantityQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<PurchasePriceScaleQuantityQuery>): PurchasePriceScaleQuantityQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: PurchasePriceScaleQuantityQuery = {
    pageIndex: currentPage.value,
    pageSize: pageSize.value,
    purchasePriceItemId: masterPurchasePriceItemId.value,
    ...overrides,
  }
  if (kw.length > 0) {
    query.keyWords = kw
  }
  const assignTrimmed = (key: keyof PurchasePriceScaleQuantityQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
async function loadData() {
  ...
  loading.value = true
  try {
    const res = await getPurchasePriceScaleQuantityList(buildListQuery())
...
/** 主表选中变更时自动加载子表 */
watch(masterPurchasePriceItemId, () => {
  reload()
})
...
async function handleEdit(record: PurchasePriceScaleQuantity) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getPurchasePriceScaleQuantityById(getPurchasePriceScaleQuantityId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.purchasePriceScaleQuantityId
    if (id) {
      await updatePurchasePriceScaleQuantity(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createPurchasePriceScaleQuantity(payload)
...
async function handleDeleteOne(record: PurchasePriceScaleQuantity) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deletePurchasePriceScaleQuantityById(getPurchasePriceScaleQuantityId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getPurchasePriceScaleQuantityId(r)).filter(Boolean)
      await deletePurchasePriceScaleQuantityBatch(ids)
      message.success(t('common.feedback.deleted', { target: pi.self() }))
      await loadData()
    },
  })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getPurchasePriceScaleQuantityTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importPurchasePriceScaleQuantity(file, sheetName)
...
async function handleExport() {
  ...
  try {
    ...
    const exportMeta = await exportPurchasePriceScaleQuantity(
