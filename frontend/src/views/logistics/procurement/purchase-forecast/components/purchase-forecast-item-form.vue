...
<!-- 文件名称：purchase-forecast-item-form.vue -->
<!-- 功能描述：Takt采购预测实体子表 purchaseForecastItem 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form purchase-forecast-item-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    <a-tabs
      ...
      class="purchase-forecast-item-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * Takt采购预测实体子表 purchaseForecastItem 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
import type { PurchaseForecastItemCreate } from '@/types/logistics/procurement/purchase-forecast-item'
...
interface Props {
  formData?: Partial<PurchaseForecastItemCreate & { purchaseForecastItemId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 purchaseForecastItemId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.purchaseForecastItemId) {
...
/** 映射为 Create/Update DTO（含主表外键 purchaseForecastId） */
function getValues(): Record<string, any> {
  ...
  payload.purchaseForecastId = props.masterId
  return payload
}
