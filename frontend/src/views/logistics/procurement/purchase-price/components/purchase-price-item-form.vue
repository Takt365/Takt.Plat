...
<!-- 功能描述：Takt采购价格实体子表 purchasePriceItem 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form purchase-price-item-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
...
function getValues(): Record<string, any> {
  ...
  if (masterRow) {
    const masterCode = masterRow.purchasePriceCode ?? masterRow.PurchasePriceCode
    const masterName = masterRow.purchasePriceName ?? masterRow.PurchasePriceName
    if (masterCode != null && masterCode !== '' && !payload.purchasePriceCode) {
      payload.purchasePriceCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.purchasePriceName) {
      payload.purchasePriceName = masterName
    }
