...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="PURCHASEPRICE_SELF_I18N_KEY"
        file-type="xlsx"
...
import { providePurchasePriceMasterContext, type PurchasePriceRowRecord } from './composables/use-purchase-price-master-context'
import { getPurchasePriceList, getPurchasePriceById, createPurchasePrice, updatePurchasePrice, deletePurchasePriceById, deletePurchasePriceBatch, getPurchasePriceTemplate, importPurchasePrice, exportPurchasePrice } from '@/api/logistics/procurement/purchase-price'
...
import {
  usePurchasePriceI18n,
  PURCHASEPRICE_LIST_FIELDS,
  PURCHASEPRICE_QUERY_STRING_FIELDS,
  PURCHASEPRICE_QUERY_FIELDS,
  PURCHASEPRICE_SELF_I18N_KEY,
} from './composables/use-purchase-price-i18n'
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<PurchasePriceRowRecord | null>(null)
/** 表格多选行 */
const selectedRows = ref<PurchasePriceRowRecord[]>([])
/** 表格多选 row-key 集合 */
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(PURCHASEPRICE_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  PURCHASEPRICE_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
/** 高级查询当前可见字段 key */
...
function syncMasterSelection(record: PurchasePriceRowRecord | null) {
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as PurchasePriceRowRecord
...
async function loadPurchasePriceDetail(record: PurchasePriceRowRecord): Promise<PurchasePrice | null> {
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceField(record, 'validFrom') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceField(record, 'remark') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: PurchasePriceRowRecord) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        onClick: (record: PurchasePriceRowRecord) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getPurchasePriceId = (record: PurchasePriceRowRecord): string => {
 ...
 * @param field 字段名
 */
const getPurchasePriceDictValue = (
  record: PurchasePriceRowRecord,
  field: string,
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: PurchasePriceRowRecord[]) => {
  ...
  onSelect: (record: PurchasePriceRowRecord, selected: boolean) => {
    if (selected) {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: PurchasePriceRowRecord[]) => {
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: PurchasePriceRowRecord) {
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importPurchasePrice(file, sheetName)
...
/** 删除单行 */
async function handleDeleteOne(record: PurchasePriceRowRecord) {
  Modal.confirm({
