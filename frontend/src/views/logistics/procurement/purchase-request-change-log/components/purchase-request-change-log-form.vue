...
<!-- 功能描述：Takt采购申请实体子表 purchaseRequestChangeLog 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form purchase-request-change-log-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
...
/**
 * Takt采购申请实体子表 purchaseRequestChangeLog 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 purchaseRequestChangeLogId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.purchaseRequestChangeLogId) {
