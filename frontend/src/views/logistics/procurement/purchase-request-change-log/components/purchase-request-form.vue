...
<template>
  ...
  >
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="purchaseRequestChangeLogTableRef"
      v-model="childPurchaseRequestChangeLogRows"
      :columns="purchaseRequestChangeLogFormColumns"
      :title="t('entity.purchaserequestchangelog._self')"
      :add-button-entity="t('entity.purchaserequestchangelog._self')"
      id-field="purchaseRequestChangeLogId"
      :default-row="createDefaultPurchaseRequestChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const childPurchaseRequestChangeLogRows = ref<Record<string, unknown>[]>([])
const purchaseRequestChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 purchaseRequestChangeLog 可编辑列 */
const purchaseRequestChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'requestCode',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.purchaserequestchangelog.changefields') }),
  },
  {
    key: 'changeTime',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.purchaserequestchangelog.changeby') }),
  },
  {
    key: 'changeReason',
    title: t('entity.purchaserequestchangelog.changereason'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.purchaserequestchangelog.changereason') }),
  },
  {
    key: 'extField',
...
function syncChildRowsFromFormData(val: Partial<PurchaseRequestCreate & { purchaseRequestId?: string }> | null | undefined) {
  childPurchaseRequestChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultPurchaseRequestChangeLogRow(): Record<string, unknown> {
  return {
    requestCode: '',
    changeFields: '',
    changeTime: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    changeLogs: purchaseRequestChangeLogTableRef.value?.getRows?.() ?? childPurchaseRequestChangeLogRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await purchaseRequestChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childPurchaseRequestChangeLogRows.value = []
  purchaseRequestChangeLogTableRef.value?.resetRows?.()
