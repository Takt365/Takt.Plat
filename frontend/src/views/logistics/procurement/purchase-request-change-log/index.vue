...
<!-- 命名空间：@/views/logistics/procurement/purchase-request-change-log -->
<!-- 文件名称：index.vue -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:procurement:purchase:request:change:log:create"
      update-permission="logistics:procurement:purchase:request:change:log:update"
      delete-permission="logistics:procurement:purchase:request:change:log:delete"
      import-permission="logistics:procurement:purchase:request:change:log:import"
      export-permission="logistics:procurement:purchase:request:change:log:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'requestStatus'">
          <a-switch
            :checked="getPurchaseRequestField(record, 'requestStatus') === 1"
      ...
      <template #detail>
        <PurchaseRequestChangeLogPanel
          ref="purchaseRequestChangeLogPanelRef"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-procurement-purchase-request-change-log'"
 ...
 * @module views/logistics/procurement/purchase-request-change-log
 */
...
import PurchaseRequestChangeLogPanel from './components/purchase-request-change-log-panel.vue'
import { providePurchaseRequestMasterContext } from './composables/use-purchase-request-master-context'
...
const queryFieldsMeta = computed(() => [
  ...
  { key: 'requiredArrivalDateEnd', label: t('common.page.entity.createdatend').replace(t('common.page.entity.createdat'), t('entity.purchaserequest.requiredarrivaldate')) },
...
const purchaseRequestChangeLogPanelRef = ref<InstanceType<typeof PurchaseRequestChangeLogPanel> | null>(null)
...
function buildListQuery(overrides?: Partial<PurchaseRequestQuery>): PurchaseRequestQuery {
  ...
  assignTrimmed('requestId', form.requestId)
  assignTrimmed('requestBy', form.requestBy)
...
function syncMasterSelection(record: PurchaseRequest | null) {
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as PurchaseRequest
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:procurement:purchase:request:change:log:update',
      ...
      },
      {
        ...
        permission: 'logistics:procurement:purchase:request:change:log:delete',
...
async function handleFormSubmit() {
  ...
  purchaseRequestChangeLogPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
 ...
 * @param checked 是否启用
 */
async function handleRequestStatusChange(record: PurchaseRequest, checked: boolean) {
  const newVal = checked ? 1 : 0
  const oldVal = getPurchaseRequestField(record, 'requestStatus')
  const id = getPurchaseRequestId(record)
  const row = dataSource.value.find((item) => getPurchaseRequestId(item) === id)
  if (row) {
    row.requestStatus = newVal
  }
  try {
    await updatePurchaseRequestStatus({ purchaseRequestId: id, requestStatus: newVal })
    message.success(t('common.feedback.updated'))
    
  } catch (error: unknown) {
    if (row) {
      row.requestStatus = oldVal
    }
