...
<!-- 命名空间：@/views/logistics/procurement/purchase-group -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt采购组主数据实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    >
      <PurchaseGroupForm
        :key="formData?.purchaseGroupId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-procurement-purchase-group'"
...
/**
 * Takt采购组主数据实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/procurement/purchase-group
 */
...
/** 分页列表数据 */
const dataSource = ref<PurchaseGroup[]>([])
/** 当前页码 */
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<PurchaseGroup> | null>(null)
/** 表单提交 loading */
...
const entityIdName = 'purchaseGroupId'
/** 工具栏「编辑」是否禁用（须恰好选中一行） */
 ...
 */
function buildListQuery(overrides?: Partial<PurchaseGroupQuery>): PurchaseGroupQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: PurchaseGroupQuery = {
  ...
  const assignTrimmed = (key: keyof PurchaseGroupQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
const onClickRow = (record: PurchaseGroupRowRecord) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getPurchaseGroupId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[PurchaseGroup] 加载数据失败', { error })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getPurchaseGroupTemplate(sheetName, fileName)
