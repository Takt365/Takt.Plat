...
<!-- 命名空间：@/views/logistics/procurement/source-of-supply -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt货源清单实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <TaktSingleTable
      ...
      :id-column-key="'sourceOfSupplyId'"
      table-mode="single"
      :data-source="dataSource"
      :loading="loading"
      :stripe="true"
      :row-key="getSourceOfSupplyId"
    ...
    >
      <SourceOfSupplyForm
        :key="formData?.sourceOfSupplyId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-procurement-source-of-supply'"
...
/**
 * Takt货源清单实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/procurement/source-of-supply
 */
...
import SourceOfSupplyForm from './components/source-of-supply-form.vue'
import { getSourceOfSupplyList, getSourceOfSupplyById, createSourceOfSupply, updateSourceOfSupply, deleteSourceOfSupplyById, deleteSourceOfSupplyBatch, getSourceOfSupplyTemplate, importSourceOfSupply, exportSourceOfSupply, updateSourceOfSupplyStatus } from '@/api/logistics/procurement/source-of-supply'
import type { SourceOfSupply, SourceOfSupplyQuery } from '@/types/logistics/procurement/source-of-supply'
...
/** 分页列表数据 */
const dataSource = ref<SourceOfSupply[]>([])
/** 当前页码 */
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<SourceOfSupply> | null>(null)
/** 表单提交 loading */
...
const entityIdName = 'sourceOfSupplyId'
/** 工具栏「编辑」是否禁用（须恰好选中一行） */
 ...
 * @returns {SourceOfSupplyQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<SourceOfSupplyQuery>): SourceOfSupplyQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: SourceOfSupplyQuery = {
  ...
  const assignTrimmed = (key: keyof SourceOfSupplyQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
const onClickRow = (record: SourceOfSupplyRowRecord) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getSourceOfSupplyId(item)))
...
async function loadData() {
  loading.value = true
  try {
    const res = await getSourceOfSupplyList(buildListQuery())
    dataSource.value = res.data ?? []
    total.value = res.total ?? 0
  } catch (error: any) {
    logger.error('[SourceOfSupply] 加载数据失败', { error })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getSourceOfSupplyTemplate(sheetName, fileName)
...
async function handleExport() {
  try {
    loading.value = true
    const exportMeta = await exportSourceOfSupply(
...
async function handleSourceStatusChange(record: SourceOfSupplyRowRecord, checked: boolean) {
  ...
  if (row) {
    row.sourceStatus = newVal
  }
  try {
    await updateSourceOfSupplyStatus({ sourceOfSupplyId: id, sourceStatus: newVal })
    message.success(t('common.feedback.updated'))
    
  } catch (error: unknown) {
    if (row) {
      row.sourceStatus = oldVal
    }
