...
<!-- 文件名称：operation-panel.vue -->
<!-- 功能描述：APS 排程订单主表实体右侧明细 apsOperation 独立 CRUD（按主表选中 apsOrderId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="operation-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="operation-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-logistics-customer-service-order-operation"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('workCenterResourceId')">
        <TaktSelect
          v-model:value="advancedQueryForm.workCenterResourceId"
          api-url="TaktWorkCenterResources/options"
          :placeholder="pi.queryPh('workCenterResourceId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('plannedDurationMinutes')">
        <a-input-number
          v-model:value="advancedQueryForm.plannedDurationMinutes"
          :placeholder="pi.queryPh('plannedDurationMinutes', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('operationStatus')">
        <TaktSelect
          v-model:value="advancedQueryForm.operationStatus"
          dict-type="aps_operation_status"
          :placeholder="pi.queryPh('operationStatus', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="APSOPERATION_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  ...
  importApsOperation,
  exportApsOperation,
} from '@/api/logistics/manufacturing/aps/operation'
import type { ApsOperation, ApsOperationQuery } from '@/types/logistics/manufacturing/aps/operation'

import {
  useApsOperationI18n,
  APSOPERATION_DEFAULT_VISIBLE_COLUMN_KEYS,
  APSOPERATION_SUMMARY_SUM_FIELDS,
  APSOPERATION_QUERY_STRING_FIELDS,
  APSOPERATION_QUERY_FIELDS,
  APSOPERATION_SELF_I18N_KEY,
} from '../composables/use-operation-i18n'
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(APSOPERATION_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  APSOPERATION_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...APSOPERATION_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...APSOPERATION_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'apsOperationId'
const masterApsOrderId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['apsOrderId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterApsOrderId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: ApsOperation }) =>
      String(getApsOperationField(record, 'apsOrderId') ?? ''),
  },
  {
  ...
  {
    title: pi.label('workCenterResourceId'),
  ...
  {
    title: pi.label('plannedDurationMinutes'),
  ...
  {
    title: pi.label('changeoverMinutes'),
  ...
  {
    title: pi.label('operationStatus'),
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: ApsOperation }) =>
      String(getApsOperationField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const summarySumFieldSet = new Set<string>(APSOPERATION_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    APSOPERATION_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof APSOPERATION_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of APSOPERATION_SUMMARY_SUM_FIELDS) {
      const num = Number(getApsOperationField(row, field))
...
function buildListQuery(overrides?: Partial<ApsOperationQuery>): ApsOperationQuery {
  ...
  for (const key of APSOPERATION_QUERY_STRING_FIELDS) {
