...
<!-- 命名空间：@/views/logistics/serial/inbound -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：序列号入库主表实体管理页面，含查询、增删改，由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getSerialInboundId"
    ...
    >
      <SerialInboundForm
        :key="formData?.serialInboundId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-serial-inbound'"
...
/**
 * 序列号入库主表实体管理页 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
 * @module views/logistics/serial/inbound
 */
...
/** 分页列表数据 */
const dataSource = ref<SerialInbound[]>([])
/** 当前页码 */
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<SerialInbound> | null>(null)
/** 表单提交 loading */
...
const entityIdName = 'serialInboundId'
/** 工具栏「编辑」是否禁用（须恰好选中一行） */
 ...
 * @returns {SerialInboundQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<SerialInboundQuery>): SerialInboundQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: SerialInboundQuery = {
  ...
  const assignTrimmed = (key: keyof SerialInboundQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
async function loadSerialInboundDetail(record: SerialInboundRowRecord): Promise<SerialInbound | null> {
  ...
  try {
    const detail = await getSerialInboundById(id)
    const index = dataSource.value.findIndex((row) => getSerialInboundId(row) === id)
    if (index !== -1) {
      dataSource.value[index] = { ...dataSource.value[index], ...detail } as SerialInbound
    }
    return detail
  } catch (error: any) {
...
async function loadData() {
  ...
  try {
    ...
    const res = await getSerialInboundList(buildListQuery())
    dataSource.value = res.data ?? []
    total.value = res.total ?? 0
  } catch (error: any) {
    logger.error('[SerialInbound] 加载数据失败', { error })
    message.error(error?.message || t('common.feedback.load.data.failed'))
    dataSource.value = []
    total.value = 0
  } finally {
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getSerialInboundTemplate(sheetName, fileName)
...
async function handleExport() {
  try {
    ...
    const exportMeta = await exportSerialInbound(
