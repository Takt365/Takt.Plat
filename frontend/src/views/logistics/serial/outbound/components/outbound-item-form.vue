...
<!-- 文件名称：outbound-item-form.vue -->
<!-- 功能描述：序列号出库主表实体子表 serialOutboundItem 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form outbound-item-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
            ...
            <a-col :span="12">
              ...
              >
                <a-input
                  ...
                  :maxlength="20"
                  allow-clear
                  :disabled="!!formData?.serialOutboundItemId"
                />
              </a-form-item>
            </a-col>
            <a-col :span="12">
...
/**
 * 序列号出库主表实体子表 serialOutboundItem 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
const formFields = ["tenantCode","companyCode","cultureCode","lineNumber","outboundSerialCode","referenceInboundId","referenceInboundCode","referenceInboundLineNumber","isObsolete"]



/** 父级传入的编辑 DTO；新增时为 undefined 或空对象 */
interface Props {
  formData?: Partial<SerialOutboundItemCreate & { serialOutboundItemId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
...
watch(
  ...
  () => {
    if (!props.formData?.serialOutboundItemId) {
...
function getValues(): Record<string, any> {
  ...
  if ('referenceInboundLineNumber' in payload) {
    const rawreferenceInboundLineNumber = payload.referenceInboundLineNumber
    if (rawreferenceInboundLineNumber === undefined || rawreferenceInboundLineNumber === null || rawreferenceInboundLineNumber === '') {
      delete payload.referenceInboundLineNumber
    } else {
      const numreferenceInboundLineNumber = typeof rawreferenceInboundLineNumber === 'number' ? rawreferenceInboundLineNumber : Number(rawreferenceInboundLineNumber)
      if (Number.isFinite(numreferenceInboundLineNumber)) payload.referenceInboundLineNumber = numreferenceInboundLineNumber
      else delete payload.referenceInboundLineNumber
    }
  }
  ...
  if (props.formData?.serialOutboundItemId) {
    payload.serialOutboundItemId = props.formData.serialOutboundItemId
  }
  ...
  if (masterRow) {
    const masterCode = masterRow.serialOutboundCode ?? masterRow.SerialOutboundCode
    const masterName = masterRow.serialOutboundName ?? masterRow.SerialOutboundName
    if (masterCode != null && masterCode !== '' && !payload.serialOutboundCode) {
      payload.serialOutboundCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.serialOutboundName) {
      payload.serialOutboundName = masterName
    }
    if (masterCode != null && masterCode !== '' && !payload.outboundCode) {
      payload.outboundCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.outboundName) {
      payload.outboundName = masterName
    }
...
function resetFields() {
  ...
  applyFormDefaults(formState)
  applyScopeDefaults(formState as Record<string, unknown>, !props.formData?.serialOutboundItemId)
