...
<!-- 文件名称：index.vue -->
<!-- 功能描述：产品序列号入库主表实体管理页面，含查询、增删改，由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成 -->
  ...
  <div class="logistics-serial-product-serial-inbound">
    ...
    >
      ...
      <template #expandedRowRender="{ record }">
        <div class="p-4">
          <div class="mb-2 text-sm font-medium">{{ t('entity.productSerialInboundItem._self') }}</div>
          <a-table
            v-if="hasProductSerialInboundItemRows(record)"
            :columns="productSerialInboundItemExpandColumns"
            :data-source="getProductSerialInboundItemRows(record)"
            :row-key="(row: ProductSerialInboundItem, index?: number) => row?.productSerialInboundItemId || String(index ?? 0)"
            :pagination="false"
            size="small"
            bordered
            class="mb-4"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-serial-product-serial-inbound'"
    ...
    >
      ...
      <a-form-item :label="t('entity.productSerialInbound.inbounddatestart')">
        <a-date-picker
          v-model:value="advancedQueryForm.inboundDateStart"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.productSerialInbound.inbounddatestart') })"
      ...
      <a-form-item :label="t('entity.productSerialInbound.inbounddateend')">
        <a-date-picker
          v-model:value="advancedQueryForm.inboundDateEnd"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.productSerialInbound.inbounddateend') })"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
    ...
    <TaktModal
      ...
      :title="t('common.dialog.title.import', { entity: t('entity.productSerialInbound._self') })"
      :width="600"
      :footer="null"
...
/**
 * 产品序列号入库主表实体管理页 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
...
import * as productSerialInboundItemApi from '@/api/logistics/serial/product-serial-inbound-item'
import type { ProductSerialInboundItem, ProductSerialInboundItemQuery } from '@/types/logistics/serial/product-serial-inbound-item'
...
const queryFieldsMeta = computed(() => [
  { key: 'plantCode', label: t('entity.productSerialInbound.plantcode') },
  { key: 'inboundNo', label: t('entity.productSerialInbound.inboundno') },
  { key: 'inboundDateStart', label: t('entity.productSerialInbound.inbounddatestart') },
  { key: 'inboundDateEnd', label: t('entity.productSerialInbound.inbounddateend') },
  { key: 'inboundType', label: t('entity.productSerialInbound.inboundtype') },
  { key: 'warehouseCode', label: t('entity.productSerialInbound.warehousecode') },
  { key: 'locationCode', label: t('entity.productSerialInbound.locationcode') },
  { key: 'totalQuantity', label: t('entity.productSerialInbound.totalquantity') },
  { key: 'relatedCompany', label: t('entity.productSerialInbound.relatedcompany') },
...
/** 展开行预览：productSerialInboundItem 列 */
const productSerialInboundItemExpandColumns = computed(() => [
  {
...
/** 读取主表行上的 productSerialInboundItem 子表缓存 */
function getProductSerialInboundItemRows(record: ProductSerialInbound): ProductSerialInboundItem[] {
  return (record as any)?.items ?? []
}

/** 主表行是否已加载 productSerialInboundItem 子表 */
function hasProductSerialInboundItemRows(record: ProductSerialInbound): boolean {
  return getProductSerialInboundItemRows(record).length > 0
}


/** 加载主表详情并回填当前页 dataSource */
async function loadProductSerialInboundDetail(record: ProductSerialInbound): Promise<ProductSerialInbound | null> {
  const id = getProductSerialInboundId(record)
  if (!id) {
    return null
  }
  try {
    const detail = await getProductSerialInboundById(id)
    const index = dataSource.value.findIndex((row) => getProductSerialInboundId(row) === id)
    if (index !== -1) {
      dataSource.value[index] = { ...dataSource.value[index], ...detail } as ProductSerialInbound
    }
    return detail
  } catch (error: any) {
...
/** 懒加载 productSerialInboundItem 子表（ProductSerialInboundItemQuery + productSerialInboundItemApi，与主表 ProductSerialInboundQuery 分离） */
async function loadProductSerialInboundItemForProductSerialInbound(record: ProductSerialInbound): Promise<ProductSerialInboundItem[]> {
  const masterId = getProductSerialInboundId(record)
  if (!masterId) {
    return []
  }
  try {
    const childQuery: ProductSerialInboundItemQuery = {
      pageIndex: 1,
      pageSize: 500,
      productSerialInboundId: masterId,
    }
    const result = await productSerialInboundItemApi.getProductSerialInboundItemList(childQuery)
    const rows = result?.data ?? []
    const index = dataSource.value.findIndex((row) => getProductSerialInboundId(row) === masterId)
    if (index !== -1) {
      const row = dataSource.value[index]
      dataSource.value[index] = { ...row, items: rows } as ProductSerialInbound
    }
    return rows
  } catch (error: any) {
    message.error(error?.message || t('common.feedback.load.data.failed'))
...
/** 展开前确保各子表已懒加载 */
async function ensureProductSerialInboundChildrenLoaded(record: ProductSerialInbound) {
  if (!hasProductSerialInboundItemRows(record)) {
    await loadProductSerialInboundItemForProductSerialInbound(record)
  }
}

/** 主表展开行：手风琴懒加载子表 */
async function handleExpand(expanded: boolean, record: ProductSerialInbound) {
  ...
  await ensureProductSerialInboundChildrenLoaded(record)
...
function handleCreate() {
  formTitle.value = t('common.dialog.title.create', { entity: t('entity.productSerialInbound._self') })
  formData.value = {}
  formVisible.value = true
}
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: ProductSerialInbound) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.productSerialInbound._self') })
  formLoading.value = true
  try {
    const detail = await loadProductSerialInboundDetail(record)
