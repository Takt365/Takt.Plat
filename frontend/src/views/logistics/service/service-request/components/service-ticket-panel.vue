...
<!-- 功能描述：服务请求实体主表实体右侧明细 serviceTicket 独立 CRUD（按主表选中 serviceRequestId 分页） -->
  ...
  <div class="service-ticket-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    >
      <ServiceTicketForm
        ref="formRef"
        :form-data="formData"
        :master-id="masterServiceRequestId"
        :loading="formLoading"
      />
    </TaktModal>
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-logistics-service-service-request-service-ticket"
...
import { useServiceRequestMasterContext } from '../composables/use-service-request-master-context'
import {
...
const { t } = useI18n()
const { selectedMasterRow } = useServiceRequestMasterContext()
...
const hasMasterSelection = computed(() => !!selectedMasterRow.value?.serviceRequestId)
const masterServiceRequestId = computed(() => selectedMasterRow.value?.serviceRequestId ?? '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: ServiceTicket }) =>
      String(getServiceTicketField(record, 'serviceOrderId') ?? ''),
  },
  {
...
function buildListQuery(overrides?: Partial<ServiceTicketQuery>): ServiceTicketQuery {
  ...
  const query: ServiceTicketQuery = {
    ...
    serviceRequestId: masterServiceRequestId.value,
    ...overrides,
  }
  if (kw.length > 0) {
    query.keyWords = kw
