...
/**
 * 服务合同实体维护表单 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
...
interface Props {
  formData?: Partial<ServiceContractCreate & { serviceContractId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.serviceContractId
    if (isCreate) {
