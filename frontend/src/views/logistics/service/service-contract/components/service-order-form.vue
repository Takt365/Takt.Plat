...
<!-- 功能描述：服务合同实体子表 serviceOrder 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
/**
 * 服务合同实体子表 serviceOrder 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
const formFields = ["plantCode","serviceOrderCode","clientId","clientCode","clientName","serviceRequestId","serviceRequestCode","orderDate"]
...
/** 映射为 Create/Update DTO（含主表外键 serviceContractId） */
function getValues(): Record<string, any> {
  const payload = { ...formState }
  if ('sortOrder' in payload) delete payload.sortOrder
  payload.serviceContractId = props.masterId
  return payload
}
