...
<!-- 命名空间：@/views/logistics/materials/manufacturer-material/components -->
<!-- 文件名称：manufacturer-material-form.vue -->
<!-- 功能描述：Takt制造商物料明细实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.manufacturerMaterial.manufacturerid')"
                name="manufacturerId"
              >
                <a-input
                  v-model:value="formState.manufacturerId"
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.manufacturerMaterial.manufacturercode')"
...
/**
 * Takt制造商物料明细实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/logistics/materials/manufacturer-material/components
 */
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","manufacturerId","manufacturerCode","lineNumber","materialType","manufacturerMaterialCode","manufacturerMaterialName","manufacturerMaterialSpecification","materialCode","extFieldJson","remark"]
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.manufacturerMaterialId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  manufacturerId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.manufacturerMaterial.manufacturerid') }),
      trigger: 'blur'
    }
  ],
  manufacturerCode: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.manufacturerMaterial.manufacturercode') }),
      trigger: 'blur'
    }
  ],
  lineNumber: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.manufacturerMaterial.linenumber') }),
      trigger: 'change'
    }
  ],
  materialType: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.manufacturerMaterial.materialtype') }),
      trigger: 'change'
    }
  ],
