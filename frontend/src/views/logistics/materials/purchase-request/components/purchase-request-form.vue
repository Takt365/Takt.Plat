...
<!-- 命名空间：@/views/logistics/materials/purchase-request/components -->
<!-- 文件名称：purchase-request-form.vue -->
<!-- 功能描述：Takt采购申请实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              ...
              >
                <a-input-number
                  ...
                  :placeholder="t('common.page.form.placeholder.required', { field: t('entity.purchaseRequest.requeststatus') })"
                  size="small"
                  style="width: 100%"
                />
              </a-form-item>
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.purchaseRequest.flowinstanceid')"
                name="flowInstanceId"
              >
                <a-input
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.purchaseRequest.items')"
                name="items"
              >
                <a-input
                  v-model:value="formState.items"
                  :placeholder="t('common.page.form.placeholder.required', { field: t('entity.purchaseRequest.items') })"
                  size="small"
                  allow-clear
                />
              </a-form-item>
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.purchaseRequest.changelogs')"
                name="changeLogs"
              >
                <a-input
                  v-model:value="formState.changeLogs"
                  :placeholder="t('common.page.form.placeholder.required', { field: t('entity.purchaseRequest.changelogs') })"
                  size="small"
                  allow-clear
                />
              </a-form-item>
...
/**
 * Takt采购申请实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/logistics/materials/purchase-request/components
 */
import { reactive, watch, computed, ref } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import type { PurchaseRequestCreate } from '@/types/logistics/materials/purchase-request'
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","plantCode","purchaseRequestCode","requestDate","requiredArrivalDate","requestId","requestBy","totalQuantity","totalAmount","convertedQuantity","convertedAmount","requestStatus","convertedStatus","flowInstanceId","requestReason","items","changeLogs","extFieldJson","remark"]
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  totalQuantity: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseRequest.totalquantity') }),
      trigger: 'change'
    }
  ],
  totalAmount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseRequest.totalamount') }),
      trigger: 'change'
    }
  ],
  convertedQuantity: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseRequest.convertedquantity') }),
      trigger: 'change'
    }
  ],
  convertedAmount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseRequest.convertedamount') }),
      trigger: 'change'
    }
  ],
  requestStatus: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseRequest.requeststatus') }),
      trigger: 'change'
    }
  ],
  convertedStatus: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseRequest.convertedstatus') }),
      trigger: 'change'
    }
  ],
}))
