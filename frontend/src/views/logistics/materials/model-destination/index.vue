...
<!-- 命名空间：@/views/logistics/materials/model-destination -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt型号目的地实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:materials:model:destination:create"
      update-permission="logistics:materials:model:destination:update"
      delete-permission="logistics:materials:model:destination:delete"
      import-permission="logistics:materials:model:destination:import"
      export-permission="logistics:materials:model:destination:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      <ModelDestinationForm
        :key="formData?.modelDestinationId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-materials-model-destination'"
    ...
    >
      ...
      <a-form-item :label="t('entity.modeldestination.materialcode')">
        <a-input
      ...
      <a-form-item :label="t('entity.modeldestination.modelcode')">
        <a-input
      ...
      <div v-show="isFieldVisible('destinationCode')">
      <a-form-item :label="t('entity.modeldestination.destinationcode')">
        <a-input
      ...
      <div v-show="isFieldVisible('destinationName')">
    ...
    <TaktModal
      ...
      :title="t('common.dialog.title.import', { entity: t('entity.modeldestination._self') })"
      :width="600"
      :footer="null"
...
/**
 * Takt型号目的地实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/materials/model-destination
 */
...
import { getModelDestinationList, getModelDestinationById, createModelDestination, updateModelDestination, deleteModelDestinationById, deleteModelDestinationBatch, getModelDestinationTemplate, importModelDestination, exportModelDestination } from '@/api/logistics/materials/model-destination'
import type { ModelDestination, ModelDestinationQuery } from '@/types/logistics/materials/model-destination'
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<ModelDestination> | null>(null)
/** 表单提交 loading */
...
const queryFieldsMeta = computed(() => [
  ...
  { key: 'destinationName', label: t('entity.modeldestination.destinationname') },
 ...
 * @returns {ModelDestinationQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<ModelDestinationQuery>): ModelDestinationQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: ModelDestinationQuery = {
  ...
  const assignTrimmed = (key: keyof ModelDestinationQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
  ...
  assignTrimmed('destinationName', form.destinationName)
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: t('entity.modeldestination.materialcode'),
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getModelDestinationField(record, 'modelCode') ?? ''
  },
  {
  ...
  {
    title: t('entity.modeldestination.destinationcode'),
    dataIndex: 'destinationCode',
    key: 'destinationCode',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getModelDestinationField(record, 'destinationCode') ?? ''
  },
  {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:materials:model:destination:update',
      ...
      },
      {
        ...
        permission: 'logistics:materials:model:destination:delete',
...
async function loadData() {
  loading.value = true
  try {
    const res = await getModelDestinationList(buildListQuery())
...
function handleCreate() {
  formTitle.value = t('common.dialog.title.create', { entity: t('entity.modeldestination._self') })
  formData.value = null
...
function handleEdit(record: ModelDestination) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.modeldestination._self') })
...
async function handleExport() {
  try {
    loading.value = true
    const exportMeta = await exportModelDestination(
