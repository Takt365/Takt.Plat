...
<!-- 命名空间：@/views/logistics/materials/purchase-order/components -->
<!-- 文件名称：purchase-order-form.vue -->
<!-- 功能描述：Takt采购订单实体维护弹窗内嵌表单。由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    ...
    >
      ...
      <!-- 子表：purchaseOrderItem -->
      <a-tab-pane
        key="child-items"
        :tab="t('entity.purchaseOrderItem._self')"
        force-render
      >
        <div class="mb-2">
          <a-button type="primary" size="small" @click="handleAddPurchaseOrderItemRow">
            {{ t('common.page.button.create') }}{{ t('entity.purchaseOrderItem._self') }}
          </a-button>
        </div>
        <a-table
          :columns="purchaseOrderItemFormColumns"
          :data-source="childPurchaseOrderItemRows"
          :pagination="false"
        ...
        >
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-else-if="column.key === 'requestCode'">
              <a-input
                v-model:value="record.requestCode"
                :placeholder="t('common.page.form.placeholder.required', { field: t('entity.purchaseOrderItem.requestcode') })"
                size="small"
                allow-clear
              />
            </template>
            <template v-else-if="column.key === 'requestLineNumber'">
              <a-input-number
                v-model:value="record.requestLineNumber"
            ...
            <template v-else-if="column.key === '__action'">
              <a-button type="link" danger size="small" @click="handleRemovePurchaseOrderItemRow(index)">
      ...
      <!-- 子表：purchaseOrderChangeLog -->
      <a-tab-pane
        key="child-changeLogs"
        :tab="t('entity.purchaseOrderChangeLog._self')"
        force-render
      >
        <div class="mb-2">
          <a-button type="primary" size="small" @click="handleAddPurchaseOrderChangeLogRow">
            {{ t('common.page.button.create') }}{{ t('entity.purchaseOrderChangeLog._self') }}
          </a-button>
        </div>
        <a-table
          :columns="purchaseOrderChangeLogFormColumns"
          :data-source="childPurchaseOrderChangeLogRows"
          :pagination="false"
          :row-key="(row: Record<string, unknown>, index?: number) => String(row.__rowKey ?? index ?? 0)"
          size="small"
          bordered
          ...
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-else-if="column.key === 'changeTime'">
              <a-input
                ...
                :placeholder="t('common.page.form.placeholder.required', { field: t('entity.purchaseOrderChangeLog.changetime') })"
                size="small"
                allow-clear
              />
            </template>
            ...
            <template v-else-if="column.key === '__action'">
              <a-button type="link" danger size="small" @click="handleRemovePurchaseOrderChangeLogRow(index)">
                {{ t('common.page.button.delete') }}
              </a-button>
            </template>
          </template>
 ...
 * @module views/logistics/materials/purchase-order/components
 */
import { reactive, watch, computed, ref } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import type { PurchaseOrderCreate, PurchaseOrderItemCreate, PurchaseOrderItem, PurchaseOrderChangeLogCreate, PurchaseOrderChangeLog } from '@/types/logistics/materials/purchase-order'
...
/** 子表 purchaseOrderChangeLog 表单列定义 */
const purchaseOrderChangeLogFormColumns = computed(() => [
  {
    title: t('common.page.entity.tenantcode'),
    dataIndex: 'tenantCode',
    key: 'tenantCode',
...
function syncChildRowsFromFormData(val: Partial<PurchaseOrderCreate & { purchaseOrderId?: string }> | null | undefined) {
  childPurchaseOrderItemRows.value = ((val as any)?.items ?? []).map((item: Record<string, unknown>, index: number) => ({
    ...item,
    __rowKey: item.purchaseOrderItemId ?? `new-${index}`,
  }))
  childPurchaseOrderChangeLogRows.value = ((val as any)?.changeLogs ?? []).map((item: Record<string, unknown>, index: number) => ({
    ...item,
    __rowKey: item.purchaseOrderChangeLogId ?? `new-${index}`,
  }))
}

/** 表单 Tab 内新增 purchaseOrderItem 行 */
function handleAddPurchaseOrderItemRow() {
  childPurchaseOrderItemRows.value.push({
...
/** 表单 Tab 内删除 purchaseOrderItem 行 */
function handleRemovePurchaseOrderItemRow(index: number) {
  childPurchaseOrderItemRows.value.splice(index, 1)
}

/** 表单 Tab 内新增 purchaseOrderChangeLog 行 */
function handleAddPurchaseOrderChangeLogRow() {
  childPurchaseOrderChangeLogRows.value.push({
    __rowKey: `new-${Date.now()}`,
      tenantCode: tenantStore.tenantCode,
      companyCode: tenantStore.companyCode,
      companyDefaultCulture: userStore.userInfo?.companyDefaultCulture ?? '',
...
/** 表单 Tab 内删除 purchaseOrderChangeLog 行 */
function handleRemovePurchaseOrderChangeLogRow(index: number) {
  childPurchaseOrderChangeLogRows.value.splice(index, 1)
}
...
function buildSubmitPayload() {
  return {
    ...formState,
    items: childPurchaseOrderItemRows.value.map(({ __rowKey, ...rest }) => rest),
    changeLogs: childPurchaseOrderChangeLogRows.value.map(({ __rowKey, ...rest }) => rest),
  }
}
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  totalQuantity: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.totalquantity') }),
      trigger: 'change'
    }
  ],
  totalAmount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.totalamount') }),
      trigger: 'change'
    }
  ],
  discountAmount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.discountamount') }),
      trigger: 'change'
    }
  ],
  taxAmount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.taxamount') }),
      trigger: 'change'
    }
  ],
  actualAmount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.actualamount') }),
      trigger: 'change'
    }
  ],
  receivedQuantity: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.receivedquantity') }),
      trigger: 'change'
    }
  ],
  receivedAmount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.receivedamount') }),
      trigger: 'change'
    }
  ],
  paidAmount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.paidamount') }),
      trigger: 'change'
    }
  ],
  orderStatus: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.orderstatus') }),
      trigger: 'change'
    }
  ],
  deliveryStatus: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.deliverystatus') }),
      trigger: 'change'
    }
  ],
  paymentMethod: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.paymentmethod') }),
      trigger: 'change'
    }
  ],
  deliveryMethod: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.purchaseOrder.deliverymethod') }),
      trigger: 'change'
    }
  ],
}))
