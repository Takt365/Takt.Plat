...
<!-- 命名空间：@/views/logistics/materials/storage-location -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt库位主数据实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:materials:storagelocation:create"
      update-permission="logistics:materials:storagelocation:update"
      delete-permission="logistics:materials:storagelocation:delete"
      import-permission="logistics:materials:storagelocation:import"
      export-permission="logistics:materials:storagelocation:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'locationStatus'">
          <a-switch
            :checked="getStorageLocationField(record, 'locationStatus') === 1"
            :checked-children="t('common.page.button.enable')" :un-checked-children="t('common.page.button.disable')"
            @change="(checked: unknown) => handleLocationStatusChange(record, Boolean(checked))"
          />
        </template>
        <template v-else-if="column.key === 'isBuiltIn'">
          <TaktDictTag
            :value="getStorageLocationField(record, 'isBuiltIn')"
    ...
    >
      <StorageLocationForm
        :key="formData?.storageLocationId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-materials-storage-location'"
      :form-model="advancedQueryForm"
      @submit="handleAdvancedQuerySubmit"
      @reset="handleAdvancedQueryReset"
    >
      <template #default="{ isFieldVisible }">
      <div v-show="isFieldVisible('warehouseId')">
      <a-form-item :label="t('entity.storagelocation.warehouseid')">
        <a-input
          v-model:value="advancedQueryForm.warehouseId"
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.storagelocation.warehouseid') })"
          show-count
          :maxlength="20"
          allow-clear
        />
...
/**
 * Takt库位主数据实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/materials/storage-location
 */
...
import { getStorageLocationList, getStorageLocationById, createStorageLocation, updateStorageLocation, deleteStorageLocationById, deleteStorageLocationBatch, getStorageLocationTemplate, importStorageLocation, exportStorageLocation, updateStorageLocationStatus } from '@/api/logistics/materials/storage-location'
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<StorageLocation> | null>(null)
/** 表单提交 loading */
...
const queryFieldsMeta = computed(() => [
  { key: 'warehouseId', label: t('entity.storagelocation.warehouseid') },
...
function buildListQuery(overrides?: Partial<StorageLocationQuery>): StorageLocationQuery {
  ...
  assignTrimmed('warehouseId', form.warehouseId)
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: any }) => getStorageLocationField(record, 'storageLocationId') ?? ''
  },
  {
    title: t('entity.storagelocation.warehouseid'),
    dataIndex: 'warehouseId',
    key: 'warehouseId',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getStorageLocationField(record, 'warehouseId') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getStorageLocationField(record, 'plantCode') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getStorageLocationField(record, 'warehouseCode') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getStorageLocationField(record, 'locationCode') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getStorageLocationField(record, 'locationName') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getStorageLocationField(record, 'locationType') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getStorageLocationField(record, 'warehouse') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:materials:storagelocation:update',
        onClick: (record: StorageLocation) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        permission: 'logistics:materials:storagelocation:delete',
        onClick: (record: StorageLocation) => handleDeleteOne(record)
      }
    ]
  })
])
...
/** 表格 row-key（优先实体主键字段） */
const getStorageLocationId = (record: any): string => record?.[entityIdName] ?? ''
/**
 * 读取行字段值
 * @param record 行数据
 * @param field 字段名
 */
const getStorageLocationField = (record: any, field: string): any => record?.[field]
...
const onClickRow = (record: StorageLocation) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getStorageLocationId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[StorageLocation] 加载数据失败', { error })
...
/** 打开编辑弹窗 */
function handleEdit(record: StorageLocation) {
...
function handleUpdate() {
  if (selectedRow.value) {
    ...
    message.warning(t('common.tip.select.to.action', { action: t('common.page.button.edit'), entity: t('entity.storagelocation._self') }))
  }
}
/** 提交新增/编辑表单 */
async function handleFormSubmit() {
  ...
  try {
    ...
    if (id) {
      await updateStorageLocation(id, payload as any)
      message.success(t('common.feedback.updated', { target: t('entity.storagelocation._self') }))
    } else {
      await createStorageLocation(payload as any)
...
async function handleExport() {
  try {
    ...
    logger.error('[StorageLocation] 导出失败', { error })
    message.error(error?.message || t('common.feedback.export.failed', { target: t('entity.storagelocation._self') }))
  } finally {
    loading.value = false
  }
}
...
async function handleDeleteOne(record: StorageLocation) {
  Modal.confirm({
    ...
    content: t('common.tip.confirm.delete.entity', { entity: t('entity.storagelocation._self'), name: t('common.tip.this.target', { target: t('entity.storagelocation._self') }) }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      await deleteStorageLocationById((record as any)[entityIdName])
...
async function handleDelete() {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.tip.select.to.action', { action: t('common.page.button.delete'), entity: t('entity.storagelocation._self') }))
    return
  }
  Modal.confirm({
    title: t('common.tip.confirm.delete.title'),
    content: t('common.tip.confirm.delete.count', { entity: t('entity.storagelocation._self'), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
 ...
 * @param checked 是否启用
 */
async function handleLocationStatusChange(record: StorageLocation, checked: boolean) {
  const newVal = checked ? 1 : 0
  const oldVal = getStorageLocationField(record, 'locationStatus')
  const id = getStorageLocationId(record)
  const row = dataSource.value.find((item) => getStorageLocationId(item) === id)
  if (row) {
    row.locationStatus = newVal
  }
  try {
    await updateStorageLocationStatus({ storageLocationId: id, locationStatus: newVal })
    message.success(t('common.feedback.updated'))
    
  } catch (error: unknown) {
    if (row) {
      row.locationStatus = oldVal
    }
