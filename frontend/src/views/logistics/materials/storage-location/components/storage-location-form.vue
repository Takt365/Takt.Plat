...
<!-- 命名空间：@/views/logistics/materials/storage-location/components -->
<!-- 文件名称：storage-location-form.vue -->
<!-- 功能描述：Takt库位主数据实体维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.storagelocation.warehouseid')"
                name="warehouseId"
              >
                <a-input
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.storagelocation.warehousecode')"
                name="warehouseCode"
              >
                <a-input
...
/**
 * Takt库位主数据实体维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/materials/storage-location/components
 */
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","warehouseId","plantCode","warehouseCode","locationCode","locationName","locationType","locationStatus","isBuiltIn","extField","remark"]
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.storageLocationId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  warehouseId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.storagelocation.warehouseid') }),
      trigger: 'blur'
    }
  ],
  plantCode: [
  ...
  warehouseCode: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.storagelocation.warehousecode') }),
      trigger: 'blur'
    }
  ],
  locationCode: [
...
function resetFields() {
  ...
  applyFormDefaults(formState)
  applyScopeDefaults(formState as Record<string, unknown>, !props.formData?.storageLocationId)
