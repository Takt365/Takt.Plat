...
<!-- 命名空间：@/views/logistics/materials/purchase-price-item -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt采购价格明细实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- 免责声明：此软件使用 MIT License，作者不承担任何使用风险。 -->
<!-- ======================================== -->

<template>
  <div class="logistics-materials-purchase-price-item">
    <!-- 查询栏 -->
    <TaktQueryBar
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:materials:purchasepriceitem:create"
      update-permission="logistics:materials:purchasepriceitem:update"
      delete-permission="logistics:materials:purchasepriceitem:delete"
      import-permission="logistics:materials:purchasepriceitem:import"
      export-permission="logistics:materials:purchasepriceitem:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      <a-form-item :label="t('entity.purchasePriceItem.purchasepriceid')">
        <a-input
          v-model:value="advancedQueryForm.purchasePriceId"
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.purchasePriceItem.purchasepriceid') })"
          allow-clear
        />
      </a-form-item>
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.purchasePriceItem._self')"
      :width="600"
      :footer="null"
...
/**
 * Takt采购价格明细实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/logistics/materials/purchase-price-item
 */
...
import { getPurchasePriceItemList, getPurchasePriceItemById, createPurchasePriceItem, updatePurchasePriceItem, deletePurchasePriceItemById, deletePurchasePriceItemBatch, getPurchasePriceItemTemplate, importPurchasePriceItem, exportPurchasePriceItem } from '@/api/logistics/materials/purchase-price-item'
import type { PurchasePriceItem, PurchasePriceItemQuery, PurchasePriceItemCreate, PurchasePriceItemUpdate } from '@/types/logistics/materials/purchase-price-item'
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: t('entity.purchasePriceItem.purchasepriceid'),
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'purchasePriceName') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'lineNumber') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'materialCode') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'materialName') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'materialSpecification') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'purchaseUnit') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'purchasePrice') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'minPurchaseQuantity') ?? ''
  },
  {
    title: t('entity.purchasePriceItem.maxpurchasequantity'),
    dataIndex: 'maxPurchaseQuantity',
    key: 'maxPurchaseQuantity',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'maxPurchaseQuantity') ?? ''
  },
  {
    title: t('entity.purchasePriceItem.scales'),
    dataIndex: 'scales',
    key: 'scales',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceItemField(record, 'scales') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:materials:purchasepriceitem:update',
        onClick: (record: PurchasePriceItem) => handleEdit(record)
      },
      {
        key: 'delete',
        ...
        permission: 'logistics:materials:purchasepriceitem:delete',
        onClick: (record: PurchasePriceItem) => handleDeleteOne(record)
      }
    ]
  })
])

const getPurchasePriceItemId = (record: any): string => record?.[entityIdName] ?? ''
...
const onClickRow = (record: PurchasePriceItem) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getPurchasePriceItemId(item)))
...
async function loadData() {
  ...
  try {
    ...
    const params: PurchasePriceItemQuery = {
    ...
    const res = await getPurchasePriceItemList(params)
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.purchasePriceItem._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: PurchasePriceItem) {
  formTitle.value = t('common.page.button.edit') + t('entity.purchasePriceItem._self')
...
function handleUpdate() {
  if (selectedRow.value) {
    ...
    message.warning(t('common.tip.select.to.action', { action: t('common.page.button.edit'), entity: t('entity.purchasePriceItem._self') }))
  }
}
...
async function handleExport() {
  try {
    loading.value = true
    const kw = (queryKeyword.value ?? '').trim()
    const exportQuery: PurchasePriceItemQuery = {
      pageIndex: 1,
      pageSize: 100000,
      ...advancedQueryForm.value
    }
    ...
    const exportMeta = await exportPurchasePriceItem(exportQuery, excelNames.sheet, excelNames.fileBase)
    const ts = new Date()
    ...
    message.error(error?.message || t('common.feedback.export.failed', { target: t('entity.purchasePriceItem._self') }))
  } finally {
    loading.value = false
  }
}
async function handleDeleteOne(record: PurchasePriceItem) {
  Modal.confirm({
    ...
    content: t('common.tip.confirm.delete.entity', { entity: t('entity.purchasePriceItem._self'), name: t('common.tip.this.target', { target: t('entity.purchasePriceItem._self') }) }),
...
async function handleDelete() {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.tip.select.to.action', { action: t('common.page.button.delete'), entity: t('entity.purchasePriceItem._self') }))
    return
  }
  Modal.confirm({
    title: t('common.tip.confirm.delete.title'),
    content: t('common.tip.confirm.delete.count', { entity: t('entity.purchasePriceItem._self'), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
...
.logistics-materials-purchase-price-item {
  padding: 16px;
  display: flex;
