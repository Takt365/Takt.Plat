...
<!-- 命名空间：@/views/logistics/materials/purchase-request-change-log/components -->
<!-- 文件名称：purchase-request-change-log-form.vue -->
<!-- 功能描述：采购申请变更记录实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="purchase-request-change-log-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.purchaseRequestChangeLog.purchaserequestid')"
            ...
            <a-col :span="12">
              ...
              >
                <a-input
                  ...
                  :placeholder="t('common.page.form.placeholder.required', { field: t('entity.purchaseRequestChangeLog.changetime') })"
                  size="small"
                  allow-clear
                />
              </a-form-item>
...
/**
 * 采购申请变更记录实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/logistics/materials/purchase-request-change-log/components
 */
import { reactive, watch, computed, ref } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import type { PurchaseRequestChangeLogCreate } from '@/types/logistics/materials/purchase-request-change-log'
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","purchaseRequestId","requestCode","changeFields","changeTime","changeBy","changeReason","extFieldJson","remark"]


interface Props {
  formData?: Partial<PurchaseRequestChangeLogCreate & { purchaseRequestChangeLogId?: string }> | null
  loading?: boolean
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.purchaseRequestChangeLogId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  purchaseRequestId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.purchaseRequestChangeLog.purchaserequestid') }),
      trigger: 'blur'
    }
  ],
  requestCode: [
  ...
  changeTime: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.purchaseRequestChangeLog.changetime') }),
      trigger: 'blur'
    }
  ],
}))
