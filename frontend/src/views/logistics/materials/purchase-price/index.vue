...
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt采购价格实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="logistics-materials-purchase-price">
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.purchasePrice._self')"
      :width="600"
      :footer="null"
...
/**
 * Takt采购价格实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceField(record, 'priceStatus') ?? ''
  },
  {
    title: t('entity.purchasePrice.items'),
    dataIndex: 'items',
    key: 'items',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceField(record, 'items') ?? ''
  },
  {
    title: t('entity.purchasePrice.changelogs'),
    dataIndex: 'changeLogs',
    key: 'changeLogs',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPurchasePriceField(record, 'changeLogs') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.purchasePrice._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: PurchasePrice) {
  formTitle.value = t('common.page.button.edit') + t('entity.purchasePrice._self')
