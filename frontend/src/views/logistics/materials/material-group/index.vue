...
<!-- 命名空间：@/views/logistics/materials/material-group -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：Takt物料组主数据实体管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="logistics:materials:materialgroup:create"
      update-permission="logistics:materials:materialgroup:update"
      delete-permission="logistics:materials:materialgroup:delete"
      import-permission="logistics:materials:materialgroup:import"
      export-permission="logistics:materials:materialgroup:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      <MaterialGroupForm
        :key="formData?.materialGroupId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-logistics-materials-material-group'"
...
/**
 * Takt物料组主数据实体管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/logistics/materials/material-group
 */
...
import MaterialGroupForm from './components/material-group-form.vue'
import { getMaterialGroupList, getMaterialGroupById, createMaterialGroup, updateMaterialGroup, deleteMaterialGroupById, deleteMaterialGroupBatch, getMaterialGroupTemplate, importMaterialGroup, exportMaterialGroup } from '@/api/logistics/materials/material-group'
...
/** 分页列表数据 */
const dataSource = ref<MaterialGroup[]>([])
/** 当前页码 */
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<MaterialGroup> | null>(null)
/** 表单提交 loading */
...
const entityIdName = 'materialGroupId'
/** 工具栏「编辑」是否禁用（须恰好选中一行） */
 ...
 */
function buildListQuery(overrides?: Partial<MaterialGroupQuery>): MaterialGroupQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: MaterialGroupQuery = {
  ...
  const assignTrimmed = (key: keyof MaterialGroupQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'logistics:materials:materialgroup:update',
      ...
      },
      {
        ...
        permission: 'logistics:materials:materialgroup:delete',
...
const onClickRow = (record: MaterialGroup) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getMaterialGroupId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[MaterialGroup] 加载数据失败', { error })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getMaterialGroupTemplate(sheetName, fileName)
