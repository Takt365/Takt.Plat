...
<!-- 文件名称：index.vue -->
<!-- 功能描述：Quartz 定时任务实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="foundation-quartz-task">
    ...
    >
      ...
      <a-form-item :label="t('entity.quartzTask.jobtype')">
        <a-input
          v-model:value="advancedQueryForm.jobType"
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.quartzTask.jobtype') })"
          allow-clear
        />
      </a-form-item>
      <a-form-item :label="t('entity.quartzTask.jobparams')">
        <a-input
          v-model:value="advancedQueryForm.jobParams"
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.quartzTask._self')"
      :width="600"
      :footer="null"
...
/**
 * Quartz 定时任务实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: t('entity.quartzTask.jobtype'),
    dataIndex: 'jobType',
    key: 'jobType',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getQuartzTaskField(record, 'jobType') ?? ''
  },
  {
    title: t('entity.quartzTask.jobparams'),
    dataIndex: 'jobParams',
    key: 'jobParams',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getQuartzTaskField(record, 'jobParams') ?? ''
  },
  {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.quartzTask._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: QuartzTask) {
  formTitle.value = t('common.page.button.edit') + t('entity.quartzTask._self')
