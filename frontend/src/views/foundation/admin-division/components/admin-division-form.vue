...
<!-- 命名空间：@/views/foundation/admin-division/components -->
<!-- 文件名称：admin-division-form.vue -->
<!-- 功能描述：行政区划实体树表维护表单（ParentId + TaktTreeSelect），由 generate-vue-tree-from-api.cjs 自动生成.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="admin-division-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
        ...
        <div :class="formContentClass">
            ...
            <a-col :span="24">
              ...
              >
                <TaktTreeSelect
                  ...
                  api-url="TaktAdminDivisions/tree-options"
                  :lazy="true"
          ...
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              ...
              >
                <a-input
                  v-model:value="formState.divisionCode"
                  :placeholder="pi.ph('divisionCode')"
                  show-count
                  :maxlength="40"
                  allow-clear
                  :disabled="!!formData?.adminDivisionId"
            ...
            <a-col :span="12">
              ...
              >
                <a-input
                  v-model:value="formState.divisionName"
                  :placeholder="pi.ph('divisionName')"
                  show-count
                  :maxlength="200"
                  allow-clear
                />
            ...
            <a-col :span="12">
              ...
              >
                <a-input
                  v-model:value="formState.divisionPath"
                  :placeholder="pi.ph('divisionPath')"
                  show-count
                  :maxlength="500"
                  allow-clear
                />
...
/**
 * 行政区划实体维护表单 · 由 generate-vue-tree-from-api.cjs 根据 types/api 生成
 * @module views/foundation/admin-division/components
 */
import { reactive, watch, computed, ref, onMounted } from 'vue'
import { useI18n } from 'vue-i18n'
import TaktTreeSelect from '@/components/business/takt-tree-select/index.vue'
import type { Rule } from 'ant-design-vue/es/form'
import { useAdminDivisionI18n } from '../composables/use-admin-division-i18n'

/** 实体字段 i18n */
const pi = useAdminDivisionI18n()

import type { AdminDivisionCreate } from '@/types/foundation/admin-division'
...
interface Props {
  formData?: Partial<AdminDivisionCreate & { adminDivisionId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 adminDivisionId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.adminDivisionId) {
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.adminDivisionId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  divisionStatus: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(pi.ph('divisionStatus'))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(pi.ph('divisionStatus'))
...
function getValues(): Record<string, any> {
  ...
  if ('divisionStatus' in payload) {
    const rawdivisionStatus = payload.divisionStatus
    payload.divisionStatus = typeof rawdivisionStatus === 'number' ? rawdivisionStatus : Number(rawdivisionStatus)
  }
...
function resetFields() {
  ...
  applyFormDefaults(formState)
  applyScopeDefaults(formState as Record<string, unknown>, !props.formData?.adminDivisionId)
