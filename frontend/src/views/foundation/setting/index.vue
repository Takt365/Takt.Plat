...
<!-- 文件名称：index.vue -->
<!-- 功能描述：系统设置实体 存储系统的各种配置参数管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="foundation-setting">
    ...
    >
      ...
      <a-form-item :label="t('entity.setting.description')">
        <a-input
          ...
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.setting.description') })"
          allow-clear
        />
      </a-form-item>
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.setting._self')"
      :width="600"
      :footer="null"
...
/**
 * 系统设置实体 存储系统的各种配置参数管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getSettingField(record, 'isBuiltIn') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getSettingField(record, 'isReadonly') ?? ''
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getSettingField(record, 'isEncrypted') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.setting._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: Setting) {
  formTitle.value = t('common.page.button.edit') + t('entity.setting._self')
...
async function handleExport() {
  try {
    loading.value = true
    const kw = (queryKeyword.value ?? '').trim()
    const exportQuery: SettingQuery = {
      pageIndex: 1,
      pageSize: 100000,
      ...advancedQueryForm.value
    }
    ...
    const exportMeta = await exportSetting(exportQuery, excelNames.sheet, excelNames.fileBase)
    const ts = new Date()
