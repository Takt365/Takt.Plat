...
<!-- 文件名称：dict-data-panel.vue -->
<!-- 功能描述：字典类型实体 用于定义系统中使用的各种字典分类主表实体右侧明细 dictData 独立 CRUD（按主表选中 dictTypeId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="dict-data-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="dict-data-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="DICTDATA_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 字典类型实体 用于定义系统中使用的各种字典分类子表 dictData 右栏面板
...
import { useDictTypeMasterContext } from '../composables/use-dict-type-master-context'
import {
  getDictDataList,
  getDictDataById,
  createDictData,
  ...
  importDictData,
  exportDictData,
} from '@/api/foundation/dict-data'
import type { DictData, DictDataQuery } from '@/types/foundation/dict-data'

import {
  useDictDataI18n,
  DICTDATA_DEFAULT_VISIBLE_COLUMN_KEYS,
  DICTDATA_SUMMARY_SUM_FIELDS,
  DICTDATA_QUERY_STRING_FIELDS,
  DICTDATA_QUERY_FIELDS,
  DICTDATA_SELF_I18N_KEY,
} from '../composables/use-dict-data-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useDictDataI18n()

const { t } = useI18n()
const { selectedMasterRow } = useDictTypeMasterContext()

/** Excel 导入/导出默认 sheet 名与文件名前缀 */
const excelNames = taktExcelEntityNames('TaktDictData')
/** 快捷查询占位文案 */
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...DICTDATA_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...DICTDATA_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'dictDataId'
const masterDictTypeId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['dictTypeId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterDictTypeId.value !== '')
const updateDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length !== 1)
const deleteDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length === 0)

function getDictDataId(record: DictData | Record<string, unknown>): string {
  return String((record as DictData)?.[entityIdName] ?? '')
}

function getDictDataField(record: DictData | Record<string, unknown>, field: string): unknown {
  return (record as DictData)?.[field as keyof DictData]
}
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: DictData }) =>
      String(getDictDataField(record, 'dictDataId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: DictData }) =>
      String(getDictDataField(record, 'cultureCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: DictData }) =>
      String(getDictDataField(record, 'isDefault') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: DictData }) =>
      String(getDictDataField(record, 'remark') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: DictData) => void handleEdit(record),
      },
      {
        key: 'delete',
        ...
        onClick: (record: DictData) => void handleDeleteOne(record),
      },
    ],
  }),
])
...
const resolvedSummaryColumns = computed(() => {
  ...
  const merged = mergeDefaultColumns(userCols, t, true, 'tenant-culture')
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'dictDataId',
    actionColumnKey: 'action',
    tableMode: 'masterDetailDetail',
    entityScope: 'tenant-culture',
  })
})

const summarySumFieldSet = new Set<string>(DICTDATA_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    DICTDATA_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof DICTDATA_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of DICTDATA_SUMMARY_SUM_FIELDS) {
      const num = Number(getDictDataField(row, field))
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record: DictData, selected: boolean) => {
    ...
    } else if (selectedRow.value && getDictDataId(selectedRow.value) === getDictDataId(record)) {
 ...
 * @param record 行数据
 */
function onClickRow(record: DictData) {
  const key = getDictDataId(record)
  return {
    onClick: () => {
 ...
 * @returns {DictDataQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<DictDataQuery>): DictDataQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: DictDataQuery = {
    pageIndex: currentPage.value,
    pageSize: pageSize.value,
    dictTypeId: masterDictTypeId.value,
    ...overrides,
  }
  if (kw.length > 0) {
    query.keyWords = kw
  }
  const assignTrimmed = (key: keyof DictDataQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
      query[key] = v as never
    }
  }
  for (const key of DICTDATA_QUERY_STRING_FIELDS) {
...
async function loadData() {
  ...
  loading.value = true
  try {
    const res = await getDictDataList(buildListQuery())
...
async function handleEdit(record: DictData) {
  ...
  try {
    const detail = await getDictDataById(getDictDataId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.dictDataId
    if (id) {
      await updateDictData(id, payload)
...
async function handleDeleteOne(record: DictData) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteDictDataById(getDictDataId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getDictDataId(r)).filter(Boolean)
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getDictDataTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importDictData(file, sheetName)
...
async function handleExport() {
  ...
  try {
    ...
    const exportMeta = await exportDictData(
