...
<!-- 文件名称：index.vue -->
<!-- 功能描述：成本要素实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="accounting-controlling-cost-element">
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.costElement._self')"
      :width="600"
      :footer="null"
...
/**
 * 成本要素实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
import { getCostElementList, getCostElementById, createCostElement, updateCostElement, deleteCostElementById, deleteCostElementBatch, getCostElementTemplate, importCostElement, exportCostElement } from '@/api/accounting/controlling/cost-element'
import type { CostElement, CostElementQuery, CostElementCreate, CostElementUpdate } from '@/types/accounting/controlling/cost-element'
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getCostElementField(record, 'costElementStatus') ?? ''
  },
  {
...
const onClickRow = (record: CostElement) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getCostElementId(item)))
...
async function loadData() {
  ...
  try {
    ...
    const res = await getCostElementList(params)
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.costElement._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: CostElement) {
  formTitle.value = t('common.page.button.edit') + t('entity.costElement._self')
...
async function handleExport() {
  try {
    loading.value = true
    const kw = (queryKeyword.value ?? '').trim()
    const exportQuery: CostElementQuery = {
      pageIndex: 1,
      pageSize: 100000,
      ...advancedQueryForm.value
    }
    ...
    const exportMeta = await exportCostElement(exportQuery, excelNames.sheet, excelNames.fileBase)
    const ts = new Date()
