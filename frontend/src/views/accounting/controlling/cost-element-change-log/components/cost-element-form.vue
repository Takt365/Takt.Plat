...
<!-- 功能描述：成本要素实体维护弹窗内嵌表单（上主下从级联保存）。由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form cost-element-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="costElementChangeLogTableRef"
      v-model="childCostElementChangeLogRows"
      :columns="costElementChangeLogFormColumns"
      :title="t('entity.costelementchangelog._self')"
      :add-button-entity="t('entity.costelementchangelog._self')"
      id-field="costElementChangeLogId"
      :default-row="createDefaultCostElementChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
/**
 * 成本要素实体维护表单 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","costElementCode","costElementName","costElementType","costElementCategory","parentId","costElementLevel","costElementStatus","validFrom","validTo","extField","remark"]

import type { TaktEditableTableColumn } from '@/components/business/takt-editable-table/types'

const childCostElementChangeLogRows = ref<Record<string, unknown>[]>([])
const costElementChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 costElementChangeLog 可编辑列 */
const costElementChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'changeFields',
    title: t('entity.costelementchangelog.changefields'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.costelementchangelog.changefields') }),
  },
  {
    key: 'changeTime',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.costelementchangelog.changeby') }),
  },
  {
    key: 'changeReason',
    title: t('entity.costelementchangelog.changereason'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.costelementchangelog.changereason') }),
  },
  {
    key: 'extField',
...
function syncChildRowsFromFormData(val: Partial<CostElementCreate & { costElementId?: string }> | null | undefined) {
  childCostElementChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultCostElementChangeLogRow(): Record<string, unknown> {
  return {
    changeFields: '',
    changeTime: '',
    changeBy: '',
...
function buildSubmitPayload() {
  const masterId = props.formData?.costElementId ?? ''
  return {
    ...formState,
    changeLogs: costElementChangeLogTableRef.value?.getRows?.() ?? childCostElementChangeLogRows.value.map((rest) => ({
      ...rest,
      tenantCode: tenantStore.tenantCode,
      companyCode: tenantStore.companyCode,
      companyDefaultCulture: userStore.userInfo?.companyDefaultCulture ?? '',
      costElementId: masterId,
    })),
  }
}
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  parentId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.costelement.parentid') }),
      trigger: 'blur'
    }
  ],
  costElementLevel: [{
...
async function validate() {
  ...
  await costElementChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childCostElementChangeLogRows.value = []
  costElementChangeLogTableRef.value?.resetRows?.()
