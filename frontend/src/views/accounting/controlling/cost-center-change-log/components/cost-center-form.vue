...
<!-- 功能描述：成本中心实体维护弹窗内嵌表单（上主下从级联保存）。由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form cost-center-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
...
/**
 * 成本中心实体维护表单 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
...
function syncChildRowsFromFormData(val: Partial<CostCenterCreate & { costCenterId?: string }> | null | undefined) {
  childCostCenterChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultCostCenterChangeLogRow(): Record<string, unknown> {
  return {
    changeFields: '',
    changeTime: '',
    changeBy: '',
...
function buildSubmitPayload() {
  const masterId = props.formData?.costCenterId ?? ''
  return {
    ...formState,
    changeLogs: costCenterChangeLogTableRef.value?.getRows?.() ?? childCostCenterChangeLogRows.value.map((rest) => ({
      ...rest,
      tenantCode: tenantStore.tenantCode,
      companyCode: tenantStore.companyCode,
      companyDefaultCulture: userStore.userInfo?.companyDefaultCulture ?? '',
      costCenterId: masterId,
    })),
  }
}
...
async function validate() {
  ...
  await costCenterChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childCostCenterChangeLogRows.value = []
  costCenterChangeLogTableRef.value?.resetRows?.()
