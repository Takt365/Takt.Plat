...
<!-- 文件名称：index.vue -->
<!-- 功能描述：资产实体管理页面，含查询、增删改，由 generate-vue-master-detail-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getAssetId"
      :master-row-selection="rowSelection"
      master-id-column-key="assetId"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-accounting-financial-asset-change-log'"
...
/**
 * 资产实体管理页 · 由 generate-vue-master-detail-from-api.cjs 根据 types/api 生成
...
import AssetChangeLogPanel from './components/asset-change-log-panel.vue'
import { provideAssetMasterContext } from './composables/use-asset-master-context'
...
/** 主表选中行上下文（右侧明细面板读取） */
const { selectedMasterRow } = provideAssetMasterContext()
const assetChangeLogPanelRef = ref<InstanceType<typeof AssetChangeLogPanel> | null>(null)
...
function syncMasterSelection(record: Asset | null) {
  selectedMasterRow.value = record
  selectedMasterKey.value = record ? getAssetId(record) : ''
}
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as Asset
  const key = getAssetId(row)
...
async function loadAssetDetail(record: Asset): Promise<Asset | null> {
  ...
  try {
    ...
    const index = dataSource.value.findIndex((row) => getAssetId(row) === id)
    if (index !== -1) {
      dataSource.value[index] = { ...dataSource.value[index], ...detail } as Asset
    }
    return detail
  } catch (error: any) {
...
/** 打开编辑弹窗（主子表：先拉详情含子表） */
async function handleEdit(record: Asset) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.asset._self') })
  formLoading.value = true
  try {
    const detail = await loadAssetDetail(record)
...
async function handleFormSubmit() {
  ...
  assetChangeLogPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
