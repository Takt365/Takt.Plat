...
<!-- 文件名称：account-title-change-log-panel.vue -->
<!-- 功能描述：会计科目实体主表实体右侧明细 accountTitleChangeLog 独立 CRUD（按主表选中 accountTitleId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="account-title-change-log-panel flex h-full min-h-0 flex-col overflow-hidden">
    <div class="mb-2 text-sm font-medium text-text">
      {{ t('entity.accounttitlechangelog._self') }}
    </div>
    <TaktQueryBar
    ...
    />
    <div class="account-title-change-log-panel__table-wrap min-h-0 flex-1 overflow-hidden">
      <TaktSingleTable
    ...
    >
      <AccountTitleChangeLogForm
        ref="formRef"
        :form-data="formData"
        :master-id="masterAccountTitleId"
        :loading="formLoading"
      />
    </TaktModal>
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-accounting-financial-account-title-change-log-account-title-change-log"
    ...
    >
      ...
      <a-form-item :label="t('entity.accounttitlechangelog.changetimestart')">
        <a-date-picker
          v-model:value="advancedQueryForm.changeTimeStart"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.accounttitlechangelog.changetimestart') })"
      ...
      <a-form-item :label="t('entity.accounttitlechangelog.changetimeend')">
        <a-date-picker
          v-model:value="advancedQueryForm.changeTimeEnd"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.accounttitlechangelog.changetimeend') })"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
...
/**
 * 会计科目实体子表 accountTitleChangeLog 右栏面板
...
import AccountTitleChangeLogForm from './account-title-change-log-form.vue'
import { useAccountTitleMasterContext } from '../composables/use-account-title-master-context'
import {
  getAccountTitleChangeLogList,
  getAccountTitleChangeLogById,
  createAccountTitleChangeLog,
  updateAccountTitleChangeLog,
  deleteAccountTitleChangeLogById,
  deleteAccountTitleChangeLogBatch,
  exportAccountTitleChangeLog,
} from '@/api/accounting/financial/account-title-change-log'
import type { AccountTitleChangeLog, AccountTitleChangeLogQuery } from '@/types/accounting/financial/account-title-change-log'

const { t } = useI18n()
const { selectedMasterRow } = useAccountTitleMasterContext()
...
const queryFieldsMeta = computed(() => [
  ...
  { key: 'changeBy', label: t('entity.accounttitlechangelog.changeby') },
  { key: 'changeReason', label: t('entity.accounttitlechangelog.changereason') },
...
const hasMasterSelection = computed(() => !!selectedMasterRow.value?.accountTitleId)
const masterAccountTitleId = computed(() => selectedMasterRow.value?.accountTitleId ?? '')
const updateDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length !== 1)
const deleteDisabled = computed(() => !hasMasterSelection.value || selectedRows.value.length === 0)

function getAccountTitleChangeLogId(record: AccountTitleChangeLog | Record<string, unknown>): string {
  return String((record as AccountTitleChangeLog)?.[entityIdName] ?? '')
}

function getAccountTitleChangeLogField(record: AccountTitleChangeLog | Record<string, unknown>, field: string): unknown {
  return (record as AccountTitleChangeLog)?.[field as keyof AccountTitleChangeLog]
}
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: AccountTitleChangeLog }) =>
      String(getAccountTitleChangeLogField(record, 'changeFields') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: AccountTitleChangeLog }) =>
      String(getAccountTitleChangeLogField(record, 'changeTime') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: AccountTitleChangeLog }) =>
      String(getAccountTitleChangeLogField(record, 'changeBy') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: AccountTitleChangeLog }) =>
      String(getAccountTitleChangeLogField(record, 'changeReason') ?? ''),
  },
  {
    title: t('entity.accounttitlechangelog.accounttitle'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: AccountTitleChangeLog }) =>
      String(getAccountTitleChangeLogField(record, 'accountTitle') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: AccountTitleChangeLog) => void handleEdit(record),
      },
      {
        key: 'delete',
        ...
        onClick: (record: AccountTitleChangeLog) => void handleDeleteOne(record),
      },
    ],
  }),
])
 ...
 * @param record 行数据
 */
function onClickRow(record: AccountTitleChangeLog) {
 ...
 * @returns {AccountTitleChangeLogQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<AccountTitleChangeLogQuery>): AccountTitleChangeLogQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: AccountTitleChangeLogQuery = {
    pageIndex: currentPage.value,
    pageSize: pageSize.value,
    accountTitleId: masterAccountTitleId.value,
    ...overrides,
  }
  if (kw.length > 0) {
    query.keyWords = kw
  }
  const assignTrimmed = (key: keyof AccountTitleChangeLogQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
async function loadData() {
  ...
  loading.value = true
  try {
    const res = await getAccountTitleChangeLogList(buildListQuery())
...
function handleCreate() {
  ...
  formTitle.value = t('common.dialog.title.create', { entity: t('entity.accounttitlechangelog._self') })
  formData.value = {}
  formVisible.value = true
}

async function handleEdit(record: AccountTitleChangeLog) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.accounttitlechangelog._self') })
  formLoading.value = true
  try {
    const detail = await getAccountTitleChangeLogById(getAccountTitleChangeLogId(record))
...
function handleUpdate() {
  if (selectedRow.value) {
    ...
    message.warning(t('common.tip.select.to.action', {
      ...
      entity: t('entity.accounttitlechangelog._self'),
    }))
  }
}
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.accountTitleChangeLogId
    if (id) {
      await updateAccountTitleChangeLog(id, payload)
      message.success(t('common.feedback.updated', { target: t('entity.accounttitlechangelog._self') }))
    } else {
      await createAccountTitleChangeLog(payload)
...
async function handleDeleteOne(record: AccountTitleChangeLog) {
  Modal.confirm({
    ...
    content: t('common.tip.confirm.delete.entity', {
      entity: t('entity.accounttitlechangelog._self'),
      name: t('common.tip.this.target', { target: t('entity.accounttitlechangelog._self') }),
    }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      await deleteAccountTitleChangeLogById(getAccountTitleChangeLogId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getAccountTitleChangeLogId(r)).filter(Boolean)
...
async function handleExport() {
  ...
  try {
    loading.value = true
    const exportMeta = await exportAccountTitleChangeLog(
    ...
    const err = error as { message?: string }
    message.error(err?.message || t('common.feedback.export.failed', { target: t('entity.accounttitlechangelog._self') }))
  } finally {
    loading.value = false
  }
}
