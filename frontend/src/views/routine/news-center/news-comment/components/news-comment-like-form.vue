...
<!-- 功能描述：新闻中心评论实体 支持多级回复子表 newsCommentLike 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form news-comment-like-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
            ...
            <a-col :span="12">
              ...
              >
                <a-date-picker
                  ...
                  :placeholder="t('common.page.form.placeholder.select', { field: t('entity.newscommentlike.liketime') })"
...
/**
 * 新闻中心评论实体 支持多级回复子表 newsCommentLike 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
const formFields = ["commentId","userId","userName","likeTime","extField","remark"]
...
/** 编辑态灌入 formData；新增态恢复默认值（须含 newsCommentLikeId 才视为编辑） */
watch(
  () => props.formData,
  (val) => {
    if (val?.newsCommentLikeId) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  likeTime: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.newscommentlike.liketime') }),
      trigger: 'change'
    }
  ],
}))
...
/** 映射为 Create/Update DTO（含主表外键 newsCommentId） */
function getValues(): Record<string, any> {
  const payload = { ...formState }
  if ('sortOrder' in payload) delete payload.sortOrder
  payload.newsCommentId = props.masterId
  return payload
}
