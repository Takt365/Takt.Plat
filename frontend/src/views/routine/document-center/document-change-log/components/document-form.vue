...
<template>
  ...
  >
    ...
    <!-- 下：子表 changeLogs -->
    <TaktEditableTable
      ref="documentChangeLogTableRef"
      v-model="childDocumentChangeLogRows"
      :columns="documentChangeLogFormColumns"
      :title="t('entity.documentchangelog._self')"
      :add-button-entity="t('entity.documentchangelog._self')"
      id-field="documentChangeLogId"
      :default-row="createDefaultDocumentChangeLogRow"
      :disabled="loading"
      section-border
    />
  </a-form>
...
const documentChangeLogTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 documentChangeLog 可编辑列 */
const documentChangeLogFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'documentTitle',
    title: t('entity.documentchangelog.documenttitle'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.documentchangelog.documenttitle') }),
  },
  {
    key: 'changeType',
  ...
  },
  {
    ...
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.documentchangelog.changesummary') }),
  },
  {
    key: 'changeFields',
    title: t('entity.documentchangelog.changefields'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.documentchangelog.changefields') }),
  },
  {
    key: 'changeReason',
    title: t('entity.documentchangelog.changereason'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: t('common.page.form.placeholder.optional', { field: t('entity.documentchangelog.changereason') }),
  },
  {
    key: 'versionAtChange',
  ...
  {
    key: 'ExtField',
    title: t('entity.documentchangelog.extfield'),
    editor: 'textarea',
    rows: 1,
...
function syncChildRowsFromFormData(val: Partial<DocumentCreate & { documentId?: string }> | null | undefined) {
  childDocumentChangeLogRows.value = ((val as any)?.changeLogs ?? []) as Record<string, unknown>[]
}

function createDefaultDocumentChangeLogRow(): Record<string, unknown> {
  return {
    documentTitle: '',
    changeType: 0,
    changeSummary: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    changeLogs: documentChangeLogTableRef.value?.getRows?.() ?? childDocumentChangeLogRows.value.map((rest) => ({
      ...rest,
...
async function validate() {
  ...
  await documentChangeLogTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  documentChangeLogTableRef.value?.resetRows?.()
