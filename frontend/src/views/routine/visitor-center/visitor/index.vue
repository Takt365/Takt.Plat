...
<!-- 文件名称：index.vue -->
<!-- 功能描述：来访接待主实体管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="routine-visitor-center-visitor">
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.visitor._self')"
      :width="600"
      :footer="null"
...
/**
 * 来访接待主实体管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: t('entity.visitor.companions'),
    dataIndex: 'companions',
    key: 'companions',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getVisitorField(record, 'companions') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.visitor._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: Visitor) {
  formTitle.value = t('common.page.button.edit') + t('entity.visitor._self')
