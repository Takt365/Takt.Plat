...
<!-- 命名空间：@/views/routine/visitor-center/visitor-companion/components -->
<!-- 文件名称：visitor-companion-form.vue -->
<!-- 功能描述：来访人员子实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="visitor-companion-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.visitorCompanion.visitorid')"
                name="visitorId"
              >
                <a-input
                  v-model:value="formState.visitorId"
...
/**
 * 来访人员子实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/routine/visitor-center/visitor-companion/components
 */
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","visitorId","department","jobTitle","companionName","extFieldJson","remark"]


interface Props {
  formData?: Partial<VisitorCompanionCreate & { visitorCompanionId?: string }> | null
  loading?: boolean
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.visitorCompanionId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  visitorId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.visitorCompanion.visitorid') }),
      trigger: 'blur'
    }
  ],
  department: [
