...
<!-- 命名空间：@/views/routine/conference-center/conference-room/components -->
<!-- 文件名称：conference-room-form.vue -->
<!-- 功能描述：会议室实体 维护线下会议室编码、位置、容量与设施维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="conference-room-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * 会议室实体 维护线下会议室编码、位置、容量与设施维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/routine/conference-center/conference-room/components
 */
...
const formFields = ["tenantCode","companyCode","cultureCode","roomCode","roomName","building","floor","locationDetail","capacity","facilities","roomStatus","extField","remark"]

/** 父级传入的编辑 DTO；新增时为 undefined 或空对象 */
interface Props {
  formData?: Partial<ConferenceRoomCreate & { conferenceRoomId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.conferenceRoomId
    if (isCreate) {
