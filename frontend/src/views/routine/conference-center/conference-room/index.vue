...
<!-- 命名空间：@/views/routine/conference-center/conference-room -->
<!-- 文件名称：index.vue -->
...
<template>
  <div class="p-4">
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-routine-conference-center-conference-room'"
 ...
 * @module views/routine/conference-center/conference-room
 */
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<ConferenceRoom | null>(null)
/** 表格多选行 */
const selectedRows = ref<ConferenceRoom[]>([])
/** 表格多选 row-key 集合 */
...
const entityIdName = 'conferenceRoomId'
/** 工具栏「编辑」是否禁用（须恰好选中一行） */
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getConferenceRoomField(record, 'roomStatus') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
/** 表格 row-key（优先实体主键字段） */
const getConferenceRoomId = (record: any): string => record?.[entityIdName] ?? ''
/**
 * 读取行字段值
 * @param record 行数据
 * @param field 字段名
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: ConferenceRoom[]) => {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: ConferenceRoom[]) => {
    selectedRow.value = selected && selectedRowsData.length === 1 ? (selectedRowsData[0] ?? null) : null
  }
}))

/** 行点击切换选中（与 rowSelection 联动） */
const onClickRow = (record: ConferenceRoom) => ({
  onClick: () => {
    const key = getConferenceRoomId(record)
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getConferenceRoomId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[ConferenceRoom] 加载数据失败', { error })
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getConferenceRoomTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件 */
async function handleImportFile(file: File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> {
  return await importConferenceRoom(file, sheetName)
}
