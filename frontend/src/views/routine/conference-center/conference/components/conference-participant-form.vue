...
<!-- 功能描述：会议中心主实体 支持内部/外部/视频/混合会议排期、议程及参与人管理子表 conferenceParticipant 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form conference-participant-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
...
/**
 * 会议中心主实体 支持内部/外部/视频/混合会议排期、议程及参与人管理子表 conferenceParticipant 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
watch(
  () => props.formData,
  (val) => {
    if (val?.conferenceParticipantId) {
