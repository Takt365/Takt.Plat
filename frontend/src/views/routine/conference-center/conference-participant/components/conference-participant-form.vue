...
<!-- 命名空间：@/views/routine/conference-center/conference-participant/components -->
<!-- 文件名称：conference-participant-form.vue -->
<!-- 功能描述：会议参与人子实体维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="conference-participant-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.conferenceParticipant.conferenceid')"
                name="conferenceId"
              >
                <a-input
...
/**
 * 会议参与人子实体维护表单 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/routine/conference-center/conference-participant/components
 */
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","conferenceId","userId","userName","participantRole","attendanceStatus","checkInTime","checkOutTime","extFieldJson","remark"]


interface Props {
  formData?: Partial<ConferenceParticipantCreate & { conferenceParticipantId?: string }> | null
  loading?: boolean
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.conferenceParticipantId
    if (isCreate) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  conferenceId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.conferenceParticipant.conferenceid') }),
      trigger: 'blur'
    }
  ],
  userId: [
  ...
  participantRole: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.conferenceParticipant.participantrole') }),
      trigger: 'change'
    }
  ],
  attendanceStatus: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.conferenceParticipant.attendancestatus') }),
      trigger: 'change'
    }
  ],
}))
