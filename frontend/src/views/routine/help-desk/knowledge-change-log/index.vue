...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getKnowledgeId"
      :master-row-selection="rowSelection"
      master-id-column-key="knowledgeId"
    ...
    >
      <template #detail>
        <KnowledgeChangeLogPanel
          ref="knowledgeChangeLogPanelRef"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-routine-help-desk-knowledge-change-log'"
...
import KnowledgeChangeLogPanel from './components/knowledge-change-log-panel.vue'
import { provideKnowledgeMasterContext } from './composables/use-knowledge-master-context'
...
/** 主表选中行上下文（右侧明细面板读取） */
const { selectedMasterRow } = provideKnowledgeMasterContext()
const knowledgeChangeLogPanelRef = ref<InstanceType<typeof KnowledgeChangeLogPanel> | null>(null)
...
function syncMasterSelection(record: Knowledge | null) {
  selectedMasterRow.value = record
  selectedMasterKey.value = record ? getKnowledgeId(record) : ''
}
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as Knowledge
  const key = getKnowledgeId(row)
...
async function handleFormSubmit() {
  ...
  knowledgeChangeLogPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
