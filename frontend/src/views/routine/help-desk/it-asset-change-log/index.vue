...
<!-- 命名空间：@/views/routine/help-desk/it-asset-change-log -->
<!-- 文件名称：index.vue -->
...
<template>
  <div class="p-4 flex flex-col min-h-0 h-full">
    ...
    <TaktMasterDetailTableLr
      ...
      :master-row-key="getItAssetId"
      :master-row-selection="rowSelection"
      master-id-column-key="itAssetId"
    ...
    >
      <template #detail>
        <ItAssetChangeLogPanel
          ref="itAssetChangeLogPanelRef"
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-routine-help-desk-it-asset-change-log'"
 ...
 * @module views/routine/help-desk/it-asset-change-log
 */
...
import ItAssetChangeLogPanel from './components/it-asset-change-log-panel.vue'
import { provideItAssetMasterContext } from './composables/use-it-asset-master-context'
...
/** 主表选中行上下文（右侧明细面板读取） */
const { selectedMasterRow } = provideItAssetMasterContext()
const itAssetChangeLogPanelRef = ref<InstanceType<typeof ItAssetChangeLogPanel> | null>(null)
...
function syncMasterSelection(record: ItAsset | null) {
  selectedMasterRow.value = record
  selectedMasterKey.value = record ? getItAssetId(record) : ''
}
...
function handleMasterSelect(record: Record<string, unknown>) {
  const row = record as unknown as ItAsset
  const key = getItAssetId(row)
...
async function handleFormSubmit() {
  ...
  itAssetChangeLogPanelRef.value?.reload?.()
    }
    loadData()
  } finally {
    formLoading.value = false
