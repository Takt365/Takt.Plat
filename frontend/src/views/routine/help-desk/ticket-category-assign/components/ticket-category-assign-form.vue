...
<!-- 命名空间：@/views/routine/help-desk/ticket-category-assign/components -->
<!-- 文件名称：ticket-category-assign-form.vue -->
<!-- 功能描述：工单分类默认处理人维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
/**
 * 工单分类默认处理人维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/routine/help-desk/ticket-category-assign/components
 */
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","categoryCode","assigneeId","assigneeName","sortOrder","extFieldJson","remark"]


/** 父级传入的编辑 DTO；新增时为 undefined 或空对象 */
interface Props {
  formData?: Partial<TicketCategoryAssignCreate & { ticketCategoryAssignId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.ticketCategoryAssignId
    if (isCreate) {
