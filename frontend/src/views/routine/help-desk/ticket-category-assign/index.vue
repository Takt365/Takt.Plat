...
<!-- 文件名称：index.vue -->
<!-- 功能描述：工单分类默认处理人管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="routine:help:desk:ticket:category:assign:create"
      update-permission="routine:help:desk:ticket:category:assign:update"
      delete-permission="routine:help:desk:ticket:category:assign:delete"
      import-permission="routine:help:desk:ticket:category:assign:import"
      export-permission="routine:help:desk:ticket:category:assign:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      <TicketCategoryAssignForm
        :key="formData?.ticketCategoryAssignId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-routine-help-desk-ticket-category-assign'"
    ...
    <TaktModal
      ...
      :title="t('common.dialog.title.import', { entity: t('entity.ticketcategoryassign._self') })"
      :width="600"
      :footer="null"
...
/**
 * 工单分类默认处理人管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
...
import type { TicketCategoryAssign, TicketCategoryAssignQuery } from '@/types/routine/help-desk/ticket-category-assign'
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<TicketCategoryAssign> | null>(null)
/** 表单提交 loading */
...
const queryFieldsMeta = computed(() => [
  { key: 'categoryCode', label: t('entity.ticketcategoryassign.categorycode') },
  { key: 'assigneeId', label: t('entity.ticketcategoryassign.assigneeid') },
  { key: 'assigneeName', label: t('entity.ticketcategoryassign.assigneename') },
 ...
 * @returns {TicketCategoryAssignQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<TicketCategoryAssignQuery>): TicketCategoryAssignQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: TicketCategoryAssignQuery = {
  ...
  const assignTrimmed = (key: keyof TicketCategoryAssignQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'routine:help:desk:ticket:category:assign:update',
      ...
      },
      {
        ...
        permission: 'routine:help:desk:ticket:category:assign:delete',
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record: TicketCategoryAssign, selected: boolean) => {
    ...
    } else if (selectedRow.value && getTicketCategoryAssignId(selectedRow.value) === getTicketCategoryAssignId(record)) {
...
async function loadData() {
  loading.value = true
  try {
    const res = await getTicketCategoryAssignList(buildListQuery())
...
function handleCreate() {
  formTitle.value = t('common.dialog.title.create', { entity: t('entity.ticketcategoryassign._self') })
  formData.value = null
...
function handleEdit(record: TicketCategoryAssign) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.ticketcategoryassign._self') })
...
async function handleExport() {
  try {
    loading.value = true
    const exportMeta = await exportTicketCategoryAssign(
