...
const rowSelection = computed(() => ({
  ...
  onSelectAll: (selected: boolean, selectedRowsData: SelfService[]) => {
...
const onClickRow = (record: SelfService) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getSelfServiceId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[SelfService] 加载数据失败', { error })
