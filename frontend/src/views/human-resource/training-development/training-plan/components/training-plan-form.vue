...
<!-- 命名空间：@/views/human-resource/training-development/training-plan/components -->
<!-- 文件名称：training-plan-form.vue -->
<!-- 功能描述：培训计划维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
            ...
            <a-col :span="12">
              ...
              >
                <a-input-number
                  ...
                  :placeholder="t('common.page.form.placeholder.required', { field: t('entity.trainingPlan.status') })"
                  size="small"
                  style="width: 100%"
                />
              </a-form-item>
...
/**
 * 培训计划维护表单 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/human-resource/training-development/training-plan/components
 */
import { reactive, watch, computed, ref } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import type { TrainingPlanCreate } from '@/types/human-resource/training-development/training-plan'
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","planCode","planName","planYear","planType","applicableDepartment","startDate","endDate","trainingObjectives","plannedHeadcount","trainingBudget","description","trainingPlanStatus","relatedPlant","extFieldJson","remark"]
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  planYear: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.trainingPlan.planyear') }),
      trigger: 'change'
    }
  ],
  planType: [
  ...
  plannedHeadcount: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.trainingPlan.plannedheadcount') }),
      trigger: 'change'
    }
  ],
  trainingBudget: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.trainingPlan.trainingbudget') }),
      trigger: 'change'
    }
  ],
  description: [
  ...
  trainingPlanStatus: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.trainingPlan.status') }),
      trigger: 'change'
    }
  ],
}))
