...
<!-- 文件名称：index.vue -->
<!-- 功能描述：假日实体 假日条目管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- 免责声明：此软件使用 MIT License，作者不承担任何使用风险。 -->
<!-- ======================================== -->

<template>
  <div class="human-resource-attendance-leave-holiday">
    <!-- 查询栏 -->
    <TaktQueryBar
    ...
    >
      ...
      <a-form-item :label="t('entity.holiday.greeting')">
        <a-input
          ...
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.holiday.greeting') })"
          allow-clear
        />
      </a-form-item>
      <a-form-item :label="t('entity.holiday.quote')">
        <a-input
          ...
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.holiday.quote') })"
          allow-clear
        />
      </a-form-item>
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.holiday._self')"
      :width="600"
      :footer="null"
...
/**
 * 假日实体 假日条目管理页 · 由 generate-vue-from-api 根据 types/api 生成
 * @module views/human-resource/attendance-leave/holiday
 */
...
import type { Holiday, HolidayQuery, HolidayCreate, HolidayUpdate } from '@/types/human-resource/attendance/holiday'
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getHolidayField(record, 'holidayType') ?? ''
  },
  {
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getHolidayField(record, 'isWorkingDay') ?? ''
  },
  {
...
const rowSelection = computed(() => ({
  ...
  onSelect: (record: Holiday, selected: boolean) => {
    ...
    } else if (getHolidayId(selectedRow.value) === getHolidayId(record)) {
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.holiday._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: Holiday) {
  formTitle.value = t('common.page.button.edit') + t('entity.holiday._self')
...
async function handleExport() {
  try {
    loading.value = true
    const kw = (queryKeyword.value ?? '').trim()
    const exportQuery: HolidayQuery = {
      pageIndex: 1,
      pageSize: 100000,
      ...advancedQueryForm.value
    }
    ...
    const exportMeta = await exportHoliday(exportQuery, excelNames.sheet, excelNames.fileBase)
    const ts = new Date()
...
.human-resource-attendance-leave-holiday {
  padding: 16px;
  display: flex;
