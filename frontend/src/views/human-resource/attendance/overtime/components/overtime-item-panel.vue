...
<template>
  <div class="overtime-item-panel flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div
      ...
      class="overtime-item-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('plannedHours')">
        <a-input-number
          v-model:value="advancedQueryForm.plannedHours"
          :placeholder="pi.queryPh('plannedHours', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('actualHours')">
        <a-input-number
          v-model:value="advancedQueryForm.actualHours"
          :placeholder="pi.queryPh('actualHours', 'required')"
          style="width: 100%"
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="OVERTIMEITEM_SELF_I18N_KEY"
        file-type="xlsx"
...
import {
  useOvertimeItemI18n,
  OVERTIMEITEM_DEFAULT_VISIBLE_COLUMN_KEYS,
  OVERTIMEITEM_SUMMARY_SUM_FIELDS,
  OVERTIMEITEM_QUERY_STRING_FIELDS,
  OVERTIMEITEM_QUERY_FIELDS,
  OVERTIMEITEM_SELF_I18N_KEY,
} from '../composables/use-overtime-item-i18n'
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of OVERTIMEITEM_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(OVERTIMEITEM_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
    (typeof OVERTIMEITEM_QUERY_STRING_FIELDS)[number],
    string
  >
  return {
    ...form,
...
const queryFieldsMeta = computed(() =>
  OVERTIMEITEM_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...OVERTIMEITEM_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...OVERTIMEITEM_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'overtimeItemId'
const masterOvertimeId = computed((): string => {
  const id = (selectedMasterRow.value as Record<string, unknown> | null)?.['overtimeId']
  return id != null ? String(id) : ''
})
const hasMasterSelection = computed(() => masterOvertimeId.value !== '')
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: OvertimeItem }) =>
      String(getOvertimeItemField(record, 'overtimeId') ?? ''),
  },
  {
    title: pi.label('overtimeName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: OvertimeItem }) =>
      String(getOvertimeItemField(record, 'overtimeName') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: OvertimeItem }) =>
      String(getOvertimeItemField(record, 'isObsolete') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const summarySumFieldSet = new Set<string>(OVERTIMEITEM_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    OVERTIMEITEM_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof OVERTIMEITEM_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of OVERTIMEITEM_SUMMARY_SUM_FIELDS) {
      const num = Number(getOvertimeItemField(row, field))
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importOvertimeItem(file, sheetName)
