...
<!-- 命名空间：@/views/human-resource/attendance/work-shift/components -->
<!-- 文件名称：work-shift-form.vue -->
<!-- 功能描述：班次定义维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
/**
 * 班次定义维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
 * @module views/human-resource/attendance/work-shift/components
 */
...
const formFields = ["tenantCode","companyCode","cultureCode","shiftCode","shiftName","startTime","endTime","crossMidnight","plantCode","extField","remark"]

/** 父级传入的编辑 DTO；新增时为 undefined 或空对象 */
interface Props {
  formData?: Partial<WorkShiftCreate & { workShiftId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
}
...
watch(
  ...
  () => {
    const isCreate = !props.formData?.workShiftId
    if (isCreate) {
