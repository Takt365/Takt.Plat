...
<!-- 文件名称：index.vue -->
<!-- 功能描述：工厂日历管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="human:resource:attendance:calendar:create"
      update-permission="human:resource:attendance:calendar:update"
      delete-permission="human:resource:attendance:calendar:delete"
      import-permission="human:resource:attendance:calendar:import"
      export-permission="human:resource:attendance:calendar:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      <CalendarForm
        :key="formData?.calendarId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-human-resource-attendance-calendar'"
      :form-model="advancedQueryForm"
      @submit="handleAdvancedQuerySubmit"
      @reset="handleAdvancedQueryReset"
    >
      <template #default="{ isFieldVisible }">
      <div v-show="isFieldVisible('calendarDateStart')">
      <a-form-item :label="t('entity.calendar.datestart')">
        <a-date-picker
          v-model:value="advancedQueryForm.calendarDateStart"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.calendar.datestart') })"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('calendarDateEnd')">
      <a-form-item :label="t('entity.calendar.dateend')">
        <a-date-picker
          v-model:value="advancedQueryForm.calendarDateEnd"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.calendar.dateend') })"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <div v-show="isFieldVisible('holidayId')">
    ...
    <TaktModal
      ...
      :title="t('common.dialog.title.import', { entity: t('entity.calendar._self') })"
      :width="600"
      :footer="null"
...
/**
 * 工厂日历管理页 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
...
import type { Calendar, CalendarQuery } from '@/types/human-resource/attendance/calendar'
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<Calendar> | null>(null)
/** 表单提交 loading */
...
const queryFieldsMeta = computed(() => [
  ...
  { key: 'plantCode', label: t('entity.calendar.relatedplant') },
 ...
 * @returns {CalendarQuery} 查询 DTO
 */
function buildListQuery(overrides?: Partial<CalendarQuery>): CalendarQuery {
  ...
  const assignTrimmed = (key: keyof CalendarQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
      query[key] = v as never
    }
  }
  assignTrimmed('calendarDateStart', form.calendarDateStart)
  assignTrimmed('calendarDateEnd', form.calendarDateEnd)
  if (form.isWorkingDay !== undefined && form.isWorkingDay !== null) {
    query.isWorkingDay = form.isWorkingDay
  }
  assignTrimmed('holidayId', form.holidayId)
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getCalendarField(record, 'plantCode') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'human:resource:attendance:calendar:update',
      ...
      },
      {
        ...
        permission: 'human:resource:attendance:calendar:delete',
...
async function loadData() {
  loading.value = true
  try {
    const res = await getCalendarList(buildListQuery())
...
function handleCreate() {
  formTitle.value = t('common.dialog.title.create', { entity: t('entity.calendar._self') })
  formData.value = null
...
function handleEdit(record: Calendar) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.calendar._self') })
...
async function handleExport() {
  try {
    loading.value = true
    const exportMeta = await exportCalendar(
