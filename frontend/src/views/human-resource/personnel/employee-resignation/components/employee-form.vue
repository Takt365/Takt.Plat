...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeResignations -->
    <TaktEditableTable
      ref="employeeResignationTableRef"
      v-model="childEmployeeResignationRows"
      :columns="employeeResignationFormColumns"
      :title="employeeResignationPi.self()"
      :add-button-entity="employeeResignationPi.self()"
      id-field="employeeResignationId"
      :default-row="createDefaultEmployeeResignationRow"
      :disabled="loading"
      :enable-vertical-scroll="false"
      section-border
      class="w-full min-w-0"
    >
      <template #cell-resignationType="{ record }">
        <TaktSelect
          v-model:value="record.resignationType"
          dict-type="hr_resignation_category"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeResignationPi.ph('resignationType')"
          :disabled="loading"
          allow-clear
        />
      </template>
...
const employeeResignationPi = useEmployeeResignationI18n()
...
const childEmployeeResignationRows = ref<Record<string, unknown>[]>([])
const employeeResignationTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeResignation 可编辑列 */
const employeeResignationFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'resignationType',
    title: employeeResignationPi.label('resignationType'),
    width: 140,
  },
  {
    key: 'applyDate',
    title: employeeResignationPi.label('applyDate'),
    editor: 'datePicker',
  ...
  {
    key: 'lastWorkDate',
    title: employeeResignationPi.label('lastWorkDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
  {
    key: 'terminationDate',
    title: employeeResignationPi.label('terminationDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
  {
    key: 'reason',
    title: employeeResignationPi.label('reason'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeResignationPi.ph('reason'),
  },
  {
    key: 'handoverNotes',
    title: employeeResignationPi.label('handoverNotes'),
    editor: 'textarea',
    rows: 1,
    placeholder: employeeResignationPi.ph('handoverNotes'),
    width: 180,
  },
])
...
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeResignation = ((val as any)?.employeeResignations ?? []) as Record<string, unknown>[]
  childEmployeeResignationRows.value = rows_employeeResignation
}

function createDefaultEmployeeResignationRow(): Record<string, unknown> {
  return {
    resignationType: 0,
    applyDate: '',
    lastWorkDate: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeResignations: employeeResignationTableRef.value?.getRows?.() ?? childEmployeeResignationRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeResignations
...
async function validate() {
  ...
  await employeeResignationTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeResignationRows.value = []
  employeeResignationTableRef.value?.resetRows?.()
