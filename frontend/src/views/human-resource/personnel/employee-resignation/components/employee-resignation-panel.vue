...
<!-- 命名空间：@/views/human-resource/personnel/employee-resignation/components -->
<!-- 文件名称：employee-resignation-panel.vue -->
<!-- 功能描述：员工实体主表实体右侧明细 employeeResignation 独立 CRUD（按主表选中 employeeId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="employee-resignation-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="employee-resignation-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-human-resource-personnel-employee-resignation-employee-resignation"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('applyDateStart')">
        <a-date-picker
          v-model:value="advancedQueryForm.applyDateStart"
          :placeholder="pi.queryPh('applyDateStart', 'select')"
      ...
      <a-form-item :label="pi.queryLabel('applyDateEnd')">
        <a-date-picker
          v-model:value="advancedQueryForm.applyDateEnd"
          :placeholder="pi.queryPh('applyDateEnd', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('lastWorkDateStart')">
        <a-date-picker
          v-model:value="advancedQueryForm.lastWorkDateStart"
          :placeholder="pi.queryPh('lastWorkDateStart', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('lastWorkDateEnd')">
        <a-date-picker
          v-model:value="advancedQueryForm.lastWorkDateEnd"
          :placeholder="pi.queryPh('lastWorkDateEnd', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('terminationDateStart')">
        <a-date-picker
          v-model:value="advancedQueryForm.terminationDateStart"
          :placeholder="pi.queryPh('terminationDateStart', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('terminationDateEnd')">
        <a-date-picker
          v-model:value="advancedQueryForm.terminationDateEnd"
          :placeholder="pi.queryPh('terminationDateEnd', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('handoverNotes')">
        <a-textarea
          v-model:value="advancedQueryForm.handoverNotes"
          :placeholder="pi.queryPh('handoverNotes', 'optional')"
          :rows="2"
          allow-clear
        />
      </a-form-item>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="EMPLOYEERESIGNATION_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 员工实体子表 employeeResignation 右栏面板
 * @module views/human-resource/personnel/employee-resignation/components
 */
...
import EmployeeResignationForm from './employee-resignation-form.vue'
...
import {
  ...
  deleteEmployeeResignationById,
  deleteEmployeeResignationBatch,
  getEmployeeResignationTemplate,
  importEmployeeResignation,
  exportEmployeeResignation,
} from '@/api/human-resource/personnel/employee-resignation'
import type { EmployeeResignation, EmployeeResignationQuery } from '@/types/human-resource/personnel/employee-resignation'

import {
  useEmployeeResignationI18n,
  EMPLOYEERESIGNATION_DEFAULT_VISIBLE_COLUMN_KEYS,
  EMPLOYEERESIGNATION_SUMMARY_SUM_FIELDS,
  EMPLOYEERESIGNATION_QUERY_STRING_FIELDS,
  EMPLOYEERESIGNATION_QUERY_FIELDS,
  EMPLOYEERESIGNATION_SELF_I18N_KEY,
} from '../composables/use-employee-resignation-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useEmployeeResignationI18n()
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of EMPLOYEERESIGNATION_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(EMPLOYEERESIGNATION_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
    (typeof EMPLOYEERESIGNATION_QUERY_STRING_FIELDS)[number],
    string
  >
  return {
    ...form,
...
const queryFieldsMeta = computed(() =>
  EMPLOYEERESIGNATION_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...EMPLOYEERESIGNATION_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...EMPLOYEERESIGNATION_DEFAULT_VISIBLE_COLUMN_KEYS]
}
...
function getEmployeeResignationId(record: EmployeeResignation | Record<string, unknown>): string {
  return String((record as EmployeeResignation)?.[entityIdName] ?? '')
}

function getEmployeeResignationField(record: EmployeeResignation | Record<string, unknown>, field: string): unknown {
  return (record as EmployeeResignation)?.[field as keyof EmployeeResignation]
}
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'employeeResignationId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'employeeId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'employeeCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'employeeName') ?? ''),
  },
  {
    title: pi.label('resignationType'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'resignationType') ?? ''),
  },
  {
    title: pi.label('applyDate'),
    dataIndex: 'applyDate',
    key: 'applyDate',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'applyDate') ?? ''),
  },
  {
    title: pi.label('lastWorkDate'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'lastWorkDate') ?? ''),
  },
  {
    title: pi.label('terminationDate'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'terminationDate') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'reason') ?? ''),
  },
  {
    title: pi.label('handoverNotes'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeResignation }) =>
      String(getEmployeeResignationField(record, 'handoverNotes') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'employeeResignationId',
...
const summarySumFieldSet = new Set<string>(EMPLOYEERESIGNATION_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    EMPLOYEERESIGNATION_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof EMPLOYEERESIGNATION_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of EMPLOYEERESIGNATION_SUMMARY_SUM_FIELDS) {
      const num = Number(getEmployeeResignationField(row, field))
 ...
 * @param record 行数据
 */
function onClickRow(record: EmployeeResignation) {
...
async function handleEdit(record: EmployeeResignation) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getEmployeeResignationById(getEmployeeResignationId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.employeeResignationId
    if (id) {
      await updateEmployeeResignation(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createEmployeeResignation(payload)
...
async function handleDeleteOne(record: EmployeeResignation) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteEmployeeResignationById(getEmployeeResignationId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getEmployeeResignationId(r)).filter(Boolean)
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importEmployeeResignation(file, sheetName)
