...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeContracts -->
    <TaktEditableTable
      ref="employeeContractTableRef"
      v-model="childEmployeeContractRows"
      :columns="employeeContractFormColumns"
      :title="employeeContractPi.self()"
      :add-button-entity="employeeContractPi.self()"
      id-field="employeeContractId"
      :default-row="createDefaultEmployeeContractRow"
      :disabled="loading"
      :enable-vertical-scroll="false"
      section-border
      class="w-full min-w-0"
    >
      <template #cell-contractType="{ record }">
        <TaktSelect
          v-model:value="record.contractType"
          dict-type="humanresource_personnel_employee_contract_type"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeContractPi.ph('contractType')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-contractStatus="{ record }">
        <TaktSelect
          v-model:value="record.contractStatus"
          dict-type="humanresource_personnel_employee_contract_status"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeContractPi.ph('contractStatus')"
          :disabled="loading"
          allow-clear
        />
      </template>
...
import { useEmployeeContractI18n } from '../composables/use-employee-contract-i18n'

const employeeContractPi = useEmployeeContractI18n()
...
const childEmployeeContractRows = ref<Record<string, unknown>[]>([])
const employeeContractTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeContract 可编辑列 */
const employeeContractFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'contractCode',
    title: employeeContractPi.label('contractCode'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'contractType',
    title: employeeContractPi.label('contractType'),
    width: 140,
  },
  {
    key: 'startDate',
    title: employeeContractPi.label('startDate'),
    editor: 'datePicker',
  ...
  {
    key: 'endDate',
    title: employeeContractPi.label('endDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
  {
    ...
    title: employeeContractPi.label('probationEndDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
  {
    key: 'signDate',
    title: employeeContractPi.label('signDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
  {
    key: 'signCompany',
    title: employeeContractPi.label('signCompany'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeContractPi.ph('signCompany'),
  },
  {
    key: 'contractStatus',
    title: employeeContractPi.label('contractStatus'),
    width: 140,
  },
])
...
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeContract = ((val as any)?.employeeContracts ?? []) as Record<string, unknown>[]
  childEmployeeContractRows.value = rows_employeeContract
}

function createDefaultEmployeeContractRow(): Record<string, unknown> {
  return {
    contractCode: '',
    contractType: 0,
    startDate: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeContracts: employeeContractTableRef.value?.getRows?.() ?? childEmployeeContractRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeContracts
...
async function validate() {
  ...
  await employeeContractTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeContractRows.value = []
  employeeContractTableRef.value?.resetRows?.()
