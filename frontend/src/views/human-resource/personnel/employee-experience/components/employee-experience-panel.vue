...
<!-- 命名空间：@/views/human-resource/personnel/employee-experience/components -->
<!-- 文件名称：employee-experience-panel.vue -->
<!-- 功能描述：员工实体主表实体右侧明细 employeeExperience 独立 CRUD（按主表选中 employeeId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="employee-experience-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="employee-experience-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
        ...
        :stripe="true"
        :virtual="true"
        :row-key="getEmployeeExperienceId"
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-human-resource-personnel-employee-experience-employee-experience"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('positionName')">
        <a-input
          v-model:value="advancedQueryForm.positionName"
          :placeholder="pi.queryPh('positionName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('jobContent')">
        <a-textarea
          v-model:value="advancedQueryForm.jobContent"
          :placeholder="pi.queryPh('jobContent', 'optional')"
          :rows="2"
          allow-clear
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('witnessName')">
        <a-input
          v-model:value="advancedQueryForm.witnessName"
          :placeholder="pi.queryPh('witnessName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('witnessPhone')">
        <a-input
          v-model:value="advancedQueryForm.witnessPhone"
          :placeholder="pi.queryPh('witnessPhone', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="EMPLOYEEEXPERIENCE_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 员工实体子表 employeeExperience 右栏面板
 * @module views/human-resource/personnel/employee-experience/components
 */
...
import EmployeeExperienceForm from './employee-experience-form.vue'
...
import {
  ...
  deleteEmployeeExperienceBatch,
  getEmployeeExperienceTemplate,
  importEmployeeExperience,
  exportEmployeeExperience,
} from '@/api/human-resource/personnel/employee-experience'
import type { EmployeeExperience, EmployeeExperienceQuery } from '@/types/human-resource/personnel/employee-experience'

import {
  useEmployeeExperienceI18n,
  EMPLOYEEEXPERIENCE_DEFAULT_VISIBLE_COLUMN_KEYS,
  EMPLOYEEEXPERIENCE_SUMMARY_SUM_FIELDS,
  EMPLOYEEEXPERIENCE_QUERY_STRING_FIELDS,
  EMPLOYEEEXPERIENCE_QUERY_FIELDS,
  EMPLOYEEEXPERIENCE_SELF_I18N_KEY,
} from '../composables/use-employee-experience-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useEmployeeExperienceI18n()
...
const dataSource = ref<EmployeeExperience[]>([])
const currentPage = ref(getTaktDefaultPageIndex())
const pageSize = ref(getTaktDefaultPageSize())
const total = ref(0)
const queryKeyword = ref('')
const selectedRow = ref<EmployeeExperience | null>(null)
const selectedRows = ref<EmployeeExperience[]>([])
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of EMPLOYEEEXPERIENCE_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(EMPLOYEEEXPERIENCE_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
    (typeof EMPLOYEEEXPERIENCE_QUERY_STRING_FIELDS)[number],
    string
  >
  return {
    ...form,
...
const queryFieldsMeta = computed(() =>
  EMPLOYEEEXPERIENCE_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...EMPLOYEEEXPERIENCE_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...EMPLOYEEEXPERIENCE_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'employeeExperienceId'
...
function getEmployeeExperienceId(record: EmployeeExperience | Record<string, unknown>): string {
  return String((record as EmployeeExperience)?.[entityIdName] ?? '')
}

function getEmployeeExperienceField(record: EmployeeExperience | Record<string, unknown>, field: string): unknown {
  return (record as EmployeeExperience)?.[field as keyof EmployeeExperience]
}

const columns = computed<TableColumnsType>(() => [
  {
    title: t('common.page.entity.id'),
    dataIndex: 'employeeExperienceId',
    key: 'employeeExperienceId',
    width: 80,
    resizable: true,
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'employeeExperienceId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'employeeId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'employeeCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'employeeName') ?? ''),
  },
  {
    title: pi.label('companyName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'companyName') ?? ''),
  },
  {
    title: pi.label('positionName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'positionName') ?? ''),
  },
  {
    title: pi.label('jobContent'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'jobContent') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'startDate') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'endDate') ?? ''),
  },
  {
    title: pi.label('witnessName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'witnessName') ?? ''),
  },
  {
    title: pi.label('witnessPhone'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeExperience }) =>
      String(getEmployeeExperienceField(record, 'witnessPhone') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: EmployeeExperience) => void handleEdit(record),
      },
      {
        key: 'delete',
        ...
        onClick: (record: EmployeeExperience) => void handleDeleteOne(record),
      },
    ],
  }),
])
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: EmployeeExperience[]) => {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: EmployeeExperience[]) => {
 ...
 * @param record 行数据
 */
function onClickRow(record: EmployeeExperience) {
  const key = getEmployeeExperienceId(record)
  return {
    onClick: () => {
...
async function handleEdit(record: EmployeeExperience) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getEmployeeExperienceById(getEmployeeExperienceId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.employeeExperienceId
    if (id) {
      await updateEmployeeExperience(id, payload)
...
async function handleDeleteOne(record: EmployeeExperience) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteEmployeeExperienceById(getEmployeeExperienceId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getEmployeeExperienceId(r)).filter(Boolean)
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getEmployeeExperienceTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importEmployeeExperience(file, sheetName)
