...
<!-- 文件名称：employee-experience-form.vue -->
<!-- 功能描述：员工实体子表 employeeExperience 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form employee-experience-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    <a-tabs
      ...
      class="employee-experience-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="pi.label('positionName')"
                name="positionName"
              >
                <a-input
                  v-model:value="formState.positionName"
                  :placeholder="pi.ph('positionName')"
                  show-count
                  :maxlength="20"
                  allow-clear
                />
            ...
            <a-col :span="24">
              <a-form-item
                :label="pi.label('jobContent')"
                name="jobContent"
              >
                <a-textarea
                  v-model:value="formState.jobContent"
                  :placeholder="pi.ph('jobContent')"
                  :rows="2"
                />
              </a-form-item>
            </a-col>
            ...
            <a-col :span="12">
              <a-form-item
                :label="pi.label('witnessName')"
                name="witnessName"
              >
                <a-input
                  v-model:value="formState.witnessName"
                  :placeholder="pi.ph('witnessName')"
                  show-count
                  :maxlength="20"
                  allow-clear
                />
            ...
            <a-col :span="12">
              <a-form-item
                :label="pi.label('witnessPhone')"
                name="witnessPhone"
              >
                <a-input
                  v-model:value="formState.witnessPhone"
                  :placeholder="pi.ph('witnessPhone')"
                  show-count
                  :maxlength="20"
                  allow-clear
                />
...
/**
 * 员工实体子表 employeeExperience 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
const formFields = ["tenantCode","companyCode","cultureCode","plantCode","companyName","positionName","jobContent","startDate","endDate","witnessName","witnessPhone"]



/** 父级传入的编辑 DTO；新增时为 undefined 或空对象 */
interface Props {
  formData?: Partial<EmployeeExperienceCreate & { employeeExperienceId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
...
watch(
  ...
  () => {
    if (!props.formData?.employeeExperienceId) {
...
function getValues(): Record<string, any> {
  ...
  if (props.formData?.employeeExperienceId) {
    payload.employeeExperienceId = props.formData.employeeExperienceId
  }
