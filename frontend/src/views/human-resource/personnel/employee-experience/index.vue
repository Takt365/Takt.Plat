...
<!-- 命名空间：@/views/human-resource/personnel/employee-experience -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：员工外部工作经历管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="human-resource-personnel-employee-experience">
    ...
    >
      ...
      <a-form-item :label="t('entity.employeeExperience.jobcontent')">
        <a-input
          ...
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.employeeExperience.jobcontent') })"
          allow-clear
        />
      </a-form-item>
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.employeeExperience._self')"
      :width="600"
      :footer="null"
...
/**
 * 员工外部工作经历管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getEmployeeExperienceField(record, 'witnessPhone') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
const getEmployeeExperienceId = (record: any): string => record?.[entityIdName] ?? ''
...
const onClickRow = (record: EmployeeExperience) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getEmployeeExperienceId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[EmployeeExperience] 加载数据失败', { error })
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.employeeExperience._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: EmployeeExperience) {
  formTitle.value = t('common.page.button.edit') + t('entity.employeeExperience._self')
...
async function handleImportFile(file: File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> {
  return await importEmployeeExperience(file, sheetName)
}
