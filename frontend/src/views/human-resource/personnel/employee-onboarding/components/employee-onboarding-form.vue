...
<!-- 文件名称：employee-onboarding-form.vue -->
<!-- 功能描述：员工实体子表 employeeOnboarding 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form employee-onboarding-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    <a-tabs
      ...
      class="employee-onboarding-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="pi.label('todoCode')"
                name="todoCode"
              >
                <a-input
                  v-model:value="formState.todoCode"
                  :placeholder="pi.ph('todoCode')"
                  show-count
                  :maxlength="20"
                  allow-clear
...
/**
 * 员工实体子表 employeeOnboarding 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
const formFields = ["tenantCode","companyCode","cultureCode","plantCode","offerId","todoCode","plannedJoinedDate","candidateName","mobile","employeeJoinedId","reason","todoStatus"]



/** 父级传入的编辑 DTO；新增时为 undefined 或空对象 */
interface Props {
  formData?: Partial<EmployeeOnboardingCreate & { employeeOnboardingId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
...
function getValues(): Record<string, any> {
  ...
  if ('todoStatus' in payload) {
    const rawtodoStatus = payload.todoStatus
    if (rawtodoStatus === undefined || rawtodoStatus === null || rawtodoStatus === '') {
      delete payload.todoStatus
    } else {
      const numtodoStatus = typeof rawtodoStatus === 'number' ? rawtodoStatus : Number(rawtodoStatus)
      if (Number.isFinite(numtodoStatus)) payload.todoStatus = numtodoStatus
      else delete payload.todoStatus
    }
  }
  ...
  if (props.formData?.employeeOnboardingId) {
    payload.employeeOnboardingId = props.formData.employeeOnboardingId
  }
...
function resetFields() {
  ...
  applyFormDefaults(formState)
  applyScopeDefaults(formState as Record<string, unknown>, !props.formData?.employeeOnboardingId)
