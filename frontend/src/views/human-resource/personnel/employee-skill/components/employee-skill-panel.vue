...
<!-- 文件名称：employee-skill-panel.vue -->
<!-- 功能描述：员工实体主表实体右侧明细 employeeSkill 独立 CRUD（按主表选中 employeeId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="employee-skill-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="employee-skill-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-human-resource-personnel-employee-skill-employee-skill"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('skillName')">
        <a-input
          v-model:value="advancedQueryForm.skillName"
          :placeholder="pi.queryPh('skillName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('certificateName')">
        <a-input
          v-model:value="advancedQueryForm.certificateName"
          :placeholder="pi.queryPh('certificateName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('certificateCode')">
        <a-input
          v-model:value="advancedQueryForm.certificateCode"
          :placeholder="pi.queryPh('certificateCode', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('obtainedDateStart')">
        <a-date-picker
          v-model:value="advancedQueryForm.obtainedDateStart"
          :placeholder="pi.queryPh('obtainedDateStart', 'select')"
      ...
      <a-form-item :label="pi.queryLabel('obtainedDateEnd')">
        <a-date-picker
          v-model:value="advancedQueryForm.obtainedDateEnd"
          :placeholder="pi.queryPh('obtainedDateEnd', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="EMPLOYEESKILL_SELF_I18N_KEY"
        file-type="xlsx"
...
import EmployeeSkillForm from './employee-skill-form.vue'
...
import {
  ...
  createEmployeeSkill,
  updateEmployeeSkill,
  deleteEmployeeSkillById,
  deleteEmployeeSkillBatch,
  getEmployeeSkillTemplate,
  importEmployeeSkill,
  exportEmployeeSkill,
} from '@/api/human-resource/personnel/employee-skill'
import type { EmployeeSkill, EmployeeSkillQuery } from '@/types/human-resource/personnel/employee-skill'

import {
  useEmployeeSkillI18n,
  EMPLOYEESKILL_DEFAULT_VISIBLE_COLUMN_KEYS,
  EMPLOYEESKILL_SUMMARY_SUM_FIELDS,
  EMPLOYEESKILL_QUERY_STRING_FIELDS,
  EMPLOYEESKILL_QUERY_FIELDS,
  EMPLOYEESKILL_SELF_I18N_KEY,
} from '../composables/use-employee-skill-i18n'
...
const dataSource = ref<EmployeeSkill[]>([])
const currentPage = ref(getTaktDefaultPageIndex())
const pageSize = ref(getTaktDefaultPageSize())
const total = ref(0)
const queryKeyword = ref('')
const selectedRow = ref<EmployeeSkill | null>(null)
const selectedRows = ref<EmployeeSkill[]>([])
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of EMPLOYEESKILL_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(EMPLOYEESKILL_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  EMPLOYEESKILL_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...EMPLOYEESKILL_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...EMPLOYEESKILL_DEFAULT_VISIBLE_COLUMN_KEYS]
}
const importVisible = ref(false)

const entityIdName = 'employeeSkillId'
...
function getEmployeeSkillId(record: EmployeeSkill | Record<string, unknown>): string {
  return String((record as EmployeeSkill)?.[entityIdName] ?? '')
}

function getEmployeeSkillField(record: EmployeeSkill | Record<string, unknown>, field: string): unknown {
  return (record as EmployeeSkill)?.[field as keyof EmployeeSkill]
}

const columns = computed<TableColumnsType>(() => [
  {
    title: t('common.page.entity.id'),
    dataIndex: 'employeeSkillId',
    key: 'employeeSkillId',
    width: 80,
    resizable: true,
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'employeeSkillId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'employeeId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'employeeCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'employeeName') ?? ''),
  },
  {
    title: pi.label('skillName'),
    dataIndex: 'skillName',
    key: 'skillName',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'skillName') ?? ''),
  },
  {
    title: pi.label('skillLevel'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'skillLevel') ?? ''),
  },
  {
    title: pi.label('certificateName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'certificateName') ?? ''),
  },
  {
    title: pi.label('certificateCode'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'certificateCode') ?? ''),
  },
  {
    title: pi.label('obtainedDate'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'obtainedDate') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeSkill }) =>
      String(getEmployeeSkillField(record, 'expiryDate') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const summarySumFieldSet = new Set<string>(EMPLOYEESKILL_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    EMPLOYEESKILL_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof EMPLOYEESKILL_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of EMPLOYEESKILL_SUMMARY_SUM_FIELDS) {
      const num = Number(getEmployeeSkillField(row, field))
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: EmployeeSkill[]) => {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: EmployeeSkill[]) => {
 ...
 * @param record 行数据
 */
function onClickRow(record: EmployeeSkill) {
  const key = getEmployeeSkillId(record)
  return {
    onClick: () => {
...
async function handleEdit(record: EmployeeSkill) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getEmployeeSkillById(getEmployeeSkillId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.employeeSkillId
    if (id) {
      await updateEmployeeSkill(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createEmployeeSkill(payload)
...
async function handleDeleteOne(record: EmployeeSkill) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteEmployeeSkillById(getEmployeeSkillId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getEmployeeSkillId(r)).filter(Boolean)
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getEmployeeSkillTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件（归一化后端 SuccessCount/successCount） */
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importEmployeeSkill(file, sheetName)
