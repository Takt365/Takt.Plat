...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeSkills -->
    <TaktEditableTable
      ref="employeeSkillTableRef"
      v-model="childEmployeeSkillRows"
      :columns="employeeSkillFormColumns"
      :title="employeeSkillPi.self()"
      :add-button-entity="employeeSkillPi.self()"
      id-field="employeeSkillId"
      :default-row="createDefaultEmployeeSkillRow"
      :disabled="loading"
      :enable-vertical-scroll="false"
      section-border
      class="w-full min-w-0"
    >
      <template #cell-skillLevel="{ record }">
        <TaktSelect
          v-model:value="record.skillLevel"
          dict-type="humanresource_personnel_employee_skill_level"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeSkillPi.ph('skillLevel')"
          :disabled="loading"
          allow-clear
        />
      </template>
...
const employeeSkillPi = useEmployeeSkillI18n()
...
const childEmployeeSkillRows = ref<Record<string, unknown>[]>([])
const employeeSkillTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeSkill 可编辑列 */
const employeeSkillFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'skillName',
    title: employeeSkillPi.label('skillName'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'skillLevel',
    title: employeeSkillPi.label('skillLevel'),
    width: 140,
  },
  {
    key: 'certificateName',
    title: employeeSkillPi.label('certificateName'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeSkillPi.ph('certificateName'),
  },
  {
    key: 'certificateCode',
    title: employeeSkillPi.label('certificateCode'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeSkillPi.ph('certificateCode'),
  },
  {
    key: 'obtainedDate',
    title: employeeSkillPi.label('obtainedDate'),
    editor: 'datePicker',
  ...
  {
    key: 'expiryDate',
    title: employeeSkillPi.label('expiryDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
...
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeSkill = ((val as any)?.employeeSkills ?? []) as Record<string, unknown>[]
  childEmployeeSkillRows.value = rows_employeeSkill
}

function createDefaultEmployeeSkillRow(): Record<string, unknown> {
  return {
    skillName: '',
    skillLevel: 0,
    certificateName: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeSkills: employeeSkillTableRef.value?.getRows?.() ?? childEmployeeSkillRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeSkills
...
async function validate() {
  ...
  await employeeSkillTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeSkillRows.value = []
  employeeSkillTableRef.value?.resetRows?.()
