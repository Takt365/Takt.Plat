...
<!-- 功能描述：员工技能与证书维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="employee-skill-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * 员工技能与证书维护表单 · 由 generate-vue-from-api 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","employeeId","skillName","skillLevel","certificateName","certificateNo","obtainedDate","expiryDate","extFieldJson","remark"]
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  skillLevel: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.employeeSkill.skilllevel') }),
      trigger: 'change'
    }
  ],
}))
