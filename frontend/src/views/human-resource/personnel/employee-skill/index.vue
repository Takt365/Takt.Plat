...
<!-- 文件名称：index.vue -->
<!-- 功能描述：员工技能与证书管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="human-resource-personnel-employee-skill">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="humanresource:personnel:employeeskill:create"
      update-permission="humanresource:personnel:employeeskill:update"
      delete-permission="humanresource:personnel:employeeskill:delete"
      import-permission="humanresource:personnel:employeeskill:import"
      export-permission="humanresource:personnel:employeeskill:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.employeeSkill._self')"
      :width="600"
      :footer="null"
...
/**
 * 员工技能与证书管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getEmployeeSkillField(record, 'expiryDate') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'humanresource:personnel:employeeskill:update',
      ...
      },
      {
        ...
        permission: 'humanresource:personnel:employeeskill:delete',
...
const getEmployeeSkillId = (record: any): string => record?.[entityIdName] ?? ''
...
const onClickRow = (record: EmployeeSkill) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getEmployeeSkillId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[EmployeeSkill] 加载数据失败', { error })
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.employeeSkill._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: EmployeeSkill) {
  formTitle.value = t('common.page.button.edit') + t('entity.employeeSkill._self')
...
async function handleImportFile(file: File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> {
  return await importEmployeeSkill(file, sheetName)
}
