...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeDelegations -->
    <TaktEditableTable
      ref="employeeDelegationTableRef"
      v-model="childEmployeeDelegationRows"
      :columns="employeeDelegationFormColumns"
      :title="employeeDelegationPi.self()"
      :add-button-entity="employeeDelegationPi.self()"
      id-field="employeeDelegationId"
      :default-row="createDefaultEmployeeDelegationRow"
      :disabled="loading"
      :enable-vertical-scroll="false"
      section-border
      class="w-full min-w-0"
    >
      <template #cell-proxyEmployeeId="{ record }">
        <TaktSelect
          v-model:value="record.proxyEmployeeId"
          api-url="TaktEmployees/options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeDelegationPi.queryPh('proxyEmployeeId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-delegationType="{ record }">
        <TaktSelect
          v-model:value="record.delegationType"
          dict-type="hr_employee_delegation_type"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeDelegationPi.ph('delegationType')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-scopeType="{ record }">
        <TaktSelect
          v-model:value="record.scopeType"
          dict-type="hr_employee_delegation_scope_type"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeDelegationPi.ph('scopeType')"
          :disabled="loading"
          allow-clear
        />
      </template>
...
const employeeDelegationPi = useEmployeeDelegationI18n()
...
const childEmployeeDelegationRows = ref<Record<string, unknown>[]>([])
const employeeDelegationTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeDelegation 可编辑列 */
const employeeDelegationFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'proxyEmployeeId',
    title: employeeDelegationPi.label('proxyEmployeeId'),
    width: 140,
  },
  {
    key: 'delegationType',
    title: employeeDelegationPi.label('delegationType'),
    width: 140,
  },
  {
    key: 'scopeType',
    title: employeeDelegationPi.label('scopeType'),
    width: 140,
  },
  {
    key: 'scopeId',
    title: employeeDelegationPi.label('scopeId'),
    editor: 'textarea',
    rows: 1,
    placeholder: employeeDelegationPi.ph('scopeId'),
    width: 180,
  },
  {
    key: 'reason',
    title: employeeDelegationPi.label('reason'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'startDate',
    title: employeeDelegationPi.label('startDate'),
    editor: 'datePicker',
  ...
  {
    key: 'endDate',
    title: employeeDelegationPi.label('endDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
...
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeDelegation = ((val as any)?.employeeDelegations ?? []) as Record<string, unknown>[]
  childEmployeeDelegationRows.value = rows_employeeDelegation
}

function createDefaultEmployeeDelegationRow(): Record<string, unknown> {
  return {
    proxyEmployeeId: '',
    delegationType: 0,
    scopeType: 0,
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeDelegations: employeeDelegationTableRef.value?.getRows?.() ?? childEmployeeDelegationRows.value.map((rest) => ({
      ...rest,
      tenantCode: tenantStore.tenantCode,
      companyCode: tenantStore.companyCode,
      cultureCode: userStore.userInfo?.companyDefaultCulture ?? userStore.userInfo?.cultureCode ?? '',
      originalEmployeeId: masterId,
    })),
  }
}
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeDelegations
...
async function validate() {
  ...
  await employeeDelegationTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeDelegationRows.value = []
  employeeDelegationTableRef.value?.resetRows?.()
