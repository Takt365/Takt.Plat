...
<!-- 文件名称：employee-delegation-form.vue -->
<!-- 功能描述：员工实体子表 employeeDelegation 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form employee-delegation-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
  >
    <a-tabs
      ...
      class="employee-delegation-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="pi.label('proxyEmployeeId')"
                name="proxyEmployeeId"
              >
                <TaktSelect
                  v-model:value="formState.proxyEmployeeId"
                  api-url="TaktEmployees/options"
                  :placeholder="pi.ph('proxyEmployeeId')"
                />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item
                :label="pi.label('delegationType')"
                name="delegationType"
              >
                <TaktSelect
            ...
            <a-col :span="24">
              <a-form-item
                :label="pi.label('scopeId')"
                name="scopeId"
              >
                <a-textarea
                  v-model:value="formState.scopeId"
                  :placeholder="pi.ph('scopeId')"
                  :rows="2"
                />
              </a-form-item>
            </a-col>
...
/**
 * 员工实体子表 employeeDelegation 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
 * @module views/human-resource/personnel/employee-delegation/components
 */
import { reactive, watch, computed, ref, onMounted } from 'vue'
import { useI18n } from 'vue-i18n'
import type { Rule } from 'ant-design-vue/es/form'
import { useEmployeeDelegationI18n } from '../composables/use-employee-delegation-i18n'
...
const formFields = ["tenantCode","companyCode","cultureCode","plantCode","proxyEmployeeId","delegationType","scopeType","scopeId","reason","startDate","endDate"]



/** 父级传入的编辑 DTO；新增时为 undefined 或空对象 */
interface Props {
  formData?: Partial<EmployeeDelegationCreate & { employeeDelegationId?: string }> | null
  /** 父级提交 loading，禁用表单项 */
  loading?: boolean
...
watch(
  ...
  () => {
    if (!props.formData?.employeeDelegationId) {
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  delegationType: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(pi.ph('delegationType'))
      }
  ...
  scopeType: [{
    validator: async (_rule, value) => {
      if (value === undefined || value === null || value === '') {
        return Promise.reject(pi.ph('scopeType'))
      }
      const num = typeof value === 'number' ? value : Number(value)
      if (!Number.isFinite(num)) {
        return Promise.reject(pi.ph('scopeType'))
...
/** 映射为 Create/Update DTO（含主表外键 originalEmployeeId） */
function getValues(): Record<string, any> {
  const payload = { ...formState }
  if ('delegationType' in payload) {
    const rawdelegationType = payload.delegationType
    if (rawdelegationType === undefined || rawdelegationType === null || rawdelegationType === '') {
      delete payload.delegationType
    } else {
      const numdelegationType = typeof rawdelegationType === 'number' ? rawdelegationType : Number(rawdelegationType)
      if (Number.isFinite(numdelegationType)) payload.delegationType = numdelegationType
      else delete payload.delegationType
    }
  }
  if ('scopeType' in payload) {
    const rawscopeType = payload.scopeType
    if (rawscopeType === undefined || rawscopeType === null || rawscopeType === '') {
      delete payload.scopeType
    } else {
      const numscopeType = typeof rawscopeType === 'number' ? rawscopeType : Number(rawscopeType)
      if (Number.isFinite(numscopeType)) payload.scopeType = numscopeType
      else delete payload.scopeType
    }
  }
  ...
  if (props.formData?.employeeDelegationId) {
    payload.employeeDelegationId = props.formData.employeeDelegationId
  }
  payload.originalEmployeeId = props.masterId
  ...
  if (masterRow) {
    ...
    if (masterCode != null && masterCode !== '' && !payload.originalEmployeeCode) {
      payload.originalEmployeeCode = masterCode
    }
    if (masterName != null && masterName !== '' && !payload.originalEmployeeName) {
      payload.originalEmployeeName = masterName
    }
