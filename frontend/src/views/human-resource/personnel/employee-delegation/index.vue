...
<!-- 命名空间：@/views/human-resource/personnel/employee-delegation -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：员工代理关系实体 独立记录所有代理场景管理页面，含查询、增删改，由 generate-vue-from-api 根据 types/api 自动生成 -->
...
<template>
  <div class="human-resource-personnel-employee-delegation">
    ...
    >
      ...
      <a-form-item :label="t('entity.employeeDelegation.scopetype')">
        <a-input
          ...
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.employeeDelegation.scopetype') })"
          allow-clear
        />
      </a-form-item>
      <a-form-item :label="t('entity.employeeDelegation.scopeid')">
        <a-input
          ...
          :placeholder="t('common.page.form.placeholder.required', { field: t('entity.employeeDelegation.scopeid') })"
          allow-clear
        />
      </a-form-item>
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('entity.employeeDelegation._self')"
      :width="600"
      :footer="null"
...
/**
 * 员工代理关系实体 独立记录所有代理场景管理页 · 由 generate-vue-from-api 根据 types/api 生成
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getEmployeeDelegationField(record, 'endDate') ?? ''
  },
  CreateActionColumn({
    actions: [
      {
...
const getEmployeeDelegationId = (record: any): string => record?.[entityIdName] ?? ''
...
const onClickRow = (record: EmployeeDelegation) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getEmployeeDelegationId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[EmployeeDelegation] 加载数据失败', { error })
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('entity.employeeDelegation._self')
  formData.value = {}
  formVisible.value = true
}
function handleEdit(record: EmployeeDelegation) {
  formTitle.value = t('common.page.button.edit') + t('entity.employeeDelegation._self')
...
async function handleImportFile(file: File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> {
  return await importEmployeeDelegation(file, sheetName)
}
