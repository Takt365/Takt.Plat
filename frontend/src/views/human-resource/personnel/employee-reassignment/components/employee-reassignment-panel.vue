...
<!-- 文件名称：employee-reassignment-panel.vue -->
<!-- 功能描述：员工实体主表实体右侧明细 employeeReassignment 独立 CRUD（按主表选中 employeeId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="employee-reassignment-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="employee-reassignment-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-human-resource-personnel-employee-reassignment-employee-reassignment"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('fromDeptId')">
        <TaktSelect
          v-model:value="advancedQueryForm.fromDeptId"
          api-url="TaktDepts/tree-options"
          :placeholder="pi.queryPh('fromDeptId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('fromDeptName')">
      <a-form-item :label="pi.queryLabel('fromDeptName')">
        <a-input
          v-model:value="advancedQueryForm.fromDeptName"
          :placeholder="pi.queryPh('fromDeptName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('fromPostId')">
        <TaktSelect
          v-model:value="advancedQueryForm.fromPostId"
          api-url="TaktPosts/options"
          :placeholder="pi.queryPh('fromPostId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('fromPostName')">
      <a-form-item :label="pi.queryLabel('fromPostName')">
        <a-input
          v-model:value="advancedQueryForm.fromPostName"
          :placeholder="pi.queryPh('fromPostName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('toDeptId')">
        <TaktSelect
          v-model:value="advancedQueryForm.toDeptId"
          api-url="TaktDepts/tree-options"
          :placeholder="pi.queryPh('toDeptId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('toDeptName')">
      <a-form-item :label="pi.queryLabel('toDeptName')">
        <a-input
          v-model:value="advancedQueryForm.toDeptName"
          :placeholder="pi.queryPh('toDeptName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('toPostId')">
        <TaktSelect
          v-model:value="advancedQueryForm.toPostId"
          api-url="TaktPosts/options"
          :placeholder="pi.queryPh('toPostId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('toPostName')">
      <a-form-item :label="pi.queryLabel('toPostName')">
        <a-input
          v-model:value="advancedQueryForm.toPostName"
          :placeholder="pi.queryPh('toPostName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="EMPLOYEEREASSIGNMENT_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 员工实体子表 employeeReassignment 右栏面板
...
import EmployeeReassignmentForm from './employee-reassignment-form.vue'
import { useEmployeeMasterContext } from '../composables/use-employee-master-context'
import {
  getEmployeeReassignmentList,
  getEmployeeReassignmentById,
  createEmployeeReassignment,
  updateEmployeeReassignment,
  deleteEmployeeReassignmentById,
  deleteEmployeeReassignmentBatch,
  getEmployeeReassignmentTemplate,
  importEmployeeReassignment,
  exportEmployeeReassignment,
} from '@/api/human-resource/personnel/employee-reassignment'
import type { EmployeeReassignment, EmployeeReassignmentQuery } from '@/types/human-resource/personnel/employee-reassignment'

import {
  useEmployeeReassignmentI18n,
  EMPLOYEEREASSIGNMENT_DEFAULT_VISIBLE_COLUMN_KEYS,
  EMPLOYEEREASSIGNMENT_SUMMARY_SUM_FIELDS,
  EMPLOYEEREASSIGNMENT_QUERY_STRING_FIELDS,
  EMPLOYEEREASSIGNMENT_QUERY_FIELDS,
  EMPLOYEEREASSIGNMENT_SELF_I18N_KEY,
} from '../composables/use-employee-reassignment-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useEmployeeReassignmentI18n()
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of EMPLOYEEREASSIGNMENT_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(EMPLOYEEREASSIGNMENT_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  EMPLOYEEREASSIGNMENT_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...EMPLOYEEREASSIGNMENT_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...EMPLOYEEREASSIGNMENT_DEFAULT_VISIBLE_COLUMN_KEYS]
}
...
function getEmployeeReassignmentId(record: EmployeeReassignment | Record<string, unknown>): string {
  return String((record as EmployeeReassignment)?.[entityIdName] ?? '')
}

function getEmployeeReassignmentField(record: EmployeeReassignment | Record<string, unknown>, field: string): unknown {
  return (record as EmployeeReassignment)?.[field as keyof EmployeeReassignment]
}
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'employeeReassignmentId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'employeeId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'employeeCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'employeeName') ?? ''),
  },
  {
    title: pi.label('reassignmentType'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'reassignmentType') ?? ''),
  },
  {
    title: pi.label('fromDeptId'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'fromDeptId') ?? ''),
  },
  {
    title: pi.label('fromDeptName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'fromDeptName') ?? ''),
  },
  {
    title: pi.label('fromPostId'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'fromPostId') ?? ''),
  },
  {
    title: pi.label('fromPostName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'fromPostName') ?? ''),
  },
  {
    title: pi.label('toDeptId'),
    dataIndex: 'toDeptId',
    key: 'toDeptId',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'toDeptId') ?? ''),
  },
  {
    title: pi.label('toDeptName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'toDeptName') ?? ''),
  },
  {
    title: pi.label('toPostId'),
    dataIndex: 'toPostId',
    key: 'toPostId',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'toPostId') ?? ''),
  },
  {
    title: pi.label('toPostName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'toPostName') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'effectiveDate') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeReassignment }) =>
      String(getEmployeeReassignmentField(record, 'reason') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
...
const resolvedSummaryColumns = computed(() => {
  ...
  return filterMergedColumnsByDefaultVisible(merged, userCols, {
    idColumnKey: 'employeeReassignmentId',
...
const summarySumFieldSet = new Set<string>(EMPLOYEEREASSIGNMENT_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    EMPLOYEEREASSIGNMENT_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof EMPLOYEEREASSIGNMENT_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of EMPLOYEEREASSIGNMENT_SUMMARY_SUM_FIELDS) {
      const num = Number(getEmployeeReassignmentField(row, field))
 ...
 * @param record 行数据
 */
function onClickRow(record: EmployeeReassignment) {
...
async function handleEdit(record: EmployeeReassignment) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getEmployeeReassignmentById(getEmployeeReassignmentId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.employeeReassignmentId
    if (id) {
      await updateEmployeeReassignment(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createEmployeeReassignment(payload)
...
async function handleDeleteOne(record: EmployeeReassignment) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteEmployeeReassignmentById(getEmployeeReassignmentId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getEmployeeReassignmentId(r)).filter(Boolean)
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importEmployeeReassignment(file, sheetName)
