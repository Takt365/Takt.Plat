...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeReassignments -->
    <TaktEditableTable
      ref="employeeReassignmentTableRef"
      v-model="childEmployeeReassignmentRows"
      :columns="employeeReassignmentFormColumns"
      :title="employeeReassignmentPi.self()"
      :add-button-entity="employeeReassignmentPi.self()"
      id-field="employeeReassignmentId"
      :default-row="createDefaultEmployeeReassignmentRow"
      :disabled="loading"
      :enable-vertical-scroll="false"
      section-border
      class="w-full min-w-0"
    >
      <template #cell-reassignmentType="{ record }">
        <TaktSelect
          v-model:value="record.reassignmentType"
          dict-type="humanresource_personnel_reassignment_type"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeReassignmentPi.ph('reassignmentType')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-fromDeptId="{ record }">
        <TaktSelect
          v-model:value="record.fromDeptId"
          api-url="TaktDepts/tree-options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeReassignmentPi.queryPh('fromDeptId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-fromPostId="{ record }">
        <TaktSelect
          v-model:value="record.fromPostId"
          api-url="TaktPosts/options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeReassignmentPi.queryPh('fromPostId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-toDeptId="{ record }">
        <TaktSelect
          v-model:value="record.toDeptId"
          api-url="TaktDepts/tree-options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeReassignmentPi.queryPh('toDeptId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-toPostId="{ record }">
        <TaktSelect
          v-model:value="record.toPostId"
          api-url="TaktPosts/options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeReassignmentPi.queryPh('toPostId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
...
import { useEmployeeReassignmentI18n } from '../composables/use-employee-reassignment-i18n'

const employeeReassignmentPi = useEmployeeReassignmentI18n()
...
const childEmployeeReassignmentRows = ref<Record<string, unknown>[]>([])
const employeeReassignmentTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeReassignment 可编辑列 */
const employeeReassignmentFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'reassignmentType',
    title: employeeReassignmentPi.label('reassignmentType'),
    width: 140,
  },
  {
    key: 'fromDeptId',
    title: employeeReassignmentPi.label('fromDeptId'),
    width: 140,
  },
  {
    key: 'fromDeptName',
    title: employeeReassignmentPi.label('fromDeptName'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'fromPostId',
    title: employeeReassignmentPi.label('fromPostId'),
    width: 140,
  },
  {
    key: 'fromPostName',
    title: employeeReassignmentPi.label('fromPostName'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeReassignmentPi.ph('fromPostName'),
  },
  {
    key: 'toDeptId',
    title: employeeReassignmentPi.label('toDeptId'),
    width: 140,
  },
  {
    key: 'toDeptName',
    title: employeeReassignmentPi.label('toDeptName'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'toPostId',
    title: employeeReassignmentPi.label('toPostId'),
    width: 140,
  },
  {
    key: 'toPostName',
    title: employeeReassignmentPi.label('toPostName'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeReassignmentPi.ph('toPostName'),
  },
  {
    key: 'effectiveDate',
    title: employeeReassignmentPi.label('effectiveDate'),
    editor: 'datePicker',
  ...
  {
    key: 'reason',
    title: employeeReassignmentPi.label('reason'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeReassignmentPi.ph('reason'),
  },
])

/** 编辑态从 formData 同步各子表行 */
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeReassignment = ((val as any)?.employeeReassignments ?? []) as Record<string, unknown>[]
  childEmployeeReassignmentRows.value = rows_employeeReassignment
}

function createDefaultEmployeeReassignmentRow(): Record<string, unknown> {
  return {
    reassignmentType: 0,
    fromDeptId: '',
    fromDeptName: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeReassignments: employeeReassignmentTableRef.value?.getRows?.() ?? childEmployeeReassignmentRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeReassignments
...
async function validate() {
  ...
  await employeeReassignmentTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeReassignmentRows.value = []
  employeeReassignmentTableRef.value?.resetRows?.()
