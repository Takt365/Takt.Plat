...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeEducations -->
    <TaktEditableTable
      ref="employeeEducationTableRef"
      v-model="childEmployeeEducationRows"
      :columns="employeeEducationFormColumns"
      :title="employeeEducationPi.self()"
      :add-button-entity="employeeEducationPi.self()"
      id-field="employeeEducationId"
      :default-row="createDefaultEmployeeEducationRow"
      :disabled="loading"
      :enable-vertical-scroll="false"
      section-border
      class="w-full min-w-0"
    >
      <template #cell-educationLevel="{ record }">
        <TaktSelect
          v-model:value="record.educationLevel"
          dict-type="hr_education_level_category"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeEducationPi.ph('educationLevel')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-degreeLevel="{ record }">
        <TaktSelect
          v-model:value="record.degreeLevel"
          dict-type="hr_degree_level_category"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeEducationPi.ph('degreeLevel')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-isHighest="{ record }">
        <TaktSelect
          v-model:value="record.isHighest"
          dict-type="sys_yes_no"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeEducationPi.ph('isHighest')"
          :disabled="loading"
          allow-clear
        />
      </template>
...
const employeeEducationPi = useEmployeeEducationI18n()
...
const childEmployeeEducationRows = ref<Record<string, unknown>[]>([])
const employeeEducationTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeEducation 可编辑列 */
const employeeEducationFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'schoolName',
    title: employeeEducationPi.label('schoolName'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'educationLevel',
    title: employeeEducationPi.label('educationLevel'),
    width: 140,
  },
  {
    key: 'degreeLevel',
    title: employeeEducationPi.label('degreeLevel'),
    width: 140,
  },
  {
    key: 'majorName',
    title: employeeEducationPi.label('majorName'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeEducationPi.ph('majorName'),
  },
  {
    key: 'certificateCode',
    title: employeeEducationPi.label('certificateCode'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeEducationPi.ph('certificateCode'),
  },
  {
    key: 'startDate',
    title: employeeEducationPi.label('startDate'),
    editor: 'datePicker',
  ...
  {
    key: 'endDate',
    title: employeeEducationPi.label('endDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
  {
    key: 'isHighest',
    title: employeeEducationPi.label('isHighest'),
    width: 140,
  },
])
...
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeEducation = ((val as any)?.employeeEducations ?? []) as Record<string, unknown>[]
  childEmployeeEducationRows.value = rows_employeeEducation
}

function createDefaultEmployeeEducationRow(): Record<string, unknown> {
  return {
    schoolName: '',
    educationLevel: 0,
    degreeLevel: 0,
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeEducations: employeeEducationTableRef.value?.getRows?.() ?? childEmployeeEducationRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeEducations
...
async function validate() {
  ...
  await employeeEducationTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeEducationRows.value = []
  employeeEducationTableRef.value?.resetRows?.()
