...
<!-- 文件名称：employee-family-panel.vue -->
<!-- 功能描述：员工实体主表实体右侧明细 employeeFamily 独立 CRUD（按主表选中 employeeId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="employee-family-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="employee-family-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-human-resource-personnel-employee-family-employee-family"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('memberName')">
        <a-input
          v-model:value="advancedQueryForm.memberName"
          :placeholder="pi.queryPh('memberName', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('workUnit')">
        <a-input
          v-model:value="advancedQueryForm.workUnit"
          :placeholder="pi.queryPh('workUnit', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
      ...
      <a-form-item :label="pi.queryLabel('isEmergencyContact')">
        <TaktSelect
          v-model:value="advancedQueryForm.isEmergencyContact"
          dict-type="sys_yes_no"
          :placeholder="pi.queryPh('isEmergencyContact', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="EMPLOYEEFAMILY_SELF_I18N_KEY"
        file-type="xlsx"
...
import EmployeeFamilyForm from './employee-family-form.vue'
import { useEmployeeMasterContext } from '../composables/use-employee-master-context'
import {
  getEmployeeFamilyList,
  getEmployeeFamilyById,
  createEmployeeFamily,
  updateEmployeeFamily,
  deleteEmployeeFamilyById,
  deleteEmployeeFamilyBatch,
  getEmployeeFamilyTemplate,
  importEmployeeFamily,
  exportEmployeeFamily,
} from '@/api/human-resource/personnel/employee-family'
import type { EmployeeFamily, EmployeeFamilyQuery } from '@/types/human-resource/personnel/employee-family'

import {
  useEmployeeFamilyI18n,
  EMPLOYEEFAMILY_DEFAULT_VISIBLE_COLUMN_KEYS,
  EMPLOYEEFAMILY_SUMMARY_SUM_FIELDS,
  EMPLOYEEFAMILY_QUERY_STRING_FIELDS,
  EMPLOYEEFAMILY_QUERY_FIELDS,
  EMPLOYEEFAMILY_SELF_I18N_KEY,
} from '../composables/use-employee-family-i18n'
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of EMPLOYEEFAMILY_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(EMPLOYEEFAMILY_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  EMPLOYEEFAMILY_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...EMPLOYEEFAMILY_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...EMPLOYEEFAMILY_DEFAULT_VISIBLE_COLUMN_KEYS]
}
...
function getEmployeeFamilyId(record: EmployeeFamily | Record<string, unknown>): string {
  return String((record as EmployeeFamily)?.[entityIdName] ?? '')
}

function getEmployeeFamilyField(record: EmployeeFamily | Record<string, unknown>, field: string): unknown {
  return (record as EmployeeFamily)?.[field as keyof EmployeeFamily]
}
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'employeeFamilyId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'employeeId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'employeeCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'employeeName') ?? ''),
  },
  {
    title: pi.label('memberName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'memberName') ?? ''),
  },
  {
    title: pi.label('relationType'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'relationType') ?? ''),
  },
  {
    title: pi.label('phoneNumber'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'phoneNumber') ?? ''),
  },
  {
    title: pi.label('workUnit'),
    dataIndex: 'workUnit',
    key: 'workUnit',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'workUnit') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'jobTitle') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'birthDate') ?? ''),
  },
  {
    title: pi.label('isEmergencyContact'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeFamily }) =>
      String(getEmployeeFamilyField(record, 'isEmergencyContact') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: EmployeeFamily) => void handleEdit(record),
      },
      {
        key: 'delete',
        ...
        onClick: (record: EmployeeFamily) => void handleDeleteOne(record),
      },
    ],
  }),
])
...
const summarySumFieldSet = new Set<string>(EMPLOYEEFAMILY_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    EMPLOYEEFAMILY_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof EMPLOYEEFAMILY_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of EMPLOYEEFAMILY_SUMMARY_SUM_FIELDS) {
      const num = Number(getEmployeeFamilyField(row, field))
 ...
 * @param record 行数据
 */
function onClickRow(record: EmployeeFamily) {
...
async function handleEdit(record: EmployeeFamily) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getEmployeeFamilyById(getEmployeeFamilyId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.employeeFamilyId
    if (id) {
      await updateEmployeeFamily(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createEmployeeFamily(payload)
...
async function handleDeleteOne(record: EmployeeFamily) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteEmployeeFamilyById(getEmployeeFamilyId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getEmployeeFamilyId(r)).filter(Boolean)
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importEmployeeFamily(file, sheetName)
