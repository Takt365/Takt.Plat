...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeFamilies -->
    <TaktEditableTable
      ref="employeeFamilyTableRef"
      v-model="childEmployeeFamilyRows"
      :columns="employeeFamilyFormColumns"
      :title="employeeFamilyPi.self()"
      :add-button-entity="employeeFamilyPi.self()"
      id-field="employeeFamilyId"
      :default-row="createDefaultEmployeeFamilyRow"
      :disabled="loading"
      :enable-vertical-scroll="false"
      section-border
      class="w-full min-w-0"
    >
      <template #cell-relationType="{ record }">
        <TaktSelect
          v-model:value="record.relationType"
          dict-type="hr_employee_family_relation_type"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeFamilyPi.ph('relationType')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-isEmergencyContact="{ record }">
        <TaktSelect
          v-model:value="record.isEmergencyContact"
          dict-type="sys_yes_no"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeFamilyPi.ph('isEmergencyContact')"
          :disabled="loading"
          allow-clear
        />
      </template>
...
const employeeFamilyPi = useEmployeeFamilyI18n()
...
const childEmployeeFamilyRows = ref<Record<string, unknown>[]>([])
const employeeFamilyTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeFamily 可编辑列 */
const employeeFamilyFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'memberName',
    title: employeeFamilyPi.label('memberName'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'relationType',
    title: employeeFamilyPi.label('relationType'),
    width: 140,
  },
  {
    key: 'phoneNumber',
    title: employeeFamilyPi.label('phoneNumber'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeFamilyPi.ph('phoneNumber'),
  },
  {
    key: 'workUnit',
    title: employeeFamilyPi.label('workUnit'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeFamilyPi.ph('workUnit'),
  },
  {
    key: 'jobTitle',
    title: employeeFamilyPi.label('jobTitle'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeFamilyPi.ph('jobTitle'),
  },
  {
    key: 'birthDate',
    title: employeeFamilyPi.label('birthDate'),
    editor: 'datePicker',
  ...
  },
  {
    ...
    title: employeeFamilyPi.label('isEmergencyContact'),
    width: 140,
  },
])
...
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeFamily = ((val as any)?.employeeFamilies ?? []) as Record<string, unknown>[]
  childEmployeeFamilyRows.value = rows_employeeFamily
}

function createDefaultEmployeeFamilyRow(): Record<string, unknown> {
  return {
    memberName: '',
    relationType: 0,
    phoneNumber: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeFamilies: employeeFamilyTableRef.value?.getRows?.() ?? childEmployeeFamilyRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeFamilies
...
async function validate() {
  ...
  await employeeFamilyTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeFamilyRows.value = []
  employeeFamilyTableRef.value?.resetRows?.()
