...
<!-- 功能描述：员工家庭成员维护弹窗内嵌表单。由 generate-vue-from-api 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="employee-family-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
...
/**
 * 员工家庭成员维护表单 · 由 generate-vue-from-api 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","employeeId","memberName","relationType","phoneNumber","workUnit","jobTitle","birthDate","isEmergencyContact","extFieldJson","remark"]
...
const rules = computed<Record<string, Rule[]>>(() => ({
  ...
  relationType: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.employeeFamily.relationtype') }),
      trigger: 'change'
    }
  ],
  isEmergencyContact: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.employeeFamily.isemergencycontact') }),
      trigger: 'change'
    }
  ],
}))
