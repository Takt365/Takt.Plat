...
<!-- 功能描述：员工实体子表 employeeAttachment 独立 CRUD 弹窗表单；defineExpose validate/getValues/resetFields。由 generate-vue-master-detail-from-api.cjs 生成，风格与主表 *-form 一致 -->
...
<template>
  <a-form
    ref="formRef"
    class="takt-generated-form employee-attachment-form flex flex-col min-h-0"
    :model="formState"
    :rules="rules"
    layout="horizontal"
    label-align="right"
...
/**
 * 员工实体子表 employeeAttachment 维护表单 · 由 generate-vue-master-detail-from-api.cjs 生成
...
watch(
  ...
  () => {
    if (!props.formData?.employeeAttachmentId) {
...
function getValues(): Record<string, any> {
  ...
  if (props.formData?.employeeAttachmentId) {
    payload.employeeAttachmentId = props.formData.employeeAttachmentId
  }
