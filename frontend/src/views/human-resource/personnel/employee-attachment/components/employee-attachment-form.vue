...
<!-- 功能描述：员工档案附件维护弹窗内嵌表单。由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成；defineExpose 提供 validate、getValues、resetFields -->
...
<template>
  ...
  >
    <a-tabs
      ...
      class="employee-attachment-form-tabs"
    >
      <a-tab-pane
        key="tab-0"
      ...
      >
        <div :class="formContentClass">
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              <a-form-item
                :label="t('entity.employeeAttachment.filetype')"
                name="fileType"
              >
                <a-input
...
/**
 * 员工档案附件维护表单 · 由 generate-vue-crud-from-api.cjs 根据 types/api 生成
...
const formFields = ["tenantCode","companyCode","companyDefaultCulture","employeeId","fileId","fileCode","fileName","filePath","fileSize","fileType","attachmentType","attachmentDescription","sortOrder","extFieldJson","remark"]
...
const rules = computed<Record<string, Rule[]>>(() => ({
  employeeId: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.employeeAttachment.employeeid') }),
      trigger: 'blur'
    }
  ],
  fileName: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.employeeAttachment.filename') }),
      trigger: 'blur'
    }
  ],
  fileSize: [
    {
      required: true,
      message: t('common.page.form.placeholder.required', { field: t('entity.employeeAttachment.filesize') }),
      trigger: 'blur'
    }
  ],
  attachmentType: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.employeeAttachment.attachmenttype') }),
      trigger: 'change'
    }
  ],
  sortOrder: [
    {
      required: true,
      message: t('common.page.form.placeholder.select', { field: t('entity.employeeAttachment.sortorder') }),
      trigger: 'change'
    }
  ],
}))
