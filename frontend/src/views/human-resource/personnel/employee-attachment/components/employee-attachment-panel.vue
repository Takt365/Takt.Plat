...
<!-- 文件名称：employee-attachment-panel.vue -->
<!-- 功能描述：员工实体主表实体右侧明细 employeeAttachment 独立 CRUD（按主表选中 employeeId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="employee-attachment-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="employee-attachment-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-human-resource-personnel-employee-attachment-employee-attachment"
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="EMPLOYEEATTACHMENT_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 员工实体子表 employeeAttachment 右栏面板
...
import EmployeeAttachmentForm from './employee-attachment-form.vue'
...
import {
  ...
  importEmployeeAttachment,
  exportEmployeeAttachment,
...
import {
  useEmployeeAttachmentI18n,
  EMPLOYEEATTACHMENT_DEFAULT_VISIBLE_COLUMN_KEYS,
  EMPLOYEEATTACHMENT_SUMMARY_SUM_FIELDS,
  EMPLOYEEATTACHMENT_QUERY_STRING_FIELDS,
  EMPLOYEEATTACHMENT_QUERY_FIELDS,
  EMPLOYEEATTACHMENT_SELF_I18N_KEY,
} from '../composables/use-employee-attachment-i18n'
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of EMPLOYEEATTACHMENT_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(EMPLOYEEATTACHMENT_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
    (typeof EMPLOYEEATTACHMENT_QUERY_STRING_FIELDS)[number],
    string
  >
  return {
    ...form,
...
const queryFieldsMeta = computed(() =>
  EMPLOYEEATTACHMENT_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...EMPLOYEEATTACHMENT_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...EMPLOYEEATTACHMENT_DEFAULT_VISIBLE_COLUMN_KEYS]
}
...
function getEmployeeAttachmentId(record: EmployeeAttachment | Record<string, unknown>): string {
  return String((record as EmployeeAttachment)?.[entityIdName] ?? '')
}

function getEmployeeAttachmentField(record: EmployeeAttachment | Record<string, unknown>, field: string): unknown {
  return (record as EmployeeAttachment)?.[field as keyof EmployeeAttachment]
}
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    customRender: ({ record }: { record: EmployeeAttachment }) =>
      String(getEmployeeAttachmentField(record, 'employeeAttachmentId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeAttachment }) =>
      String(getEmployeeAttachmentField(record, 'employeeId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeAttachment }) =>
      String(getEmployeeAttachmentField(record, 'employeeCode') ?? ''),
  },
  {
  ...
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeAttachment }) =>
      String(getEmployeeAttachmentField(record, 'accessUrl') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
 ...
 * @param record 行数据
 */
function onClickRow(record: EmployeeAttachment) {
...
async function handleEdit(record: EmployeeAttachment) {
  ...
  try {
    const detail = await getEmployeeAttachmentById(getEmployeeAttachmentId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.employeeAttachmentId
    if (id) {
      await updateEmployeeAttachment(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createEmployeeAttachment(payload)
...
async function handleDeleteOne(record: EmployeeAttachment) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteEmployeeAttachmentById(getEmployeeAttachmentId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getEmployeeAttachmentId(r)).filter(Boolean)
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importEmployeeAttachment(file, sheetName)
