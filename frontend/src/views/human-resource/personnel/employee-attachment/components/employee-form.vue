...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeAttachments -->
    <TaktEditableTable
      ref="employeeAttachmentTableRef"
      v-model="childEmployeeAttachmentRows"
      :columns="employeeAttachmentFormColumns"
      :title="employeeAttachmentPi.self()"
      :add-button-entity="employeeAttachmentPi.self()"
      id-field="employeeAttachmentId"
      :default-row="createDefaultEmployeeAttachmentRow"
      :disabled="loading"
...
const employeeAttachmentPi = useEmployeeAttachmentI18n()

const childEmployeeAttachmentRows = ref<Record<string, unknown>[]>([])
const employeeAttachmentTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeAttachment 可编辑列 */
const employeeAttachmentFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'attachmentName',
    title: employeeAttachmentPi.label('attachmentName'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'accessUrl',
    title: employeeAttachmentPi.label('accessUrl'),
    editor: 'input',
    width: 140,
  },
])
...
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeAttachment = ((val as any)?.employeeAttachments ?? []) as Record<string, unknown>[]
  childEmployeeAttachmentRows.value = rows_employeeAttachment
}

function createDefaultEmployeeAttachmentRow(): Record<string, unknown> {
  return {
    attachmentName: '',
    accessUrl: '',
  }
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeAttachments: employeeAttachmentTableRef.value?.getRows?.() ?? childEmployeeAttachmentRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeAttachments
...
async function validate() {
  ...
  await employeeAttachmentTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeAttachmentRows.value = []
  employeeAttachmentTableRef.value?.resetRows?.()
