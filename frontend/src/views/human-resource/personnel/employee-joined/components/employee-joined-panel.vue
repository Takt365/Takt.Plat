...
<!-- 命名空间：@/views/human-resource/personnel/employee-joined/components -->
<!-- 文件名称：employee-joined-panel.vue -->
<!-- 功能描述：员工实体主表实体右侧明细 employeeJoined 独立 CRUD（按主表选中 employeeId 分页） -->
<!-- 版权信息：Copyright (c) 2025 Takt  All rights reserved. -->
<!-- ======================================== -->

<template>
  <div class="employee-joined-panel flex h-full min-h-0 flex-col overflow-hidden">
    <TaktQueryBar
    ...
    <div
      ...
      class="employee-joined-panel__table-wrap min-h-0 flex-1 overflow-hidden"
    >
      <TaktSingleTable
    ...
    <TaktQueryDrawer
      ...
      storage-key="takt-query-fields-human-resource-personnel-employee-joined-employee-joined"
    ...
    >
      ...
      <a-form-item :label="pi.queryLabel('onboardingId')">
        <TaktSelect
          v-model:value="advancedQueryForm.onboardingId"
          api-url="TaktEmployeeOnboardings/options"
          :placeholder="pi.queryPh('onboardingId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('joinedDateStart')">
      <a-form-item :label="pi.queryLabel('joinedDateStart')">
        <a-date-picker
          v-model:value="advancedQueryForm.joinedDateStart"
          :placeholder="pi.queryPh('joinedDateStart', 'select')"
      ...
      <a-form-item :label="pi.queryLabel('joinedDateEnd')">
        <a-date-picker
          v-model:value="advancedQueryForm.joinedDateEnd"
          :placeholder="pi.queryPh('joinedDateEnd', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('probationEndDateStart')">
        <a-date-picker
          v-model:value="advancedQueryForm.probationEndDateStart"
          :placeholder="pi.queryPh('probationEndDateStart', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('probationEndDateEnd')">
        <a-date-picker
          v-model:value="advancedQueryForm.probationEndDateEnd"
          :placeholder="pi.queryPh('probationEndDateEnd', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('regularDateStart')">
        <a-date-picker
          v-model:value="advancedQueryForm.regularDateStart"
          :placeholder="pi.queryPh('regularDateStart', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('regularDateEnd')">
        <a-date-picker
          v-model:value="advancedQueryForm.regularDateEnd"
          :placeholder="pi.queryPh('regularDateEnd', 'select')"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="pi.queryLabel('postId')">
        <TaktSelect
          v-model:value="advancedQueryForm.postId"
          api-url="TaktPosts/options"
          :placeholder="pi.queryPh('postId', 'select')"
          allow-clear
        />
      </a-form-item>
      </div>
      ...
      <a-form-item :label="pi.queryLabel('jobTitle')">
        <a-input
          v-model:value="advancedQueryForm.jobTitle"
          :placeholder="pi.queryPh('jobTitle', 'required')"
          show-count
          :maxlength="20"
          allow-clear
        />
    ...
    >
      <TaktImportFile
        v-if="importVisible"
        :entity-i18n-key="EMPLOYEEJOINED_SELF_I18N_KEY"
        file-type="xlsx"
...
/**
 * 员工实体子表 employeeJoined 右栏面板
 * @module views/human-resource/personnel/employee-joined/components
 */
...
import EmployeeJoinedForm from './employee-joined-form.vue'
import { useEmployeeMasterContext } from '../composables/use-employee-master-context'
import {
  getEmployeeJoinedList,
  getEmployeeJoinedById,
  createEmployeeJoined,
  updateEmployeeJoined,
  deleteEmployeeJoinedById,
  deleteEmployeeJoinedBatch,
  getEmployeeJoinedTemplate,
  importEmployeeJoined,
  exportEmployeeJoined,
} from '@/api/human-resource/personnel/employee-joined'
import type { EmployeeJoined, EmployeeJoinedQuery } from '@/types/human-resource/personnel/employee-joined'

import {
  useEmployeeJoinedI18n,
  EMPLOYEEJOINED_DEFAULT_VISIBLE_COLUMN_KEYS,
  EMPLOYEEJOINED_SUMMARY_SUM_FIELDS,
  EMPLOYEEJOINED_QUERY_STRING_FIELDS,
  EMPLOYEEJOINED_QUERY_FIELDS,
  EMPLOYEEJOINED_SELF_I18N_KEY,
} from '../composables/use-employee-joined-i18n'

/** 实体字段 i18n（标签/占位符统一入口） */
const pi = useEmployeeJoinedI18n()
...
function hasAnyListQueryFilter(): boolean {
  ...
  for (const key of EMPLOYEEJOINED_QUERY_STRING_FIELDS) {
...
function createEmptyAdvancedQueryForm() {
  const form = Object.fromEntries(EMPLOYEEJOINED_QUERY_STRING_FIELDS.map((key) => [key, ''])) as Record<
...
const queryFieldsMeta = computed(() =>
  EMPLOYEEJOINED_QUERY_FIELDS.map((key) => ({ key, label: pi.queryLabel(key) })),
)
...
/** 表格当前可见列 key */
const visibleColumnKeys = ref<string[]>([...EMPLOYEEJOINED_DEFAULT_VISIBLE_COLUMN_KEYS])
...
function handleColumnSettingReset() {
  visibleColumnKeys.value = [...EMPLOYEEJOINED_DEFAULT_VISIBLE_COLUMN_KEYS]
}
...
function getEmployeeJoinedId(record: EmployeeJoined | Record<string, unknown>): string {
  return String((record as EmployeeJoined)?.[entityIdName] ?? '')
}

function getEmployeeJoinedField(record: EmployeeJoined | Record<string, unknown>, field: string): unknown {
  return (record as EmployeeJoined)?.[field as keyof EmployeeJoined]
}
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'employeeJoinedId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'employeeId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'employeeCode') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'employeeName') ?? ''),
  },
  {
    title: pi.label('onboardingId'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'onboardingId') ?? ''),
  },
  {
    title: pi.label('onboardingName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'onboardingName') ?? ''),
  },
  {
    title: pi.label('joinedDate'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'joinedDate') ?? ''),
  },
  {
    title: pi.label('probationEndDate'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'probationEndDate') ?? ''),
  },
  {
    title: pi.label('regularDate'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'regularDate') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'deptId') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'deptName') ?? ''),
  },
  {
    title: pi.label('postId'),
    dataIndex: 'postId',
    key: 'postId',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'postId') ?? ''),
  },
  {
    title: pi.label('postName'),
    dataIndex: 'postName',
    key: 'postName',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'postName') ?? ''),
  },
  {
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'jobTitle') ?? ''),
  },
  {
    title: pi.label('workNature'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'workNature') ?? ''),
  },
  {
    title: pi.label('employmentType'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'employmentType') ?? ''),
  },
  {
    title: pi.label('directManagerId'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'directManagerId') ?? ''),
  },
  {
    title: pi.label('directManagerName'),
    ...
    ellipsis: true,
    customRender: ({ record }: { record: EmployeeJoined }) =>
      String(getEmployeeJoinedField(record, 'directManagerName') ?? ''),
  },
  CreateActionColumn({
    actions: [
      {
        ...
        onClick: (record: EmployeeJoined) => void handleEdit(record),
      },
      {
        key: 'delete',
        ...
        onClick: (record: EmployeeJoined) => void handleDeleteOne(record),
      },
    ],
  }),
])
...
const summarySumFieldSet = new Set<string>(EMPLOYEEJOINED_SUMMARY_SUM_FIELDS)
...
const summaryFieldTotals = computed(() => {
  const totals = Object.fromEntries(
    EMPLOYEEJOINED_SUMMARY_SUM_FIELDS.map((field) => [field, 0]),
  ) as Record<(typeof EMPLOYEEJOINED_SUMMARY_SUM_FIELDS)[number], number>
  for (const row of dataSource.value) {
    for (const field of EMPLOYEEJOINED_SUMMARY_SUM_FIELDS) {
      const num = Number(getEmployeeJoinedField(row, field))
 ...
 * @param record 行数据
 */
function onClickRow(record: EmployeeJoined) {
...
async function handleEdit(record: EmployeeJoined) {
  formTitle.value = t('common.dialog.title.edit', { entity: pi.self() })
  formLoading.value = true
  try {
    const detail = await getEmployeeJoinedById(getEmployeeJoinedId(record))
...
async function handleFormSubmit() {
  ...
  try {
    ...
    const id = formData.value?.employeeJoinedId
    if (id) {
      await updateEmployeeJoined(id, payload)
      message.success(t('common.feedback.updated', { target: pi.self() }))
    } else {
      await createEmployeeJoined(payload)
...
async function handleDeleteOne(record: EmployeeJoined) {
  Modal.confirm({
    ...
    onOk: async () => {
      await deleteEmployeeJoinedById(getEmployeeJoinedId(record))
...
async function handleDelete() {
  ...
  Modal.confirm({
    ...
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r) => getEmployeeJoinedId(r)).filter(Boolean)
...
async function handleImportFile(file: File, sheetName?: string): Promise<TaktImportResult> {
  const raw = await importEmployeeJoined(file, sheetName)
