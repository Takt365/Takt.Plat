...
<template>
  ...
  >
    ...
    <!-- 下：子表 employeeJoineds -->
    <TaktEditableTable
      ref="employeeJoinedTableRef"
      v-model="childEmployeeJoinedRows"
      :columns="employeeJoinedFormColumns"
      :title="employeeJoinedPi.self()"
      :add-button-entity="employeeJoinedPi.self()"
      id-field="employeeJoinedId"
      :default-row="createDefaultEmployeeJoinedRow"
      :disabled="loading"
      :enable-vertical-scroll="false"
      section-border
      class="w-full min-w-0"
    >
      <template #cell-onboardingId="{ record }">
        <TaktSelect
          v-model:value="record.onboardingId"
          api-url="TaktEmployeeOnboardings/options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeJoinedPi.queryPh('onboardingId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-deptId="{ record }">
        <TaktSelect
          v-model:value="record.deptId"
          api-url="TaktDepts/tree-options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeJoinedPi.queryPh('deptId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
      <template #cell-postId="{ record }">
        <TaktSelect
          v-model:value="record.postId"
          api-url="TaktPosts/options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeJoinedPi.queryPh('postId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
      ...
      <template #cell-directManagerId="{ record }">
        <TaktSelect
          v-model:value="record.directManagerId"
          api-url="TaktEmployees/options"
          class="w-full"
          :get-popup-container="getSelectPopupContainer"
          :placeholder="employeeJoinedPi.queryPh('directManagerId', 'select')"
          :disabled="loading"
          allow-clear
        />
      </template>
...
const employeeJoinedPi = useEmployeeJoinedI18n()
...
const childEmployeeJoinedRows = ref<Record<string, unknown>[]>([])
const employeeJoinedTableRef = ref<{
  getRows: () => Record<string, unknown>[]
  validate: () => Promise<unknown>
  resetRows: () => void
} | null>(null)

/** 子表 employeeJoined 可编辑列 */
const employeeJoinedFormColumns = computed<TaktEditableTableColumn[]>(() => [
  {
    key: 'onboardingId',
    title: employeeJoinedPi.label('onboardingId'),
    width: 140,
  },
  {
    key: 'joinedDate',
    title: employeeJoinedPi.label('joinedDate'),
    editor: 'datePicker',
  ...
  },
  {
    ...
    title: employeeJoinedPi.label('probationEndDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
  {
    key: 'regularDate',
    title: employeeJoinedPi.label('regularDate'),
    editor: 'datePicker',
    valueFormat: 'YYYY-MM-DD',
    width: 140,
  },
  {
    key: 'deptId',
    title: employeeJoinedPi.label('deptId'),
    width: 140,
  },
  {
    key: 'deptName',
    title: employeeJoinedPi.label('deptName'),
    editor: 'input',
    width: 140,
  },
  {
    key: 'postId',
    title: employeeJoinedPi.label('postId'),
    width: 140,
  },
  {
    key: 'postName',
    title: employeeJoinedPi.label('postName'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeJoinedPi.ph('postName'),
  },
  {
    key: 'jobTitle',
    title: employeeJoinedPi.label('jobTitle'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeJoinedPi.ph('jobTitle'),
  },
  {
    key: 'workNature',
    title: employeeJoinedPi.label('workNature'),
    width: 140,
  },
  {
    key: 'employmentType',
    title: employeeJoinedPi.label('employmentType'),
    width: 140,
  },
  {
    key: 'directManagerId',
    title: employeeJoinedPi.label('directManagerId'),
    width: 140,
  },
  {
    key: 'directManagerName',
    title: employeeJoinedPi.label('directManagerName'),
    editor: 'input',
    width: 140, allowClear: true, placeholder: employeeJoinedPi.ph('directManagerName'),
  },
])

/** 编辑态从 formData 同步各子表行 */
function syncChildRowsFromFormData(val: Partial<EmployeeCreate & { employeeId?: string }> | null | undefined) {
  const rows_employeeJoined = ((val as any)?.employeeJoineds ?? []) as Record<string, unknown>[]
  childEmployeeJoinedRows.value = rows_employeeJoined
}

function createDefaultEmployeeJoinedRow(): Record<string, unknown> {
  return {
    onboardingId: '',
    joinedDate: '',
    probationEndDate: '',
...
function buildSubmitPayload() {
  ...
  return {
    ...formState,
    employeeJoineds: employeeJoinedTableRef.value?.getRows?.() ?? childEmployeeJoinedRows.value.map((rest) => ({
      ...rest,
...
watch(
  ...
  (val) => {
    ...
    delete (next as any).employeeJoineds
...
async function validate() {
  ...
  await employeeJoinedTableRef.value?.validate?.()
  return formState
}
...
function resetFields() {
  ...
  childEmployeeJoinedRows.value = []
  employeeJoinedTableRef.value?.resetRows?.()
