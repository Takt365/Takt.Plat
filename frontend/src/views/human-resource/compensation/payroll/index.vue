...
<!-- 命名空间：@/views/human-resource/compensation/payroll -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：薪酬体系管理页面，含查询、增删改，由 generate-vue-crud-from-api.cjs 根据 types/api 自动生成 -->
...
<template>
  <div class="p-4">
    ...
    <TaktQueryDrawer
      ...
      :storage-key="'takt-query-fields-human-resource-compensation-payroll'"
      :form-model="advancedQueryForm"
      @submit="handleAdvancedQuerySubmit"
      @reset="handleAdvancedQueryReset"
    >
      <template #default="{ isFieldVisible }">
      <div v-show="isFieldVisible('payrollCode')">
      <a-form-item :label="t('entity.payroll.code')">
        <a-input
      ...
      <div v-show="isFieldVisible('payrollName')">
      <a-form-item :label="t('entity.payroll.name')">
        <a-input
      ...
      <a-form-item :label="t('entity.payroll.payscaleid')">
        <a-input
      ...
      <a-form-item :label="t('entity.payroll.formulasetcode')">
        <a-input
      ...
      <a-form-item :label="t('entity.payroll.effectivedatestart')">
        <a-date-picker
          v-model:value="advancedQueryForm.effectiveDateStart"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.payroll.effectivedatestart') })"
      ...
      <a-form-item :label="t('entity.payroll.effectivedateend')">
        <a-date-picker
          v-model:value="advancedQueryForm.effectiveDateEnd"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.payroll.effectivedateend') })"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="t('entity.payroll.expirydatestart')">
        <a-date-picker
          v-model:value="advancedQueryForm.expiryDateStart"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.payroll.expirydatestart') })"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      ...
      <a-form-item :label="t('entity.payroll.expirydateend')">
        <a-date-picker
          v-model:value="advancedQueryForm.expiryDateEnd"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.payroll.expirydateend') })"
          value-format="YYYY-MM-DD"
          style="width: 100%"
        />
      </a-form-item>
      </div>
      <div v-show="isFieldVisible('payrollStatus')">
      <a-form-item :label="t('entity.payroll.status')">
        <TaktSelect
          v-model:value="advancedQueryForm.payrollStatus"
      ...
      <a-form-item :label="t('entity.payroll.description')">
        <a-textarea
    ...
    <TaktModal
      ...
      :title="t('common.dialog.title.import', { entity: t('entity.payroll._self') })"
      :width="600"
      :footer="null"
    ...
    >
      <TaktImportFile
        entity-i18n-key="entity.payroll._self"
        file-type="xlsx"
...
const excelNames = taktExcelEntityNames('TaktPayroll')
/** 列表快捷查询占位文案 */
const searchPlaceholder = computed(
  () => t('common.page.form.placeholder.search', { keyword: t('entity.payroll._self') })
)
...
const total = ref(0)
/** 工具栏单选时当前行 */
const selectedRow = ref<Payroll | null>(null)
/** 表格多选行 */
const selectedRows = ref<Payroll[]>([])
/** 表格多选 row-key 集合 */
...
const queryFieldsMeta = computed(() => [
  { key: 'payrollCode', label: t('entity.payroll.code') },
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    ellipsis: true,
    fixed: 'left',
    customRender: ({ record }: { record: any }) => getPayrollField(record, 'payrollId') ?? ''
  },
  {
    title: t('entity.payroll.code'),
    dataIndex: 'payrollCode',
    key: 'payrollCode',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPayrollField(record, 'payrollCode') ?? ''
  },
  {
    title: t('entity.payroll.name'),
    dataIndex: 'payrollName',
    key: 'payrollName',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPayrollField(record, 'payrollName') ?? ''
  },
  {
    title: t('entity.payroll.payscaleid'),
    dataIndex: 'payScaleId',
    key: 'payScaleId',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPayrollField(record, 'payScaleId') ?? ''
  },
  {
    title: t('entity.payroll.formulasetcode'),
    dataIndex: 'formulaSetCode',
    key: 'formulaSetCode',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPayrollField(record, 'formulaSetCode') ?? ''
  },
  {
    title: t('entity.payroll.effectivedate'),
    dataIndex: 'effectiveDate',
    key: 'effectiveDate',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPayrollField(record, 'effectiveDate') ?? ''
  },
  {
    title: t('entity.payroll.expirydate'),
    dataIndex: 'expiryDate',
    key: 'expiryDate',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPayrollField(record, 'expiryDate') ?? ''
  },
  {
    title: t('entity.payroll.status'),
  ...
  {
    title: t('entity.payroll.description'),
    dataIndex: 'description',
    key: 'description',
    width: 120,
    resizable: true,
    ellipsis: true,
    customRender: ({ record }: { record: any }) => getPayrollField(record, 'description') ?? ''
  },
  {
...
/** 表格 row-key（优先实体主键字段） */
const getPayrollId = (record: any): string => record?.[entityIdName] ?? ''
/**
 * 读取行字段值
 * @param record 行数据
 * @param field 字段名
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: (string | number)[], rows: Payroll[]) => {
  ...
  onSelectAll: (selected: boolean, selectedRowsData: Payroll[]) => {
    selectedRow.value = selected && selectedRowsData.length === 1 ? (selectedRowsData[0] ?? null) : null
  }
}))

/** 行点击切换选中（与 rowSelection 联动） */
const onClickRow = (record: Payroll) => ({
  onClick: () => {
    const key = getPayrollId(record)
    ...
    selectedRows.value = dataSource.value.filter((item) => selectedRowKeys.value.includes(getPayrollId(item)))
...
async function loadData() {
  ...
  try {
    ...
    logger.error('[Payroll] 加载数据失败', { error })
...
function handleEdit(record: Payroll) {
  formTitle.value = t('common.dialog.title.edit', { entity: t('entity.payroll._self') })
...
function handleUpdate() {
  if (selectedRow.value) {
    ...
    message.warning(t('common.tip.select.to.action', { action: t('common.page.button.edit'), entity: t('entity.payroll._self') }))
  }
}
/** 提交新增/编辑表单 */
async function handleFormSubmit() {
  ...
  try {
    ...
    if (id) {
      await updatePayroll(id, payload as any)
      message.success(t('common.feedback.updated', { target: t('entity.payroll._self') }))
    } else {
      await createPayroll(payload as any)
...
async function handleDownloadTemplate(sheetName?: string, fileName?: string): Promise<Blob> {
  const res = await getPayrollTemplate(sheetName, fileName)
  return (res as any)?.data ?? res
}

/** 上传并导入 Excel 文件 */
async function handleImportFile(file: File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> {
  return await importPayroll(file, sheetName)
}
...
async function handleExport() {
  try {
    ...
    message.success(t('common.feedback.export.success', { target: t('entity.payroll._self') }))
  } catch (error: any) {
    logger.error('[Payroll] 导出失败', { error })
    message.error(error?.message || t('common.feedback.export.failed', { target: t('entity.payroll._self') }))
  } finally {
    loading.value = false
  }
}
/** 删除单行 */
async function handleDeleteOne(record: Payroll) {
  Modal.confirm({
    title: t('common.tip.confirm.delete.title'),
    content: t('common.tip.confirm.delete.entity', { entity: t('entity.payroll._self'), name: t('common.tip.this.target', { target: t('entity.payroll._self') }) }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      await deletePayrollById((record as any)[entityIdName])
      message.success(t('common.feedback.deleted', { target: t('entity.payroll._self') }))
      loadData()
    }
  })
}
...
async function handleDelete() {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.tip.select.to.action', { action: t('common.page.button.delete'), entity: t('entity.payroll._self') }))
    return
  }
  Modal.confirm({
    title: t('common.tip.confirm.delete.title'),
    content: t('common.tip.confirm.delete.count', { entity: t('entity.payroll._self'), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      const ids = selectedRows.value.map((r: any) => r[entityIdName]).filter(Boolean)
      await deletePayrollBatch(ids)
      message.success(t('common.feedback.deleted', { target: t('entity.payroll._self') }))
      loadData()
    }
  })
