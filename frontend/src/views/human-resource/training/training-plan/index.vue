...
<template>
  <div class="p-4">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="human:resource:training:plan:create"
      update-permission="human:resource:training:plan:update"
      delete-permission="human:resource:training:plan:delete"
      import-permission="human:resource:training:plan:import"
      export-permission="human:resource:training:plan:export"
      :show-create="true"
      :show-update="true"
      :show-delete="true"
      :show-import="true"
    ...
    >
      <TrainingPlanForm
        :key="formData?.trainingPlanId ?? 'create'"
        ref="formRef"
        :form-data="formData"
        :loading="formLoading"
      />
...
import TrainingPlanForm from './components/plan-form.vue'
import { getTrainingPlanList, getTrainingPlanById, createTrainingPlan, updateTrainingPlan, deleteTrainingPlanById, deleteTrainingPlanBatch, getTrainingPlanTemplate, importTrainingPlan, exportTrainingPlan, updateTrainingPlanStatus } from '@/api/human-resource/training/plan'
import type { TrainingPlan, TrainingPlanQuery } from '@/types/human-resource/training/plan'
...
const formTitle = ref('')
/** 传入内嵌表单的编辑数据 */
const formData = ref<Partial<TrainingPlan> | null>(null)
/** 表单提交 loading */
 ...
 */
function buildListQuery(overrides?: Partial<TrainingPlanQuery>): TrainingPlanQuery {
  const form = advancedQueryForm.value
  const kw = (queryKeyword.value ?? '').trim()
  const query: TrainingPlanQuery = {
  ...
  const assignTrimmed = (key: keyof TrainingPlanQuery, value: string | undefined) => {
    const v = (value ?? '').trim()
    if (v.length > 0) {
...
async function handleTrainingPlanStatusChange(record: TrainingPlanRowRecord, checked: boolean) {
  ...
  if (row) {
    row.trainingPlanStatus = newVal
  }
  try {
    await updateTrainingPlanStatus({ trainingPlanId: id, trainingPlanStatus: newVal })
    message.success(t('common.feedback.updated'))
    
  } catch (error: unknown) {
    if (row) {
      row.trainingPlanStatus = oldVal
    }
