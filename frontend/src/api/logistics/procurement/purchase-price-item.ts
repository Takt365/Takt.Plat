...
/**
 * 更新采购价格明细作废状态
 * @param {PurchasePriceItemObsolete} dto 作废 DTO
 * @returns {Promise<PurchasePriceItem>} 采购价格明细DTO
 */
export function updatePurchasePriceItemObsolete(dto: PurchasePriceItemObsolete): Promise<PurchasePriceItem> {
  return request<PurchasePriceItem>({
    url: `${PURCHASE_PRICE_ITEM_API_BASE}/obsolete`,
    method: 'put',
    data: dto,
  });
}
 ...
 */
export function getPurchasePriceItemOptions(
  plantCode?: string,
  keyword?: string
 ...
 */
export function importPurchasePriceItem(file: globalThis.File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> {
