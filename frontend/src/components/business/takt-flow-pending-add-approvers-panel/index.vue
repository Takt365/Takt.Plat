...
<!-- 命名空间：@/components/business/takt-flow-pending-add-approvers-panel -->
<!-- 文件名称：index.vue -->
<!-- 功能描述：流程实例未处理加签列表；可选减签（pendingAddApprovers、reduce-sign API） -->
<!-- ======================================== -->
<template>
  <div
  ...
  >
    ...
    >
      ...
      >
        {{ t('common.page.button.reducesign') }}
      </a-button>
    </div>
  </div>
</template>
...
import type { FlowInstanceDetail, FlowPendingAddApprover } from '@/types/workflow/flow-engine'
...
async function onReduce(flowAddSignId: string) {
  ...
  const flowInstanceId = d?.flowInstanceId
