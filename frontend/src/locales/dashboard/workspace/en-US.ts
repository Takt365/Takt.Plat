// ========================================
// 项目名称：节拍工厂·Takt Plat
// 命名空间：frontend/src/locales/dashboard/workspace
// 文件名称：en-US.ts
// 创建时间：2026-06-16
// 创建人：Takt365(Cursor AI)
// 功能描述：dashboard/workspace page static copy; keys dashboard.workspace.page.*
//
// 版权信息：Copyright (c) 2025 Takt  All rights reserved.
// 免责声明：此软件使用 MIT License，作者不承担任何使用风险。
// ========================================

export default {
  page: {
    addmodule: "Add Module",
    removemodule: "Remove Module",
    manageshortcuts: "Manage shortcuts",
    shortcuttip: "Select up to 16 menu items as shortcuts (2 rows, 8 per row).",
    addsuccess: "Module added",
    removesuccess: "Module removed",
    selectmoduletype: "Select module type",
    currenttimelabel: "Current time",
    dayofyearlabel: "Day {n}",
    todoplaceholder: "No todos. Add a Todo module via \"Add Module\".",
    noticeplaceholder: "No notices. Add a Announcement module via \"Add Module\".",
    customplaceholder: "Custom content. Editable in future.",
    dragtoreorder: "Drag {title} to reorder",
    layoutlabel: "Layout",
    layoutfullrow: "One row one column",
    layouthalfrow: "One row two columns",
    modules: {
      welcome: "Welcome Greeting",
      shortcut: "Shortcuts",
      todo: "Todo",
      announcement: "Announcement",
      custom: "Custom",
    },
  },
};
